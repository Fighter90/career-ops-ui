...
> **🆕 Son sürüm — v1.228.3** — **Belge asistanı İngilizce yanıt veremiyordu.** Sorun getirmede değildi: bağlam oluşturucu sığmayan ilk bölümde `break` yapıp bağlamı boş bırakıyordu. İngilizce §5, 14 336 baytlık bütçeye karşı 16 081 bayta ulaşmıştı. 6 KB üstü bölümler artık `###` sınırlarından bölünüyor (32 → 75 parça). Ayrıca: Telegram şirket etiketleri adı değil cümleyi veriyordu ve `GET /api/ping` hiçbir şey sızdırmayan bir canlılık probu. **2907 test.**
...
- **Telegram kanalları (v1.228.0)** — herhangi bir **herkese açık** kanal: `telegram_channels:` bloğunda listeleyin, `t.me/s/<kanal>` önizlemesinden okunur — bot jetonu yok, API anahtarı yok. Telegram RSS yayımlamaz ve Bot API, botunuzun yönetmediği bir kanalı okuyamaz; önizleme tek yoldur. **Bir gönderi düzyazıdır, iş kaydı değil**: başlık ilk anlamlı satırdan, şirket ve konum yalnızca açık etiketlerden gelir; gerçek ilanları reklamdan ayırmak `title_filter`'ınızın işidir.
...
├─ server/
│  ├─ index.mjs              # ~130 LOC orkestratör: middleware + 37 register<Topic>Routes(app) çağrısı + SPA catch-all
│  └─ lib/
...
│     └─ routes/             # 37 route modülü — konu başına bir tane (P-2)
...
│  └─ reviews/REVIEW-*.md
└─ tests/                    # 2907 birim + 101 Playwright + 23/23 e2e:full + 21 e2e:smoke (baseline @ v1.121.0)
...
```bash
npm test                       # 2907 birim/entegrasyon testi
...
| Paket                       | Test | Ne                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (birim + entegrasyon) | **2907** | Her uç nokta, geçici sunucu, ağ yok. 329 dosya: ayrıştırıcılar, tarayıcılar (taklit edilmiş), çalıştırıcılar, anthropic/openai, güvenlik başlıkları, XSS, iş tanımı temizleme, URL doğrulama, i18n eşitliği, + v1.55→v1.56 UX-düzeltme paketleri. |
...
- Push'tan önce `npm test` çalıştırın — **2907 kontrol yeşil** çıtadır (arayüze dokunursanız artı 101 Playwright).
