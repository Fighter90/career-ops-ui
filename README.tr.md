...
> **🆕 En son sürüm — v1.131.2**
>
> **`app.css` bölünmesi (yeniden yapılandırma).** v1.131.2, bir dosya-boyutu-sözleşmesi borcunu kapatıyor: ~1990 satırlık `app.css`, üç sıralı stil sayfasına bölündü — `app.css` (temel/çerçeve/düğmeler), **`components.css`** (kartlar, tablolar, formlar, rozetler, sayfalayıcı, …), **`overlays.css`** (toast, modal, çekmece, `[dir="rtl"]`, docs-fab, usage-hud) — her biri 800 satırlık sert hedefin içinde. Kesim bitişik ve sırayla yapıldı, böylece kaskad **bayt bayt aynı**; `index.html` üçünü sıralı `<link>`ler olarak yüklüyor. Davranış/biçimlendirme/i18n değişikliği yok; tarayıcıda doğrulandı (üç sayfa da ayrıştırılıyor ve uygulanıyor, 0 konsol hatası). **2138 test**. **Adaptör ana bilgisayar sabitleme sıkılaştırması (yama).** v1.131.1, v1.130.0 kaynaklarındaki kod incelemesi takip işlerini ele alıyor (derinlemesine savunma, geçerli girdiler için davranış değişikliği yok): **a16z Speedrun** adaptörü artık uç nokta geçersiz kılmasını yeniden doğruluyor (HTTPS + tam ana bilgisayar) ve kanonik beslemeye geri dönüyor — **Cryptocurrency Jobs** ile parite — ve Cryptocurrency Jobs ayrıştırıcısının `cleanUrl`'i, SSRF kontrolüyle aynı tam ana bilgisayar koruyucusunu kullanıyor (önceden alt alan adlarını kabul eden `endsWith` idi), böylece ayrıştırıcı asla koruyucudan daha gevşek değil. **2135 test**. **CRM aşama-sekmesi panosu.** v1.131.0, üst projenin web uygulamasının `/pipeline` görünümünü `#/tracker`'a taşıyor: huni-çipi çubuğu + durum açılır menüsü bir **aşama-sekmesi şeridi**ne dönüşüyor — **Tümü** artı her kanonik durum için bir sekme (Evaluated · Applied · Responded · Interview · Offer · Rejected · Discarded · SKIP · Hired), her biri canlı bir sayıyla, **sıfır sayılı aşamalar dahil** olmak üzere, böylece tüm huni her zaman görünür kalıyor. Sekmeler yeni bir salt-okunur `GET /api/tracker/stages`'ten geliyor (kanonik etiketler + `templates/states.yml`'den takma ad katlama — istemci beyaz listeyi asla sabit kodlamaz), satırlar isteğe bağlı marka logoları kazanıyor ve erişilebilir adlar sayıları taşıyor. **2133 test**. **Üst proje v1.24.0 paritesi.** v1.130.0 iki yeni tarama kaynağı taşıyor — **a16z Speedrun** (a16z Speedrun yetenek ağı panosu genelindeki besleme) ve **Cryptocurrency Jobs** (Web3 panosu `cryptocurrencyjobs.co`, herkese açık RSS'i üzerinden) — kayıt defterini **72 kaynağa (67 İngilizce + 5 Rusça)** çıkarıyor ve ikisini de `#/scan` Kaynak filtresinde ve cvstart.org açılış sayfasında gösteriyor. Ayrıca iki üst proje düzeltmesini yansıtıyor: **echojobs** artık **hibrit** rolleri uzaktandan ayırt edilebilir tutuyor (`Şehir · Hibrit`, asla `Remote`'a indirgenmiyor) ve **radancy** eski TalentBrew biçimlendirmesini ve JSON sonuç-parçası taşımasını öğreniyor. **2123 test**. **AI inceleme takipleri (yama).** v1.129.1, v1.128/v1.129 web taşımalarındaki tavsiye bulgularını düzeltir: job-facets seviye önceliği (`Senior Engineering Manager` → `senior`, `lead` değil), `states.mjs` açılışta üst proje aksaklığında yedeğini artık sabitlemiyor (+bozuk dosyada uyarır), ton puansız satırı nötr gösterir (kırmızı değil) ve `domainFromName` ASCII olmayan slug'ları atlar. **Tarama seviye faseti + yaş sütunu.** v1.129.0, v1.128.0'deki `job-facets.js` kütüphanesini `#/scan`'e bağlar: yeni bir **Seviye** filtresi (lead/staff/senior/orta/junior/stajyer, Ülke faseti gibi sonuçlardan otomatik dolar), bir Seviye rozet sütunu ve token'sız bir **Yaş** sütunu (`bugün` / `Ng`). **Üst projenin web uygulamasından dört taşıma.** v1.128.0, career-ops'un kendi Next.js uygulamasındaki dört çözümü saf JS ile yeniden yazar: izleyici kanonik durumları `templates/states.yml`'den canlı okur (sabit kodlu beyaz liste yok), ATS barındırmalı satırlarda ad→alan adı eşlemesiyle marka logoları, daha ince 4 kademeli puan tonu ve token'sız bir job-facets kütüphanesi. **2066 test**. **Üst proje v1.23.0 paritesi.** v1.127.0 üç yeni tarama kaynağı — **Flowxtra**, **VDAB**, **iCIMS** (kayıt defteri artık **70 adaptör**, 65 EN + 5 RU) — taşır, agenticjobs'un HTML→REST geçişini, Greenhouse şehir kurtarmayı ve role-matcher düzeltmelerini yansıtır ve **Cursor**'ı CLI listesine geri ekler (parent #2115 — `cli-detect` 10 araç bildirir). **CLI listesi kayması yaması.** v1.126.1, v1.126.0 taramasının kaçırdığı iki noktayı düzeltir: `#/config` API keys sekmesi girişi (i18n ×17) artık Antigravity + Grok Build listeliyor ve yardım kılavuzu ×17'deki (ve CI'da derlenen site help'indeki) eski `Cursor / Gemini CLI` satırı artık tam 8-CLI listesini taşıyor. **Doküman & CLI listesi yeniden senkronu.** v1.126.0 her doküman yüzeyini (help ×17, README ×17, wiki) canlı career-ops.org/docs ile (31 sayfanın tümü okundu) uzlaştırır ve `#/config` AI CLI araçları taramasına 8 birinci sınıf CLI'nin tümünü algılamayı öğretir — **Grok Build CLI** ve **Kimi CLI**, Claude Code, Cursor, Codex, OpenCode, Antigravity, Qwen ve Copilot'a katılır (artık **9 araç**, hâlâ asla çalıştırılmaz). **Bakım senkronu.** v1.125.4, dependabot site güncellemelerini (`sharp` 0.35.3, `svgo` 4.0.2, `fast-uri` 3.1.4) topluyor ve üst proje parite taramasını (v1.22.0 sonrası, #2108–#2168) kaydediyor: set-status yanlış-satır koruması, yerelleştirilmiş modların Risk Summary'si ve güncelleyici manifest doğrulaması hepsi CLI tarafında — taşınacak bir şey yok (**1969 test** değişmedi). **İstem yerel ayarı düzeltmesi.** v1.125.3 Danca ve Hintçeyi her AI isteminde birinci sınıf yapar — deep research (canlı + manuel), modlar, değerlendirme, mülakat, networking ve CV Studio artık bu iki dilde `# Output language` yönergesini üretiyor (yanıtlar İngilizce geliyordu) ve yerel ayar kapısı artık 17 dilin tümünü tarıyor (**1969 test**). **Dış katkı paketi.** v1.125.2, [@Alien10140](https://github.com/Alien10140) tarafından gönderilen ilk topluluk PR'larını birleştiriyor — canlı Deep research HTTP 502 düzeltmesi (API sağlayıcıları için araçsız headless istemler) ve kullanımdan kaldırılan `gemini-2.0-flash` yerine `gemini-3.6-flash` olan Gemini varsayılanları; yeni 5 testlik kayma kapısıyla sabitlendi (**1957 test**). v1.125.1, çok markalı SuccessFactors RMK kiracılarının marka yolunu koruyor (üst proje #2099). **Landingde iş kaynakları.** v1.125.0, **70 tarama kaynağının tümünü** tıklanabilir çipler olarak listeleyen bir cvstart.org bölümü ekliyor; kayıt defteriyle build sırasında senkronize ve sapmaya karşı korunuyor. Bundan önce **üst proje v1.22.0 paritesi.** v1.124.0, **beş tarama kaynağını** taşıyor — Welcome to the Jungle, Agentic Engineering Jobs, Jobvite, Gem ve Alibaba Group (artık **70 adaptör**) — buna ek olarak Arbeitsagentur'un tamamen-uzaktan doğrulamasını (`homeofficetyp=VOLLSTAENDIG`) ve SmartRecruiters'ın herkese açık URL düzeltmesini de taşıyor.
>
> _app.css bölünmesi (3 stil sayfası) · dosya-boyutu-sözleşmesi borcu kapatma · bayt-bayt aynı kaskad · adaptör ana bilgisayar sabitleme sıkılaştırması · a16z geçersiz kılma yeniden doğrulaması · cryptocurrencyjobs cleanUrl tam ana bilgisayar · CRM aşama-sekmesi panosu · izleyici /pipeline taşıması · aşama sekmeleri + sayılar · GET /api/tracker/stages · izleyici marka logoları · üst proje v1.24 paritesi · +2 kaynak (a16z Speedrun/Cryptocurrency Jobs) · 72 kaynak · echojobs hibrit · radancy TalentBrew · AI inceleme yaması · seviye önceliği · states yedeği · nötr ton · seviye faseti · yaş sütunu · job-facets bağlandı · 4 web/ taşıma · states.yml canlı · ad→alan adı logoları · puan tonu · facet · üst proje v1.23 paritesi · +3 kaynak · 70 adaptör · Cursor · CLI listesi kayması yaması · doküman & CLI listesi yeniden senkronu · bakım senkronu · da/hi istem yerel ayarı düzeltmesi · dış katkı paketi · deep research 502 düzeltmesi · Gemini 3.6 varsayılanları · SF marka yolu düzeltmesi · landing kaynak bölümü · 5 kaynak · 67 adaptör · üst proje v1.22 paritesi · Oracle Cloud · 62 adaptör · üst proje v1.21 paritesi · Hintçe · 17 dil · manifesto · cvstart.org'da methodology/license/changelog sayfaları · üst proje v1.19 paritesi · 61 adaptör · landingde canlı yıldızlar + katkıda bulunanlar · üst parite v1.18 · 54 adaptör · Hired durumu · toplam istatistik · parite paketi · 50 adaptör · kullanım göstergesi yeniden yapımı · tasarım rötuşu · kullanım göstergesi · yüzen yardım asistanı · doküman & QA konsolidasyonu · güvenlik backlog kapanışı · docs & QA ×16 · Scan Hariç Tut · pipeline genel bakışı · güvenlik sıkılaştırması 2 · temizleyici sıkılaştırması · güvenlik sıkılaştırması · AI kullanımı & maliyeti · şirket logoları · yapay zeka CLI araçları · belgelere sor · CV uyarlama + ön yazı · two-pager otomatik doldurma · DOCX dışa aktarımı · portal sağlığı · uygulama içi hata bildirici · 16 yerel dil · 6 LLM sağlayıcısı · 46 tarayıcı adaptörü · kariyer yönelimi · kariyer planı · istatistiklerin yeniden tasarımı · bellek katmanı · CV Studio · networking planlayıcı · deneme mülakatı · two-pager ile pazar uyumu · üst career-ops v1.16.0 eşdeğerliği._
...
![career-ops-ui — Komuta Merkezi](./images/dashboard-tr.png)
