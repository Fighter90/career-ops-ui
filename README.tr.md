...
│  └─ reviews/REVIEW-*.md
└─ tests/                    # 1824 birim + 90 Playwright + 23/23 e2e:full + 20 e2e:smoke (baseline @ v1.118.4)
...
```bash
npm test                       # 1824 birim/entegrasyon testi
...
| Paket                       | Test | Ne                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (birim + entegrasyon) | **1824** | Her uç nokta, geçici sunucu, ağ yok. 215 dosya: ayrıştırıcılar, tarayıcılar (taklit edilmiş), çalıştırıcılar, anthropic/openai, güvenlik başlıkları, XSS, iş tanımı temizleme, URL doğrulama, i18n eşitliği, + v1.55→v1.56 UX-düzeltme paketleri. |
