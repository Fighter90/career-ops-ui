...
> **🆕 En son sürüm — v1.103.0**
>
> **Ayarlar: yapay zeka CLI araçları.** Uygulama ayarlarındaki yeni **Yapay zeka CLI araçları** sekmesi, hangi ajan CLI'larının — Claude Code, Codex, Gemini CLI, OpenCode, GitHub Copilot CLI, Qwen, Antigravity — sunucuyu çalıştıran makinede kurulu olduğunu ve yollarını gösterir. Salt okunur bir PATH taraması: yalnızca her ikili dosyanın var olup olmadığını kontrol eder ve **asla çalıştırmaz** (`--version` yok, yürütme yok), hiçbir şey yazmaz, kullanıcı verisi okumaz.
>
> _yapay zeka CLI araçları · belgelere sor · CV uyarlama + ön yazı · two-pager otomatik doldurma · DOCX dışa aktarımı · portal sağlığı · uygulama içi hata bildirici · 16 yerel dil · 6 LLM sağlayıcısı · 46 tarayıcı adaptörü · kariyer yönelimi · kariyer planı · istatistiklerin yeniden tasarımı · bellek katmanı · CV Studio · networking planlayıcı · deneme mülakatı · two-pager ile pazar uyumu · üst career-ops v1.16.0 eşdeğerliği._
