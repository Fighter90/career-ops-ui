...
> **🆕 Son sürüm — v1.228.2** — **İki kelimelik arama, kelimeleri değil öbeği arıyordu.** `/api/scan-results?q=` tek bir `includes(q)` yapıyordu: her iki kelimeyi 162 satır taşırken 32 satır buluyordu. Dar bir eşleşme böyle yanlış cevaba dönüşür. Terimler artık ayrı ayrı karşılaştırılıyor (VE). **2899 test.**
...
│  └─ reviews/REVIEW-*.md
└─ tests/                    # 2899 birim + 101 Playwright + 23/23 e2e:full + 21 e2e:smoke (baseline @ v1.121.0)
...
```bash
npm test                       # 2899 birim/entegrasyon testi
...
| Paket                       | Test | Ne                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (birim + entegrasyon) | **2899** | Her uç nokta, geçici sunucu, ağ yok. 329 dosya: ayrıştırıcılar, tarayıcılar (taklit edilmiş), çalıştırıcılar, anthropic/openai, güvenlik başlıkları, XSS, iş tanımı temizleme, URL doğrulama, i18n eşitliği, + v1.55→v1.56 UX-düzeltme paketleri. |
...
- Push'tan önce `npm test` çalıştırın — **2899 kontrol yeşil** çıtadır (arayüze dokunursanız artı 101 Playwright).
