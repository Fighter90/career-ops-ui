...
> **🆕 En son sürüm — v1.100.0**
>
> **Two-pager yapay zeka otomatik doldurma + dışa aktarım.** Two-pager (`#/two-pager`) artık yapılandırılmış sağlayıcınızla özgeçmişiniz + profilinizden kendini doldurur (hiçbir şey uydurmadan; anahtar yoksa istemi-kopyala geri dönüşü), bir **Önizle ve dışa aktar** görünümü ekler ve **PDF / DOCX / Markdown** dışa aktarımı kazanır — DOCX yeni bağımlılıksız bir `.docx` üreticisiyle, pazar, kariyer planı ve yönlendirme raporlarına da bağlı.
>
> _two-pager otomatik doldurma · DOCX dışa aktarımı · portal sağlığı · uygulama içi hata bildirici · 16 yerel dil · 6 LLM sağlayıcısı · 46 tarayıcı adaptörü · kariyer yönelimi · kariyer planı · istatistiklerin yeniden tasarımı · bellek katmanı · CV Studio · networking planlayıcı · deneme mülakatı · two-pager ile pazar uyumu · üst career-ops v1.16.0 eşdeğerliği._
