...
> **🆕 En son sürüm — v1.106.0**
>
> **Güvenlik sıkılaştırması (CodeQL triyajı).** Rota render hata yolu artık hata mesajını DOM'a ulaşmadan önce kaçışlıyor (bir sunucu hatası kullanıcı girdisini yansıtabilir — XSS sınırı) ve profil/yapılandırma özellik yazımları `__proto__` / `constructor` / `prototype` anahtarlarını reddediyor (prototip kirliliği koruması). Kalan uyarıların çoğu tarayıcının meşru `data/*` G/Ç'si üzerindeki yanlış pozitiflerdir ve gerekçeyle reddedildi.
>
> _güvenlik sıkılaştırması · AI kullanımı & maliyeti · şirket logoları · yapay zeka CLI araçları · belgelere sor · CV uyarlama + ön yazı · two-pager otomatik doldurma · DOCX dışa aktarımı · portal sağlığı · uygulama içi hata bildirici · 16 yerel dil · 6 LLM sağlayıcısı · 46 tarayıcı adaptörü · kariyer yönelimi · kariyer planı · istatistiklerin yeniden tasarımı · bellek katmanı · CV Studio · networking planlayıcı · deneme mülakatı · two-pager ile pazar uyumu · üst career-ops v1.16.0 eşdeğerliği._
