...
> **🆕 En son sürüm — v1.108.0**
>
> **Güvenlik sıkılaştırması (CodeQL triyajı, 2. tur).** Prompt oluşturucu rol satırını kendi anahtarı + `typeof === function` ile çözer, PDF dosya adı slug'ı regex'inden önce sınırlandırılır (polinom ReDoS) ve belge içe aktarma bir dizi `filename`'i dizeye zorlar (type-confusion). v1.106–v1.108 boyunca statik analiz birikimi 167'den ~14'e düştü, her gerçek bulgu düzeltildi ve kalanı gerekçeyle reddedildi.
>
> _güvenlik sıkılaştırması 2 · temizleyici sıkılaştırması · güvenlik sıkılaştırması · AI kullanımı & maliyeti · şirket logoları · yapay zeka CLI araçları · belgelere sor · CV uyarlama + ön yazı · two-pager otomatik doldurma · DOCX dışa aktarımı · portal sağlığı · uygulama içi hata bildirici · 16 yerel dil · 6 LLM sağlayıcısı · 46 tarayıcı adaptörü · kariyer yönelimi · kariyer planı · istatistiklerin yeniden tasarımı · bellek katmanı · CV Studio · networking planlayıcı · deneme mülakatı · two-pager ile pazar uyumu · üst career-ops v1.16.0 eşdeğerliği._
