...
> **🆕 En son sürüm — v1.104.0**
>
> **Tarama tablosunda şirket logoları (gizliliği koruyan).** Yeni bir Görünüm anahtarı `#/scan` üzerinde her şirketin logosunu adının yanına çizer. Logo, şirketin **kendi alan adından favicon**'udur ve SSRF güvenli getirici üzerinden sunucu tarafında proxy'lenir (`GET /api/logo`) — böylece hiçbir üçüncü taraf logo servisi hangi işverenlere baktığınızı öğrenemez. Paylaşılan iş ilanı sunucuları harf rozeti gösterir; varsayılan kapalı.
>
> _şirket logoları · yapay zeka CLI araçları · belgelere sor · CV uyarlama + ön yazı · two-pager otomatik doldurma · DOCX dışa aktarımı · portal sağlığı · uygulama içi hata bildirici · 16 yerel dil · 6 LLM sağlayıcısı · 46 tarayıcı adaptörü · kariyer yönelimi · kariyer planı · istatistiklerin yeniden tasarımı · bellek katmanı · CV Studio · networking planlayıcı · deneme mülakatı · two-pager ile pazar uyumu · üst career-ops v1.16.0 eşdeğerliği._
