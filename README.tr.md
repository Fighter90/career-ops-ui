...
> **🆕 Son sürüm — v1.160.0** — **Dürüst sağlayıcı metni (düzeltme)** — `#/config` ve pano yalnızca Anthropic/Gemini ile puanladığını (OpenAI'nin "kullanılmadığını") söylüyordu — 7 sağlayıcı zincirinden beri yanlış. Artık yedisi de adlandırılıyor, Değerlendir kartı tarafsız ve `Keys: N/5` → `N/7`. **2413 test.**
>
