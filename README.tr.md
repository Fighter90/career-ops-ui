...
> **🆕 En son sürüm — v1.107.0**
>
> **Temizleyici sıkılaştırması (durağan XSS).** Özgeçmiş/ilan markdown temizleyicisi artık tehlikeli HTML'i **bir sabit noktaya kadar** kaldırıyor (yükü yeniden oluşturan bir kaldırmayı yakalamak için), sonunda çöp bulunan script/style kapanış etiketleriyle (`</script foo>`) eşleşiyor ve kapatılmamış bir yürütülebilir açıcıyı kaldırıyor. Render'da-kaçışlı istemcinin arkasında derinlemesine savunma; ilgili CodeQL bulgularını kapatır.
>
> _temizleyici sıkılaştırması · güvenlik sıkılaştırması · AI kullanımı & maliyeti · şirket logoları · yapay zeka CLI araçları · belgelere sor · CV uyarlama + ön yazı · two-pager otomatik doldurma · DOCX dışa aktarımı · portal sağlığı · uygulama içi hata bildirici · 16 yerel dil · 6 LLM sağlayıcısı · 46 tarayıcı adaptörü · kariyer yönelimi · kariyer planı · istatistiklerin yeniden tasarımı · bellek katmanı · CV Studio · networking planlayıcı · deneme mülakatı · two-pager ile pazar uyumu · üst career-ops v1.16.0 eşdeğerliği._
