...
**Слой памяти (Epic 24).** Новая страница `#/memory` хранит короткую редактируемую заметку «запомни это обо мне», которую ассистент держит в уме при **every** задаче:

- **Одна заметка везде** — поскольку она встраивается в `bundleProjectContext`, заметка автоматически доходит до каждого AI-запроса (оценка, mock interview, нетворкинг, CV Studio) по всем **all** провайдерам. Напишите её один раз — она направляет всё.
