...
> **🆕 Dernière version — v1.137.0** — **corrections de lisibilité et de rendu** : blanc sur blanc / noir sur noir en mode sombre sur de nombreux écrans (`#/pipeline`, onglets de `#/stats`, `#/config`, `#/two-pager`) corrigé à la racine — quinze tokens de thème n'étaient jamais déclarés et retombaient sur des valeurs claires/noires codées en dur ; les libellés des graphiques de `#/stats` s'affichent désormais avec une ellipse au lieu d'être tronqués en plein mot ; et `#/career-plan` s'affiche en texte formaté et lisible au lieu de Markdown brut. **2345 tests.**
>
