...
> **🆕 Dernière version — v1.234.0** — **Parité parent avec career-ops `main` @ `56cce8f` : un seul identifiant d'offre malformé ne vide plus toute la page.** `encodeURIComponent` lève une exception sur un substitut UTF-16 isolé, et une charge utile JSON peut en transporter un. Quinze sources construisaient l'URL de chaque offre à partir d'un identifiant contrôlé par l'hôte *à l'intérieur* de la boucle d'analyse, si bien qu'une seule offre défectueuse interrompait la boucle et perdait silencieusement tous les postes de cette page. L'utilitaire `_safe-url.mjs` du parent est reproduit en miroir dans `server/lib/sources/`, et chaque boucle ne supprime désormais que l'offre concernée — sur les treize sources miroir, plus **jobstreet et trudvsem**, deux sources propres à web-ui que la nouvelle garde au niveau des sources a repérées dès sa première exécution. Les valeurs dérivées de la configuration (`office` de garena, `corpName` de csod) conservent leur encodage strict et échouent bruyamment. Le décodage du repli de slug de rheinmetall est protégé de la même façon, et le registre adopte la convention du parent du préfixe `_` pour les utilitaires. Rien d'autre dans le delta de 58 commits n'est porté — chaque élément est listé avec sa justification. Nombre de sources inchangé à **92**. **3042 tests · 116 navigateur.**
