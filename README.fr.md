...
> **🆕 Dernière version — v1.131.2**
>
> **Scission d'`app.css` (refactorisation).** v1.131.2 résorbe une dette de contrat de taille de fichier : l'`app.css` d'environ 1990 lignes est scindé en trois feuilles de style ordonnées — `app.css` (base/chrome/boutons), **`components.css`** (cartes, tableaux, formulaires, badges, paginateur, …), **`overlays.css`** (toast, modale, tiroir, `[dir="rtl"]`, docs-fab, usage-hud) — chacune dans la limite stricte de 800 lignes. Le découpage est contigu et dans l'ordre, si bien que la cascade est **identique octet pour octet** ; `index.html` charge les trois fichiers comme des `<link>` ordonnés. Aucun changement de comportement, de balisage ou d'i18n ; vérifié dans le navigateur (les trois feuilles s'analysent et s'appliquent, 0 erreur console). **2138 tests**. **Durcissement de l'épinglage d'hôte des adaptateurs (correctif).** v1.131.1 traite les suites de revue de code sur les sources de v1.130.0 (défense en profondeur, aucun changement de comportement pour les entrées valides) : l'adaptateur **a16z Speedrun** revalide désormais la surcharge de point de terminaison (HTTPS + hôte exact) et se replie sur le flux canonique — parité avec **Cryptocurrency Jobs** — et le `cleanUrl` du parseur Cryptocurrency Jobs utilise la même garde d'hôte exact que son contrôle SSRF (c'était `endsWith`, qui acceptait les sous-domaines), de sorte que le parseur n'est jamais plus permissif que la garde. **2135 tests**. **Tableau à onglets d'étape CRM.** v1.131.0 porte la vue `/pipeline` de l'application web du parent vers `#/tracker` : la barre de puces d'entonnoir et le menu déroulant de statut deviennent une **bande d'onglets d'étape** — **All** plus un onglet par statut canonique (Evaluated · Applied · Responded · Interview · Offer · Rejected · Discarded · SKIP · Hired), chacun avec un compte en direct, **étapes à zéro incluses** afin que l'entonnoir complet reste toujours visible. Les onglets proviennent d'une nouvelle route en lecture seule `GET /api/tracker/stages` (libellés canoniques + repli des alias depuis `templates/states.yml` — le client ne code jamais en dur la liste blanche), les lignes gagnent des logos de marque optionnels, et les noms accessibles portent les comptes. **2133 tests**. **Parité avec le parent v1.24.0.** v1.130.0 porte deux nouvelles sources de scan — **a16z Speedrun** (le flux global du board talent-network d'a16z Speedrun) et **Cryptocurrency Jobs** (le board Web3 `cryptocurrencyjobs.co`, via son flux RSS public) —, portant le registre à **72 sources (67 anglaises + 5 russes)** et les faisant apparaître dans le filtre Source de `#/scan` et sur la landing cvstart.org. Elle reflète aussi deux correctifs du parent : **echojobs** garde désormais les postes **hybrides** distincts du remote (`Ville · Hybrid`, jamais fondu dans `Remote`), et **radancy** apprend le balisage TalentBrew hérité ainsi que le transport par fragment JSON de résultats. **2123 tests**. **Suites de la revue IA (correctif).** v1.129.1 corrige les points consultatifs des ports web de v1.128/v1.129 : précédence de niveau dans job-facets (`Senior Engineering Manager` → `senior`, pas `lead`), `states.mjs` ne fige plus son fallback lors d'un incident parent au démarrage (+alerte sur fichier malformé), la teinte affiche neutre une ligne sans score (pas rouge), et `domainFromName` ignore les slugs non-ASCII. **Facette Niveau + colonne Ancienneté au scan.** v1.129.0 relie la lib `job-facets.js` de v1.128.0 à `#/scan` : un nouveau filtre **Niveau** (lead/staff/senior/confirmé/junior/stagiaire, rempli depuis les résultats comme la facette Pays), une colonne badge Niveau et une colonne **Ancienneté** sans token (`auj.` / `Nj`). **Quatre ports depuis la web app du parent.** v1.128.0 réécrit en JS vanilla quatre solutions de l'app Next.js de career-ops : le tracker lit les statuts canoniques en direct depuis `templates/states.yml` (fini la whitelist codée en dur), des logos de marque sur les lignes hébergées par un ATS via une carte nom→domaine, une teinte de score à 4 niveaux et une lib de facettes sans token. **2066 tests**. **Parité parent v1.23.0.** v1.127.0 porte trois nouvelles sources de scan — **Flowxtra**, **VDAB**, **iCIMS** (registre désormais **70 adaptateurs**, 65 EN + 5 RU) —, reflète le passage HTML→REST d'agenticjobs, la récupération de ville de Greenhouse et les correctifs de role-matcher, et réintègre **Cursor** au roster des CLIs (parent #2115 — `cli-detect` rapporte 10 outils). **Correctif de dérive du roster CLI.** v1.126.1 corrige deux points manqués par le balayage v1.126.0 : l'intro de l'onglet API keys de `#/config` (i18n ×17) liste désormais Antigravity + Grok Build, et une ligne obsolète `Cursor / Gemini CLI` dans le guide d'aide ×17 (et le help du site) porte désormais le roster complet de 8 CLIs. **Resync docs & roster CLI.** v1.126.0 réconcilie chaque surface de docs (help ×17, README ×17, wiki) avec la career-ops.org/docs vivante (les 31 pages lues) et apprend au scan AI CLI tools de `#/config` à détecter les 8 CLIs de premier rang — **Grok Build CLI** et **Kimi CLI** rejoignent Claude Code, Cursor, Codex, OpenCode, Antigravity, Qwen et Copilot (désormais **9 outils**, toujours jamais exécutés). **Sync de maintenance.** v1.125.4 regroupe les bumps dependabot du site (`sharp` 0.35.3, `svgo` 4.0.2, `fast-uri` 3.1.4) et consigne un balayage de parité avec le parent (post-v1.22.0, #2108–#2168) : le garde-fou de mauvaise ligne de set-status, le Risk Summary des modes localisés et la vérification de manifeste de l'updater sont tous côté CLI — rien à porter (**1969 tests** inchangés). **Correctif de locale des prompts.** v1.125.3 fait du danois et du hindi des locales de premier rang dans chaque prompt IA — deep research (live et manuel), modes, évaluation, entretien, networking et CV Studio émettent désormais la directive `# Output language` dans ces deux locales (les réponses arrivaient en anglais), et le garde-fou parcourt maintenant les 17 locales (**1969 tests**). **Pack de contributions externes.** v1.125.2 fusionne les premiers PR communautaires, de [@Alien10140](https://github.com/Alien10140) : le correctif du HTTP 502 du Deep research en direct (prompts headless sans outils pour les fournisseurs d'API) et les modèles Gemini par défaut passés du déprécié `gemini-2.0-flash` à `gemini-3.6-flash`, épinglés par une nouvelle porte anti-dérive de 5 tests (**1957 tests**). v1.125.1 conserve aux tenants RMK multimarques de SuccessFactors leur chemin de marque (parent #2099). **Sources d'offres sur la landing.** La v1.125.0 ajoute une section sur cvstart.org qui liste les **70 sources de scan** sous forme de puces cliquables, synchronisée avec le registre à chaque build et protégée contre toute dérive. Avant cela, v1.124.0 porte **cinq sources de scan** — Welcome to the Jungle, Agentic Engineering Jobs, Jobvite, Gem et Alibaba Group (**70 adaptateurs** désormais) — ainsi que la vérification du full remote pour Arbeitsagentur (`homeofficetyp=VOLLSTAENDIG`) et le correctif des URL publiques de SmartRecruiters.
>
> _scission d'app.css (3 feuilles de style) · résorption de la dette de contrat de taille de fichier · cascade identique octet pour octet · durcissement de l'épinglage d'hôte des adaptateurs · revalidation de la surcharge a16z · cleanUrl à hôte exact de cryptocurrencyjobs · tableau à onglets d'étape CRM · portage /pipeline du tracker · onglets d'étape + comptes · GET /api/tracker/stages · logos de marque du tracker · parité parent v1.24 · +2 sources (a16z Speedrun/Cryptocurrency Jobs) · 72 sources · echojobs hybrid · radancy TalentBrew · correctif revue IA · précédence de niveau · fallback states · teinte neutre · facette niveau · colonne ancienneté · job-facets reliée · 4 ports web/ · states.yml en direct · logos nom→domaine · teinte · facettes · parité parent v1.23 · +3 sources · 70 adaptateurs · Cursor · correctif de dérive du roster CLI · resync docs & roster CLI · sync de maintenance · correctif locale prompts da/hi · pack de contributions externes · correctif 502 du deep research · Gemini 3.6 par défaut · correctif chemin de marque SF · section des sources sur la landing · 5 sources · 67 adaptateurs · parité parent v1.22 · Oracle Cloud · 62 adaptateurs · parité parent v1.21 · Hindi · 17 locales · manifeste · pages méthodologie/licence/changelog de cvstart.org · parité parent v1.19 · 61 adaptateurs · étoiles en direct + contributeurs sur la landing · parité parent v1.18 · 54 adaptateurs · statut Hired · stats cumulées · pack de parité · 50 adaptateurs · refonte compteur d'utilisation · peaufinage du design · compteur d'utilisation · assistant d'aide flottant · consolidation docs & QA · clôture backlog sécurité · docs & QA ×16 · Exclure Scan · aperçu pipeline · durcissement de sécurité 2 · durcissement du désinfectant · durcissement de sécurité · utilisation & coût IA · logos d'entreprise · outils CLI d'IA · demander au guide · adapter CV + lettre · remplissage auto du two-pager · export DOCX · santé des portails · rapporteur de bugs intégré · 16 locales · 6 fournisseurs LLM · 46 adaptateurs de scanner · orientation de carrière · plan de carrière · refonte des statistiques · couche mémoire · CV Studio · planificateur de networking · mock interview · adéquation au marché via le two-pager · parité avec le career-ops v1.16.0._
