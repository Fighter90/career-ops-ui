...
> **🆕 Dernière version — v1.228.3** — **L'assistant de documentation ne répondait pas en anglais.** Pas la recherche : le constructeur de contexte faisait `break` sur la première section trop grande, laissant le contexte vide. Le §5 anglais atteignait 16 081 octets pour un budget de 14 336. Les sections de plus de 6 Ko sont désormais découpées à leurs `###` (32 → 75 blocs). Et : les étiquettes d'entreprise Telegram donnaient la phrase, pas le nom, et `GET /api/ping` est une sonde qui ne divulgue rien. **2907 tests.**
...
│  └─ reviews/REVIEW-*.md
└─ tests/                    # 2907 unit + 101 Playwright + 23/23 e2e:full + 21 e2e:smoke (baseline @ v1.121.0)
...
```bash
npm test                       # 2907 unit/integration tests
...
| Suite                       | Tests | Quoi                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + intégration) | **2907** | Chaque endpoint, serveur éphémère, sans réseau. 329 fichiers : parsers, scanners (mockés), runners, anthropic/openai, en-têtes de sécurité, XSS, désinfection d'offre, validation d'URL, parité i18n, + les suites de fixes UX v1.55→v1.56. |
