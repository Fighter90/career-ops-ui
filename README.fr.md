...
> **🆕 Dernière version — v1.228.2** — **Une recherche de deux mots cherchait la phrase, pas les mots.** `/api/scan-results?q=` faisait un seul `includes(q)` : 32 lignes trouvées là où 162 contiennent les deux mots. C'est ainsi qu'une correspondance étroite devient une réponse fausse. Les termes sont désormais comparés indépendamment (ET). **2899 tests.**
...
| Suite                       | Tests | Quoi                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + intégration) | **2899** | Chaque endpoint, serveur éphémère, sans réseau. 329 fichiers : parsers, scanners (mockés), runners, anthropic/openai, en-têtes de sécurité, XSS, désinfection d'offre, validation d'URL, parité i18n, + les suites de fixes UX v1.55→v1.56. |
