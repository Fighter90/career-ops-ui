...
> **🆕 Ultima release — v1.228.0** — **I canali Telegram sono ora una fonte di scansione.** Elencali in un blocco `telegram_channels:` e ognuno viene letto dall'anteprima pubblica `t.me/s/<canale>`: 15 canali, **299 post** in una esecuzione reale. Registro: **86 fonti** (81 EN + 5 RU). Inoltre: l'assistente Telegram rispondeva da una frazione dei dati — riportava **9** corrispondenze dove lo snapshot ne aveva centinaia, perché `/api/scan-results` sapeva restituire solo lo snapshot intero da ~2 MB; ora accetta una query paginata. **2893 test.**
...
| Suite                       | Test | Cosa                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **2893** | Ogni endpoint, server effimero, nessuna rete. 218 file: parser, scanner (mockati), runner, anthropic/openai, header di sicurezza, XSS, sanitizzazione JD, validazione URL, parità i18n, + le suite di correzioni UX v1.55→v1.56. |
...
| **Totale**                   | **2893** | **0 fallimenti, 0 flake**                                                                                    |
...
- Esegui `npm test` prima del push — **2893 checks green** è il minimo (più 101 Playwright se tocchi la UI).
