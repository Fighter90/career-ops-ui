...
> **🆕 Ultima release — v1.104.0**
>
> **Logo aziendali nella tabella di scansione (rispettosi della privacy).** Un nuovo interruttore Aspetto disegna il logo di ogni azienda accanto al nome su `#/scan`. Il logo è la **favicon dell'azienda dal suo dominio**, messa in proxy lato server (`GET /api/logo`) tramite il fetcher sicuro contro l'SSRF — nessun servizio di logo di terze parti scopre quali datori di lavoro guardi. Gli host di portali condivisi mostrano un badge con una lettera; disattivato per impostazione predefinita.
>
> _logo aziendali · strumenti CLI IA · chiedi alla guida · adatta CV + lettera · compilazione auto del two-pager · esportazione DOCX · salute dei portali · segnalatore di bug integrato · 16 lingue · 6 provider LLM · 46 adattatori di scansione · orientamento professionale · piano di carriera · rielaborazione delle statistiche · livello di memoria · CV Studio · pianificatore di networking · colloquio simulato · fit di mercato con il two-pager · parità con career-ops principale v1.16.0._
