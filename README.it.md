...
> **🆕 Ultima release — v1.228.3** — **L'assistente della documentazione non rispondeva in inglese.** Non il recupero: il costruttore del contesto faceva `break` sulla prima sezione troppo grande, lasciando il contesto vuoto. Il §5 inglese era arrivato a 16 081 byte contro un budget di 14 336. Le sezioni oltre 6 KB ora si dividono ai loro `###` (32 → 75 blocchi). Inoltre: le etichette azienda di Telegram davano la frase, non il nome, e `GET /api/ping` è una sonda che non rivela nulla. **2907 test.**
...
| Suite                       | Test | Cosa                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **2907** | Ogni endpoint, server effimero, nessuna rete. 329 file: parser, scanner (mockati), runner, anthropic/openai, header di sicurezza, XSS, sanitizzazione JD, validazione URL, parità i18n, + le suite di correzioni UX v1.55→v1.56. |
...
- Esegui `npm test` prima del push — **2907 checks green** è il minimo (più 101 Playwright se tocchi la UI).
