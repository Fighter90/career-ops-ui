...
> **🆕 Ultima release — v1.231.5** — **Sicurezza: sette avvisi chiusi, uno un'esecuzione di codice remoto critica.** Nessun file sorgente modificato. Prima `multer` 2.2.0→2.3.0 e `svgo` 4.0.2→4.1.0; la riscansione ne ha rivelati altri cinque, tutti già dentro il `^` esistente: **`astro` 7.1.0→7.3.2** (**critica** — RCE tramite ottimizzazione immagini AVIF), **`sharp` 0.35.3→0.35.4** e **`js-yaml` 4.3.1→4.3.2 in entrambi i manifesti**. Pubblicato con **3013/3013** test e una ricostruzione completa del sito. `npm audit`: **0 vulnerabilità**. **3013 test.**
