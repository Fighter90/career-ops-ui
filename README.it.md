...
> **🆕 Ultima release — v1.228.2** — **Una ricerca di due parole cercava la frase, non le parole.** `/api/scan-results?q=` faceva un solo `includes(q)`: 32 righe trovate dove 162 contengono entrambe. Così una corrispondenza stretta diventa una risposta sbagliata. Ora i termini si confrontano separatamente (E). **2899 test.**
...
| Suite                       | Test | Cosa                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **2899** | Ogni endpoint, server effimero, nessuna rete. 329 file: parser, scanner (mockati), runner, anthropic/openai, header di sicurezza, XSS, sanitizzazione JD, validazione URL, parità i18n, + le suite di correzioni UX v1.55→v1.56. |
...
- Esegui `npm test` prima del push — **2899 checks green** è il minimo (più 101 Playwright se tocchi la UI).
