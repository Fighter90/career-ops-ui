...
> **🆕 Ultima release — v1.228.5** — **La versione riportata descriveva il file su disco, non il codice in esecuzione.** `/api/health` rileggeva `package.json` a ogni richiesta: un processo avviato prima di un deploy riportava la versione del file nuovo mentre serviva il codice vecchio — il QA ha trovato un'istanza che rispondeva `1.228.4` e restituiva 404 su una rotta aggiunta in 1.228.3. È così che un deploy che copia i file senza riavviare sembra riuscito. La versione ora si cattura al caricamento del modulo: un processo obsoleto riporta la versione **vecchia**, cioè la verità. **2909 test.**
...
| Suite                       | Test | Cosa                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **2909** | Ogni endpoint, server effimero, nessuna rete. 329 file: parser, scanner (mockati), runner, anthropic/openai, header di sicurezza, XSS, sanitizzazione JD, validazione URL, parità i18n, + le suite di correzioni UX v1.55→v1.56. |
...
| **Totale** (tutte e quattro le suite)                   | **3040** | **0 fallimenti, 0 flake**                                                                                    |
...
- Esegui `npm test` prima del push — **2909 checks green** è il minimo (più 101 Playwright se tocchi la UI).
