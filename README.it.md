...
> **🆕 Ultima release — v1.106.0**
>
> **Rafforzamento della sicurezza (triage CodeQL).** Il percorso di errore del rendering ora effettua l'escape del messaggio di errore prima che raggiunga il DOM (un errore del server può riflettere input dell'utente — confine XSS), e le scritture di proprietà profilo/config rifiutano le chiavi `__proto__` / `constructor` / `prototype` (protezioni anti prototype-pollution). La maggior parte degli avvisi rimanenti sono falsi positivi sull'I/O legittimo dello scanner in `data/*` e sono stati respinti con motivazione.
>
> _rafforzamento della sicurezza · uso e costo IA · logo aziendali · strumenti CLI IA · chiedi alla guida · adatta CV + lettera · compilazione auto del two-pager · esportazione DOCX · salute dei portali · segnalatore di bug integrato · 16 lingue · 6 provider LLM · 46 adattatori di scansione · orientamento professionale · piano di carriera · rielaborazione delle statistiche · livello di memoria · CV Studio · pianificatore di networking · colloquio simulato · fit di mercato con il two-pager · parità con career-ops principale v1.16.0._
