...
[career-ops](https://career-ops.org) è un sistema open-source per la ricerca di lavoro che gira come slash command dentro qualsiasi CLI di coding con IA (Claude Code, Gemini CLI, Codex, Qwen Code, OpenCode, GitHub Copilot CLI, Antigravity CLI — funzionano anche altre CLI compatibili con Claude tramite la stessa superficie di slash command). Indipendente dal modello. Valuta ogni annuncio rispetto al tuo CV con una griglia a sei dimensioni da 0.0 a 5.0, genera CV in PDF su misura e tiene traccia di ogni candidatura localmente — nessun account cloud, nessuna telemetria, nessun invio automatico.
