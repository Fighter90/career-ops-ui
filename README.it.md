...
> **🆕 Ultima release — v1.131.2**
>
> **Divisione di `app.css` (refactor).** v1.131.2 salda un debito da contratto sulla dimensione dei file: l'`app.css` di ~1990 LOC è diviso in tre fogli di stile ordinati — `app.css` (base/chrome/pulsanti), **`components.css`** (card, tabelle, form, badge, paginatore, …), **`overlays.css`** (toast, modale, cassetto, `[dir="rtl"]`, docs-fab, usage-hud) — ciascuno entro il limite rigido di 800 LOC. Il taglio è contiguo e in ordine, quindi la cascata è **identica byte per byte**; `index.html` carica tutti e tre come `<link>` ordinati. Nessuna modifica di comportamento/markup/i18n; verificato nel browser (tutti e tre i fogli vengono analizzati e applicati, 0 errori console). **2138 test**. **Rafforzamento del pinning dell'host degli adattatori (patch).** v1.131.1 agisce sui follow-up di code review sulle sorgenti v1.130.0 (difesa in profondità, nessuna modifica di comportamento per input validi): l'adattatore **a16z Speedrun** ora rivalida l'override dell'endpoint (HTTPS + host esatto) e ripiega sul feed canonico — parità con **Cryptocurrency Jobs** — e il `cleanUrl` del parser Cryptocurrency Jobs usa la stessa guardia dell'host esatto del suo controllo SSRF (era `endsWith`, che accettava sottodomini), così il parser non è mai più permissivo della guardia. **2135 test**. **Bacheca a schede di stadio CRM.** v1.131.0 porta la vista `/pipeline` dell'app web del padre su `#/tracker`: la barra a chip del funnel + il menu a tendina di stato diventano una **striscia di schede di stadio** — **Tutti** più una scheda per ogni stato canonico (Evaluated · Applied · Responded · Interview · Offer · Rejected · Discarded · SKIP · Hired), ciascuna con un conteggio live, **incluse le fasi a conteggio zero** così l'intero funnel è sempre visibile. Le schede provengono da una nuova rotta di sola lettura `GET /api/tracker/stages` (etichette canoniche + fusione degli alias da `templates/states.yml` — il client non codifica mai staticamente la whitelist), le righe guadagnano loghi di brand opzionali, e i nomi accessibili portano i conteggi. **2133 test**. **Parità genitore v1.24.0.** v1.130.0 porta due nuove sorgenti di scansione — **a16z Speedrun** (il feed dell'intera bacheca della talent-network a16z Speedrun) e **Cryptocurrency Jobs** (il board Web3 `cryptocurrencyjobs.co`, via il suo RSS pubblico) — portando il registro a **72 sorgenti (67 inglesi + 5 russe)** e mostrandole entrambe nel filtro Sorgente di `#/scan` e sulla landing di cvstart.org. Rispecchia inoltre due correzioni del padre: **echojobs** ora mantiene i ruoli **ibridi** distinguibili dal remoto (`Città · Ibrido`, mai collassati in `Remoto`), e **radancy** impara il markup legacy TalentBrew più il trasporto del frammento di risultati JSON. **2123 test**. **Follow-up della revisione IA (patch).** v1.129.1 corregge i rilievi consultivi dei port web di v1.128/v1.129: precedenza di livello in job-facets (`Senior Engineering Manager` → `senior`, non `lead`), `states.mjs` non blocca più il fallback su un intoppo del padre al boot (+avvisa su file malformato), il tono mostra neutra una riga senza punteggio (non rossa), e `domainFromName` salta gli slug non-ASCII. **Faccetta Livello + colonna Età nello scan.** v1.129.0 collega la lib `job-facets.js` di v1.128.0 a `#/scan`: un nuovo filtro **Livello** (lead/staff/senior/intermedio/junior/stagista, autopopolato dai risultati come la faccetta Paese), una colonna badge Livello e una colonna **Età** senza token (`oggi` / `Ng`). **Quattro port dalla web app del padre.** v1.128.0 reimplementa in JS vanilla quattro soluzioni dell'app Next.js di career-ops: il tracker legge gli stati canonici dal vivo da `templates/states.yml` (niente più whitelist hardcoded), loghi di brand ora sulle righe con host ATS tramite una mappa nome→dominio, un tono di punteggio a 4 livelli più fine e una lib di facet senza token. **2066 test**. **Parità genitore v1.23.0.** v1.127.0 porta tre nuove sorgenti di scansione — **Flowxtra**, **VDAB**, **iCIMS** (registro ora **70 adattatori**, 65 EN + 5 RU) —, rispecchia il passaggio HTML→REST di agenticjobs, il recupero città di Greenhouse e le correzioni di role-matcher, e reintegra **Cursor** nel roster delle CLI (parent #2115 — `cli-detect` riporta 10 strumenti). **Patch di deriva del roster CLI.** v1.126.1 corregge due punti mancati dallo sweep v1.126.0: l'intro della scheda API keys di `#/config` (i18n ×17) ora elenca Antigravity + Grok Build, e una riga obsoleta `Cursor / Gemini CLI` nella guida di aiuto ×17 (e nell'help del sito) ora porta il roster completo di 8 CLI. **Resync docs & roster CLI.** v1.126.0 riconcilia ogni superficie di docs (help ×17, README ×17, wiki) con la career-ops.org/docs viva (lette tutte e 31 le pagine) e insegna allo scan AI CLI tools di `#/config` a rilevare tutte le 8 CLI di prima classe — **Grok Build CLI** e **Kimi CLI** si uniscono a Claude Code, Cursor, Codex, OpenCode, Antigravity, Qwen e Copilot (ora **9 strumenti**, mai eseguiti). **Sync di manutenzione.** v1.125.4 raccoglie i bump dependabot di site (`sharp` 0.35.3, `svgo` 4.0.2, `fast-uri` 3.1.4) e registra uno sweep di parità col padre (post-v1.22.0, #2108–#2168): il guard della riga sbagliata di set-status, il Risk Summary dei modi localizzati e la verifica del manifest dell'updater sono tutti lato CLI — niente da portare (**1969 test** invariati). **Fix locale dei prompt.** v1.125.3 rende danese e hindi di prima classe in ogni prompt AI — deep research (live + manuale), modalità, valutazione, colloquio, networking e CV Studio ora emettono la direttiva `# Output language` in quelle due lingue (le risposte arrivavano in inglese), e il gate delle locale ora scorre tutte le 17 (**1969 test**). **Pacchetto di contributi esterni.** v1.125.2 unisce le prime PR della community, di [@Alien10140](https://github.com/Alien10140): la correzione dell'HTTP 502 del Deep research live (prompt headless senza strumenti per i provider API) e i modelli Gemini predefiniti aggiornati dal deprecato `gemini-2.0-flash` a `gemini-3.6-flash`, fissati da un nuovo gate anti-deriva di 5 test (**1957 test**). v1.125.1 mantiene i tenant RMK multi-brand di SuccessFactors sul loro percorso di brand (parent #2099). **Fonti di lavoro sulla landing.** La v1.125.0 aggiunge una sezione su cvstart.org che elenca tutte le **70 sorgenti di scansione** come chip cliccabili, sincronizzata con il registro a ogni build e protetta da disallineamenti. Prima, v1.124.0 porta **cinque sorgenti di scansione** — Welcome to the Jungle, Agentic Engineering Jobs, Jobvite, Gem e Alibaba Group (ora **70 adattatori**) — oltre alla verifica del full-remote di Arbeitsagentur (`homeofficetyp=VOLLSTAENDIG`) e alla correzione dell'URL pubblico di SmartRecruiters.
>
> _divisione di app.css (3 fogli di stile) · saldo del debito da contratto sulla dimensione dei file · cascata identica byte per byte · rafforzamento del pinning dell'host degli adattatori · rivalidazione dell'override a16z · cryptocurrencyjobs cleanUrl host esatto · bacheca a schede di stadio CRM · port `/pipeline` del tracker · schede di stadio + conteggi · `GET /api/tracker/stages` · loghi di brand nel tracker · parità genitore v1.24 · +2 sorgenti (a16z Speedrun/Cryptocurrency Jobs) · 72 sorgenti · echojobs ibrido · radancy TalentBrew · patch revisione IA · precedenza di livello · fallback states · tono neutro · faccetta livello · colonna età · job-facets collegata · 4 port web/ · states.yml dal vivo · loghi nome→dominio · tono · facet · parità genitore v1.23 · +3 sorgenti · 70 adattatori · Cursor · patch di deriva del roster CLI · resync docs & roster CLI · sync di manutenzione · fix locale prompt da/hi · pacchetto contributi esterni · fix 502 deep research · default Gemini 3.6 · fix percorso brand SF · sezione fonti sulla landing · 5 fonti · 67 adattatori · parità con il parent v1.22 · Oracle Cloud · 62 adattatori · parità con il parent v1.21 · hindi · 17 lingue · manifesto · pagine methodology/license/changelog su cvstart.org · parità padre v1.19 · 61 adapter · stelle live + contributori sulla landing · parità padre v1.18 · 54 adapter · stato Hired · statistiche totali · pacchetto di parità · 50 adattatori · rifacimento contatore di utilizzo · rifinitura del design · contatore di utilizzo · assistente guida fluttuante · consolidamento docs & QA · chiusura backlog sicurezza · docs & QA ×16 · Escludi Scan · panoramica pipeline · rafforzamento della sicurezza 2 · rafforzamento del sanitizzatore · rafforzamento della sicurezza · uso e costo IA · logo aziendali · strumenti CLI IA · chiedi alla guida · adatta CV + lettera · compilazione auto del two-pager · esportazione DOCX · salute dei portali · segnalatore di bug integrato · 16 lingue · 6 provider LLM · 46 adattatori di scansione · orientamento professionale · piano di carriera · rielaborazione delle statistiche · livello di memoria · CV Studio · pianificatore di networking · colloquio simulato · fit di mercato con il two-pager · parità con career-ops principale v1.16.0._
...
Le modalità completamente guidate da LLM (`oferta`, `deep`, `contacto`, `apply`, `batch`, `patterns`, `followup`) hanno bisogno di un LLM per girare davvero. La web UI risolve un provider dall'ordine `auto` **Anthropic → Gemini → OpenAI → Qwen → OpenRouter** (o qualunque cosa fissi `LLM_PROVIDER`):
