...
> **🆕 Ultima release — v1.236.0** — **Parità con il padre career-ops `main` @ `6a9c84c`: due nuove sorgenti, e due correzioni che qui perdevano silenziosamente le offerte.** Un ruolo **Workday** aperto in più città riporta un _conteggio_ — `"53 Locations"` — dove ogni altra offerta porta una sede, quindi non corrispondeva a nessuna voce `allow: [austin]` ed era scartato (53 of 291 offerte a monte). Le sue sedi reali provengono ora dal documento di dettaglio CXS, con un tetto di 200 richieste per annuncio. **`role-matcher`** non aveva alcuna gestione dei livelli, quindi `"Insurance Specialist"` e `"Insurance Specialist II"` collassavano in un unico repost — l'intero meccanismo del padre è stato portato, con romano/arabo ripiegati. Una bacheca **Avature** fissata a un filtro percorreva l'intera bacheca globale, perché la query string veniva scartata quando l'URL di ricerca era ricostruito. Nuove sorgenti: **Python.org Jobs** e **Generalist World** — **92 → 94** (89 EN + 5 RU). Inoltre `devalue` 5.8.1 → 5.9.2, che chiude l'avviso Dependabot aperto. **3164 test · 116 browser.**
