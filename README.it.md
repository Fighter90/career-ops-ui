...
> **🆕 Ultima release — v1.231.4** — **la v1.231.3 ha sistemato le conseguenze del clic su Doctor, non il momento.** Ripristinava gli `<span>` nel `finally`, ma il segnale di occupato scriveva ancora `'⏳ ' + etichetta` dentro il pulsante *mentre la richiesta girava* — quindi per tutta la durata di `doctor.mjs` il quadrato da 36 px era una pillola larga `⏳ 🩺Doctor` che rompeva la riga, e su desktop un `⏳🩺Doctor` ammassato. **Un pulsante con figli elemento non viene più riscritto**: il suo segnale è la classe `.is-loading`, che scambia l'icona con una clessidra in CSS — **36 px a riposo, 36 px in volo, 36 px dopo**. **3013 test.**
