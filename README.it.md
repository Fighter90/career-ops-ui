...
> **🆕 Ultima release — v1.233.1** — **La guida non ha mai descritto il controllo aggiunto dalla v1.233.0.** Ogni menù a tendina inizia ora con «Usa il valore predefinito (…)», che esprime uno stato che l'interfaccia non sapeva mostrare: **la chiave non è impostata**. Sparisce anche una riga di `### Recognized keys` che non nominava alcuna chiave e invitava a registrarsi su dev.hh.ru — inutile dalla **v1.65.0** — assieme ad altre due menzioni obsolete. `tests/help-recognized-keys.test.mjs` è il controllo che mancava. **3021 test · 116 browser.**
