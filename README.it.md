...
> **🆕 Ultima release — v1.235.0** — **Parità con il padre career-ops `main` @ `68e6b94`: una seconda offerta dalla stessa bacheca MokaHR spariva invece di essere aggiunta.** La chiave di deduplica scartava ogni frammento URL — corretto per `#apply`, sbagliato per MokaHR, i cui annunci condividono il percorso di un tenant e portano l'id dell'offerta **solo** in `#/job/{id}`. Incollare due offerte diverse nella pipeline lasciava **una sola riga**, senza errori, e il filtro di freschezza dello scanner scartava il resto del tenant come «già visto». Una rotta riconosciuta `#/job/{id}` / `#/jobs/{id}` viene ora promossa a chiave di confronto prima che il frammento venga scartato — aggiunta, senza mai sovrascrivere quella già esistente — mentre ogni frammento cosmetico continua a collassare. Inoltre: `accept-encoding` è fissato su `gzip, deflate, br` su ogni richiesta verso le sorgenti (preventivo qui — Node 18/20/22 non offrono mai zstd; l'override senza distinzione tra maiuscole e minuscole è la parte che cambia il comportamento). Sorgenti invariate a **92**. **3066 test · 116 browser.**
