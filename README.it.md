...
> **🆕 Ultima release — v1.228.1** — **Un symlink rotto nel repository troncava silenziosamente i deploy.** `.claude/skills/refero-design` puntava a un percorso mai esistito qui; rsync si interrompe su di esso, così un deploy ha riportato successo mentre il server restava alla versione precedente. Rimosso. La fonte dei canali Telegram è nella v1.228.0.
...
| Suite                       | Test | Cosa                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **2895** | Ogni endpoint, server effimero, nessuna rete. 329 file: parser, scanner (mockati), runner, anthropic/openai, header di sicurezza, XSS, sanitizzazione JD, validazione URL, parità i18n, + le suite di correzioni UX v1.55→v1.56. |
...
- Esegui `npm test` prima del push — **2895 checks green** è il minimo (più 101 Playwright se tocchi la UI).
