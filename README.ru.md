...
[![tests](https://img.shields.io/badge/tests-2895%20passed-brightgreen)](#тесты)
...
> **🆕 Последний релиз — v1.228.1** — **Висячий симлинк в репозитории молча обрезал деплой.** `.claude/skills/refero-design` указывал на путь, которого здесь никогда не было; rsync прерывается на нём посреди передачи, и деплой отчитался об успехе, пока сервер работал на прежней версии. Удалён. Источник Telegram-каналов — в v1.228.0.
...
```bash
npm test                       # 2895 unit/integration-теста
...
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **2895** | Каждый эндпоинт, эфемерный сервер, без сети. Включая парсеры, сканер (mocked), runner, anthropic, заголовки безопасности, XSS, sanitize JD, валидацию URL, защиту от DNS-rebind, состояние гонки на трекере, rate-limit, path-traversal, паритет i18n. |
...
- Перед push выполняйте `npm test` — **2895 проверок green** — это минимальная планка (плюс 101 Playwright, если изменения касаются UI).
