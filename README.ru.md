...
> **🆕 Последний релиз — v1.232.1** — **Сохранение на `#/config` записывало поля, которых пользователь не касался.** Два разумных решения вместе давали запись: v1.57.1 засевает незаданный контрол его `defaultValue`, чтобы поле показывало то, что сервер и вправду использует, а `save()` спрашивал `dirty` **только для секретов** и отправлял всё остальное всегда — значит значение, подставленное для *показа*, уходило как выбор пользователя. Смена одного дропдауна отправляла **28 ключей и закрепляла 18** нетронутых; Save вообще без изменений отправлял 27. В день записи ничего не меняется — записанное и есть умолчания, — но настройка переходит из «не задана, следуй умолчанию проекта» в «закреплена в `.env`», и следующая смена умолчания молча не дойдёт до тех, кто хоть раз нажал Save. **Интересна тут основа сравнения:** сравнения с загруженным `cfg.values` недостаточно, потому что незаданный select показывает своё умолчание, а сохранённое значение пусто — тело сократилось с 27 ключей до 19, а не до одного. Теперь сравнение идёт с тем, **чем контрол был проинициализирован**. Заодно снова работает счётчик в тосте: он жил внутри fallback-аргумента `t()`, а ключ есть во всех 17 словарях, поэтому число не показывалось. **3018 тестов · 111 браузерных.**
