...
> **🆕 Последний релиз — v1.86.0**
>
> **Статистика по целевым ролям** (`#/stats`): новая страница аналитики читает ваши **целевые роли из профиля** (не зашитые в код) и последний скан, чтобы построить графики **вакансий и медианной зарплаты по странам**, с фильтрами по роли/стране и **трендом по сохранённым снимкам** — агрегируется на стороне клиента из разреженных данных, которые сканеры уже собирают (без выдуманных чисел), на всех 16 языках. Основан на v1.85.0 (немецкая/итальянская/турецкая локали), v1.84.0 (кулдаун повторных заявок) и v1.83.0 (детектор повторных публикаций).
>
> _16 locales · 6 LLM-провайдеров · 41 адаптер сканера · рыночная статистика по целевым ролям · детектор повторных публикаций · паритет с родительским career-ops v1.16.0._
