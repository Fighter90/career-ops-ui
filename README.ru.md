...
> **🆕 Последний релиз — v1.97.0**
>
> **Источник сканера Dassault Systèmes + улучшение качества.** Паритет с родительским career-ops (#1498): новый zero-token-источник сканирования **Dassault Systèmes** (публичная лента карточного поиска Exalead за 3ds.com/careers) присоединяется к реестру как **46-й адаптер** — выбирается по провайдеру (`provider: dassault`) или автоматически определяется по хосту 3ds.com, с привязкой хоста для защиты от SSRF. Вместе с ним аудит по трём направлениям (сервер + SPA + перевод на 16 языков) исправил реальные дефекты: **асинхронную гонку вкладок статистики** (рендер медленной вкладки мог затереть более новую), **лимит размера в `safe-fetch`, который мог зависнуть** при превышающем лимит запросе предпросмотра/пайплайна, мёртвое логирование активности SSE, два подтверждения удаления с пустым телом и непереведённые значения словаря (uk/ru/it). Перенесены исправления надёжности из родительского проекта для источников Avature / Get on Board / SuccessFactors.
>
> _16 locales · 6 LLM-провайдеров · 46 адаптеров сканера · карьерная ориентация · план карьеры · переработка статистики · слой памяти · CV Studio · планировщик нетворкинга · mock interview · рыночное соответствие через two-pager · паритет с родительским career-ops v1.16.0._
