...
> **🆕 Последний релиз — v1.94.0**
>
> **Статистика, переработана (Epic 25):** страница **`#/stats`** теперь представляет собой раздел **Статистика** с тремя вкладками, с настоящими графиками и куда большим объёмом данных. Новая вкладка **Отчёт по рынку** генерирует AI-анализ зарплат и рынка труда по вашим целевым ролям — по грейдам с процентилями P10–P90, ведущие работодатели, востребованные навыки, бенефиты, распределение удалёнки, тренды и рекомендации по переговорам, в выбранных вами регионе и **валюте** (цифры помечены как ориентировочные оценки модели, а не собранные данные). Новая вкладка **Мой pipeline** строит графики по вашему собственному трекеру (баллы, воронка, ведущие компании/роли, таймлайн, коэффициенты конверсии). Каждый отчёт экспортируется в **Markdown или PDF**. Основан на v1.93.0 (слой памяти) и v1.92.0 (CV Studio).
>
> _переработка статистики · 16 locales · 6 LLM-провайдеров · 45 адаптеров сканера · слой памяти · CV Studio · планировщик нетворкинга · mock interview · рыночное соответствие через two-pager · детектор повторных публикаций · паритет с родительским career-ops v1.16.0._
