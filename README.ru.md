...
[![tests](https://img.shields.io/badge/tests-1844%20passed-brightgreen)](#тесты)
...
```bash
npm test                       # 1844 unit/integration-теста
...
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **1844** | Каждый эндпоинт, эфемерный сервер, без сети. Включая парсеры, сканер (mocked), runner, anthropic, заголовки безопасности, XSS, sanitize JD, валидацию URL, защиту от DNS-rebind, состояние гонки на трекере, rate-limit, path-traversal, паритет i18n. |
...
- Перед push выполняйте `npm test` — **1844 проверок green** — это минимальная планка (плюс 90 Playwright, если изменения касаются UI).
