...
[![tests](https://img.shields.io/badge/tests-3164%20passed-brightgreen)](#тесты)
...
> **🆕 Последний релиз — v1.236.0** — **Паритет с родителем, career-ops `main` @ `6a9c84c`: два новых источника и два исправления, которые незаметно теряли вакансии.** Вакансия **Workday**, открытая в нескольких городах, отдаёт *счётчик* — `"53 Locations"` — там, где любая другая вакансия несёт место, поэтому она не совпадала ни с одной записью `allow: [austin]` и отбрасывалась (53 из 291 вакансии выше по течению). Её настоящие места теперь берутся из CXS-документа detail, с потолком в 200 запросов на запись. **`role-matcher`** вообще не имел обработки уровня, поэтому `"Insurance Specialist"` и `"Insurance Specialist II"` схлопывались в один репост — перенесён весь механизм родителя целиком, со свёрткой римских и арабских цифр. Доска **Avature**, закреплённая за фильтром, обходила всю глобальную доску, потому что строка запроса отбрасывалась при пересборке URL поиска. Новые источники: **Python.org Jobs** и **Generalist World** — **92 → 94** (89 EN + 5 RU). Плюс `devalue` 5.8.1 → 5.9.2, закрывающий открытое уведомление Dependabot. **3164 тест · 116 браузерных.**
