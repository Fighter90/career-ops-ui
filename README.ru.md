...
> **🆕 Последний релиз — v1.100.0**
>
> **ИИ-автозаполнение two-pager + экспорт.** Two-pager (`#/two-pager`) теперь сам заполняется из вашего резюме + профиля через настроенный провайдер (ничего не выдумывая; при отсутствии ключа — откат на копирование промпта), добавляет вид **Просмотр и экспорт** и получает экспорт в **PDF / DOCX / Markdown** — DOCX через новый генератор `.docx` без зависимостей, также подключённый к отчётам по рынку, карьерному плану и профориентации.
>
> _автозаполнение two-pager · экспорт в DOCX · здоровье порталов · встроенный репортёр багов · 16 locales · 6 LLM-провайдеров · 46 адаптеров сканера · карьерная ориентация · план карьеры · переработка статистики · слой памяти · CV Studio · планировщик нетворкинга · mock interview · рыночное соответствие через two-pager · паритет с родительским career-ops v1.16.0._
