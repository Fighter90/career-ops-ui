...
[![tests](https://img.shields.io/badge/tests-2602%20passed-brightgreen)](#тесты)
...
> **🆕 Последний релиз — v1.204.0** — **«Доктор настройки» в Настройках** — проверка без токенов `cv.md` + `config/profile.yml`, перечисляющая блокирующие проблемы (отсутствующие файлы/поля) и предупреждения (демо-данные, зашитые метрики), чтобы исправить неполную настройку до того, как она ослабит сканы и адаптацию. **2602 тестов.**
>
