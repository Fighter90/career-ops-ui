...
[![tests](https://img.shields.io/badge/tests-2360%20passed-brightgreen)](#тесты)
...
> **🆕 Последний релиз — v1.139.0** — **понятность: подсказки `?`** — переиспользуемая CSP-безопасная `?` рядом с заголовками, 5 вкладками `#/stats` и 8 заголовками ИИ/аналитических экранов открывает локализованное всплывающее окно «что это / как работает / чего ожидать» (доступно, RTL, зависит от темы). 14 новых ключей ×17 локалей. **2360 тестов.**
>
