...
[![tests](https://img.shields.io/badge/tests-1845%20passed-brightgreen)](#тесты)
...
career-ops — первая референсная реализация [Манифеста CareerOps](https://career-ops.org/manifesto?utm_source=career-ops-ui) — практики ведения поиска работы с доказательствами, дисциплиной и инструментами на стороне кандидата за столом переговоров. Прочитайте его. Если он говорит то, во что вы верите, подпишите — ваша подпись становится коммитом. Приложение ссылается на него из футера боковой панели.
...
```bash
npm test                       # 1845 unit/integration-теста
...
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **1845** | Каждый эндпоинт, эфемерный сервер, без сети. Включая парсеры, сканер (mocked), runner, anthropic, заголовки безопасности, XSS, sanitize JD, валидацию URL, защиту от DNS-rebind, состояние гонки на трекере, rate-limit, path-traversal, паритет i18n. |
...
- Перед push выполняйте `npm test` — **1845 проверок green** — это минимальная планка (плюс 90 Playwright, если изменения касаются UI).
