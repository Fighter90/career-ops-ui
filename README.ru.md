...
[![tests](https://img.shields.io/badge/tests-2837%20passed-brightgreen)](#тесты)
...
> **🆕 Последний релиз — v1.227.0** — **Гонконг и точнее фоллоу-апы** — JobsDB Гонконг присоединяется к Jobstreet/SEEK (теперь на API v5), на доске каденции появился фильтр «только срочные», а нераскрытые работодатели показывают агентство-посредника — паритет с 1.30.0 родительского проекта. **2837 тестов.**
