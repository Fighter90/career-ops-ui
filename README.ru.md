...
[![tests](https://img.shields.io/badge/tests-2356%20passed-brightgreen)](#тесты)
...
> **🆕 Последний релиз — v1.138.0** — **генерация на языке интерфейса** — каждая AI-генерация (карьерный план, ориентация, отчёт по рынку, мок-интервью, нетворкинг, «спросить документацию», заметка памяти, two-pager) теперь отвечает на выбранном в интерфейсе языке, а не всегда на английском; плюс усиление тестов по итогам ревью (канарей CSS-ролей цвета и самопроверка XSS-загрузчика `UI.md()`). **2356 тестов.**
>
