...
> **🆕 Последний релиз — v1.85.0**
>
> **Немецкая, итальянская и турецкая локали** (паритет локалей с родительским career-ops v1.16.0): интерфейс теперь доступен на **16 языках** — Deutsch 🇩🇪, Italiano 🇮🇹 и Türkçe 🇹🇷 присоединяются к 13 существующим, с полным переводом встроенной справки, README и CHANGELOG, а также локализованными заготовками LLM-промптов. Основан на v1.84.0 (кулдаун повторных заявок + компенсация), v1.83.0 (детектор повторных публикаций) и v1.82.0 (NoDesk).
>
> _16 locales · 6 LLM-провайдеров · 41 адаптер сканера · детектор повторных публикаций · кулдаун повторных заявок · паритет с родительским career-ops v1.16.0._
