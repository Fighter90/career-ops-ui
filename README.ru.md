...
[![tests](https://img.shields.io/badge/tests-2909%20passed-brightgreen)](#тесты)
...
> **🆕 Последний релиз — v1.228.5** — **Сообщаемая версия описывала файл на диске, а не исполняемый код.** `/api/health` перечитывал `package.json` на каждый запрос, поэтому процесс, запущенный до развёртывания, рапортовал версию нового файла, обслуживая старый код — QA поймал локальный инстанс, отвечавший `1.228.4` и дававший 404 на маршрут из 1.228.3. Именно так выглядит успешным развёртывание, которое скопировало файлы и не перезапустило процесс. Версия теперь фиксируется при загрузке модуля: устаревший процесс сообщит **старую** версию, то есть правду. **2909 тестов.**
...
```bash
npm test                       # 2909 unit/integration-теста
...
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **2909** | Каждый эндпоинт, эфемерный сервер, без сети. Включая парсеры, сканер (mocked), runner, anthropic, заголовки безопасности, XSS, sanitize JD, валидацию URL, защиту от DNS-rebind, состояние гонки на трекере, rate-limit, path-traversal, паритет i18n. |
...
| **Итого** (все четыре набора)                   | **3040** | **0 fails, 0 flakes**                                                                                   |
...
- Перед push выполняйте `npm test` — **2909 проверок green** — это минимальная планка (плюс 101 Playwright, если изменения касаются UI).
