...
[![tests](https://img.shields.io/badge/tests-3201%20passed-brightgreen)](#тесты)
...
> **🆕 Последний релиз — v1.237.0** — **Паритет с родителем, career-ops `main` @ `93c4302b`: четыре зеркальных исправления, два из которых незаметно теряли вакансии здесь.** **Jobstreet / SEEK** строил каждый URL детали как `/id/job/<id>` — но `/id/` это *индонезийский языковой префикс*, и любой другой хост SEEK отвечает на него **404**, поэтому каждая возвращённая нами австралийская, новозеландская, сингапурская, малайзийская и гонконгская вакансия вела на мёртвую страницу; теперь путь определяется хостом. **Oracle Cloud** обрывал обход на короткой странице, но ORC отдаёт короткие страницы *в середине списка* — American Express сообщает **454** вакансии и отдаёт их страницами 200 / 199 / 54, поэтому последние **54 вакансии, 12% доски**, так и не были получены. **Liveness** пропускал `"This role is closed"` (111 из 111 неопределённых вакансий на одной доске) и теперь защищается от составного `closed-loop`; `job expired` переехал в мягкий уровень, который больше не может перебить видимую кнопку отклика — ложное истечение записывается в историю сканирования и исключает настоящую вакансию из *каждого* последующего скана. Плюс опциональное закрепление тенанта **Personio** через `personio: <slug>` для встроенных в iframe досок, и Jobstreet `appendWorkType`. Число источников не изменилось — **94** источника (89 EN + 5 RU). **3201 тест · 116 браузерных.**
...
Само это приложение тоже построено с помощью агентов кодирования — по [Паттернам агентного кодинга](https://mokevnin.github.io/agentic-coding-design-patterns/ru/) (*Agentic Coding Design Patterns*; [английское издание](https://mokevnin.github.io/agentic-coding-design-patterns/en/)) — исполняемые ограждения, упакованные рабочие процессы, словарь предметной области, ADR и оценки рабочих процессов. Страница вики [Agentic Practices](https://github.com/Fighter90/career-ops-ui/wiki/Agentic-Practices) фиксирует, какие из этих практик применяются на самом деле.
...
- **Telegram-каналы (v1.228.0)** — любой **публичный** канал: перечислите его в блоке `telegram_channels:`, и он читается через веб-превью `t.me/s/<канал>` — без токена бота и без API-ключа. RSS у Telegram нет, а Bot API не читает канал, которым бот не управляет, так что превью — единственный путь. **Пост канала — это проза, а не запись о вакансии**: заголовок берётся из первой содержательной строки, компания и локация — только из явных меток, а отделять настоящие вакансии от рекламы — работа вашего `title_filter`. Это подсказки, а не список.
...
## 📘 Как этот репозиторий строится с помощью агентов

Большая часть этого проекта написана агентами для кодинга. Это практика со своими характерными сбоями, поэтому она ведётся осознанно — по книге [«Паттерны агентного кодинга»](https://mokevnin.github.io/agentic-coding-design-patterns/ru/) (*Agentic Coding Design Patterns*; [английское издание](https://mokevnin.github.io/agentic-coding-design-patterns/en/)) Кирилла Мокевнина (Kirill Mokevnin). Файлы ниже — эта практика, воплощённая в жизнь. Ни один из них не влияет на работающее приложение — они существуют для того, чтобы новая сессия продолжала с того места, где остановилась предыдущая, а не заново выводила, заново решала и заново ломала одно и то же.
...
| **[`evals/workflow/`](evals/workflow/)** | Фиксированный набор задач с грейдерами, которые проверяют поведение *конвейера*, а не продукта. | До и после изменения скилла, промпта или инструмента. |
| **[`.claude/skills/`](.claude/skills/)** | Упакованные workflow. `parent-sync` проводит целый релиз паритета за девять контролируемых фаз. | Вместо импровизации релиза. |
| **[`.githooks/pre-commit`](.githooks/pre-commit)** | Исполняемые ограждения — детерминированный минимум, который жёстко валит сборку, плюс необязательный ИИ-слой, который никогда не блокирует. | Он запускается сам. |

### Как ими пользоваться

Документы не требуют настройки — это обычный Markdown, его читаете и вы, и агент. Две вещи подключены, и о них стоит знать:

```bash
# Ограждения: хуки принадлежат репозиторию, поэтому проверки применяются ко всем, а не только к вашей машине.
git config core.hooksPath .githooks

# Оценки workflow: сверяют состояние репозитория с правилами из docs/adr/ и PROGRESS.md.
node evals/workflow/run.mjs              # все грейдеры
node evals/workflow/run.mjs --task qa-prompt-mandatory
```

Каждый грейдер в `evals/workflow/tasks.yml` соответствует ошибке, которая однажды всё же попала в релиз — затёртой исторической атрибуции, сборке сайта, запущенной посреди фан-аута, пропущенному запрету на порт. Результат и траектория оцениваются отдельно, а грейдер, который нельзя решить по репозиторию, сообщает `SKIP`, а не проходит молча: проверка, которая не может выполниться, никогда не должна выглядеть зелёной.

### Как поддерживать их актуальность

Устаревший словарь хуже, чем его отсутствие, поэтому работа над этими файлами — часть самого релиза: `parent-sync`, фаза 0, загружает их, а фаза 8 возвращает в них то, что было узнано. Добавляйте термин в `CONTEXT.md` тем же коммитом, что вводит понятие; открывайте ADR, когда решение будет выглядеть произвольным через шесть месяцев; фиксируйте отвергнутый подход в `PROGRESS.md` в тот момент, когда его отвергли.

Какие паттерны из книги здесь реально реализованы, а какие нет, фиксируется на странице вики [Agentic Practices](https://github.com/Fighter90/career-ops-ui/wiki/Agentic-Practices).
