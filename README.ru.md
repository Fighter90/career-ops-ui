...
> **🆕 Последний релиз — v1.97.1**
>
> **Усиление по итогам ревью и паритет документации.** Продолжение v1.97.0: значок соответствия `◎` при сканировании (`fit-score.js`) больше не превращает ставку меньше годовой («500 EUR/day») в фиктивный годовой минимум зарплаты, а сопоставление стран теперь идёт по целому слову, поэтому «Germany» не может совпасть с прилагательным «German». Каждый локализованный README и встроенная справка теперь единообразно заявляют **16 локалей** и **46 адаптеров сканера** (часть текста всё ещё держалась на счётчиках до v1.85). В v1.97.0 добавлен источник сканирования **Dassault Systèmes** (46-й адаптер, zero-token Exalead XML, с привязкой хоста от SSRF) и аудит по трём направлениям (сервер / SPA / 16 языков).
>
