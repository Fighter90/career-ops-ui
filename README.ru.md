...
> **🆕 Последний релиз — v1.93.0**
>
> **Слой памяти (Epic 24):** новая страница **`#/memory`** хранит короткую редактируемую заметку «запомни это обо мне». Поскольку она встраивается в общий контекст проекта, она направляет **every** AI-запрос — оценка, mock interview, нетворкинг, CV Studio — по всем **all** провайдерам. Она фиксирует ваши предпочтения и стиль работы (никогда не новые факты о вас), а помощник **предложить из ваших данных** набрасывает поведенческие пункты из вашего трекера для просмотра. Основан на v1.92.0 (CV Studio), v1.91.0 (планировщик нетворкинга) и v1.90.0 (mock interview).
>
> _16 locales · 6 LLM-провайдеров · 45 адаптеров сканера · слой памяти · CV Studio · планировщик нетворкинга · mock interview · рыночное соответствие через two-pager · рыночная статистика по целевым ролям · детектор повторных публикаций · паритет с родительским career-ops v1.16.0._
