...
> **🆕 Последний релиз — v1.131.2**
>
> **Разделение `app.css` (рефакторинг).** v1.131.2 закрывает техдолг по контракту размера файла: `app.css` объёмом ~1990 строк разделён на три упорядоченных таблицы стилей — `app.css` (база/хром/кнопки), **`components.css`** (карточки, таблицы, формы, бейджи, пагинатор, …), **`overlays.css`** (toast, модалка, шторка, `[dir="rtl"]`, docs-fab, usage-hud) — каждая в пределах жёсткого лимита в 800 строк. Разрез непрерывный и в исходном порядке, поэтому каскад **побайтово идентичен**; `index.html` подключает все три как упорядоченные `<link>`. Без изменений поведения/разметки/i18n; проверено вживую в браузере (все три таблицы стилей парсятся и применяются, 0 ошибок в консоли). **2138 тестов**. **Харденинг host-pinning адаптеров (патч).** v1.131.1 закрывает доработки по code-review на источниках v1.130.0 (defense-in-depth, поведение для валидных входных данных не меняется): адаптер **a16z Speedrun** теперь повторно валидирует override эндпоинта (HTTPS + точное совпадение хоста) и откатывается на канонический фид — паритет с **Cryptocurrency Jobs** — а `cleanUrl` парсера Cryptocurrency Jobs использует тот же гард точного совпадения хоста, что и его SSRF-проверка (было `endsWith`, который принимал поддомены), так что парсер никогда не более permissive, чем гард. **2135 тестов**. **CRM-доска со вкладками стадий.** v1.131.0 портирует вид `/pipeline` веб-приложения родителя на `#/tracker`: полоска funnel-чипов и дропдаун статуса становятся **полосой вкладок стадий** — **All** плюс по одной вкладке на каждый канонический статус (Evaluated · Applied · Responded · Interview · Offer · Rejected · Discarded · SKIP · Hired), каждая с живым счётчиком, **включая стадии с нулевым счётчиком**, чтобы вся воронка всегда была видна. Вкладки берутся из нового read-only `GET /api/tracker/stages` (канонические метки + свёртка алиасов из `templates/states.yml` — клиент никогда не хардкодит whitelist), строки получают опциональные брендовые логотипы, а accessible-имена несут счётчики. **2133 теста**. **Паритет с родителем v1.24.0.** v1.130.0 портирует два новых источника сканирования — **a16z Speedrun** (общедоступный фид всей доски talent-network a16z Speedrun) и **Cryptocurrency Jobs** (доска Web3 `cryptocurrencyjobs.co`, через её публичный RSS) — доводя реестр до **72 источников (67 английских + 5 русских)** и показывая оба в фильтре Source на `#/scan` и на лендинге cvstart.org. Также зеркалит два родительских фикса: **echojobs** теперь сохраняет **гибридные** вакансии отличимыми от удалённых (`Город · Hybrid`, никогда не схлопывается в `Remote`), а **radancy** учится устаревшей разметке TalentBrew плюс транспорту JSON results-фрагмента. **2123 теста**. **Доработки по AI-ревью (патч).** v1.129.1 чинит адвайзори-находки портов web из v1.128/v1.129: порядок грейда в job-facets (`Senior Engineering Manager` → `senior`, не `lead`), `states.mjs` больше не пиннит fallback при сбое родителя на буте (+warn на битом файле), тон показывает строку без оценки нейтральной (не красной), а `domainFromName` пропускает не-ASCII slug. **Фасет «Грейд» + колонка «Свежесть» в скане.** v1.129.0 вплетает библиотеку `job-facets.js` из v1.128.0 в `#/scan`: новый фильтр **Грейд** (lead/staff/senior/mid/junior/intern, автозаполнение из результатов как у фасета «Страна»), колонка-бейдж «Грейд» и zero-token колонка **Свежесть** (`сегодня` / `Nдн`). **Четыре порта из веб-приложения родителя.** v1.128.0 переносит на ванильный JS четыре решения из Next.js-приложения career-ops: трекер читает канонические статусы вживую из `templates/states.yml` (без хардкод-whitelist), бренд-логотипы теперь на ATS-хостованных строках через карту имя→домен, более точный 4-уровневый тон оценки и zero-token библиотека фасетов. **2066 тестов**. **Паритет родителя v1.23.0.** v1.127.0 портирует три новых источника — **Flowxtra**, **VDAB**, **iCIMS** (в реестре теперь **70 адаптеров**, 65 EN + 5 RU) — зеркалит переход agenticjobs на REST, восстановление города в Greenhouse и фиксы role-matcher, и возвращает **Cursor** в ростер CLI (parent #2115 — `cli-detect` сообщает 10 инструментов). **Патч дрейфа ростера CLI.** v1.126.1 исправляет две точки, пропущенные свипом v1.126.0: интро вкладки API keys в `#/config` (i18n ×17) теперь перечисляет Antigravity + Grok Build, а устаревшая строка `Cursor / Gemini CLI` в help-гайде ×17 (и в help сайта) теперь содержит полный ростер из 8 CLI. **Ресинк доков и ростера CLI.** v1.126.0 сверяет каждую поверхность доков (help ×17, README ×17, wiki) с живой career-ops.org/docs (прочитаны все 31 страница) и учит скан AI CLI tools в `#/config` детектить все 8 первоклассных CLI — **Grok Build CLI** и **Kimi CLI** присоединяются к Claude Code, Cursor, Codex, OpenCode, Antigravity, Qwen и Copilot (теперь **9 инструментов**, по-прежнему без запуска). **Сервисный синк.** v1.125.4 сводит dependabot-бампы site (`sharp` 0.35.3, `svgo` 4.0.2, `fast-uri` 3.1.4) и фиксирует парити-свип родителя (после v1.22.0, #2108–#2168): guard неверной строки в set-status, Risk Summary локализованных режимов и проверка манифеста апдейтера — всё на стороне CLI, портировать нечего (**1969 тестов** без изменений). **Фикс локали промптов.** v1.125.3 делает датский и хинди полноправными во всех AI-промптах — deep research (живой и ручной), режимы, оценка, интервью, нетворкинг и CV Studio теперь выдают директиву `# Output language` в этих двух локалях (раньше ответы приходили по-английски), а гейт локалей обходит все 17 (**1969 тестов**). **Пакет внешних контрибьюторов.** v1.125.2 вливает первые PR сообщества от [@Alien10140](https://github.com/Alien10140) — фикс HTTP 502 в живом Deep research (headless-промпты без инструментов для API-провайдеров) и обновление дефолтов Gemini с устаревшей `gemini-2.0-flash` на `gemini-3.6-flash`, закреплённое новым анти-дрейф гейтом из 5 тестов (**1957 тестов**). v1.125.1 сохраняет мультибрендовым SuccessFactors RMK-тенантам их брендовый путь (родительский #2099). **Источники вакансий на лендинге.** v1.125.0 добавляет на cvstart.org раздел, перечисляющий все **70 источников сканирования** в виде кликабельных чипов — синхронизированный с реестром при сборке и защищённый от рассинхронизации. До этого v1.124.0 добавляет **пять источников сканирования** — Welcome to the Jungle, Agentic Engineering Jobs, Jobvite, Gem и Alibaba Group (теперь **70 адаптеров**) — а также проверку полностью удалённой работы для Arbeitsagentur (`homeofficetyp=VOLLSTAENDIG`) и исправление публичных URL вакансий в SmartRecruiters.
>
> _разделение app.css (3 таблицы стилей) · погашение техдолга по контракту размера файла · побайтово идентичный каскад · харденинг host-pinning адаптеров · повторная валидация override a16z · cryptocurrencyjobs cleanUrl точный хост · CRM-доска со вкладками стадий · порт /pipeline трекера · вкладки стадий + счётчики · GET /api/tracker/stages · брендовые логотипы трекера · паритет родителя v1.24 · +2 источника (a16z Speedrun/Cryptocurrency Jobs) · 72 источника · echojobs hybrid · radancy TalentBrew · патч AI-ревью · порядок грейда · fallback states · нейтральный тон · фасет грейда · колонка свежести · job-facets вплетена · 4 порта web/ · states.yml вживую · логотипы имя→домен · тон оценки · фасеты · паритет родителя v1.23 · +3 источника · 70 адаптеров · Cursor · патч дрейфа ростера CLI · ресинк доков и ростера CLI · сервисный синк · фикс промпт-локалей da/hi · пакет внешних контрибьюторов · фикс 502 в deep research · дефолты Gemini 3.6 · фикс брендового пути SF · раздел источников на лендинге · 5 источников · 67 адаптеров · паритет с родителем v1.22 · Oracle Cloud · 62 адаптера · паритет с родителем v1.21 · хинди · 17 языков · манифест · страницы methodology/license/changelog на cvstart.org · паритет с родителем v1.19 · 61 адаптер · живые звёзды + контрибьюторы на лендинге · паритет с родителем v1.18 · 54 адаптера · статус Hired · статистика за всё время · пакет паритета · 50 адаптеров · переработка счётчика расхода · шлифовка дизайна · счётчик использования · плавающий помощник справки · консолидация доков и QA · закрытие бэклога безопасности · докум. и QA ×16 · Исключить в Scan · обзор пайплайна · усиление безопасности 2 · усиление санитайзера · усиление безопасности · расход ИИ · логотипы компаний · инструменты CLI для ИИ · спросить справку · адаптация CV + письмо · автозаполнение two-pager · экспорт в DOCX · здоровье порталов · встроенный репортёр багов · 16 locales · 6 LLM-провайдеров · 46 адаптеров сканера · карьерная ориентация · план карьеры · переработка статистики · слой памяти · CV Studio · планировщик нетворкинга · mock interview · рыночное соответствие через two-pager · паритет с родительским career-ops v1.16.0._
