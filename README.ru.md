...
[![tests](https://img.shields.io/badge/tests-2908%20passed-brightgreen)](#тесты)
...
> **🆕 Последний релиз — v1.228.4** — **Проба живости без авторизации обращалась к диску на каждый запрос.** `GET /api/ping` читал и разбирал `package.json` при каждом обращении — а это единственная точка, доступная без учётных данных, то есть рычаг отказа в обслуживании, выданный кому угодно (CodeQL: отсутствие ограничения частоты). Версия читается один раз при регистрации, ввода-вывода в обработчике нет. Ограничение частоты урезало бы ущерб; удаление работы убирает рычаг. Плюс: маскирование имени владельца профиля вне loopback закреплено тестом. **2908 тестов.**
...
```bash
npm test                       # 2908 unit/integration-теста
...
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **2908** | Каждый эндпоинт, эфемерный сервер, без сети. Включая парсеры, сканер (mocked), runner, anthropic, заголовки безопасности, XSS, sanitize JD, валидацию URL, защиту от DNS-rebind, состояние гонки на трекере, rate-limit, path-traversal, паритет i18n. |
...
- Перед push выполняйте `npm test` — **2908 проверок green** — это минимальная планка (плюс 101 Playwright, если изменения касаются UI).
