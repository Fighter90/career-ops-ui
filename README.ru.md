...
> **🆕 Последний релиз — v1.89.0**
>
> **Соответствие кандидата рынку — two-pager (Epic 14):** новая страница **`#/two-pager`** фиксирует то, что _вы_ на самом деле хотите от следующей роли (что вам нравится, обязательные условия, что вы ненавидите, deal-breaker'ы, непереговариваемое) в формате «two-pager Мнукина». **ИИ-ассистент заполнения** составляет её на основе вашего CV + профиля, каждая вакансия на `#/scan` получает оценку **`◎` соответствия-тому-что-вы-хотите**, а two-pager питает каждую оценку. Основан на v1.88.0 (i18n для Scan + гигиена API), v1.87.0 (4 новых провайдера сканирования) и v1.86.0 (статистика по целевым ролям).
>
> _16 locales · 6 LLM-провайдеров · 45 адаптеров сканера · рыночное соответствие через two-pager · рыночная статистика по целевым ролям · детектор повторных публикаций · паритет с родительским career-ops v1.16.0._
