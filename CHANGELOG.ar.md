...
**تغيير (منخفض، الماسح) — نقل تكافؤ Getro (الأصل #2640): الراتب، جميع المواقع، واكتشاف العمل عن بُعد عبر `work_mode`.**

### تغيير
- نُقل النصف الآمن من امتداد `providers/getro.mjs` في الأصل (#2640) إلى `server/lib/sources/getro.mjs`. تحمل وظيفة Getro الآن سلسلة **راتب** (من `compensation_amount_min/max_cents` + العملة، سنويًا فقط — بمحاكاة اصطلاح السلسلة في remotli/lever الذي يعيد مرشّح الراتب في العميل تحليله)، ويجمع **موقعها** **كل** `locations` (كان الأول فقط)، ويُعترف بـ `work_mode: 'remote'` كإشارة عمل عن بُعد إلى جانب علم `remote` / نص الموقع الموجودين. تخطيط بيانات خالص من استجابة الـ API التي يجلبها web-ui أصلاً — بلا مضيف جديد وبلا تغيير في سطح SSRF.

### ملاحظات
- لم يُنقل عمدًا: **الحل التلقائي** لـ `collection_id` في الأصل (يجلب `careers_url` الخاص باللوحة — مضيف عشوائي — فهو يخص تغييرًا منفصلاً يمرّ عبر حدود SSRF عبر `safeGet` في web-ui) والحد الأقصى المرفوع إلى 1500 صفحة (يحتفظ web-ui بحدّ 200 صفحة / 4000 وظيفة لعمليات المسح المُطلَقة من المتصفح). الالتزامات الأصلية الأخرى الـ 11 في هذه المزامنة ليس لها مرآة في web-ui. الاختبارات: `tests/sources-getro.test.mjs` (+5). المجموعة: **2470** (+5).
