...
test('index.html sidebar footer carries the manifesto link', () => {
  ...
  assert.ok(html.includes(`${MANIFESTO_URL}?utm_source=career-ops-ui`), 'manifesto URL missing');
...
test('every help bundle has the §29 manifesto section with the manifesto URL', () => {
  for (const lang of HELP_BUNDLES) {
    ...
    assert.ok(text.includes(MANIFESTO_URL), `${lang}.md missing manifesto URL`);
  }
});
...
test('README (en) explains the manifesto', () => {
  ...
  assert.ok(text.includes(MANIFESTO_URL), 'README manifesto URL missing');
});
...
test('cvstart.org footer links to the manifesto in every site locale file', () => {
  ...
  assert.ok(footer.includes(`${MANIFESTO_URL}?utm_source=cvstart.org`), 'Footer.astro manifesto link missing');
