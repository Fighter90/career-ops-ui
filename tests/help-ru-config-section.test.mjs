...
test('every help-bundle keeps the 21-H2 parity contract after v1.29.1 edit', () => {
  // Belt-and-suspenders: the v1.29.1 expansion is a ### subsection of
  // §5 — H2 count stays at 21 (v1.60.0 added §19 Localizing the app; v1.86.0 added §20 Statistics by target roles; v1.89.0 added §21 Your two-pager). If a future change accidentally
  ...
  assert.equal(baseline, 21, `expected 21 H2 sections, got ${baseline}`);
});
...
test('WS10: every help-bundle has identical H3 parity (en + 7 locales)', () => {
  ...
  assert.equal(baseline, 81, `expected 81 H3 subsections per bundle, got ${baseline}`); // v1.58.35 §18 added 3 H3s; v1.62.x §5 added "rss (RSS / Atom boards)"; v1.64.0 §7 added "Scanning hh.ru from outside Russia"; v1.86.0 §20 Statistics added 3 H3s; v1.89.0 §21 Your two-pager added 3 H3s
});
