...
**记忆层(Epic 24)。** 全新的 `#/memory` 页面保存一条简短、可编辑的「关于我请记住这些」的备注,助手会在 **every** 任务中将其牢记于心:

- **一条备注,处处生效** — 由于它被内联进 `bundleProjectContext`,该备注会自动抵达每一次 AI 请求(评估、模拟面试、人脉拓展、CV Studio),并横跨 **all** 提供商。写一次,它就会引导一切。
