...
>
> **Repo state.** v1.229.0 (2026-09-03 — **Four scanner sources ported from parent career-ops v1.31.0**: Built In, Feishu Jobs, Garena and MokaHR, taking the registry 86 → 90 sources (85 EN + 5 RU) and `ALL_ADAPTERS` 81 → 85. All zero-token. Built In ships no default query and refuses an unlisted market rather than quietly scanning the national board; MokaHR returns an AES-128-CBC envelope whose key ships alongside it, so decryption is part of reading the board. Also fixed: htmlToText could leak an incomplete tag opener into plain text.) 2956 unit · 101 Playwright · 21 smoke · 23 comprehensive.
