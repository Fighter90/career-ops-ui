...
>
> **Repo state.** v1.231.0 (2026-09-05 — **Two scanner sources ported from parent career-ops v1.32.0**: Collage (per-tenant; the job-site address is never guessed, because a guessed one scans somebody else's board) and Telegram (strict) — the same t.me previews web-ui's existing `telegram` source reads, but a post becomes a row only when it names an employer AND links to a vacancy. Both ship, as in the parent. Gem gained an opt-in REST mode. Fixed in the parent's own code while porting: the strict reader's LOCATIONISH_RE used ASCII `\b`, so it could not see Cyrillic and returned "Москва" as an EMPLOYER; web-ui uses Unicode lookarounds. Registry 90 → 92 sources / 87 adapters.) 3009 unit · 101 Playwright · 21 smoke · 23 comprehensive.
