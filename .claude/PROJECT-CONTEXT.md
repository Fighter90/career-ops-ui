...
>
> **Repo state.** v1.232.1 (2026-09-10 — **CONFIG-2: Save on #/config wrote fields the user never touched.** v1.57.1 seeds an unset control with its defaultValue for display; `save()` consulted `dirty` for SECRETS ONLY and sent every other field regardless — so the display value travelled as a user choice. One dropdown change posted 28 keys and pinned 18 nobody opened; changing nothing posted 27. Harm is deferred, not immediate: each setting moves from 'unset, follow the project default' to 'pinned in .env', so a later default change silently never reaches anyone who pressed Save. THE COMPARISON BASIS IS THE POINT: comparing to loaded `cfg.values` is NOT enough — an unset select shows its default while stored is '', so they differ honestly and all 17 model dropdowns still get written (27 -> 19 keys, not 1). Save now compares against what each control was SEEDED with, recorded in an `initial` map at build time. `dirty` is not a valid basis either — it fires on type-and-revert. Also: the toast's key count lived in t()'s FALLBACK arg while `config.saved` exists in all 17 dicts, so the number never rendered. Counts unchanged: 92 sources / 87 EN adapters.) 3018 unit · 111 Playwright · 21 smoke · 23 comprehensive.
