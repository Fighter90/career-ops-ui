...
>
> **Repo state.** v1.228.5 (2026-08-30 — **The reported version described the file on disk, not the running code.** `/api/health` re-read package.json per request, so a process started before a deploy reported the new file’s version while serving the old code; it is captured at module load now, so a stale process reports the OLD version. Earlier in the same series: Telegram channels as a scan source (86 sources, 81 EN + 5 RU), a paged `/api/scan-results` whose `?q=` matches terms independently rather than as one phrase, a docs-assistant that could not answer in English because one oversized section emptied the context, and `GET /api/ping` — a liveness probe that leaks nothing.) 2909 unit · 101 Playwright · 21 smoke · 23 comprehensive.
