...
>
> **Repo state.** v1.232.0 (2026-09-09 — two defects from a browser QA pass, neither from v1.231.5. **PROFILE-1:** `#/profile` scrolled the page sideways — the card value was an anonymous div with no wrapping rule, and a LinkedIn URL gives CSS no break point, so its min-content width was the whole string and `min-width:auto` on grid items refused to shrink below it; 23px of real scroll at 1280px. Fixed by a shared `.card-value` with `overflow-wrap: anywhere` (NOT `break-word` — only `anywhere` counts toward min-content) plus `.card-row > * {min-width:0}`. **CONFIG-1:** `POST /api/config` accepted any select value; `LLM_PROVIDER=not-a-provider` returned 200, was written to .env, then failed SILENTLY as the resolver fell back. `LLM_PROVIDERS` had been exported from env-config.mjs all along — the validator never consulted it. Only the provider is enforced: model lists belong to the vendor and move between releases (our own suites use `gemini-2.0-flash`, dropped from the curated list). An already-stored value is grandfathered — the form resends every non-secret field on every Save, so a strict check froze the page; a browser test caught it. `OLLAMA_API_KEY` gained a UI descriptor; server keys and UI descriptors are both 45 and gated. Counts unchanged: 92 sources / 87 EN adapters.) 3018 unit · 103 Playwright · 21 smoke · 23 comprehensive.
