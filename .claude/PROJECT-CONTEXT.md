...
>
> **Repo state.** v1.231.1 (2026-09-06 — **v1.231.0's count sweep rewrote the account name in every link.** The `90 → 92` source-count sweep matched the 90 inside `Fighter90`, where the digit boundary holds because the character before is a letter and the token simply ends: 562 links across READMEs, help bundles and site mirrors, plus 138 in the wiki, pointed at an account that does not exist. `85 → 87` likewise turned `v1.85.0` into a `v1.87.0` that was never released. A digit boundary guards against neighbouring DIGITS, not against being part of an IDENTIFIER or a VERSION STRING. Counts themselves unchanged: 92 sources / 87 EN adapters.) 3009 unit · 101 Playwright · 21 smoke · 23 comprehensive.
