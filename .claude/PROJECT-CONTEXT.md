...
>
> **Repo state.** v1.233.0 (2026-09-10 — three findings in the #/config save path. **CONFIG-3, caused by v1.232.1's own fix:** a deliberate choice equal to the seeded value was discarded as untouched. `initial` conflates seeded-from-.env (equal = not edited, skip is right) with seeded-from-defaultValue (equal is AMBIGUOUS). With HOST absent the field shows 127.0.0.1; typing that exact value posted {} and the key stayed absent — the only route was wrong value, save, right value back. Fixed with a `seededFromFile` map, which makes `dirty` a valid SECOND signal only where the ambiguity lives; CONFIG-2 cannot return because nobody opened those 18 and `dirty` is empty for them. **ADJACENT-1:** removing a key reported '· 0' — updateEnvFile returns writes only, so POST /api/config now also answers `removed`, computed against the pre-write snapshot. **ADJACENT-2:** none of the 18 selects had an empty option, so a dropdown could be set but never cleared; each now leads with a translated 'Use the default (…)' entry and an unset key selects THAT, not the default value — select-remote re-creates it after replaceChildren. New i18n key config.useDefault x17. Verified and left alone: `lang` is a literal allowlist entry not a prefix rule (langx/lang_evil/LANG all 400); facts.json is absent from dist by design (inlined at build); extensionless cv/import passthrough is documented and writes nothing. Counts unchanged: 92 sources / 87 EN adapters.) 3018 unit · 116 Playwright · 21 smoke · 23 comprehensive.
