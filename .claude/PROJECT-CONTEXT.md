...
>
> **Repo state.** v1.233.1 (2026-09-10 — documentation. **DOC-1:** the 'Use the default (…)' entry v1.233.0 added was described nowhere; it expresses 'key not set', and the difference from picking the same value explicitly is invisible and consequential — that is the drift CONFIG-2/CONFIG-3 were about. Three bullets in the existing ### Behavior list x17. Heading gates count H2/H3; coverage is not a count. **DOC-2:** a ### Recognized keys row's Key cell held `(server uses default UA)` describing HH_USER_AGENT, removed in v1.19.0, and told readers to register at dev.hh.ru — useless since v1.65.0 when hh.ru switched to scraping the public site with its own browser UA. Two more stale mentions (a health row deleted in v1.28.1; a troubleshooting row) went too. The troubleshooting row was REWRITTEN not deleted: hh.ru really does need a Russian residential IP (HTTP 451 since July 2026); the registration step was the wrong part. The keys table lists 6 of 45 while its heading promises a registry — one sentence x17 now calls it a selection and points at #/config. New gate tests/help-recognized-keys.test.mjs checks the first column of key tables against live KNOWN_KEYS, scoped so other tables (`PDF` in the export matrix) are not judged. Counts unchanged: 92 sources / 87 EN adapters; help stays 32 H2 / 122 H3 x17.) 3021 unit · 116 Playwright · 21 smoke · 23 comprehensive.
