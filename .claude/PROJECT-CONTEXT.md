...
>
> **Repo state.** v1.231.2 (2026-09-06 — **the mobile top bar was wrapping to two rows, the server was running an older parent, and the cvstart.ru redirect 404ed on every path that already named a locale.** Topbar actions are now 36 px icon-only squares; the desktop rules sit BELOW the `max-width: 900px` block in `app.css`, so at equal specificity they win — `gap` silently stayed 24 px and `.btn-ico` stayed hidden until the mobile selectors were raised to `header.topbar` / `.topbar .topbar-actions`. Measured at 320/360/390/430 px and across 560–760 px. Server parent 1.31.0 → 1.32.0 by targeted rsync of 95 runtime files, verified by checksum — the first attempt aborted mid-transfer on a file that no longer exists locally and still reported success. `cvstart.ru` now passes locale-prefixed paths through unchanged and strips `en`. 98 superseded QA prompts archived. Counts unchanged: 92 sources / 87 EN adapters.) 3009 unit · 101 Playwright · 21 smoke · 23 comprehensive.
