...
>
> **Repo state.** v1.237.1 (2026-09-22 — patch from an EXTERNAL QA pass on v1.237.0. The Jobstreet fix was confirmed from outside: /id/job/<id> is a clean 404 on AU/NZ/HK/MY and /job/<id> reaches a real Cloudflare-guarded route. BOOK-1: the book footer link localized its label but not its href — every locale pointed at /en/ while ru's label promised the Russian edition; new data-i18n-href applier (https-only) + footer.patternsUrl key ×17 SPA + ×17 site, ru→/ru/. SEEK MIGRATION: www.seek.com.au→au.seek.com and www.seek.co.nz→nz.seek.com; detail pages already 301, both pairs serve the v5 API at 200; http-json.mjs uses redirect:'error' so an old-host entry will fail the day the API path redirects — au.seek.com + nz.seek.com added to ALLOWED_JOBSTREET_HOSTS, old hosts kept. WIKI-1: unescaped | in the Agentic Practices doc-map row. New suites: footer-book-link-locale (5), sources-jobstreet-seek-hosts (4). Not closable from outside, recorded: Oracle Cloud (Akamai), Personio pin/appendWorkType (need portals.yml, UI has no delete), liveness strings (40 probes: 12 live / 28 inconclusive / 0 false expired). 3201 → 3210 tests.) Previous: v1.237.0 (2026-09-21 — parent parity with career-ops main @ 93c4302b, VERSION 1.33.0, 75 commits pulled from career-ops-hq/main. NO new sources; counts stay 94 (89 EN + 5 RU), 89 EN adapters. FIX 1 jobstreet: detail URLs were built as ${baseUrl}/id/job/${id} for EVERY market, but /id/ is the Indonesian locale prefix and every other SEEK host 404s on it — web-ui already allowed seek.com.au/seek.co.nz/sg/my/hk.jobsdb, so those postings all linked to dead pages; ID_LOCALE_HOSTS + jobDetailPath key the path on the host, and two of our own tests that had ENCODED the bug (hk.jobsdb.com/id/job/, www.jobstreet.com/id/job/) were corrected. FIX 2 oraclecloud: a SHORT page mid-list ended the walk, but ORC filters rows server-side (Amex: TotalJobsCount 454, pages 200/199/54 — last 54 postings, 12% of the board, never fetched); empty page always ends, a reported total drives paging to offset+PAGE_SIZE>=total, short-page-ends only without a total; hasMore still ignored; MAX_PAGES is now the only backstop against a bogus total and has its own test. FIX 3 liveness-core: /this job (listing )?is closed/ widened to /this (?:job|role|position)(?: listing)? is closed\b(?!-)/ — 'role' was 111 of 111 uncertain postings on one board; the \b(?!-) guard keeps 'closed-loop control' prose from expiring (\b alone rejects closedown but not closed-loop). FIX 4 liveness-core: /\bjob expired\b/ moved OUT of HARD_EXPIRED_PATTERNS into new SOFT_EXPIRED_PATTERNS checked AFTER hasApplyControl (code expired_body_soft) — it false-fires on similar-jobs carousels, filter chips and footer FAQs, and a false expired is written as skipped_expired and dedup-filters a real job out of EVERY later scan. ADDED personio tenant pin (company.personio slug, anchored allowlist, still gated by PERSONIO_HOST_RE; normalised through new URL() because web-ui's host regex is case-sensitive) and jobstreet appendWorkType (opt-in). NOT FOLLOWED: parent deleted providers/telegram.mjs — web-ui registers telegram AND telegram-channel as user-facing sources, following it would drop 94→93. jd-skill-gap is a relay, fix arrives with the deployed parent. New suites: sources-jobstreet-market-path, sources-oraclecloud-short-page, liveness-core-soft-expiry, adapters-personio-tenant-pin. 3164 → 3201 tests.) Previous: v1.236.0 (2026-09-18 — parent parity with career-ops main @ 6a9c84c, VERSION 1.33.0, 128 commits. ADDED 2 sources → 94 (89 EN + 5 RU), 89 EN adapters: pythonorg (feed; employer attribution mandatory) and generalist-world (scrape; FAILS LOUD on unparseable cards). FIX 1 workday: Workday's list endpoint reports a multi-location posting as a COUNT ('53 Locations'), which matched no location-filter allow entry and was dropped — isMultiLocationPlaceholder + locationsFromDetail + dateFromDetail now resolve the real places from the CXS DETAIL doc, capped at MAX_DETAIL_REQUESTS=200, placeholder left visible past the cap. FIX 2 role-matcher: web-ui had NO level handling — LEVEL_TOKENS/LEVEL_RE/extractLevels ported plus both guards (two stated levels must agree; a one-sided level splits only when each side also has a non-baseline word the other lacks). FIX 3 avature: resolveSearch dropped the entry's query string, so a pinned facet never narrowed the board; facets now ride every page, jobOffset applied last. SECURITY devalue 5.8.1→5.9.2 (site, via astro). NO-PORT: lever/manfred timeouts (web-ui already 60s), icims careers- doubling (origin comes from config, never constructed), deutschebahn/manfred retry. New suites: sources-workday-multi-location, role-matcher-levels, sources-avature-facets, sources-pythonorg, sources-generalist-world. 3066 → 3164 tests.) Previous: v1.235.0 (2026-09-16 — parent parity with career-ops main @ 68e6b94, VERSION 1.33.0. MIRROR 1: server/lib/url-key.mjs gains promoteKnownFragmentIdentity — a recognized `#/job/{id}`/`#/jobs/{id}` route is promoted to a comparison-only query key BEFORE the fragment is dropped (mokahr_job_id for app.mokahr.com, _career_ops_fragment_job_id elsewhere; APPEND not set). Fixed a live silent loss: all MokaHR postings on one tenant shared a dedup key, so addPipelineUrl swallowed the second job and en-scanner discarded the tenant as already-seen. MIRROR 2: accept-encoding pinned to 'gzip, deflate, br' (PINNED_ACCEPT_ENCODING + withPinnedEncoding in server/lib/http-json.mjs) at BOTH the 3 helper fetchImpl call sites AND server/lib/fetch-timeout.mjs::makeTimeoutFetch — the latter is the real chokepoint, since 25 of 92 sources call the injected fetchImpl directly and never pass through the helpers (the parent needs no equivalent: providers/_http.mjs is its only transport); a structural guard test fails CI if a source calls fetch directly, case-insensitively so a caller's Accept-Encoding REPLACES rather than joins; headers stay a PLAIN OBJECT at the fetchImpl boundary (45 suites read opts.headers['User-Agent']) — deliberate divergence from the parent's Headers instance. web-ui injects no default UA, so the parent's default-header fix is a no-port. New: tests/http-accept-encoding.test.mjs, tests/parsers-pipeline-hash-route.test.mjs. Not ported: liveness-browser, check-jd-archive, test-all, web/, manifests; batch-runner + jd-skill-gap are relayed read-only. 3042 → 3066 tests.) Previous: v1.234.0 (2026-09-15 — parent parity with career-ops main @ 56cce8f, VERSION still 1.32.0. MIRROR: providers/_safe-url.mjs → server/lib/sources/_safe-url.mjs; safeEncodeURIComponent (null on URIError from a lone UTF-16 surrogate) routed through the per-posting id/slug encode in 15 sources (13 mirrored + jobstreet + trudvsem, which the new source-level guard surfaced) so one bad posting drops instead of aborting the page; garena splits config `office` (strict, throws) from API `id` (null → skip); rheinmetall slug-fallback decodeURIComponent guarded; registry AND site/scripts/sync-assets.mjs on-disk count skip `_`-prefixed helpers. tests/sources-url-encoding-surrogate.test.mjs = helper contract + 15 per-source cases + guard (CONVERTED must import+call; no bare encodeURIComponent on a url line outside ALLOWLIST {4dayweek: SLUG_RE, csod: corpName}). Not ported: batch --cli, invite-match code-root, company-funded (json shape unchanged), web/, test-all timeout. 3022 → 3042 tests.) Previous: v1.233.2 (2026-09-10 — two high-severity CodeQL js/remote-property-injection alerts on server/lib/routes/config.mjs, closed by making an existing invariant STRUCTURAL. The loop copying saved values into process.env iterated `Object.entries(safe)` — an object built from the request body — with an explicit __proto__/constructor/prototype guard inside. Safe in fact (safe is populated 20 lines earlier from KNOWN_KEYS only) but unprovable to the analyser, which cannot follow the constraint across two loops. Now iterates KNOWN_KEYS directly and looks each key up in `safe`, so the property name is a module-level literal; the guard against an impossible case went with it. BEHAVIOUR IDENTICAL — the alerts were dated 2026-09-09 only because v1.232/v1.233 moved the line numbers. The test that pinned the old guard was a GREP OF SOURCE TEXT, which proves a string exists, not that the route is safe; replaced by a structural check (confirmed red against the old shape) plus a behavioural one posting __proto__ as RAW JSON — an object literal sets the prototype instead of creating an own property, so JSON.stringify would have sent {} and proven nothing. Counts unchanged: 92 sources / 87 EN adapters.) 3022 unit · 116 Playwright · 21 smoke · 23 comprehensive.
...
| `server/lib/sources/_safe-url.mjs` | `providers/_safe-url.mjs` (safeEncodeURIComponent — lone-surrogate guard; v1.234.0) |
