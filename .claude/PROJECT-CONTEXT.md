...
>
> **Repo state.** v1.230.0 (2026-09-05 — **Three provider fixes ported from parent career-ops v1.32.0**: Welcome to the Jungle takes a server-side Algolia `filters` expression, which shrinks the result set instead of re-ranking it — the only way a scan of a global board is exhaustive rather than a sample; Radancy stops warning "truncated" off `data-total-results`, which overstates on a majority of tenants, and cache-busts a JSON route that replayed stale pages; Greenhouse office lists are ordered deterministically, and web-ui's own `offices[0]` fallback — a second instance of that defect the parent does not have — is sorted with them. Registry unchanged at 90 sources / 85 adapters.) 2962 unit · 101 Playwright · 21 smoke · 23 comprehensive.
