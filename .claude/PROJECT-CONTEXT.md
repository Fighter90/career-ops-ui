...
>
> **Repo state.** v1.231.5 (2026-09-09 — **security: seven advisories closed, one a critical RCE. No source file changed.** Dependabot's own two merged first (`multer` 2.2.0->2.3.0 high DoS; site `svgo` 4.0.2->4.1.0 high+medium). The rescan those triggered surfaced five more, each already inside the existing `^` range so `npm update` sufficed, no manifest edit: site `astro` 7.1.0->7.3.2 (CRITICAL RCE via AVIF image optimization, fixed 7.2.8; plus an authorization bypass from a missing path-segment boundary check, 7.2.4), `sharp` 0.35.3->0.35.4 (libheif), and `js-yaml` 4.3.1->4.3.2 in BOTH manifests. Not shipped on the audit result alone: js-yaml parses every config read, multer takes every upload, and Astro crossed a MINOR version — so verified by the full suite, the browser suite, and a site rebuild yielding the same 86 pages / 17 mirrors. `npm audit` 0 in both. Counts unchanged: 92 sources / 87 EN adapters.) 3013 unit · 101 Playwright · 21 smoke · 23 comprehensive.
