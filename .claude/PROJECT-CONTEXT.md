...
>
> **Repo state.** v1.231.4 (2026-09-07 — **v1.231.3 fixed the aftermath of the Doctor click, not the moment.** It snapshotted the child nodes and restored them in `finally`, but the busy cue still ran `replaceChildren` with `'⏳ ' + label` for the DURATION of the request, so the 36px square was a wide pill the whole time `doctor.mjs` ran and the row broke exactly as before. A gate that waits for `aria-busy` to clear before measuring can only ever see the state `finally` already repaired. A button with element children is now never rewritten at all; its cue is the `.is-loading` class, which swaps the icon for an hourglass in CSS. 36px idle / in flight / after at 320px. The new browser check holds the promise open itself rather than racing a real request. Counts unchanged: 92 sources / 87 EN adapters.) 3013 unit · 101 Playwright · 21 smoke · 23 comprehensive.
