...
>
> **Repo state.** v1.231.3 (2026-09-07 — **`UI.withSpinner` flattened the button it was decorating.** Its busy cue assigned `button.textContent`, which replaces every child with ONE text node. v1.231.2 had just split each top-bar action into `.btn-ico` + `.btn-label` spans with the label hidden under `max-width: 900px`, so the first Doctor tap destroyed both spans permanently — the label returned as bare text and the 36 px square grew into a pill that overflowed the row. Only a reload healed it. Now snapshots the child NODES and restores them with `replaceChildren`; a `dataset.spinnerBusy` flag stops a re-entrant call from snapshotting the hourglass. Same defect class as the `applyI18n()` trap, different caller: **splitting an element's content into child spans makes every existing `textContent` writer on it a latent bug**, and those writers are not always in the file you are editing. Locked by a `node:vm` unit suite and a 320 px browser click, both confirmed failing against the old code. Counts unchanged: 92 sources / 87 EN adapters.) 3012 unit · 101 Playwright · 21 smoke · 23 comprehensive.
