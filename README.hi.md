...
> **🆕 नवीनतम रिलीज़ — v1.131.2**
>
> **`app.css` विभाजन (रीफ़ैक्टर)।** v1.131.2 एक फ़ाइल-आकार-अनुबंध ऋण चुकाता है: लगभग 1990-LOC वाली `app.css` को तीन क्रमबद्ध स्टाइलशीट्स में विभाजित किया गया है — `app.css` (base/chrome/buttons), **`components.css`** (cards, tables, forms, badges, paginator, …), **`overlays.css`** (toast, modal, drawer, `[dir="rtl"]`, docs-fab, usage-hud) — प्रत्येक 800-LOC कठोर लक्ष्य के भीतर। विभाजन सतत और क्रमबद्ध है, इसलिए कैस्केड **बाइट-फ़ॉर-बाइट समान** है; `index.html` तीनों को क्रमबद्ध `<link>` के रूप में लोड करता है। कोई व्यवहार/मार्कअप/i18n परिवर्तन नहीं; ब्राउज़र में सत्यापित (तीनों शीट्स पार्स और लागू होती हैं, 0 कंसोल त्रुटियाँ)। **2138 परीक्षण**। **अडैप्टर होस्ट-पिनिंग हार्डनिंग (पैच)।** v1.131.1 v1.130.0 स्रोतों पर कोड-समीक्षा फ़ॉलो-अप पर काम करता है (डिफ़ेंस-इन-डेप्थ, वैध इनपुट के लिए कोई व्यवहार परिवर्तन नहीं): **a16z Speedrun** अडैप्टर अब एंडपॉइंट ओवरराइड को पुनः-सत्यापित करता है (HTTPS + सटीक होस्ट) और कैननिकल फ़ीड पर वापस लौटता है — **Cryptocurrency Jobs** के समान — और Cryptocurrency Jobs पार्सर का `cleanUrl` अपनी SSRF जाँच के समान सटीक-होस्ट गार्ड इस्तेमाल करता है (पहले `endsWith` था, जो सबडोमेन स्वीकार कर लेता था), ताकि पार्सर कभी भी गार्ड से अधिक ढीला न हो। **2135 परीक्षण**। **CRM स्टेज-टैब बोर्ड।** v1.131.0 पैरेंट वेब ऐप के `/pipeline` व्यू को `#/tracker` पर पोर्ट करता है: फ़नल-चिप बार + स्टेटस ड्रॉपडाउन अब एक **स्टेज-टैब स्ट्रिप** बन जाते हैं — **All** प्लस हर कैननिकल स्टेटस के लिए एक टैब (Evaluated · Applied · Responded · Interview · Offer · Rejected · Discarded · SKIP · Hired), हर एक लाइव गिनती के साथ, **शून्य-गिनती वाले स्टेज शामिल** ताकि पूरा फ़नल हमेशा दृश्यमान रहे। टैब्स नए रीड-ओनली `GET /api/tracker/stages` से आते हैं (कैननिकल लेबल + `templates/states.yml` से उपनाम-फ़ोल्ड — क्लाइंट कभी भी व्हाइटलिस्ट हार्डकोड नहीं करता), पंक्तियों को वैकल्पिक ब्रांड लोगो मिलते हैं, और सुलभ नामों में गिनती शामिल होती है। **2133 परीक्षण**। **पैरेंट v1.24.0 पैरिटी।** v1.130.0 दो नए स्कैन स्रोत पोर्ट करता है — **a16z Speedrun** (a16z Speedrun टैलेंट-नेटवर्क बोर्ड-वाइड फ़ीड) और **Cryptocurrency Jobs** (Web3 बोर्ड `cryptocurrencyjobs.co`, इसकी सार्वजनिक RSS के ज़रिए) — जो रजिस्ट्री को **72 स्रोत (67 अंग्रेज़ी + 5 रूसी)** तक पहुँचाते हैं और दोनों को `#/scan` स्रोत फ़िल्टर व cvstart.org लैंडिंग पर दिखाते हैं। यह दो पैरेंट फ़िक्स भी मिरर करता है: **echojobs** अब **हाइब्रिड** भूमिकाओं को रिमोट से अलग पहचानने योग्य रखता है (`City · Hybrid`, कभी `Remote` में नहीं समेटा जाता), और **radancy** लेगेसी TalentBrew मार्कअप के साथ-साथ JSON परिणाम-फ़्रैगमेंट ट्रांसपोर्ट सीखता है। **2123 परीक्षण**। **AI समीक्षा फ़ॉलो-अप (पैच)।** v1.129.1 v1.128/v1.129 web पोर्ट की सलाहकारी टिप्पणियाँ ठीक करता है: job-facets स्तर वरीयता (`Senior Engineering Manager` → `senior`, `lead` नहीं), `states.mjs` अब बूट पर पैरेंट गड़बड़ी पर fallback पिन नहीं करता (+विकृत फ़ाइल पर चेतावनी), टोन बिना स्कोर वाली पंक्ति को तटस्थ दिखाता है (लाल नहीं), और `domainFromName` गैर-ASCII slug छोड़ता है। **स्कैन स्तर फ़ैसेट + आयु कॉलम।** v1.129.0 v1.128.0 की `job-facets.js` लाइब्रेरी को `#/scan` से जोड़ता है: नया **स्तर** फ़िल्टर (lead/staff/senior/mid/junior/intern, देश फ़ैसेट की तरह परिणामों से स्वतः भरा), स्तर बैज कॉलम, और शून्य-टोकन **आयु** कॉलम (`आज` / `Nदि`)। **पैरेंट वेब ऐप से चार पोर्ट।** v1.128.0 career-ops के अपने Next.js ऐप के चार समाधान वैनिला JS में पुनः-लागू करता है: ट्रैकर `templates/states.yml` से कैननिकल स्टेटस लाइव पढ़ता है (हार्डकोडेड व्हाइटलिस्ट नहीं), ATS-होस्टेड पंक्तियों में नाम→डोमेन मैप से ब्रांड लोगो, अधिक बारीक 4-स्तरीय स्कोर टोन, और शून्य-टोकन job-facets लाइब्रेरी। **2066 परीक्षण**। **पैरेंट v1.23.0 पैरिटी।** v1.127.0 तीन नए स्कैन स्रोत — **Flowxtra**, **VDAB**, **iCIMS** (रजिस्ट्री अब **70 अडैप्टर**, 65 EN + 5 RU) — पोर्ट करता है, agenticjobs के HTML→REST बदलाव, Greenhouse शहर पुनःप्राप्ति और role-matcher फ़िक्स को मिरर करता है, और **Cursor** को CLI रोस्टर में वापस जोड़ता है (parent #2115 — `cli-detect` अब 10 टूल्स रिपोर्ट करता है)। **CLI रोस्टर ड्रिफ्ट पैच।** v1.126.1 v1.126.0 स्वीप द्वारा छूटे दो स्थानों को ठीक करता है: `#/config` API keys टैब का इंट्रो (i18n ×17) अब Antigravity + Grok Build सूचीबद्ध करता है, और हेल्प गाइड ×17 (व CI-निर्मित साइट help) में पुरानी `Cursor / Gemini CLI` पंक्ति अब पूरा 8-CLI रोस्टर रखती है। **डॉक्स व CLI-रोस्टर पुनः-सिंक।** v1.126.0 हर दस्तावेज़ सतह (help ×17, README ×17, wiki) को लाइव career-ops.org/docs (सभी 31 पेज पढ़े) से मिलाता है और `#/config` के AI CLI टूल्स स्कैन को सभी 8 प्रथम-श्रेणी CLI का पता लगाना सिखाता है — **Grok Build CLI** और **Kimi CLI** Claude Code, Cursor, Codex, OpenCode, Antigravity, Qwen और Copilot में शामिल (अब **9 टूल्स**, फिर भी कभी नहीं चलाए जाते)। **रखरखाव सिंक।** v1.125.4 dependabot के site अपडेट (`sharp` 0.35.3, `svgo` 4.0.2, `fast-uri` 3.1.4) समेटता है और पैरेंट पैरिटी स्वीप (v1.22.0 के बाद, #2108–#2168) दर्ज करता है: set-status का गलत-पंक्ति गार्ड, स्थानीयकृत मोड का Risk Summary और अपडेटर की मैनिफ़ेस्ट जाँच — सब CLI की ओर, पोर्ट करने को कुछ नहीं (**1969 परीक्षण** अपरिवर्तित)। **प्रॉम्प्ट लोकेल सुधार।** v1.125.3 डेनिश और हिन्दी को हर AI प्रॉम्प्ट में प्रथम श्रेणी बनाता है — डीप रिसर्च (लाइव + मैनुअल), मोड, मूल्यांकन, इंटरव्यू, नेटवर्किंग और CV Studio अब इन दोनों भाषाओं में `# Output language` निर्देश देते हैं (पहले जवाब अंग्रेज़ी में आते थे), और लोकेल गेट अब सभी 17 लोकेल जाँचता है (**1969 परीक्षण**)। **बाहरी योगदानकर्ता पैक।** v1.125.2 [@Alien10140](https://github.com/Alien10140) के पहले कम्युनिटी PR मर्ज करता है — लाइव डीप रिसर्च HTTP 502 फ़िक्स (API प्रदाताओं के लिए बिना टूल वाले headless प्रॉम्प्ट) और बंद हो चुके `gemini-2.0-flash` से `gemini-3.6-flash` तक Gemini डिफ़ॉल्ट, नए 5-टेस्ट ड्रिफ़्ट गेट से स्थिर (**1957 टेस्ट**)। v1.125.1 मल्टी-ब्रांड SuccessFactors RMK टेनेंट का ब्रांड पथ बनाए रखता है (पैरेंट #2099)। **लैंडिंग पर जॉब स्रोत।** v1.125.0 cvstart.org पर एक सेक्शन जोड़ता है जो **सभी 67 स्कैनर स्रोतों** को क्लिक-योग्य चिप्स के रूप में सूचीबद्ध करता है, जो बिल्ड पर रजिस्ट्री से सिंक होता है और ड्रिफ़्ट-गेटेड है। इससे पहले, **पैरेंट v1.22.0 समता।** v1.124.0 **पाँच स्कैन स्रोत** पोर्ट करता है — Welcome to the Jungle, Agentic Engineering Jobs, Jobvite, Gem, और Alibaba Group (अब **70 अडैप्टर**) — साथ ही Arbeitsagentur की पूर्ण-रिमोट पुष्टि (`homeofficetyp=VOLLSTAENDIG`) और SmartRecruiters पब्लिक-URL फ़िक्स।
>
> _app.css विभाजन (3 स्टाइलशीट्स) · फ़ाइल-आकार-अनुबंध चुकौती · बाइट-समान कैस्केड · अडैप्टर होस्ट-पिनिंग हार्डनिंग · a16z ओवरराइड पुनः-सत्यापन · cryptocurrencyjobs cleanUrl सटीक-होस्ट · CRM स्टेज-टैब बोर्ड · ट्रैकर /pipeline पोर्ट · स्टेज टैब्स + गिनती · GET /api/tracker/stages · ट्रैकर ब्रांड लोगो · पैरेंट v1.24 पैरिटी · +2 स्रोत (a16z Speedrun/Cryptocurrency Jobs) · 72 स्रोत · echojobs हाइब्रिड · radancy TalentBrew · AI समीक्षा पैच · स्तर वरीयता · states fallback · तटस्थ टोन · स्कैन स्तर फ़ैसेट · आयु कॉलम · job-facets जुड़ी · 4 web/ पोर्ट · states.yml लाइव · नाम→डोमेन लोगो · स्कोर टोन · फ़ैसेट · पैरेंट v1.23 पैरिटी · +3 स्रोत · 70 अडैप्टर · Cursor · CLI रोस्टर ड्रिफ्ट पैच · डॉक्स व CLI-रोस्टर पुनः-सिंक · रखरखाव सिंक · da/hi प्रॉम्प्ट लोकेल सुधार · बाहरी योगदानकर्ता पैक · डीप रिसर्च 502 फ़िक्स · Gemini 3.6 डिफ़ॉल्ट · SF ब्रांड पथ फ़िक्स · लैंडिंग पर स्रोत सेक्शन · 5 स्रोत · 67 अडैप्टर · पैरेंट v1.22 समता · Oracle Cloud · 62 अडैप्टर · पैरेंट v1.21 समता · manifesto · cvstart.org methodology/license/changelog पेज · parent v1.19 parity · 61 adapters · लैंडिंग लाइव स्टार्स + contributors · parent v1.18 parity · 54 adapters · Hired स्टेटस · lifetime stats · parent parity pack · 50 adapters · usage मीटर रीवर्क · डिज़ाइन पॉलिश · usage मीटर · फ़्लोटिंग docs assistant · docs & QA consolidation · security backlog closeout · docs & QA रीफ़्रेश ×16 · scan Exclude · pipeline overview · security hardening 2 · sanitizer hardening · security hardening · AI usage & cost · कंपनी लोगो · AI CLI टूल्स · ask-the-docs · CV tailor + cover letter · two-pager auto-fill · DOCX export · 16 locales · 6 LLM providers · 46 scanner adapters · portals health · in-app bug reporter · career orientation · career plan · statistics rework · memory layer · CV Studio · networking planner · mock interview · two-pager market-fit · parent career-ops v1.16.0 parity._
...
![career-ops-ui — Command Center](./images/dashboard-hi.png)
