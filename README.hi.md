...
> **🆕 नवीनतम रिलीज़ — v1.228.5** — **बताई गई संस्करण संख्या डिस्क की फ़ाइल का वर्णन करती थी, चल रहे कोड का नहीं।** `/api/health` हर अनुरोध पर `package.json` दोबारा पढ़ता था: परिनियोजन से पहले शुरू हुई प्रक्रिया पुराना कोड परोसते हुए नई फ़ाइल का संस्करण बताती थी — QA को ऐसा इंस्टेंस मिला जो `1.228.4` उत्तर देता और 1.228.3 में जोड़े गए मार्ग पर 404 लौटाता था। फ़ाइलें कॉपी करके पुनरारंभ न करने वाला परिनियोजन ऐसा ही सफल दिखता है। संस्करण अब मॉड्यूल लोड होते समय दर्ज होता है: पुरानी प्रक्रिया **पुराना** संस्करण बताएगी — यही सच है। **2909 परीक्षण।**
...
│  └─ reviews/REVIEW-*.md
└─ tests/                    # 2909 यूनिट + 101 Playwright + 23/23 e2e:full + 21 e2e:smoke (बेसलाइन @ v1.121.0)
...
```bash
npm test                       # 2909 यूनिट/इंटीग्रेशन टेस्ट
...
| सूट                       | टेस्ट | क्या                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (यूनिट + इंटीग्रेशन) | **2909** | हर एंडपॉइंट, एफ़ेमरल सर्वर, कोई नेटवर्क नहीं। 218 फ़ाइलें: parsers, scanners (मॉक्ड), runners, anthropic/openai, सुरक्षा हेडर, XSS, JD सैनिटाइज़, URL वैलिडेशन, i18n पैरिटी, + v1.55→v1.56 UX-फ़िक्स सूट। |
...
- पुश करने से पहले `npm test` चलाएँ — **2909 चेक ग्रीन** बार है (यदि आप UI छूते हैं तो साथ में 101 Playwright भी)।
