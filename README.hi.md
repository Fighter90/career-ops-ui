...
> **🆕 नवीनतम रिलीज़ — v1.228.3** — **दस्तावेज़ सहायक अंग्रेज़ी में उत्तर नहीं दे पा रहा था।** दोष खोज का नहीं था: संदर्भ-निर्माता पहले ही बड़े खंड पर `break` कर देता था और संदर्भ खाली छोड़ देता था। अंग्रेज़ी §5 14 336 बाइट के बजट के मुक़ाबले 16 081 बाइट हो गया था। 6 KB से बड़े खंड अब `###` सीमाओं पर बँटते हैं (32 → 75 टुकड़े)। साथ ही: Telegram के कंपनी लेबल नाम के बजाय वाक्य देते थे, और `GET /api/ping` एक ऐसी जाँच है जो कुछ भी उजागर नहीं करती। **2907 परीक्षण।**
...
│  └─ reviews/REVIEW-*.md
└─ tests/                    # 2907 यूनिट + 101 Playwright + 23/23 e2e:full + 21 e2e:smoke (बेसलाइन @ v1.121.0)
...
```bash
npm test                       # 2907 यूनिट/इंटीग्रेशन टेस्ट
...
| सूट                       | टेस्ट | क्या                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (यूनिट + इंटीग्रेशन) | **2907** | हर एंडपॉइंट, एफ़ेमरल सर्वर, कोई नेटवर्क नहीं। 218 फ़ाइलें: parsers, scanners (मॉक्ड), runners, anthropic/openai, सुरक्षा हेडर, XSS, JD सैनिटाइज़, URL वैलिडेशन, i18n पैरिटी, + v1.55→v1.56 UX-फ़िक्स सूट। |
...
- पुश करने से पहले `npm test` चलाएँ — **2907 चेक ग्रीन** बार है (यदि आप UI छूते हैं तो साथ में 101 Playwright भी)।
