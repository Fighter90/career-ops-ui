...
> **🆕 नवीनतम रिलीज़ — v1.229.0** — **पैरेंट v1.31.0 से चार नए स्रोत: Built In, Feishu Jobs, Garena और MokaHR** — 86 → **90 स्रोत** (85 EN + 5 RU), सभी बिना टोकन। Built In **कोई डिफ़ॉल्ट क्वेरी नहीं** लाता और सूची से बाहर के बाज़ार को अस्वीकार करता है, चुपचाप राष्ट्रीय बोर्ड स्कैन करने के बजाय; MokaHR **एन्क्रिप्टेड उत्तर** देता है जिसकी कुंजी साथ ही आती है। यह भी ठीक हुआ: `htmlToText` अधूरे टैग ओपनर को पास होने दे सकता था। **2956 परीक्षण।**
...
│  └─ reviews/REVIEW-*.md
└─ tests/                    # 2956 यूनिट + 101 Playwright + 23/23 e2e:full + 21 e2e:smoke (बेसलाइन @ v1.121.0)
...
```bash
npm test                       # 2956 यूनिट/इंटीग्रेशन टेस्ट
...
| सूट                       | टेस्ट | क्या                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (यूनिट + इंटीग्रेशन) | **2956** | हर एंडपॉइंट, एफ़ेमरल सर्वर, कोई नेटवर्क नहीं। 218 फ़ाइलें: parsers, scanners (मॉक्ड), runners, anthropic/openai, सुरक्षा हेडर, XSS, JD सैनिटाइज़, URL वैलिडेशन, i18n पैरिटी, + v1.55→v1.56 UX-फ़िक्स सूट। |
...
- पुश करने से पहले `npm test` चलाएँ — **2956 चेक ग्रीन** बार है (यदि आप UI छूते हैं तो साथ में 101 Playwright भी)।
