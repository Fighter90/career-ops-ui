...
> **🆕 Останній реліз — v1.97.0**
>
> **Джерело сканера Dassault Systèmes + покращення якості.** Паритет із батьківським career-ops (#1498): нове zero-token-джерело сканування **Dassault Systèmes** (публічна стрічка карткового пошуку Exalead за 3ds.com/careers) приєднується до реєстру як **46-й адаптер** — обирається за провайдером (`provider: dassault`) або автоматично визначається за хостом 3ds.com, із прив'язкою хоста для захисту від SSRF. Разом із ним аудит за трьома напрямами (сервер + SPA + переклад 16 мовами) виправив реальні дефекти: **асинхронну гонку вкладок статистики** (рендер повільної вкладки міг затерти новішу), **ліміт розміру в `safe-fetch`, який міг зависнути** на запиті предперегляду/пайплайну, що перевищує ліміт, мертве логування активності SSE, два підтвердження видалення з порожнім тілом та неперекладені значення словника (uk/ru/it). Перенесено виправлення надійності з батьківського проєкту для джерел Avature / Get on Board / SuccessFactors.
>
> _16 locales · 6 LLM-провайдерів · 46 адаптерів сканера · кар'єрна орієнтація · план кар'єри · перероблення статистики · шар пам'яті · CV Studio · планувальник нетворкінгу · пробна співбесіда · ринкова відповідність через two-pager · паритет із батьківським career-ops v1.16.0._
