...
> **🆕 Останній реліз — v1.108.0**
>
> **Посилення безпеки (розбір CodeQL, раунд 2).** Збирач промптів розв'язує рядок ролі за власним ключем + `typeof === function`, slug імені PDF обмежується до його регулярки (поліноміальний ReDoS), а імпорт документа приводить масивний `filename` до рядка (type-confusion). За v1.106–v1.108 беклог статичного аналізу скоротився зі 167 до ~14, усі реальні знахідки виправлено, решту відхилено з обґрунтуванням.
>
> _посилення безпеки 2 · посилення санітайзера · посилення безпеки · витрати ШІ · логотипи компаній · інструменти CLI для ШІ · запитати довідку · адаптація CV + лист · автозаповнення two-pager · експорт у DOCX · здоров’я порталів · вбудований репортер багів · 16 locales · 6 LLM-провайдерів · 46 адаптерів сканера · кар'єрна орієнтація · план кар'єри · перероблення статистики · шар пам'яті · CV Studio · планувальник нетворкінгу · пробна співбесіда · ринкова відповідність через two-pager · паритет із батьківським career-ops v1.16.0._
