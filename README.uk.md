...
> **🆕 Останній реліз — v1.104.0**
>
> **Логотипи компаній у таблиці сканування (приватність).** Новий перемикач «Вигляд» малює логотип кожної компанії поруч із назвою на `#/scan`. Логотип — це **favicon компанії з її власного домену**, проксі на сервері (`GET /api/logo`) через SSRF-безпечний завантажувач, тож жоден сторонній сервіс логотипів не дізнається, яких роботодавців ви переглядаєте. Спільні портали показують літерний значок; типово вимкнено.
>
> _логотипи компаній · інструменти CLI для ШІ · запитати довідку · адаптація CV + лист · автозаповнення two-pager · експорт у DOCX · здоров’я порталів · вбудований репортер багів · 16 locales · 6 LLM-провайдерів · 46 адаптерів сканера · кар'єрна орієнтація · план кар'єри · перероблення статистики · шар пам'яті · CV Studio · планувальник нетворкінгу · пробна співбесіда · ринкова відповідність через two-pager · паритет із батьківським career-ops v1.16.0._
