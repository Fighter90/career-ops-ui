...
> **🆕 Останній реліз — v1.97.1**
>
> **Посилення за підсумками рев'ю та паритет документації.** Продовження v1.97.0: значок відповідності `◎` під час сканування (`fit-score.js`) більше не перетворює ставку, меншу за річну («500 EUR/day»), на фіктивний річний мінімум зарплати, а зіставлення країн тепер іде за цілим словом, тож «Germany» не може збігтися з прикметником «German». Кожен локалізований README та вбудована довідка тепер однаково заявляють **16 локалей** і **46 адаптерів сканера** (частина тексту все ще трималася на лічильниках до v1.85). У v1.97.0 додано джерело сканування **Dassault Systèmes** (46-й адаптер, zero-token Exalead XML, із прив'язкою хоста від SSRF) та аудит за трьома напрямами (сервер / SPA / 16 мов).
>
