...
> **🆕 Останній реліз — v1.100.0**
>
> **ШІ-автозаповнення two-pager + експорт.** Two-pager (`#/two-pager`) тепер сам заповнюється з вашого резюме + профілю через налаштованого провайдера (нічого не вигадуючи; за відсутності ключа — відкат на копіювання промпта), додає вид **Перегляд і експорт** і отримує експорт у **PDF / DOCX / Markdown** — DOCX через новий генератор `.docx` без залежностей, також під'єднаний до звітів по ринку, кар'єрному плану та профорієнтації.
>
> _автозаповнення two-pager · експорт у DOCX · здоров’я порталів · вбудований репортер багів · 16 locales · 6 LLM-провайдерів · 46 адаптерів сканера · кар'єрна орієнтація · план кар'єри · перероблення статистики · шар пам'яті · CV Studio · планувальник нетворкінгу · пробна співбесіда · ринкова відповідність через two-pager · паритет із батьківським career-ops v1.16.0._
