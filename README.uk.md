...
[![tests](https://img.shields.io/badge/tests-3018%20passed-brightgreen)](#тести)
...
> **🆕 Останній реліз — v1.232.0** — **Два дефекти з браузерного прогону; жоден не з v1.231.5.** **`#/profile` прокручував убік усю сторінку**: URL LinkedIn не дає CSS точок розриву, тож **min-content дорівнював усьому рядку**, а `min-width:auto` не давав стиснутися — картка розпирала рядок і документ. Виправлено `.card-value` з **`overflow-wrap: anywhere`**. **`POST /api/config` приймав будь-яке значення select-поля** і падав далі *мовчки*. Перевіряється лише провайдер; уже збережене значення отримує поблажку. **3018 тестів.**
