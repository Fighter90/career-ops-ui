...
> **🆕 Neueste Version — v1.228.5** — **Die gemeldete Version beschrieb die Datei auf der Festplatte, nicht den laufenden Code.** `/api/health` las `package.json` bei jeder Anfrage neu: ein vor einem Deployment gestarteter Prozess meldete die Version der neuen Datei, während er den alten Code auslieferte — die QA fand eine Instanz, die `1.228.4` antwortete und auf einer in 1.228.3 ergänzten Route 404 lieferte. So sieht ein Deployment aus, das Dateien kopiert und nicht neu startet. Die Version wird jetzt beim Laden des Moduls erfasst: ein veralteter Prozess meldet die **alte** Version — die Wahrheit. **2909 Tests.**
...
| Suite                       | Tests | Was                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **2909** | Jeder Endpunkt, ephemerer Server, kein Netzwerk. 329 Dateien: Parser, Scanner (gemockt), Runner, anthropic/openai, Security-Header, XSS, JD-Sanitize, URL-Validierung, i18n-Parität, + die UX-Fix-Suites v1.55→v1.56. |
...
| **Gesamt** (alle vier Suites)                   | **3040** | **0 Fehler, 0 Flakes**                                                                                    |
...
- Führen Sie `npm test` vor dem Pushen aus — **2909 checks green** ist die Messlatte (plus 101 Playwright, wenn Sie die UI berühren).
