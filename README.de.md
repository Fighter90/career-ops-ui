...
> **🆕 Neueste Version — v1.131.2**
>
> **`app.css`-Aufteilung (Refactor).** v1.131.2 tilgt eine File-Size-Contract-Schuld: Die ~1990 LOC große `app.css` wird in drei geordnete Stylesheets aufgeteilt — `app.css` (Basis/Chrome/Buttons), **`components.css`** (Karten, Tabellen, Formulare, Badges, Paginator, …), **`overlays.css`** (Toast, Modal, Drawer, `[dir="rtl"]`, docs-fab, usage-hud) — jede innerhalb des harten 800-LOC-Ziels. Der Schnitt ist zusammenhängend und in Reihenfolge, sodass die Kaskade **byte-für-byte identisch** ist; `index.html` lädt alle drei als geordnete `<link>`s. Keine Verhaltens-/Markup-/i18n-Änderung; im Browser verifiziert (alle drei Stylesheets werden geparst und greifen, 0 Konsolenfehler). **2138 Tests**. **Adapter-Host-Pinning-Härtung (Patch).** v1.131.1 setzt Code-Review-Nachträge zu den v1.130.0-Quellen um (Defense-in-Depth, kein Verhaltensunterschied bei gültigen Eingaben): Der **a16z-Speedrun**-Adapter validiert das Endpoint-Override jetzt erneut (HTTPS + exakter Host) und fällt auf den kanonischen Feed zurück — Parität mit **Cryptocurrency Jobs** —, und `cleanUrl` im Cryptocurrency-Jobs-Parser verwendet denselben exakten Host-Guard wie dessen SSRF-Prüfung (zuvor `endsWith`, was Subdomains akzeptierte), sodass der Parser nie großzügiger ist als der Guard. **2135 Tests**. **CRM-Stage-Tab-Board.** v1.131.0 portiert die `/pipeline`-Ansicht der Web-App des Elternprojekts nach `#/tracker`: Die Funnel-Chip-Leiste + das Status-Dropdown werden zu einer **Stage-Tab-Leiste** — **Alle** plus ein Tab pro kanonischem Status (Evaluated · Applied · Responded · Interview · Offer · Rejected · Discarded · SKIP · Hired), jeweils mit einer Live-Anzahl, **einschließlich Stufen mit null Treffern**, sodass der gesamte Funnel stets sichtbar bleibt. Die Tabs stammen aus einer neuen schreibgeschützten `GET /api/tracker/stages` (kanonische Labels + Alias-Faltung aus `templates/states.yml` — der Client hartcodiert die Whitelist nie), Zeilen erhalten optionale Markenlogos, und die barrierefreien Namen tragen die Anzahl. **2133 Tests**. **Eltern-v1.24.0-Parität.** v1.130.0 portiert zwei neue Scan-Quellen — **a16z Speedrun** (den board-weiten Feed des a16z-Speedrun-Talent-Netzwerks) und **Cryptocurrency Jobs** (das Web3-Board `cryptocurrencyjobs.co`, über dessen öffentlichen RSS) — und bringt die Registry auf **72 Quellen (67 englische + 5 russische)**, sichtbar sowohl im `#/scan`-Quellenfilter als auch auf der cvstart.org-Landing. Zudem spiegelt es zwei Fixes aus dem Elternprojekt: **echojobs** hält **Hybrid**-Stellen jetzt von Remote unterscheidbar (`Stadt · Hybrid`, nie zu `Remote` zusammengefasst), und **radancy** lernt Legacy-TalentBrew-Markup plus den JSON-Ergebnisfragment-Transport. **2123 Tests**. **KI-Review-Nachträge (Patch).** v1.129.1 behebt beratende Befunde der Web-Ports aus v1.128/v1.129: Level-Präzedenz in job-facets (`Senior Engineering Manager` → `senior`, nicht `lead`), `states.mjs` fixiert seinen Fallback bei einem Eltern-Aussetzer beim Boot nicht mehr (+warnt bei fehlerhafter Datei), der Ton zeigt eine Zeile ohne Score neutral (nicht rot), und `domainFromName` überspringt Nicht-ASCII-Slugs. **Scan-Level-Facette + Alter-Spalte.** v1.129.0 bindet die `job-facets.js`-Lib aus v1.128.0 an `#/scan` an: ein neues **Level**-Filter (lead/staff/senior/mid/junior/praktikant, aus den Ergebnissen befüllt wie die Land-Facette), eine Level-Badge-Spalte und eine token-freie **Alter**-Spalte (`heute` / `Nd`). **Vier Ports aus der Web-App des Elternprojekts.** v1.128.0 implementiert vier Lösungen aus career-ops' eigener Next.js-App in Vanilla-JS neu: Der Tracker liest kanonische Status live aus `templates/states.yml` (keine hartcodierte Whitelist mehr), Markenlogos jetzt in ATS-gehosteten Zeilen via Name→Domain-Map, ein feinerer 4-stufiger Score-Ton und eine token-freie Job-Facets-Lib. **2066 Tests**. **Eltern-v1.23.0-Parität.** v1.127.0 portiert drei neue Scan-Quellen — **Flowxtra**, **VDAB**, **iCIMS** (Registry nun **70 Adapter**, 65 EN + 5 RU) —, spiegelt den HTML→REST-Wechsel von agenticjobs, die Greenhouse-Stadt-Wiederherstellung und role-matcher-Fixes und fügt **Cursor** wieder ins CLI-Roster ein (parent #2115 — `cli-detect` meldet 10 Werkzeuge). **CLI-Roster-Drift-Patch.** v1.126.1 behebt zwei vom v1.126.0-Sweep übersehene Stellen: das Intro des API-keys-Tabs in `#/config` (i18n ×17) listet jetzt Antigravity + Grok Build, und eine veraltete `Cursor / Gemini CLI`-Zeile im Hilfe-Guide ×17 (und im Site-Help) trägt nun das vollständige 8-CLI-Roster. **Docs- & CLI-Roster-Resync.** v1.126.0 gleicht jede Doku-Fläche (help ×17, README ×17, wiki) mit der lebenden career-ops.org/docs ab (alle 31 Seiten gelesen) und bringt dem AI-CLI-tools-Scan in `#/config` bei, alle 8 erstklassigen CLIs zu erkennen — **Grok Build CLI** und **Kimi CLI** kommen zu Claude Code, Cursor, Codex, OpenCode, Antigravity, Qwen und Copilot hinzu (jetzt **9 Werkzeuge**, weiterhin nie ausgeführt). **Wartungs-Sync.** v1.125.4 bündelt die Dependabot-Site-Bumps (`sharp` 0.35.3, `svgo` 4.0.2, `fast-uri` 3.1.4) und protokolliert einen Paritäts-Sweep des Elternprojekts (nach v1.22.0, #2108–#2168): der Falsche-Zeile-Guard in set-status, das Risk Summary der lokalisierten Modi und die Manifest-Prüfung des Updaters sind alle CLI-seitig — nichts zu portieren (**1969 Tests** unverändert). **Prompt-Locale-Fix.** v1.125.3 macht Dänisch und Hindi in jedem KI-Prompt vollwertig — Deep Research (live + manuell), Modi, Bewertung, Interview, Networking und CV Studio geben in diesen beiden Sprachen jetzt die `# Output language`-Direktive aus (Antworten kamen zuvor auf Englisch), und das Locale-Gate durchläuft nun alle 17 Locales (**1969 Tests**). **Externes Contributor-Paket.** v1.125.2 merged die ersten Community-PRs von [@Alien10140](https://github.com/Alien10140) — den Fix des HTTP 502 im Live-Deep-Research (Headless-Prompts ohne Tools für API-Anbieter) und die Gemini-Standards, angehoben vom eingestellten `gemini-2.0-flash` auf `gemini-3.6-flash`, fixiert durch ein neues Drift-Gate mit 5 Tests (**1957 Tests**). v1.125.1 belässt Multi-Brand-SuccessFactors-RMK-Tenants auf ihrem Brand-Pfad (Parent #2099). **Job-Quellen auf der Landing.** v1.125.0 fügt einen cvstart.org-Abschnitt hinzu, der alle **70 Scanner-Quellen** als anklickbare Chips auflistet — synchronisiert mit der Registry beim Build und gegen Drift abgesichert. Zuvor **Parent-Parität v1.22.0.** v1.124.0 portiert **fünf Scan-Quellen** — Welcome to the Jungle, Agentic Engineering Jobs, Jobvite, Gem und Alibaba Group (jetzt **70 Adapter**) — sowie die Arbeitsagentur-Verifizierung für vollständig ortsunabhängige Stellen (`homeofficetyp=VOLLSTAENDIG`) und die Behebung der öffentlichen SmartRecruiters-URL.
>
> _`app.css`-Aufteilung (3 Stylesheets) · File-Size-Contract-Tilgung · byte-identische Kaskade · Adapter-Host-Pinning-Härtung · a16z-Override-Revalidierung · cryptocurrencyjobs cleanUrl exakter Host · CRM-Stage-Tab-Board · Tracker-`/pipeline`-Port · Stage-Tabs + Anzahl · GET /api/tracker/stages · Tracker-Markenlogos · Eltern-v1.24-Parität · +2 Quellen (a16z Speedrun/Cryptocurrency Jobs) · 72 Quellen · echojobs Hybrid · radancy TalentBrew · KI-Review-Patch · Level-Präzedenz · states-Fallback · neutraler Ton · Level-Facette · Alter-Spalte · job-facets angebunden · 4 web/-Ports · states.yml live · Name→Domain-Logos · Score-Ton · Facetten · Eltern-v1.23-Parität · +3 Quellen · 70 Adapter · Cursor · CLI-Roster-Drift-Patch · Docs- & CLI-Roster-Resync · Wartungs-Sync · da/hi-Prompt-Locale-Fix · externes Contributor-Paket · Deep-Research-502-Fix · Gemini-3.6-Standards · SF-Brand-Pfad-Fix · Quellen-Bereich auf der Landing · 5 Quellen · 67 Adapter · Parität mit Parent v1.22 · Oracle Cloud · 62 Adapter · Parität mit Parent v1.21 · Hindi · 17 Sprachen · Manifest · Methodology-/License-/Changelog-Seiten auf cvstart.org · Parent-Parität v1.19 · 61 Adapter · Live-Sterne + Mitwirkende auf der Landing · Parent-Parität v1.18 · 54 Adapter · Hired-Status · Gesamtstatistik · Paritätspaket · 50 Adapter · Nutzungsanzeige-Überarbeitung · Design-Feinschliff · Nutzungsanzeige · schwebender Hilfe-Assistent · Docs- & QA-Konsolidierung · Abschluss Sicherheits-Backlog · Docs & QA ×16 · Scan-Ausschluss · Pipeline-Überblick · Sicherheitshärtung 2 · Sanitizer-Härtung · Sicherheitshärtung · KI-Nutzung & Kosten · Firmenlogos · KI-CLI-Tools · Doku fragen · Lebenslauf-Anpassung + Anschreiben · Two-Pager-Autofüllung · DOCX-Export · Portal-Gesundheit · integrierter Fehlermelder · 16 Sprachen · 6 LLM-Anbieter · 46 Scanner-Adapter · Berufsorientierung · Karriereplan · Statistik-Überarbeitung · Speicherschicht · CV Studio · Networking-Planer · Mock-Interview · Markt-Fit über two-pager · Parität mit dem übergeordneten career-ops v1.16.0._
...
Die vollständig LLM-gesteuerten Modi (`oferta`, `deep`, `contacto`, `apply`, `batch`, `patterns`, `followup`) benötigen ein LLM, um tatsächlich zu laufen. Die Web-UI löst einen Anbieter aus der `auto`-Reihenfolge **Anthropic → Gemini → OpenAI → Qwen → OpenRouter** auf (oder was auch immer `LLM_PROVIDER` festlegt):
