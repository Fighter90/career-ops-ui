...
> **🆕 Neueste Version — v1.237.0** — **Eltern-Parität mit career-ops `main` @ `93c4302b`: vier gespiegelte Fixes, zwei davon verloren hier still Anzeigen.** **Jobstreet / SEEK** baute jede Detail-URL als `/id/job/<id>` auf — doch `/id/` ist das *indonesische Locale-Präfix*, und jeder andere SEEK-Host antwortet darauf mit **404**, sodass jede von uns zurückgegebene australische, neuseeländische, singapurische, malaysische und Hongkonger Anzeige auf eine tote Seite zeigte; der Pfad wird jetzt über den Host bestimmt. **Oracle Cloud** beendete seinen Durchlauf bei einer kurzen Seite, doch ORC liefert kurze Seiten *mitten in der Liste* — American Express meldet **454** Stellen und liefert 200 / 199 / 54, sodass die letzten **54 Anzeigen, 12% des Boards**, nie abgerufen wurden. **Liveness** übersah `"This role is closed"` (111 von 111 unsicheren Anzeigen auf einem Board) und schützt jetzt den zusammengesetzten Fall `closed-loop`; `job expired` wanderte in eine weiche Stufe, die einen sichtbaren Apply-Button nicht mehr schlagen kann — ein falscher Ablauf wird in den Scan-Verlauf geschrieben und wirft einen echten Job aus *jedem* späteren Scan. Dazu ein optionaler **Personio**-Tenant-Pin `personio: <slug>` für iframe-eingebettete Boards, sowie Jobstreet-`appendWorkType`. Quellenzahl unverändert bei **94** Quellen (89 EN + 5 RU). **3201 Tests · 116 Browser.**
