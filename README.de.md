...
| Suite                       | Tests | Was                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **2908** | Jeder Endpunkt, ephemerer Server, kein Netzwerk. 329 Dateien: Parser, Scanner (gemockt), Runner, anthropic/openai, Security-Header, XSS, JD-Sanitize, URL-Validierung, i18n-Parität, + die UX-Fix-Suites v1.55→v1.56. |
...
- Führen Sie `npm test` vor dem Pushen aus — **2908 checks green** ist die Messlatte (plus 101 Playwright, wenn Sie die UI berühren).
