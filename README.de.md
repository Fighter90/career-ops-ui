...
> **🆕 Neueste Version — v1.228.3** — **Der Doku-Assistent konnte auf Englisch nicht antworten.** Nicht das Retrieval: der Kontextbauer machte `break` beim ersten zu großen Abschnitt und ließ den Kontext leer. Der englische §5 war auf 16 081 Bytes bei 14 336 Budget gewachsen. Abschnitte über 6 KB werden jetzt an ihren `###` geteilt (32 → 75 Blöcke). Dazu: Telegram-Firmenlabels lieferten den Satz statt des Namens, und `GET /api/ping` ist eine Sonde, die nichts preisgibt. **2907 Tests.**
...
| Suite                       | Tests | Was                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **2907** | Jeder Endpunkt, ephemerer Server, kein Netzwerk. 329 Dateien: Parser, Scanner (gemockt), Runner, anthropic/openai, Security-Header, XSS, JD-Sanitize, URL-Validierung, i18n-Parität, + die UX-Fix-Suites v1.55→v1.56. |
...
- Führen Sie `npm test` vor dem Pushen aus — **2907 checks green** ist die Messlatte (plus 101 Playwright, wenn Sie die UI berühren).
