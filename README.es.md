...
```bash
npm test                       # 2908 tests unit/integración
...
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integración) | **2908** | Cada endpoint, servidor efímero, sin red. Incluye parser, scanner (mockeado), runner, anthropic, security headers, XSS, JD sanitize, validación de URL, DNS rebind, mutex de archivo, rate limit, path traversal y paridad i18n. |
...
- Ejecuta `npm test` antes de hacer push — **2908 checks en verde** es el mínimo (más 101 Playwright si tocas la UI).
