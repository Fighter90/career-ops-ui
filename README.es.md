...
[![release](https://img.shields.io/badge/release-v1.131.2-blue)](https://github.com/Fighter90/career-ops-ui/releases/tag/v1.131.2)

> **🆕 Última versión — v1.131.2**
>
> **División de `app.css` (refactor).** v1.131.2 salda una deuda de contrato de tamaño de archivo: el `app.css` de ~1990 LOC se divide en tres hojas de estilo ordenadas — `app.css` (base/chrome/botones), **`components.css`** (tarjetas, tablas, formularios, badges, paginador, …), **`overlays.css`** (toast, modal, cajón, `[dir="rtl"]`, docs-fab, usage-hud) — cada una dentro del límite estricto de 800 LOC. El corte es contiguo y en orden, de modo que la cascada es **byte a byte idéntica**; `index.html` carga las tres como `<link>` ordenados. Sin cambio de comportamiento/marcado/i18n; verificado en el navegador (las tres hojas se parsean y se aplican, 0 errores de consola). **2138 tests**. **Anclaje de host de adaptadores (parche).** v1.131.1 actúa sobre seguimientos de la revisión de código en las fuentes de v1.130.0 (defensa en profundidad, sin cambio de comportamiento para entradas válidas): el adaptador **a16z Speedrun** ahora revalida la anulación del endpoint (HTTPS + host exacto) y recurre al feed canónico — paridad con **Cryptocurrency Jobs** —, y el `cleanUrl` del analizador de Cryptocurrency Jobs usa la misma comprobación de host exacto que su verificación SSRF (antes era `endsWith`, que aceptaba subdominios), de modo que el analizador nunca es más permisivo que la comprobación. **2135 tests**. **Tablero de pestañas de etapa CRM.** v1.131.0 porta la vista `/pipeline` de la app web del padre a `#/tracker`: la barra de chips de embudo y el desplegable de estado se convierten en una **franja de pestañas de etapa** — **All** más una pestaña por cada estado canónico (Evaluated · Applied · Responded · Interview · Offer · Rejected · Discarded · SKIP · Hired), cada una con un recuento en vivo, **incluidas las etapas con recuento cero** para que el embudo completo sea siempre visible. Las pestañas provienen de una nueva ruta de solo lectura `GET /api/tracker/stages` (etiquetas canónicas + plegado de alias desde `templates/states.yml` — el cliente nunca fija de forma hardcodeada la whitelist), las filas ganan logos de marca opcionales, y los nombres accesibles llevan los recuentos. **2133 tests**. **Paridad con el padre v1.24.0.** v1.130.0 porta dos nuevas fuentes de escaneo — **a16z Speedrun** (el feed de todo el board de la red de talento de a16z Speedrun) y **Cryptocurrency Jobs** (la bolsa Web3 `cryptocurrencyjobs.co`, vía su RSS público) —, llevando el registro a **72 fuentes (67 en inglés + 5 en ruso)** y mostrando ambas en el filtro de Fuente de `#/scan` y en la landing de cvstart.org. También refleja dos correcciones del padre: **echojobs** ahora mantiene los puestos **híbridos** distinguibles del remoto (`Ciudad · Híbrido`, nunca colapsado en `Remote`), y **radancy** aprende el marcado legacy de TalentBrew además del transporte de fragmento de resultados JSON. **2123 tests**. **Seguimientos de la revisión de IA (parche).** v1.129.1 corrige hallazgos consultivos de los ports web de v1.128/v1.129: precedencia de nivel en job-facets (`Senior Engineering Manager` → `senior`, no `lead`), `states.mjs` ya no fija su fallback ante un fallo del padre al arranque (+avisa ante archivo malformado), el tono muestra neutral una fila sin puntuación (no roja), y `domainFromName` omite slugs no-ASCII. **Faceta de nivel + columna de antigüedad en el escaneo.** v1.129.0 integra la lib `job-facets.js` de v1.128.0 en `#/scan`: un nuevo filtro **Nivel** (lead/staff/senior/mid/junior/becario, autocompletado desde los resultados como la faceta de País), una columna de badge Nivel y una columna **Antigüedad** sin tokens (`hoy` / `Nd`). **Cuatro ports de la web app del padre.** v1.128.0 reimplementa en JS vanilla cuatro soluciones de la app Next.js de career-ops: el tracker lee estados canónicos en vivo desde `templates/states.yml` (sin whitelist hardcodeada), logos de marca ahora en filas con host ATS vía mapa nombre→dominio, un tono de puntuación de 4 niveles y una lib de facetas sin tokens. **2066 tests**. **Paridad con el padre v1.23.0.** v1.127.0 porta tres nuevas fuentes de escaneo — **Flowxtra**, **VDAB**, **iCIMS** (registro ahora **70 adaptadores**, 65 EN + 5 RU) —, refleja el cambio HTML→REST de agenticjobs, la recuperación de ciudad de Greenhouse y las correcciones de role-matcher, y devuelve **Cursor** al roster de CLIs (parent #2115 — `cli-detect` reporta 10 herramientas). **Parche de deriva del roster de CLI.** v1.126.1 corrige dos puntos que el barrido de v1.126.0 omitió: el intro de la pestaña API keys de `#/config` (i18n ×17) ahora lista Antigravity + Grok Build, y una fila obsoleta `Cursor / Gemini CLI` en la guía de ayuda ×17 (y el help del sitio) ahora tiene el roster completo de 8 CLIs. **Resync de docs y roster de CLIs.** v1.126.0 reconcilia cada superficie de docs (help ×17, README ×17, wiki) con la career-ops.org/docs viva (leídas las 31 páginas) y enseña al escaneo de AI CLI tools de `#/config` a detectar las 8 CLIs de primera clase — **Grok Build CLI** y **Kimi CLI** se unen a Claude Code, Cursor, Codex, OpenCode, Antigravity, Qwen y Copilot (ahora **9 herramientas**, sin ejecutarlas nunca). **Sync de mantenimiento.** v1.125.4 agrupa los bumps de dependabot en site (`sharp` 0.35.3, `svgo` 4.0.2, `fast-uri` 3.1.4) y registra un barrido de paridad con el padre (post-v1.22.0, #2108–#2168): el guard de fila errónea de set-status, el Risk Summary de modos localizados y la verificación de manifiesto del actualizador son solo CLI — nada que portar (**1969 pruebas** sin cambios). **Corrección de locale en prompts.** v1.125.3 hace del danés y el hindi ciudadanos de primera clase en cada prompt de IA — deep research (en vivo y manual), modos, evaluación, entrevista, networking y CV Studio ahora emiten la directiva `# Output language` en esos dos idiomas (las respuestas llegaban en inglés), y la barrera de locales ahora recorre los 17 (**1969 pruebas**). **Pack de contribuciones externas.** v1.125.2 fusiona los primeros PR de la comunidad, de [@Alien10140](https://github.com/Alien10140): el arreglo del HTTP 502 en Deep research en vivo (prompts headless sin herramientas para proveedores de API) y los modelos por defecto de Gemini actualizados del obsoleto `gemini-2.0-flash` a `gemini-3.6-flash`, fijados por una nueva puerta anti-deriva de 5 tests (**1957 tests**). v1.125.1 mantiene a los tenants RMK multimarca de SuccessFactors en su ruta de marca (padre #2099). **Fuentes de empleo en la landing.** La v1.125.0 añade una sección en cvstart.org que lista las **70 fuentes de escaneo** como chips pulsables, sincronizada con el registro en cada build y protegida contra desincronizaciones. Antes, v1.124.0 incorpora **cinco fuentes de escaneo** — Welcome to the Jungle, Agentic Engineering Jobs, Jobvite, Gem y Alibaba Group (**70 adaptadores** ahora) — además de la verificación de puestos totalmente remotos de Arbeitsagentur (`homeofficetyp=VOLLSTAENDIG`) y la corrección de la URL pública de SmartRecruiters.
>
> _división de app.css (3 hojas de estilo) · pago de deuda de contrato de tamaño · cascada idéntica byte a byte · anclaje de host de adaptadores · revalidación de la anulación de a16z Speedrun · host exacto en cleanUrl de cryptocurrencyjobs · tablero de pestañas de etapa CRM · port de /pipeline del tracker · pestañas de etapa + recuentos · GET /api/tracker/stages · logos de marca en el tracker · paridad con el padre v1.24 · +2 fuentes (a16z Speedrun/Cryptocurrency Jobs) · 72 fuentes · echojobs híbrido · radancy TalentBrew · parche revisión IA · precedencia de nivel · fallback states · tono neutral · faceta de nivel · columna antigüedad · job-facets integrada · 4 ports web/ · states.yml en vivo · logos nombre→dominio · tono · facetas · paridad padre v1.23 · +3 fuentes · 70 adaptadores · Cursor · parche de deriva del roster de CLI · resync de docs y roster de CLIs · sync de mantenimiento · fix de locale en prompts da/hi · pack de contribuciones externas · arreglo 502 de deep research · Gemini 3.6 por defecto · arreglo de ruta de marca SF · sección de fuentes en la landing · 5 fuentes · 67 adaptadores · paridad con el padre v1.22 · Oracle Cloud · 62 adaptadores · paridad con el padre v1.21 · hindi · 17 locales · manifiesto · páginas de metodología/licencia/historial de cambios en cvstart.org · paridad padre v1.19 · 61 adapters · estrellas en vivo + contribuidores en la landing · paridad padre v1.18 · 54 adaptadores · estado Hired · estadísticas históricas · paquete de paridad · 50 adaptadores · rework medidor de uso · retoque de diseño · medidor de uso · asistente flotante de ayuda · consolidación docs y QA · cierre backlog seguridad · docs y QA ×16 · Excluir en Scan · resumen pipeline · refuerzo de seguridad 2 · refuerzo del saneador · refuerzo de seguridad · uso y coste de IA · logos de empresa · herramientas CLI de IA · preguntar a la guía · adaptar CV + carta · autorrelleno del two-pager · exportación DOCX · estado de portales · reportador de errores integrado · 16 locales · 6 proveedores LLM · 46 adaptadores de escáner · orientación profesional · plan de carrera · rediseño de estadísticas · capa de memoria · CV Studio · planificador de networking · mock interview · encaje de mercado con el two-pager · paridad con el career-ops v1.16.0._
