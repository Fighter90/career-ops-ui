...
> **🆕 Última versión — v1.228.3** — **El asistente de documentación no respondía en inglés.** No era la recuperación: el constructor de contexto hacía `break` en la primera sección demasiado grande, dejando el contexto vacío. El §5 inglés llegó a 16 081 bytes frente a un presupuesto de 14 336. Las secciones de más de 6 KB ahora se dividen por sus `###` (32 → 75 fragmentos). Además: las etiquetas de empresa de Telegram daban la frase, no el nombre, y `GET /api/ping` ofrece una sonda que no filtra nada. **2907 pruebas.**
...
```bash
npm test                       # 2907 tests unit/integración
...
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integración) | **2907** | Cada endpoint, servidor efímero, sin red. Incluye parser, scanner (mockeado), runner, anthropic, security headers, XSS, JD sanitize, validación de URL, DNS rebind, mutex de archivo, rate limit, path traversal y paridad i18n. |
...
- Ejecuta `npm test` antes de hacer push — **2907 checks en verde** es el mínimo (más 101 Playwright si tocas la UI).
