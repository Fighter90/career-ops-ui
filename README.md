...
> **🆕 Latest release — v1.182.0** — **Locale-neutral salary ranges** — one-sided pay figures in scan & tracker now show as `≥` / `≤` (e.g. `≥ 120000 EUR`) instead of English "from" / "up to", so they read the same in every language. **2471 tests.**
>
