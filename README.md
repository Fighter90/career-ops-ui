...
> **🆕 Latest release — v1.159.0** — **Locale-aware report metadata (fix)** — Reports written in a non-English language showed a blank metadata strip (no score/date/legitimacy). `parseReportHeader` now reads the language-invariant `## Machine Summary` block (English reports stay byte-identical), tolerates locale number forms, and falls back to the file date. **2410 tests.**
>
