...
> **🆕 Latest release — v1.107.0**
>
> **Sanitizer hardening (at-rest XSS).** The CV/JD markdown sanitizer now strips dangerous HTML **to a fixed point** (so a removal that reforms a payload is caught), matches script/style end tags with trailing junk (`</script foo>`), and removes an unclosed executable opener. Defense-in-depth behind the escape-on-render client; closes the matching CodeQL findings.
>
> _sanitizer hardening · security hardening · AI usage & cost · company logos · AI CLI tools · ask-the-docs · CV tailor + cover letter · two-pager auto-fill · DOCX export · 16 locales · 6 LLM providers · 46 scanner adapters · portals health · in-app bug reporter · career orientation · career plan · statistics rework · memory layer · CV Studio · networking planner · mock interview · two-pager market-fit · parent career-ops v1.16.0 parity._
