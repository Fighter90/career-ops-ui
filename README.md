...
> **🆕 Latest release — v1.171.0** — **Type-scale + z-index tokens (D-4, first step)** — introduced `--font-size-*` and named `--z-*` layers; every z-index literal is migrated (values preserved → byte-identical stacking) and the core font sizes now use the ramp. No pixel changes. **2437 tests.**
>
