...
>
> **In-app bug reporter** (parent `web-v0.2.0` parity). A **🐞 Report a bug** button in the notifications drawer gathers a privacy-floored diagnostic snapshot — versions, your screen, browser, a `/api/health` check summary, and the last 20 errors from a new client-side ring buffer — plus a deterministic dedupe fingerprint, lets you **review the exact Markdown**, then opens a pre-filled GitHub issue. Nothing is auto-filed and it **never** carries your CV, profile, answers, job URLs, or keys. New libs `logbuf.js` + `bug-report.js`; 11 i18n keys ×16; `tests/bug-report.test.mjs`.
>
> _16 locales · 6 LLM providers · 46 scanner adapters · in-app bug reporter · career orientation · career plan · statistics rework · memory layer · CV Studio · networking planner · mock interview · two-pager market-fit · parent career-ops v1.16.0 parity._
