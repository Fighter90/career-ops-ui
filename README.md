...
> **🆕 Latest release — v1.195.0** — **Faster repost detection** — duplicate-posting detection no longer goes O(N²) on large scan histories; the per-company title grouping is now an inverted index. Output is byte-identical (proven by a differential test), just far quicker. **2521 tests.**
>
