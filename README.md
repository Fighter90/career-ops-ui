...
> **🆕 Latest release — v1.104.0**
>
> **Company logos in the scan table (privacy-preserving).** A new Appearance toggle draws each company's logo next to its name on `#/scan`. The logo is the company's **favicon from its own domain**, proxied server-side (`GET /api/logo`) through the SSRF-safe fetcher — so no third-party logo service learns which employers you view. Shared job-board hosts show a coloured letter badge instead; off by default.
>
> _company logos · AI CLI tools · ask-the-docs · CV tailor + cover letter · two-pager auto-fill · DOCX export · 16 locales · 6 LLM providers · 46 scanner adapters · portals health · in-app bug reporter · career orientation · career plan · statistics rework · memory layer · CV Studio · networking planner · mock interview · two-pager market-fit · parent career-ops v1.16.0 parity._
