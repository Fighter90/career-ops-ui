...
> **🆕 Latest release — v1.132.0** — the `#/scan` results-rendering subsystem is extracted into its own module (`public/js/lib/scan-results.js`) to satisfy the < 800-LOC file-size contract; behaviour is preserved byte-for-byte and verified in a real browser (`tests/playwright-scan-filters.mjs`). **2138 tests.**
>
