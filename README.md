...
> **🆕 Latest release — v1.183.0** — **Smarter duplicate detection** — the scanner now keys each job on a canonical URL, so the same posting re-listed with a tracking link (`?utm_…`), over `http`/`https`, or with a trailing slash no longer shows up twice. **2477 tests.**
>
