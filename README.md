...
> **🆕 Latest release — v1.227.3** — **Title-filter prefixes now work** — `word:` / `stem:` were matched as literal text, silently turning those filter lines into no-ops (a `word:intern` rule filtered nothing here while working in the CLI). Ported from the parent, Unicode boundaries included. **2852 tests.**
