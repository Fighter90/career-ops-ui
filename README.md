...
> **🆕 Latest release — v1.201.0** — **Localized tracker headers no longer render blank** — a `data/applications.md` with non-English or variant column headers (`empresa`/`estado`, `position`/`stage`/`link`…) now folds onto the canonical fields, so Company/Role/Status/Date/Link fill in correctly. **2563 tests.**
>
