...
> **🆕 Latest release — v1.108.0**
>
> **Security hardening (CodeQL triage, round 2).** The mode-prompt builder resolves the locale role-line by own key + `typeof === function` (a tampered locale can't dispatch to a prototype method), the PDF-filename slug is capped before its dash-trim regex (polynomial ReDoS), and document import coerces an array `filename` to a string (type-confusion). Over v1.106–v1.108 the static-analysis backlog went 167 → ~14, every real finding fixed and the rest dismissed with rationale.
>
> _security hardening 2 · sanitizer hardening · security hardening · AI usage & cost · company logos · AI CLI tools · ask-the-docs · CV tailor + cover letter · two-pager auto-fill · DOCX export · 16 locales · 6 LLM providers · 46 scanner adapters · portals health · in-app bug reporter · career orientation · career plan · statistics rework · memory layer · CV Studio · networking planner · mock interview · two-pager market-fit · parent career-ops v1.16.0 parity._
