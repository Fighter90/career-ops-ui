...
> **🆕 Latest release — v1.233.2** — **Two high-severity CodeQL `js/remote-property-injection` alerts, closed by making an existing invariant structural rather than asserted.** The loop that copies saved values into `process.env` iterated `Object.entries(safe)` — an object built from the request body — with an explicit `__proto__`/`constructor`/`prototype` guard inside. The code was safe: `safe` is populated twenty lines earlier from `KNOWN_KEYS` alone. But read on its own the loop says *assign to a property whose name came out of an object built from the request body*, and CodeQL cannot follow that constraint across two loops. It now iterates the constant array directly, so the property name is provably a module-level literal, and the guard against an impossible case went with it. **Behaviour is identical.** The test that pinned the old guard was a grep of the source text; it is replaced by a structural check (confirmed red against the old shape) and a behavioural one that posts `__proto__` as **raw JSON** — an object literal would have set the prototype instead of creating the property, so `JSON.stringify` would have sent `{}` and proven nothing. **3022 tests · 116 browser.**
