...
> **🆕 Latest release — v1.174.0** — **Localized report scores + SEO fix** — non-English reports no longer read "Score not detected" (an H1 that contained the label word was mistaken for the score); legitimacy chips drop stray `**`, long scores stop overflowing their card, and the cvstart.org description now counts the live registry (~75). **2448 tests.**
>
