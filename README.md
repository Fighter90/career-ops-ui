...
>
> **Portals health page** (`#/portals`). The scanner watches a set of companies in `portals.yml`; an ATS slug can quietly break and that employer silently vanishes from every future scan. The new **Portals** page lists every watched company and, on **Check portal health**, probes each `careers_url` through the DNS-pinned `safeGet` (SSRF-safe) and flags the dead ones (a 404 = silently dropped) — read-only. Also hardens the v1.98.0 bug reporter per review: the error ring buffer now catches network-layer fetch failures, and the scrubber redacts bare/unlabelled provider keys.
>
> _16 locales · 6 LLM providers · 46 scanner adapters · portals health · in-app bug reporter · career orientation · career plan · statistics rework · memory layer · CV Studio · networking planner · mock interview · two-pager market-fit · parent career-ops v1.16.0 parity._
