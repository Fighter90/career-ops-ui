...
> **🆕 Latest release — v1.105.0**
>
> **AI usage & cost page.** A new **AI usage** page shows how many tokens you've spent on live AI generations, per provider, over 24h / 7d / 30d / all-time, with an estimated USD cost. Each live call appends a `{provider, in, out}` record to `data/llm-usage.jsonl` (nothing leaves your machine); dollars come from an editable per-provider price table — tokens are exact, USD is an estimate, never billed.
>
> _AI usage & cost · company logos · AI CLI tools · ask-the-docs · CV tailor + cover letter · two-pager auto-fill · DOCX export · 16 locales · 6 LLM providers · 46 scanner adapters · portals health · in-app bug reporter · career orientation · career plan · statistics rework · memory layer · CV Studio · networking planner · mock interview · two-pager market-fit · parent career-ops v1.16.0 parity._
