...
> **🆕 Latest release — v1.101.0**
>
> **CV Studio: tailor to a job + cover letter.** A new **Tailor to a job** card on `#/cv-studio` turns a pasted job description into a résumé tailored to it plus a matching cover letter, then runs both through a recruiter-grade **checklist gate** (errors block, warnings advise). Distilled from career-coaching practice into generic rules — grounded only in your own CV, profile, and two-pager; it never fabricates. Export the result as Markdown / PDF / DOCX.
>
> _CV tailor + cover letter · two-pager auto-fill · DOCX export · 16 locales · 6 LLM providers · 46 scanner adapters · portals health · in-app bug reporter · career orientation · career plan · statistics rework · memory layer · CV Studio · networking planner · mock interview · two-pager market-fit · parent career-ops v1.16.0 parity._
