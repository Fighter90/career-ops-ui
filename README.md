...
> **🆕 Latest release — v1.232.1** — **Save on `#/config` wrote fields the user never touched.** Two sound decisions combined into a write: v1.57.1 seeds an unset control with its `defaultValue` so the field shows what the server really uses, while `save()` consulted `dirty` **for secrets only** and sent everything else regardless — so the value placed there for *display* travelled as if the user had chosen it. One dropdown change posted **28 keys and pinned 18** nobody had opened; changing nothing still posted 27. Nothing behaves differently that day — the values *are* the defaults — but each setting moves from *unset, follow the project default* to *pinned in `.env`*, so the next time a default changes it silently never reaches anyone who ever pressed Save. **The comparison basis is the interesting part:** comparing against the loaded `cfg.values` is not enough, because an unset select shows its default while the stored value is `''` — that cut the body from 27 keys to 19, not to one. Save now compares against what each control was **seeded** with. The toast's key count also works again: it lived inside `t()`'s fallback, and `config.saved` exists in all 17 dictionaries, so the number never rendered. **3018 tests · 111 browser.**
