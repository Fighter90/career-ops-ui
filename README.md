...
>
> **Candidate market fit — the two-pager (Epic 14):** a new **`#/two-pager`** page captures what _you_ actually want from your next role (loves, must-haves, hates, deal-breakers, non-negotiables) in the "Mnookin two-pager" format. An **AI fill assistant** drafts it from your CV + profile, every posting on `#/scan` gets a **fit-to-what-you-want** `◎` score, and the two-pager feeds every evaluation. Builds on v1.88.0 (Scan i18n + API hygiene), v1.87.0 (4 new scan providers), and v1.86.0 (target-role stats).
>
> _16 locales · 6 LLM providers · 45 scanner adapters · two-pager market-fit · target-role market stats · repost detector · parent career-ops v1.16.0 parity._
