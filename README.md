...
> **🆕 Latest release — v1.103.0**
>
> **Settings: AI CLI tools.** A new **AI CLI tools** tab in App settings shows which agent CLIs — Claude Code, Codex, Gemini CLI, OpenCode, GitHub Copilot CLI, Qwen, Antigravity — are installed on the machine running the server, and their paths. A read-only PATH scan: it only checks whether each binary exists and **never runs it** (no `--version`, no execution), writes nothing, reads no user data.
>
> _AI CLI tools · ask-the-docs · CV tailor + cover letter · two-pager auto-fill · DOCX export · 16 locales · 6 LLM providers · 46 scanner adapters · portals health · in-app bug reporter · career orientation · career plan · statistics rework · memory layer · CV Studio · networking planner · mock interview · two-pager market-fit · parent career-ops v1.16.0 parity._
