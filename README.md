...
> **🆕 Latest release — v1.106.0**
>
> **Security hardening (CodeQL triage).** The route-render error path now escapes the error message before it reaches the DOM (a server error can echo user input — XSS boundary), and the profile/config property writes reject `__proto__` / `constructor` / `prototype` keys (prototype-pollution guards). The bulk of the remaining static-analysis alerts are false positives on the scanner's legitimate `data/*` I/O and were dismissed with rationale.
>
> _security hardening · AI usage & cost · company logos · AI CLI tools · ask-the-docs · CV tailor + cover letter · two-pager auto-fill · DOCX export · 16 locales · 6 LLM providers · 46 scanner adapters · portals health · in-app bug reporter · career orientation · career plan · statistics rework · memory layer · CV Studio · networking planner · mock interview · two-pager market-fit · parent career-ops v1.16.0 parity._
