...
> **🆕 Latest release — v1.131.2**
>
> **`app.css` split (refactor).** v1.131.2 pays down a file-size-contract debt: the ~1990-LOC `app.css` is split into three ordered stylesheets — `app.css` (base/chrome/buttons), **`components.css`** (cards, tables, forms, badges, paginator, …), **`overlays.css`** (toast, modal, drawer, `[dir="rtl"]`, docs-fab, usage-hud) — each within the 800-LOC hard target. The cut is contiguous and in-order, so the cascade is **byte-for-byte identical**; `index.html` loads all three as ordered `<link>`s. No behavior/markup/i18n change; verified in-browser (all three sheets parse and apply, 0 console errors). **2138 tests**. **Adapter host-pinning hardening (patch).** v1.131.1 acts on code-review follow-ups on the v1.130.0 sources (defense-in-depth, no behavior change for valid inputs): the **a16z Speedrun** adapter now re-validates the endpoint override (HTTPS + exact host) and falls back to the canonical feed — parity with **Cryptocurrency Jobs** — and the Cryptocurrency Jobs parser's `cleanUrl` uses the same exact-host guard as its SSRF check (was `endsWith`, which accepted subdomains), so the parser is never looser than the guard. **2135 tests**. **CRM stage-tab board.** v1.131.0 ports the parent web app's `/pipeline` view to `#/tracker`: the funnel-chip bar + status dropdown become a proper **stage-tab strip** — **All** plus one tab per canonical status (Evaluated · Applied · Responded · Interview · Offer · Rejected · Discarded · SKIP · Hired), each with a live count, **zero-count stages included** so the whole funnel is always visible. Tabs come from a new read-only `GET /api/tracker/stages` (canonical labels + alias fold from `templates/states.yml` — the client never hardcodes the whitelist), rows gain optional brand logos, and the accessible names carry the counts. **2133 tests**. **Parent v1.24.0 parity.** v1.130.0 ports two new scan sources — **a16z Speedrun** (the a16z Speedrun _talent-network_ board-wide feed) and **Cryptocurrency Jobs** (the Web3 board [`cryptocurrencyjobs.co`](https://cryptocurrencyjobs.co), via its public RSS) — bringing the registry to **72 sources (67 English + 5 Russian)** and surfacing both in the `#/scan` Source filter and on the cvstart.org landing. It also mirrors two parent fixes: **echojobs** now keeps **hybrid** roles distinguishable from remote (`City · Hybrid`, never collapsed to `Remote`), and **radancy** learns legacy TalentBrew markup plus the JSON results-fragment transport. **2123 tests**. **AI-review follow-ups (patch).** v1.129.1 fixes advisory findings from the v1.128/v1.129 web-ports: job-facets seniority precedence (`Senior Engineering Manager` → `senior`, not `lead`), `states.mjs` no longer pins its fallback on a boot-time parent hiccup (+warns on a malformed file), score tone shows a no-score row neutral (not red), and `domainFromName` skips non-ASCII slugs. **Scan seniority facet + freshness column.** v1.129.0 wires the v1.128.0 `job-facets.js` lib into `#/scan`: a new **Seniority** filter (lead/staff/senior/mid/junior/intern, auto-populated from results like the Country facet), a Seniority badge column, and a zero-token **Age** column (`today` / `Nd`). **Four ports from the parent web app.** v1.128.0 re-implements four solutions from career-ops's own Next.js app in vanilla JS: the tracker reads canonical statuses live from `templates/states.yml` (no more hardcoded whitelist), brand logos now resolve on ATS-hosted rows via a name→domain map, a finer 4-tier score tone, and a zero-token job-facets lib. **2066 tests**. **Parent v1.23.0 parity.** v1.127.0 ports three new scan sources — **Flowxtra**, **VDAB**, **iCIMS** (registry now **70 adapters**, 65 EN + 5 RU) — mirrors the agenticjobs HTML→REST rewrite, Greenhouse office-city recovery and role-matcher fixes, and re-adds **Cursor** to the CLI roster (parent #2115 — `cli-detect` now reports 10 tools). **CLI-roster drift patch.** v1.126.1 fixes two spots the v1.126.0 sweep missed: the `#/config` API-keys tab intro (i18n ×17) now lists Antigravity + Grok Build, and a stale `Cursor / Gemini CLI` row in the help guide ×17 (and the CI-built site help) now carries the full 8-CLI roster. **Docs & CLI-roster resync.** v1.126.0 reconciles every doc surface (help ×17, README ×17, wiki) against the live career-ops.org/docs (all 31 pages read) and teaches the `#/config` AI-CLI-tools scan to detect all 8 first-class CLIs — **Grok Build CLI** and **Kimi CLI** join Claude Code, Cursor, Codex, OpenCode, Antigravity, Qwen and Copilot (now **9 tools** reported, still never executed). **Maintenance sync.** v1.125.4 rolls up the dependabot site bumps (`sharp` 0.35.3, `svgo` 4.0.2, `fast-uri` 3.1.4) and records a parent parity sweep (post-v1.22.0, #2108–#2168): the set-status wrong-row guard, localized-mode Risk Summary and updater manifest check are all CLI-side — nothing to port (**1969 tests** unchanged). **Prompt-locale fix.** v1.125.3 makes Danish and Hindi first-class in every AI prompt — deep research (live + manual), mode runs, evaluate, interview, networking, and CV Studio now emit the `# Output language` directive in those two locales (answers used to come back in English), and the locale gate now sweeps all 17 locales (**1969 tests**). **External-contributor pack.** v1.125.2 merges the first community PRs by [@Alien10140](https://github.com/Alien10140) — the live Deep-research HTTP 502 fix (headless no-tools prompts for API providers) and Gemini defaults bumped past the deprecated `gemini-2.0-flash` to `gemini-3.6-flash`, pinned by a new 5-test drift gate (**1957 tests**). v1.125.1 keeps multi-brand SuccessFactors RMK tenants on their brand path (parent #2099). **Job sources on the landing.** v1.125.0 adds a cvstart.org section listing all **70 scanner sources** as clickable chips, registry-synced at build and drift-gated. Before it, v1.124.0 ports **five scan sources** — Welcome to the Jungle, Agentic Engineering Jobs, Jobvite, Gem, and Alibaba Group (**70 adapters** now) — plus the Arbeitsagentur fully-remote verification (`homeofficetyp=VOLLSTAENDIG`) and the SmartRecruiters public-URL fix. Before it, v1.123.0 ported the **Oracle Recruiting Cloud** source and the repost-detector specialized-suffix fix. Before it, v1.122.0 added **हिन्दी (Hindi)** as the 17th interface language everywhere: the app UI, the in-app help guide, README, changelog, and every cvstart.org page (landing + Methodology/License/Changelog/Help). Also in this train: v1.120.0 (parent v1.20.0 parity) surfaced [the CareerOps Manifesto](https://career-ops.org/manifesto) across the app — sidebar-footer link, help §29 in 16 languages, README section. v1.121.0 adds four sections to [cvstart.org](https://cvstart.org): **Methodology** (the six-dimension scoring rubric, from [career-ops.org/methodology](https://career-ops.org/methodology)), **Compare**, **License**, and **Changelog** (all 16 languages), plus methodology links across the docs and help guide.
>
> _app.css split (3 stylesheets) · file-size-contract paydown · byte-identical cascade · adapter host-pinning hardening · a16z override re-validation · cryptocurrencyjobs cleanUrl exact-host · CRM stage-tab board · tracker /pipeline port · stage tabs + counts · GET /api/tracker/stages · tracker brand logos · parent v1.24 parity · +2 sources (a16z Speedrun/Cryptocurrency Jobs) · 72 sources · echojobs hybrid · radancy TalentBrew · AI-review patch · seniority precedence · states fallback · score-tone neutral · scan seniority facet · age column · job-facets wired · 4 web/ ports · states.yml live · name→domain logos · score tone · facets · parent v1.23 parity · +3 sources (Flowxtra/VDAB/iCIMS) · 70 adapters · Cursor re-added · CLI-roster drift patch · docs & CLI-roster resync · maintenance sync · da/hi prompt-locale fix · external-contributor pack · deep-research 502 fix · Gemini 3.6 defaults · SF brand-path fix · landing sources section · 5 sources (WTTJ/Jobvite/Gem/Alibaba/Agentic) · 67 adapters · parent v1.22 parity · Oracle Cloud · 62 adapters · parent v1.21 parity · Hindi · 17 locales · manifesto · cvstart.org methodology/license/changelog pages · parent v1.19 parity · 61 adapters · landing live stars + contributors · parent v1.18 parity · 54 adapters · Hired status · lifetime stats · parent parity pack · 50 adapters · usage meter rework · design polish · usage meter · floating docs assistant · docs & QA consolidation · security backlog closeout · docs & QA refresh ×16 · scan Exclude · pipeline overview · security hardening 2 · sanitizer hardening · security hardening · AI usage & cost · company logos · AI CLI tools · ask-the-docs · CV tailor + cover letter · two-pager auto-fill · DOCX export · 16 locales · 6 LLM providers · 46 scanner adapters · portals health · in-app bug reporter · career orientation · career plan · statistics rework · memory layer · CV Studio · networking planner · mock interview · two-pager market-fit · parent career-ops v1.16.0 parity._
...
[career-ops](https://career-ops.org) is an open-source job-search system that runs as slash commands inside any AI coding CLI (Claude Code, Cursor, Codex, OpenCode, Antigravity CLI, Grok Build CLI, Qwen Code, Kimi, GitHub Copilot CLI, Gemini CLI (legacy) — other Claude-compatible CLIs work too via the same slash-command surface). Model-agnostic. It evaluates each posting against your CV with a six-dimension 0.0–5.0 rubric, generates tailored PDF résumés, and tracks every application locally — no cloud accounts, no telemetry, no auto-submit.
