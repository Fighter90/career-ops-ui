...
> **🆕 Latest release — v1.224.0** — **Real provider logos** — the LLM provider tiles now show the real brand logo (Anthropic, OpenAI, Gemini, DeepSeek, Mistral, Ollama, …) where the provider publishes an open-source icon — still fully CSP-safe (inline SVG, no remote assets). **2784 tests.**
>
