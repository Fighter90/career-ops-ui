...
**変更 (低, スキャナー) — Getro パリティ移植 (親 #2640): 給与、全ロケーション、`work_mode` リモート判定。**

### 変更
- 親の `providers/getro.mjs` 拡張(#2640)の安全な部分を `server/lib/sources/getro.mjs` に移植しました。Getro の求人が **給与** 表示文字列(`compensation_amount_min/max_cents` + 通貨から、年額のみ — クライアントの給与フィルターが再解析する remotli/lever の文字列規約に準拠)を持ち、**ロケーション**は `locations` の**全件**を結合し(以前は最初の1件のみ)、`work_mode: 'remote'` が既存の `remote` フラグ / ロケーションテキストと並んでリモート信号として認識されます。web-ui が既に取得している API 応答の純粋なデータマッピング — 新しいホストなし、SSRF 面の変更なし。

### 備考
- 意図的に未移植: 親の `collection_id` **自動解決**(ボード自身の `careers_url`(任意ホスト)を取得するため、web-ui の `safeGet` SSRF 境界を経由する別の変更に属します)と引き上げられた 1500 ページのハードキャップ(web-ui はブラウザ起点スキャンのため 200 ページ / 4000 件のキャップを維持)。この同期の残り 11 の親コミットは web-ui にミラーがありません。テスト: `tests/sources-getro.test.mjs` (+5)。スイート: **2470** (+5)。
