...
career-ops는 [CareerOps 선언문](https://career-ops.org/manifesto?utm_source=career-ops-ui) — 증거와 규율, 그리고 지원자 쪽에 놓인 도구로 구직 활동을 운영한다는 실천 방식 — 의 첫 번째 레퍼런스 구현입니다. 한번 읽어 보세요. 당신이 믿는 바와 일치한다면 서명하세요 — 서명은 하나의 커밋이 됩니다. 이 앱은 사이드바 푸터에서 그 페이지로 연결됩니다.
...
```bash
npm test                       # 1845 unit/integration 테스트
...
| Suite                       | Tests | 내용                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **1845** | 모든 엔드포인트, 임시 서버, 네트워크 없음. parser, scanner (mocked), runner, anthropic, security headers, XSS, JD sanitize, URL validation, path traversal, DNS-rebind, file lock, rate limit, i18n 패리티 포함. |
...
- 푸시 전에 `npm test`를 실행합니다 — **1845 checks green**이 기준입니다(UI를 건드리는 경우 90개의 Playwright 테스트도 포함).
