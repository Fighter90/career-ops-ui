...
> **🆕 최신 릴리스 — v1.108.0**
>
> **보안 강화(CodeQL 분류 2차).** 모드 프롬프트 빌더가 역할 줄을 자체 키 + `typeof === function`으로 해석하고, PDF 파일명 슬러그를 정규식 이전에 제한하며(다항 ReDoS), 문서 가져오기가 배열 `filename`을 문자열로 강제 변환합니다(type-confusion). v1.106–v1.108에 걸쳐 정적 분석 백로그가 167에서 ~14로 줄고, 실제 문제는 모두 수정, 나머지는 근거와 함께 기각.
>
> _보안 강화 2 · 새니타이저 강화 · 보안 강화 · AI 사용량 · 회사 로고 · AI CLI 도구 · 문서에 질문 · CV 맞춤 + 커버레터 · 투페이저 자동 채우기 · DOCX 내보내기 · 포털 상태 · 인앱 버그 리포터 · 통계 재구성 · 16 locales · 6개 LLM 프로바이더 · 46개 스캐너 어댑터 · 커리어 방향성 · 커리어 플랜 · 메모리 레이어 · CV Studio · 네트워킹 플래너 · mock interview · 투페이저 시장 적합성 · 상위 career-ops v1.16.0 패리티._
