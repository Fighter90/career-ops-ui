...
> **🆕 최신 릴리스 — v1.95.0**
>
> **커리어 플랜 (Epic 26):** 새로운 **`#/career-plan`** 페이지는 당신의 CV와 프로필을 구체적이고 개인화된 성장 계획으로 바꿉니다. **기간**(6/12/24개월)과 선택적 포커스를 고르면, 모델은 출발점 스냅샷, 강점/성장 SWOT, **SMART / OKR / WOOP**로 된 목표, 대안 경로, 하드/소프트 스킬 계획, **월별 로드맵**, 진행 추적, 함정, 그리고 지원 방안을 작성합니다 — 모두 당신 자신의 자료에 기반하며, 당신의 이력을 지어내지 않습니다. 인라인으로 편집하고, 사용자 레이어에 **저장**하며, **Markdown 또는 PDF**로 내보낼 수 있습니다. v1.94.0(통계 재구성)과 v1.93.0(메모리 레이어)을 기반으로 합니다.
>
> _통계 재구성 · 16 locales · 6개 LLM 프로바이더 · 45개 스캐너 어댑터 · 커리어 플랜 · 메모리 레이어 · CV Studio · 네트워킹 플래너 · mock interview · 투페이저 시장 적합성 · 재게시 감지기 · 상위 career-ops v1.16.0 패리티._
