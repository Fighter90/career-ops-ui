...
> **🆕 최신 릴리스 — v1.103.0**
>
> **설정: AI CLI 도구.** 앱 설정의 새 **AI CLI 도구** 탭은 Claude Code, Codex, Gemini CLI, OpenCode, GitHub Copilot CLI, Qwen, Antigravity 중 서버를 실행하는 머신에 무엇이 설치되어 있는지와 경로를 보여줍니다. 읽기 전용 PATH 스캔: 각 바이너리 존재 여부만 확인하고 **절대 실행하지 않으며**(`--version` 없음, 실행 없음), 아무것도 쓰지 않고 사용자 데이터를 읽지 않습니다.
>
> _AI CLI 도구 · 문서에 질문 · CV 맞춤 + 커버레터 · 투페이저 자동 채우기 · DOCX 내보내기 · 포털 상태 · 인앱 버그 리포터 · 통계 재구성 · 16 locales · 6개 LLM 프로바이더 · 46개 스캐너 어댑터 · 커리어 방향성 · 커리어 플랜 · 메모리 레이어 · CV Studio · 네트워킹 플래너 · mock interview · 투페이저 시장 적합성 · 상위 career-ops v1.16.0 패리티._
