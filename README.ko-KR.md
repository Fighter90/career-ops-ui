...
> **🆕 최신 릴리스 — v1.109.0**
>
> **Scan 제외 필터 + 파이프라인 개요.** Scan 검색 상자가 쉼표를 OR로 취급하고("찾을 역할"), 새 제외 필드가 쉼표로 구분된 단어와 일치하는 행을 숨깁니다. 둘 다 저장된 검색에 유지됩니다. 파이프라인 페이지에 컴팩트 개요 스트립 추가 — 받은함 N, 추적 N, 트래커의 Applied / Responded / Interview / Offer 수, 각각 `#/tracker`로 연결.
>
> _Scan 제외 · 파이프라인 개요 · 보안 강화 2 · 새니타이저 강화 · 보안 강화 · AI 사용량 · 회사 로고 · AI CLI 도구 · 문서에 질문 · CV 맞춤 + 커버레터 · 투페이저 자동 채우기 · DOCX 내보내기 · 포털 상태 · 인앱 버그 리포터 · 통계 재구성 · 16 locales · 6개 LLM 프로바이더 · 46개 스캐너 어댑터 · 커리어 방향성 · 커리어 플랜 · 메모리 레이어 · CV Studio · 네트워킹 플래너 · mock interview · 투페이저 시장 적합성 · 상위 career-ops v1.16.0 패리티._
