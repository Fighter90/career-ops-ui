...
> **🆕 최신 릴리스 — v1.102.0**
>
> **문서에 질문 — 근거 있는 도움말 채팅.** 새 **문서에 질문 💬** 페이지는 당신의 언어로 된 앱 도움말 가이드**만** 사용해 사용법 질문에 답합니다(사용 섹션을 표시하고 이력서·구직 활동은 읽지 않음). 가이드의 `##` 섹션에 대한 의존성 없는 키워드 검색; 모델은 그로부터 답하거나 가이드가 다루지 않음을 인정합니다. LLM 키로 실시간, 없으면 복사용 프롬프트.
>
> _문서에 질문 · CV 맞춤 + 커버레터 · 투페이저 자동 채우기 · DOCX 내보내기 · 포털 상태 · 인앱 버그 리포터 · 통계 재구성 · 16 locales · 6개 LLM 프로바이더 · 46개 스캐너 어댑터 · 커리어 방향성 · 커리어 플랜 · 메모리 레이어 · CV Studio · 네트워킹 플래너 · mock interview · 투페이저 시장 적합성 · 상위 career-ops v1.16.0 패리티._
