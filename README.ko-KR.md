...
> **🆕 최신 릴리스 — v1.92.0**
>
> **CV Studio (Epic 21):** 새로운 **`#/cv-studio`** 페이지는 당신의 CV에 정직한 세 가지 도구를 제공합니다 — **이력서 진단**(항목별 설명이 딸린 결정론적 0–100 점수), **프라이버시 마스크**(이메일/전화/링크/주소/이름을 전적으로 브라우저에서 가림), 그리고 **사람처럼 만들기**(당신의 voice-dna와 글쓰기 샘플에 근거해 딱딱한 문장을 _당신의_ 목소리로 다시 쓰되, 사실을 지어내지 않음). v1.91.0(네트워킹 플래너), v1.90.0(mock interview), v1.89.0(투페이저 시장 적합성)을 기반으로 합니다.
>
> _16 locales · 6개 LLM 프로바이더 · 45개 스캐너 어댑터 · CV Studio · 네트워킹 플래너 · mock interview · 투페이저 시장 적합성 · 타깃 직무 시장 통계 · 재게시 감지기 · 상위 career-ops v1.16.0 패리티._
