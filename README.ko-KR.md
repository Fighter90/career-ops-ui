...
> **🆕 최신 릴리스 — v1.113.0**
>
> **모든 페이지의 떠 있는 "도움말에 질문하기" 어시스턴트.** 그라데이션 로봇 채팅 버튼이 모든 페이지의 오른쪽 아래(RTL은 왼쪽 아래)에 떠 있습니다 — 클릭하면 당신 언어의 인앱 도움말 가이드에만 근거해 사용법 질문에 답하는 소형 채팅이 열립니다(CV나 구직 내용은 읽지 않음). LLM 키가 있으면 실시간, 없으면 바로 실행 가능한 프롬프트.
>
> _떠 있는 도움말 어시스턴트 · 문서 및 QA 통합 · 보안 백로그 정리 · 문서 및 QA ×16 · Scan 제외 · 파이프라인 개요 · 보안 강화 2 · 새니타이저 강화 · 보안 강화 · AI 사용량 · 회사 로고 · AI CLI 도구 · 문서에 질문 · CV 맞춤 + 커버레터 · 투페이저 자동 채우기 · DOCX 내보내기 · 포털 상태 · 인앱 버그 리포터 · 통계 재구성 · 16 locales · 6개 LLM 프로바이더 · 46개 스캐너 어댑터 · 커리어 방향성 · 커리어 플랜 · 메모리 레이어 · CV Studio · 네트워킹 플래너 · mock interview · 투페이저 시장 적합성 · 상위 career-ops v1.16.0 패리티._
