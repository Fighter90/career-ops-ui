...
> **🆕 최신 릴리스 — v1.228.2** — **두 단어 검색이 단어가 아니라 구절을 찾고 있었습니다.** `/api/scan-results?q=`는 `includes(q)`를 한 번만 수행해, 두 단어를 모두 가진 행이 162개인데 32개만 찾았습니다. 좁은 일치는 이렇게 틀린 답이 됩니다. 이제 단어를 각각 대조합니다(AND). **테스트 2899개.**
...
```bash
npm test                       # 2899 unit/integration 테스트
...
| Suite                       | Tests | 내용                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **2899** | 모든 엔드포인트, 임시 서버, 네트워크 없음. parser, scanner (mocked), runner, anthropic, security headers, XSS, JD sanitize, URL validation, path traversal, DNS-rebind, file lock, rate limit, i18n 패리티 포함. |
...
- 푸시 전에 `npm test`를 실행합니다 — **2899 checks green**이 기준입니다(UI를 건드리는 경우 101개의 Playwright 테스트도 포함).
