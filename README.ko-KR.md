...
> **🆕 최신 릴리스 — v1.111.0**
>
> **보안 — CodeQL 백로그 정리.** 세 가지 심층 방어 강화가 남은 정적 분석 지적을 근본 원인에서 닫습니다: CV 마크다운 정화기는 잘린 위험 태그 여는 부분을 이스케이프해 출력에 살아 있는 `<script`/`<iframe`가 절대 존재하지 않게 하고, CV 가져오기는 버퍼 크기를 명시적 숫자 강제 변환으로 읽으며, 모드 역할 줄은 저장된 함수 대신 `String.replace`로 보간되는 템플릿 문자열이 되어 마지막 동적 디스패치 호출을 제거합니다. 사용자 표시 변화 없음.
>
> _보안 백로그 정리 · 문서 및 QA ×16 · Scan 제외 · 파이프라인 개요 · 보안 강화 2 · 새니타이저 강화 · 보안 강화 · AI 사용량 · 회사 로고 · AI CLI 도구 · 문서에 질문 · CV 맞춤 + 커버레터 · 투페이저 자동 채우기 · DOCX 내보내기 · 포털 상태 · 인앱 버그 리포터 · 통계 재구성 · 16 locales · 6개 LLM 프로바이더 · 46개 스캐너 어댑터 · 커리어 방향성 · 커리어 플랜 · 메모리 레이어 · CV Studio · 네트워킹 플래너 · mock interview · 투페이저 시장 적합성 · 상위 career-ops v1.16.0 패리티._
