...
> **🆕 최신 릴리스 — v1.89.0**
>
> **후보자-시장 적합성 — 투페이저(Epic 14):** 새로운 **`#/two-pager`** 페이지는 _당신이_ 다음 직무에서 진짜로 원하는 것(좋아하는 것, 필수 조건, 싫어하는 것, deal-breaker, 타협 불가 사항)을 "Mnookin 투페이저" 형식으로 담아냅니다. **AI 채우기 도우미**가 당신의 CV + 프로필로부터 초안을 작성하고, `#/scan`의 모든 공고에는 **원하는-것과의-적합도 `◎` 점수**가 매겨지며, 투페이저는 모든 평가에 반영됩니다. v1.88.0(Scan i18n + API 위생), v1.87.0(새로운 스캔 프로바이더 4개), v1.86.0(타깃 직무 통계)을 기반으로 합니다.
>
> _16 locales · 6개 LLM 프로바이더 · 45개 스캐너 어댑터 · 투페이저 시장 적합성 · 타깃 직무 시장 통계 · 재게시 감지기 · 상위 career-ops v1.16.0 패리티._
