...
> **🆕 최신 릴리스 — v1.144.0** — **설정 및 필터(Phase 4): 추적 포털 활성화/비활성화** — `#/portals`에서 감시 회사를 켜고 끄기(`POST /api/portals/toggle`, portals.yml 외과적 쓰기); 스캐너는 이미 `enabled: false` 회사를 건너뜁니다. **2368 테스트.**
>
