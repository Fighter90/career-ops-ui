...
> **🆕 최신 릴리스 — v1.94.0**
>
> **통계, 재구성 (Epic 25):** **`#/stats`** 페이지는 이제 실제 그래프와 훨씬 더 많은 데이터를 갖춘 세 개의 탭으로 된 **통계** 섹션입니다. 새로운 **시장 리포트** 탭은 당신의 타깃 직무에 대한 AI 급여 및 노동 시장 분석을 생성합니다 — 등급별 P10–P90 백분위, 상위 고용주, 수요가 많은 스킬, 복리후생, 원격 비율, 추세, 그리고 협상 가이드를, 당신이 선택한 지역과 **통화**로(수치는 스크레이핑이 아니라 모델의 방향성 추정치로 표기). 새로운 **내 파이프라인** 탭은 당신 자신의 트래커를 차트로 보여줍니다(점수, 퍼널, 상위 회사/직무, 타임라인, 전환율). 모든 리포트는 **Markdown 또는 PDF**로 내보낼 수 있습니다. v1.93.0(메모리 레이어)과 v1.92.0(CV Studio)을 기반으로 합니다.
>
> _통계 재구성 · 16 locales · 6개 LLM 프로바이더 · 45개 스캐너 어댑터 · 메모리 레이어 · CV Studio · 네트워킹 플래너 · mock interview · 투페이저 시장 적합성 · 재게시 감지기 · 상위 career-ops v1.16.0 패리티._
