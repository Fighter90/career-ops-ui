...
> **🆕 최신 릴리스 — v1.91.0**
>
> **네트워킹 및 심층 기업 리서치 (Epic 16):** 새로운 **`#/networking`** 페이지는 기업 하나를 실행 가능한 계획으로 바꿔줍니다 — **기업 도시에**, **누구에게 연락할지**(페르소나 + LinkedIn 검색 문자열), **가장 따뜻한 소개 경로**, 맞춤형 **아웃리치 초안** — 모두 당신의 CV와 투페이저에 근거합니다. 어떤 LLM 키로든 실시간으로 작동하거나 복사·붙여넣기용 프롬프트를 돌려주며, 기업별로 **계획을 저장**할 수 있습니다. v1.90.0(mock interview), v1.89.0(투페이저 후보자-시장 적합성), v1.88.0(Scan i18n + API 위생)을 기반으로 합니다.
>
> _16 locales · 6개 LLM 프로바이더 · 45개 스캐너 어댑터 · 네트워킹 플래너 · mock interview · 투페이저 시장 적합성 · 타깃 직무 시장 통계 · 재게시 감지기 · 상위 career-ops v1.16.0 패리티._
