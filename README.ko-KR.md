...
> **🆕 최신 릴리스 — v1.138.0** — **인터페이스 언어로 생성** — 모든 AI 생성(커리어 플랜, 오리엔테이션, 시장 리포트, 모의 면접, 네트워킹, 「문서에 묻기」, 메모리 노트, two-pager)이 이제 항상 영어가 아니라 UI에서 선택한 언어로 응답합니다. 아울러 리뷰에서 나온 테스트 강화(CSS 색 역할 카나리아와 `UI.md()` XSS 로더 자가 점검). **2356 테스트.**
>
