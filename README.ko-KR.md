...
> **🆕 최신 릴리스 — v1.105.0**
>
> **AI 사용량 및 비용 페이지.** 새 **AI 사용량** 페이지는 실시간 AI 생성에 쓴 토큰을 제공자별로 24시간 / 7일 / 30일 / 전체 기간에 걸쳐 예상 USD 비용과 함께 보여줍니다. 각 실시간 호출은 `{provider, in, out}` 레코드를 `data/llm-usage.jsonl`에 추가합니다(아무것도 기기를 떠나지 않음). 달러는 편집 가능한 제공자별 가격표에서 나오며 — 토큰은 정확하고 USD는 추정치이며 청구되지 않습니다.
>
> _AI 사용량 · 회사 로고 · AI CLI 도구 · 문서에 질문 · CV 맞춤 + 커버레터 · 투페이저 자동 채우기 · DOCX 내보내기 · 포털 상태 · 인앱 버그 리포터 · 통계 재구성 · 16 locales · 6개 LLM 프로바이더 · 46개 스캐너 어댑터 · 커리어 방향성 · 커리어 플랜 · 메모리 레이어 · CV Studio · 네트워킹 플래너 · mock interview · 투페이저 시장 적합성 · 상위 career-ops v1.16.0 패리티._
