...
> **🆕 최신 릴리스 — v1.114.0**
>
> **사이드바의 AI 사용량·비용 미터.** 모든 페이지 사이드바 하단의 컴팩트 사용량 섹션(왼쪽 아래; RTL은 오른쪽 아래)이 24시간 / 7일 / 30일 LLM 토큰 사용량을 녹색 바와 24시간 예상 비용으로 보여줍니다 — 읽기 전용 로컬 사용 로그(#/usage 페이지와 동일 소스)에서. 접을 수 있음; 비용은 추정치이고 수동 모드 실행은 무료.
>
> _사용량 미터 · 떠 있는 도움말 어시스턴트 · 문서 및 QA 통합 · 보안 백로그 정리 · 문서 및 QA ×16 · Scan 제외 · 파이프라인 개요 · 보안 강화 2 · 새니타이저 강화 · 보안 강화 · AI 사용량 · 회사 로고 · AI CLI 도구 · 문서에 질문 · CV 맞춤 + 커버레터 · 투페이저 자동 채우기 · DOCX 내보내기 · 포털 상태 · 인앱 버그 리포터 · 통계 재구성 · 16 locales · 6개 LLM 프로바이더 · 46개 스캐너 어댑터 · 커리어 방향성 · 커리어 플랜 · 메모리 레이어 · CV Studio · 네트워킹 플래너 · mock interview · 투페이저 시장 적합성 · 상위 career-ops v1.16.0 패리티._
