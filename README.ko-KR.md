...
> **🆕 최신 릴리스 — v1.107.0**
>
> **새니타이저 강화(저장 시 XSS).** 이력서/공고 마크다운 새니타이저가 이제 위험한 HTML을 **고정점까지** 제거하고(재구성되는 제거를 잡아냄), 뒤에 쓰레기가 붙은 script/style 종료 태그(`</script foo>`)와 닫히지 않은 실행 오프너를 제거합니다. 렌더 시 이스케이프 클라이언트 뒤의 심층 방어이며 해당 CodeQL 발견을 해소합니다.
>
> _새니타이저 강화 · 보안 강화 · AI 사용량 · 회사 로고 · AI CLI 도구 · 문서에 질문 · CV 맞춤 + 커버레터 · 투페이저 자동 채우기 · DOCX 내보내기 · 포털 상태 · 인앱 버그 리포터 · 통계 재구성 · 16 locales · 6개 LLM 프로바이더 · 46개 스캐너 어댑터 · 커리어 방향성 · 커리어 플랜 · 메모리 레이어 · CV Studio · 네트워킹 플래너 · mock interview · 투페이저 시장 적합성 · 상위 career-ops v1.16.0 패리티._
