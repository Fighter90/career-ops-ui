...
> **🆕 최신 릴리스 — v1.131.2**
>
> **`app.css` 분할(리팩터링).** v1.131.2는 파일 크기 계약 부채를 해소합니다: ~1990줄이던 `app.css`를 세 개의 순서가 있는 스타일시트로 분할합니다 — `app.css`(베이스/크롬/버튼), **`components.css`**(카드, 테이블, 폼, 배지, 페이지네이터 등), **`overlays.css`**(토스트, 모달, 드로어, `[dir="rtl"]`, docs-fab, usage-hud) — 각각 800줄 하드 목표 이내입니다. 분할은 연속적이며 순서를 유지하므로 캐스케이드는 **바이트 단위로 완전히 동일**합니다. `index.html`은 세 파일 모두를 순서대로 `<link>`로 로드합니다. 동작/마크업/i18n 변경 없음이며 브라우저에서 검증했습니다(세 시트 모두 파싱·적용, 콘솔 오류 0건). **2138 테스트**. **어댑터 호스트 고정 강화(패치).** v1.131.1은 v1.130.0 소스에 대한 코드 리뷰 후속 조치를 반영합니다(심층 방어, 유효한 입력에 대한 동작 변화 없음): **a16z Speedrun** 어댑터가 이제 엔드포인트 오버라이드를 재검증하고(HTTPS + 정확한 호스트) 실패 시 정규 피드로 폴백합니다 — **Cryptocurrency Jobs**와의 패리티 — 그리고 Cryptocurrency Jobs 파서의 `cleanUrl`이 이제 SSRF 검사와 동일한 정확 호스트 가드를 사용합니다(이전에는 서브도메인을 허용하는 `endsWith`였습니다), 따라서 파서는 가드보다 느슨해질 수 없습니다. **2135 테스트**. **CRM 스테이지 탭 보드.** v1.131.0은 상위 웹 앱의 `/pipeline` 뷰를 `#/tracker`로 이식합니다: 퍼널 칩 바 + 상태 드롭다운이 제대로 된 **스테이지 탭 스트립**으로 바뀝니다 — **All** 플러스 정규 상태별 탭 하나씩(Evaluated · Applied · Responded · Interview · Offer · Rejected · Discarded · SKIP · Hired), 각각 실시간 카운트를 표시하며, **카운트 0인 스테이지도 포함**되어 전체 퍼널이 항상 보입니다. 탭은 새 읽기 전용 `GET /api/tracker/stages`(정규 라벨 + `templates/states.yml`에서의 별칭 폴드 — 클라이언트는 화이트리스트를 하드코딩하지 않음)에서 오며, 행은 선택적 브랜드 로고를 얻고, 접근 가능한 이름에 카운트가 포함됩니다. **2133 테스트**. **상위 v1.24.0 패리티.** v1.130.0은 2개의 새 스캔 소스 — **a16z Speedrun**(a16z Speedrun talent-network 보드 전체 피드)과 **Cryptocurrency Jobs**(Web3 보드 `cryptocurrencyjobs.co`, 공개 RSS 경유) — 를 포팅하여 레지스트리를 **72개 소스(영문 67개 + 러시아어 5개)**로 확장하고, 둘 다 `#/scan` Source 필터와 cvstart.org 랜딩에 표시합니다. 또한 상위의 2가지 수정 사항을 미러링합니다: **echojobs**는 이제 **하이브리드** 근무 형태를 원격과 구분되게 유지하며(`City · Hybrid`, 절대 `Remote`로 뭉개지지 않음), **radancy**는 레거시 TalentBrew 마크업과 JSON 결과 프래그먼트 전송을 학습합니다. **2123 테스트**. **AI 리뷰 후속(패치).** v1.129.1은 v1.128/v1.129 웹 포트의 권고 사항을 수정: job-facets 레벨 우선순위(`Senior Engineering Manager` → `senior`, `lead` 아님), `states.mjs`가 부팅 시 부모 문제로 폴백을 더 이상 고정하지 않음(+손상 파일 경고), 점수 톤이 점수 없는 행을 중립으로 표시(빨강 아님), `domainFromName`이 비-ASCII 슬러그를 건너뜀. **스캔 레벨 패싯 + 게시일 컬럼.** v1.129.0은 v1.128.0의 `job-facets.js` 라이브러리를 `#/scan`에 연결합니다: 새 **레벨** 필터(lead/staff/senior/mid/junior/intern, 결과에서 자동 채움 — Country 패싯처럼), 레벨 배지 컬럼, 무토큰 **게시일** 컬럼(`오늘`/`N일`). **상위 웹앱에서 4개 포팅.** v1.128.0은 career-ops의 Next.js 앱 솔루션 4개를 바닐라 JS로 재구현합니다: 트래커가 `templates/states.yml`에서 정규 상태를 실시간 읽고(하드코딩 화이트리스트 제거), ATS 호스트 행에 이름→도메인 매핑으로 브랜드 로고, 더 세밀한 4단계 점수 톤, 무토큰 job-facets 라이브러리. **2066 테스트**. **상위 v1.23.0 패리티.** v1.127.0은 3개의 새 스캔 소스 — **Flowxtra**, **VDAB**, **iCIMS** (레지스트리 이제 **70개 어댑터**, 65 EN + 5 RU) — 를 포팅하고, agenticjobs HTML→REST 전환·Greenhouse 도시 복구·role-matcher 수정을 미러링하며, **Cursor**를 CLI 로스터에 복귀시킵니다(parent #2115 — `cli-detect` 10개 도구 보고). **CLI 로스터 드리프트 패치.** v1.126.1은 v1.126.0 스윕이 놓친 두 곳을 수정: `#/config` API keys 탭 인트로(i18n ×17)가 이제 Antigravity + Grok Build를 나열하고, 도움말 가이드 ×17(및 CI 빌드 사이트 help)의 구식 `Cursor / Gemini CLI` 행이 이제 전체 8-CLI 로스터를 담습니다. **문서·CLI 로스터 재동기화.** v1.126.0은 모든 문서 표면(help ×17, README ×17, wiki)을 라이브 career-ops.org/docs(31페이지 전부 읽음)와 대조하고, `#/config`의 AI CLI 도구 스캔이 8개 일급 CLI를 모두 감지하도록 합니다 — **Grok Build CLI**와 **Kimi CLI**가 Claude Code·Cursor·Codex·OpenCode·Antigravity·Qwen·Copilot에 합류(이제 **9개 도구** 보고, 여전히 실행 안 함). **유지보수 동기화.** v1.125.4는 dependabot site 업데이트(`sharp` 0.35.3, `svgo` 4.0.2, `fast-uri` 3.1.4)를 통합하고 상위 프로젝트 패리티 스윕(v1.22.0 이후, #2108–#2168)을 기록합니다: set-status 잘못된 행 가드, 로컬라이즈 모드 Risk Summary, 업데이터 매니페스트 검증은 모두 CLI 전용 — 포팅할 것 없음(**1969 테스트** 변화 없음). **프롬프트 로케일 수정.** v1.125.3은 덴마크어와 힌디어를 모든 AI 프롬프트에서 정식 지원합니다 — 딥 리서치(라이브·수동), 모드 실행, 평가, 인터뷰, 네트워킹, CV Studio가 이제 이 두 로케일에서 `# Output language` 지시문을 내보내며(이전에는 영어로 응답), 로케일 게이트는 17개 로케일 전체를 검사합니다(**1969 테스트**). **외부 기여자 팩.** v1.125.2는 [@Alien10140](https://github.com/Alien10140)의 첫 커뮤니티 PR을 병합합니다 — 라이브 딥 리서치 HTTP 502 수정(API 제공자용 도구 없는 헤드리스 프롬프트)과 지원 종료된 `gemini-2.0-flash`를 `gemini-3.6-flash`로 올린 Gemini 기본값, 새 5-테스트 드리프트 게이트로 고정(**1957 테스트**). v1.125.1은 멀티 브랜드 SuccessFactors RMK 테넌트의 브랜드 경로를 유지합니다(부모 #2099). **랜딩 페이지의 채용 공고 소스.** v1.125.0은 cvstart.org에 **스캔 소스 67개** 전체를 클릭 가능한 칩으로 나열하는 섹션을 추가했으며, 빌드 시점에 레지스트리와 동기화되고 드리프트가 감시됩니다. 그 이전에는, **상위 v1.22.0 패리티.** v1.124.0은 Welcome to the Jungle, Agentic Engineering Jobs, Jobvite, Gem, Alibaba Group까지 **스캔 소스 5개**(이제 **어댑터 67개** — EN 62개 + RU 5개)를 포팅하고, 여기에 Arbeitsagentur의 완전 원격 근무 검증(`homeofficetyp=VOLLSTAENDIG`)과 SmartRecruiters의 공개 URL 수정 사항을 더합니다.
>
> _app.css 분할(스타일시트 3개) · 파일 크기 계약 부채 해소 · 바이트 단위 동일 캐스케이드 · 어댑터 호스트 고정 강화 · a16z 오버라이드 재검증 · cryptocurrencyjobs cleanUrl 정확 호스트 · CRM 스테이지 탭 보드 · 트래커 /pipeline 포팅 · 스테이지 탭 + 카운트 · GET /api/tracker/stages · 트래커 브랜드 로고 · 상위 v1.24 패리티 · 소스 +2(a16z Speedrun/Cryptocurrency Jobs) · 소스 72개 · echojobs 하이브리드 · radancy TalentBrew · AI 리뷰 패치 · 레벨 우선순위 · states 폴백 · 중립 톤 · 스캔 레벨 패싯 · 게시일 컬럼 · job-facets 연결 · web/ 4포팅 · states.yml 실시간 · 이름→도메인 로고 · 점수 톤 · 파셋 · 상위 v1.23 패리티 · +3 소스 · 70 어댑터 · Cursor · CLI 로스터 드리프트 패치 · 문서·CLI 로스터 재동기화 · 유지보수 동기화 · da/hi 프롬프트 로케일 수정 · 외부 기여자 팩 · 딥 리서치 502 수정 · Gemini 3.6 기본값 · SF 브랜드 경로 수정 · 랜딩 소스 섹션 · 소스 5개 · 어댑터 67개 · 부모 v1.22 패리티 · Oracle Cloud · 어댑터 62개 · 부모 v1.21 패리티 · 힌디어 · 17개 언어 · 선언문 · cvstart.org 방법론/라이선스/체인지로그 페이지 · 부모 v1.19 패리티 · 61개 어댑터 · 랜딩 실시간 스타 + 기여자 · 부모 v1.18 패리티 · 어댑터 54개 · Hired 상태 · 누적 통계 · 패리티 팩 · 어댑터 50 · 사용량 미터 재작업 · 디자인 다듬기 · 사용량 미터 · 떠 있는 도움말 어시스턴트 · 문서 및 QA 통합 · 보안 백로그 정리 · 문서 및 QA ×16 · Scan 제외 · 파이프라인 개요 · 보안 강화 2 · 새니타이저 강화 · 보안 강화 · AI 사용량 · 회사 로고 · AI CLI 도구 · 문서에 질문 · CV 맞춤 + 커버레터 · 투페이저 자동 채우기 · DOCX 내보내기 · 포털 상태 · 인앱 버그 리포터 · 통계 재구성 · 16 locales · 6개 LLM 프로바이더 · 46개 스캐너 어댑터 · 커리어 방향성 · 커리어 플랜 · 메모리 레이어 · CV Studio · 네트워킹 플래너 · mock interview · 투페이저 시장 적합성 · 상위 career-ops v1.16.0 패리티._
