...
> **🆕 최신 릴리스 — v1.101.0**
>
> **CV Studio: 채용공고 맞춤 + 커버레터.** `#/cv-studio`의 새 **채용공고에 맞춤 조정** 카드는 채용공고를 맞춤 이력서와 어울리는 커버레터로 바꾸고, 둘 다 리크루터 수준의 **체크리스트 게이트**에 통과시킵니다(오류는 차단, 경고는 권고). 커리어 코칭 실무를 일반 규칙으로 증류 — 오직 당신의 이력서·프로필·투페이저에만 근거하며 절대 지어내지 않습니다. 결과를 Markdown / PDF / DOCX로 내보내세요.
>
> _CV 맞춤 + 커버레터 · 투페이저 자동 채우기 · DOCX 내보내기 · 포털 상태 · 인앱 버그 리포터 · 통계 재구성 · 16 locales · 6개 LLM 프로바이더 · 46개 스캐너 어댑터 · 커리어 방향성 · 커리어 플랜 · 메모리 레이어 · CV Studio · 네트워킹 플래너 · mock interview · 투페이저 시장 적합성 · 상위 career-ops v1.16.0 패리티._
