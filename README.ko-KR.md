...
> **🆕 최신 릴리스 — v1.93.0**
>
> **메모리 레이어 (Epic 24):** 새로운 **`#/memory`** 페이지는 짧고 편집 가능한 "나에 대해 이걸 기억해 줘" 메모를 담습니다. 공유 프로젝트 컨텍스트에 인라인으로 삽입되므로, **모든** 제공자에 걸쳐 **모든** AI 요청 — 평가, mock interview, 네트워킹, CV Studio — 을 이끕니다. 당신의 선호와 일하는 방식을 담으며(당신에 대한 새로운 사실은 절대 아님), **당신의 데이터에서 제안** 헬퍼가 당신의 트래커에서 검토할 행동 불릿을 초안으로 작성합니다. v1.92.0(CV Studio), v1.91.0(네트워킹 플래너), v1.90.0(mock interview)을 기반으로 합니다.
>
