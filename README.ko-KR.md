...
> **🆕 최신 릴리스 — v1.174.0** — **현지화 리포트 점수 + SEO 수정** — 비영어 리포트가 더 이상 "Score not detected"로 표시되지 않고(라벨 단어가 든 H1을 점수로 오인), 정당성 칩은 남은 `**`를 제거하며, 긴 점수는 카드를 넘치지 않고, cvstart.org 설명은 실제 레지스트리를 셉니다(~75). **2448개 테스트.**
>
