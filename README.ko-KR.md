...
> **🆕 최신 릴리스 — v1.104.0**
>
> **스캔 표의 회사 로고(개인정보 보호).** 새 모양 토글이 `#/scan`에서 회사 이름 옆에 로고를 그립니다. 로고는 **회사 자체 도메인의 favicon**이며 SSRF 안전 페처를 통해 서버에서 프록시됩니다(`GET /api/logo`). 따라서 제3자 로고 서비스가 당신이 보는 고용주를 알 수 없습니다. 공용 채용 보드 호스트는 글자 배지를 표시하며 기본 꺼짐입니다.
>
> _회사 로고 · AI CLI 도구 · 문서에 질문 · CV 맞춤 + 커버레터 · 투페이저 자동 채우기 · DOCX 내보내기 · 포털 상태 · 인앱 버그 리포터 · 통계 재구성 · 16 locales · 6개 LLM 프로바이더 · 46개 스캐너 어댑터 · 커리어 방향성 · 커리어 플랜 · 메모리 레이어 · CV Studio · 네트워킹 플래너 · mock interview · 투페이저 시장 적합성 · 상위 career-ops v1.16.0 패리티._
