...
> **🆕 최신 릴리스 — v1.86.0**
>
> **타깃 직무별 통계**(`#/stats`): 새로운 Analytics 페이지가 **프로필의 타깃 직무**(하드코딩 아님)와 최신 스캔을 읽어 **국가별 채용 공고 수와 중앙값 급여**를 차트로 표시하며, 직무/국가 필터와 **저장된 스냅샷 추세**를 제공합니다 — 스캐너가 이미 수집하는 희소 데이터에서 클라이언트 측으로 집계(조작된 수치 없음)하며, 16개 언어 전체를 지원합니다. v1.85.0(독일어/이탈리아어/터키어 로케일), v1.84.0(재적용 쿨다운), v1.83.0(재게시 감지기)을 기반으로 합니다.
>
> _16 locales · 6개 LLM 프로바이더 · 41개 스캐너 어댑터 · 타깃 직무 시장 통계 · 재게시 감지기 · 상위 career-ops v1.16.0 패리티._
