...
> **🆕 최신 릴리스 — v1.106.0**
>
> **보안 강화(CodeQL 분류).** 라우트 렌더 오류 경로가 이제 오류 메시지를 DOM에 도달하기 전에 이스케이프하며(서버 오류가 사용자 입력을 반영할 수 있음 — XSS 경계), 프로필/설정 속성 쓰기가 `__proto__` / `constructor` / `prototype` 키를 거부합니다(프로토타입 오염 방지). 나머지 정적 분석 경고 대부분은 스캐너의 정당한 `data/*` 입출력에 대한 오탐이며 근거와 함께 기각했습니다.
>
> _보안 강화 · AI 사용량 · 회사 로고 · AI CLI 도구 · 문서에 질문 · CV 맞춤 + 커버레터 · 투페이저 자동 채우기 · DOCX 내보내기 · 포털 상태 · 인앱 버그 리포터 · 통계 재구성 · 16 locales · 6개 LLM 프로바이더 · 46개 스캐너 어댑터 · 커리어 방향성 · 커리어 플랜 · 메모리 레이어 · CV Studio · 네트워킹 플래너 · mock interview · 투페이저 시장 적합성 · 상위 career-ops v1.16.0 패리티._
