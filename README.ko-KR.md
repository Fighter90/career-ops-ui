...
> **🆕 최신 릴리스 — v1.115.0**
>
> **디자인 다듬기(보수적, 코럴 브랜드 유지).** 공유 디자인 시스템의 가벼운 정제 — 구조·팔레트 변경 없음: 지표 카드가 호버 시 코럴 테두리로 떠오르고, 버튼은 정지 그림자와 부드러운 호버 리프트를 얻고, 큰 숫자는 tabular-nums로 정렬되며, 컨트롤은 부드러운 코럴 포커스 헤일로를 받습니다. 모션은 prefers-reduced-motion을 존중합니다.
>
> _디자인 다듬기 · 사용량 미터 · 떠 있는 도움말 어시스턴트 · 문서 및 QA 통합 · 보안 백로그 정리 · 문서 및 QA ×16 · Scan 제외 · 파이프라인 개요 · 보안 강화 2 · 새니타이저 강화 · 보안 강화 · AI 사용량 · 회사 로고 · AI CLI 도구 · 문서에 질문 · CV 맞춤 + 커버레터 · 투페이저 자동 채우기 · DOCX 내보내기 · 포털 상태 · 인앱 버그 리포터 · 통계 재구성 · 16 locales · 6개 LLM 프로바이더 · 46개 스캐너 어댑터 · 커리어 방향성 · 커리어 플랜 · 메모리 레이어 · CV Studio · 네트워킹 플래너 · mock interview · 투페이저 시장 적합성 · 상위 career-ops v1.16.0 패리티._
