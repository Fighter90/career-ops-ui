...
> **🆕 최신 릴리스 — v1.100.0**
>
> **투페이저 AI 자동 채우기 + 내보내기.** 투페이저(`#/two-pager`)가 이제 설정된 제공자로 이력서 + 프로필에서 스스로 채워지고(지어내지 않음; 키가 없으면 프롬프트 복사로 대체), **미리보기 및 내보내기** 뷰가 추가되며 **PDF / DOCX / Markdown** 내보내기를 얻습니다 — DOCX는 새 의존성 없는 `.docx` 생성기로, 시장·커리어 플랜·오리엔테이션 리포트에도 연결됩니다.
>
> _투페이저 자동 채우기 · DOCX 내보내기 · 포털 상태 · 인앱 버그 리포터 · 통계 재구성 · 16 locales · 6개 LLM 프로바이더 · 46개 스캐너 어댑터 · 커리어 방향성 · 커리어 플랜 · 메모리 레이어 · CV Studio · 네트워킹 플래너 · mock interview · 투페이저 시장 적합성 · 상위 career-ops v1.16.0 패리티._
