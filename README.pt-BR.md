...
> **🆕 Última versão — v1.131.2**
>
> **Split do `app.css` (refatoração).** v1.131.2 paga um débito do contrato de tamanho de arquivo: o `app.css` de ~1990 LOC é dividido em três folhas de estilo ordenadas — `app.css` (base/chrome/botões), **`components.css`** (cards, tabelas, formulários, badges, paginador, …), **`overlays.css`** (toast, modal, gaveta, `[dir="rtl"]`, docs-fab, usage-hud) — cada uma dentro da meta rígida de 800 LOC. O corte é contíguo e na mesma ordem, então a cascata é **byte a byte idêntica**; `index.html` carrega as três como `<link>`s ordenados. Nenhuma mudança de comportamento/markup/i18n; verificado no navegador (as três folhas são interpretadas e se aplicam, 0 erros de console). **2138 testes.** **Fixação de host (host-pinning) do adaptador reforçada (patch).** v1.131.1 atua sobre desdobramentos de revisão de código nas fontes da v1.130.0 (defesa em profundidade, sem mudança de comportamento para entradas válidas): o adaptador **a16z Speedrun** agora revalida o override de endpoint (HTTPS + host exato) e recai para o feed canônico — paridade com **Cryptocurrency Jobs** — e o `cleanUrl` do parser do Cryptocurrency Jobs usa a mesma guarda de host exato que sua checagem de SSRF (antes era `endsWith`, que aceitava subdomínios), de modo que o parser nunca é mais permissivo que a guarda. **2135 testes.** **Board de abas de estágio (CRM).** v1.131.0 porta a view `/pipeline` do web app do pai para o `#/tracker`: a barra de chips de funil + dropdown de status vira uma **faixa de abas de estágio** — **All** mais uma aba por status canônico (Evaluated · Applied · Responded · Interview · Offer · Rejected · Discarded · SKIP · Hired), cada uma com uma contagem ao vivo, **incluindo estágios com contagem zero**, para que o funil inteiro esteja sempre visível. As abas vêm de uma nova rota somente leitura `GET /api/tracker/stages` (rótulos canônicos + dobra de aliases a partir de `templates/states.yml` — o cliente nunca fixa a whitelist), as linhas ganham logos de marca opcionais, e os nomes acessíveis carregam as contagens. **2133 testes.** **Paridade com o pai v1.24.0.** v1.130.0 porta duas novas fontes de varredura — **a16z Speedrun** (o feed board-wide da rede de talentos do a16z Speedrun) e **Cryptocurrency Jobs** (o board Web3 `cryptocurrencyjobs.co`, via seu RSS público) — elevando o registro a **72 fontes (67 em inglês + 5 russas)** e exibindo ambas no filtro Fonte de `#/scan` e na landing do cvstart.org. Também espelha duas correções do pai: **echojobs** agora mantém vagas **híbridas** distinguíveis do remoto (`City · Hybrid`, nunca colapsadas em `Remote`), e **radancy** aprende o markup legado do TalentBrew além do transporte via fragmento JSON de resultados. **2123 testes**. **Desdobramentos da revisão de IA (patch).** v1.129.1 corrige achados consultivos dos ports web de v1.128/v1.129: precedência de nível em job-facets (`Senior Engineering Manager` → `senior`, não `lead`), `states.mjs` não fixa mais o fallback num problema do pai no boot (+avisa em arquivo malformado), o tom mostra neutra uma linha sem pontuação (não vermelha), e `domainFromName` ignora slugs não-ASCII. **Faceta de nível + coluna de idade na varredura.** v1.129.0 integra a lib `job-facets.js` da v1.128.0 em `#/scan`: um novo filtro **Nível** (lead/staff/sênior/pleno/júnior/estagiário, autopreenchido a partir dos resultados como a faceta de País), uma coluna de badge Nível e uma coluna **Idade** sem tokens (`hoje` / `Nd`). **Quatro ports do web app do pai.** v1.128.0 reimplementa em JS vanilla quatro soluções do app Next.js do career-ops: o tracker lê status canônicos ao vivo de `templates/states.yml` (sem whitelist hardcoded), logos de marca agora em linhas com host ATS via mapa nome→domínio, um tom de pontuação de 4 níveis e uma lib de facetas sem tokens. **2066 testes**. **Paridade com o pai v1.23.0.** v1.127.0 porta três novas fontes de varredura — **Flowxtra**, **VDAB**, **iCIMS** (registro agora **70 adaptadores**, 65 EN + 5 RU) —, espelha a mudança HTML→REST do agenticjobs, a recuperação de cidade do Greenhouse e as correções do role-matcher, e devolve **Cursor** ao roster de CLIs (parent #2115 — `cli-detect` reporta 10 ferramentas). **Patch de desvio do roster de CLI.** v1.126.1 corrige dois pontos que a varredura da v1.126.0 perdeu: o intro da aba API keys de `#/config` (i18n ×17) agora lista Antigravity + Grok Build, e uma linha obsoleta `Cursor / Gemini CLI` no guia de ajuda ×17 (e o help do site) agora tem o roster completo de 8 CLIs. **Resync de docs e roster de CLIs.** v1.126.0 reconcilia cada superfície de docs (help ×17, README ×17, wiki) com a career-ops.org/docs viva (31 páginas lidas) e ensina o scan de AI CLI tools de `#/config` a detectar as 8 CLIs de primeira classe — **Grok Build CLI** e **Kimi CLI** juntam-se a Claude Code, Cursor, Codex, OpenCode, Antigravity, Qwen e Copilot (agora **9 ferramentas**, nunca executadas). **Sync de manutenção.** v1.125.4 agrupa os bumps do dependabot no site (`sharp` 0.35.3, `svgo` 4.0.2, `fast-uri` 3.1.4) e registra uma varredura de paridade com o pai (pós-v1.22.0, #2108–#2168): o guard de linha errada do set-status, o Risk Summary dos modos localizados e a verificação de manifesto do atualizador são só CLI — nada a portar (**1969 testes** inalterados). **Correção de locale nos prompts.** v1.125.3 torna dinamarquês e hindi de primeira classe em todo prompt de IA — deep research (ao vivo e manual), modos, avaliação, entrevista, networking e CV Studio agora emitem a diretiva `# Output language` nesses dois idiomas (as respostas vinham em inglês), e o gate de locales agora varre todos os 17 (**1969 testes**). **Pacote de contribuições externas.** v1.125.2 mescla os primeiros PRs da comunidade, de [@Alien10140](https://github.com/Alien10140): a correção do HTTP 502 no Deep research ao vivo (prompts headless sem ferramentas para provedores de API) e os padrões do Gemini atualizados do descontinuado `gemini-2.0-flash` para `gemini-3.6-flash`, fixados por um novo gate anti-deriva de 5 testes (**1957 testes**). v1.125.1 mantém os tenants RMK multimarca do SuccessFactors em seu caminho de marca (pai #2099). **Fontes de vagas na landing.** A v1.125.0 adiciona uma seção no cvstart.org que lista as **70 fontes de scan** como chips clicáveis, sincronizada com o registro a cada build e protegida contra desincronizações. Antes, v1.124.0 porta **cinco fontes de scan** — Welcome to the Jungle, Agentic Engineering Jobs, Jobvite, Gem e Alibaba Group (**70 adaptadores** agora) — além da verificação de trabalho totalmente remoto do Arbeitsagentur (`homeofficetyp=VOLLSTAENDIG`) e da correção da URL pública do SmartRecruiters.
>
> _split do app.css (3 folhas de estilo) · paydown do contrato de tamanho de arquivo · cascata byte a byte idêntica · fixação de host (host-pinning) do adaptador reforçada · revalidação do override a16z · cleanUrl do cryptocurrencyjobs com host exato · board de abas de estágio (CRM) · port /pipeline do tracker · abas de estágio + contagens · GET /api/tracker/stages · logos de marca no tracker · paridade pai v1.24 · +2 fontes (a16z Speedrun/Cryptocurrency Jobs) · 72 fontes · echojobs híbrido · radancy TalentBrew · patch revisão IA · precedência de nível · fallback states · tom neutro · faceta de nível · coluna idade · job-facets integrada · 4 ports web/ · states.yml ao vivo · logos nome→domínio · tom · facetas · paridade pai v1.23 · +3 fontes · 70 adaptadores · Cursor · patch de desvio do roster de CLI · resync de docs e roster de CLIs · sync de manutenção · correção de locale da/hi nos prompts · pacote de contribuições externas · correção 502 do deep research · padrões Gemini 3.6 · correção do caminho de marca SF · seção de fontes na landing · 5 fontes · 67 adaptadores · paridade com o pai v1.22 · Oracle Cloud · 62 adaptadores · paridade com o pai v1.21 · Hindi · 17 idiomas · manifesto · páginas de metodologia/licença/changelog do cvstart.org · paridade pai v1.19 · 61 adapters · estrelas ao vivo + contribuidores na landing · paridade pai v1.18 · 54 adaptadores · status Hired · estatísticas históricas · pacote de paridade · 50 adaptadores · rework medidor de uso · acabamento de design · medidor de uso · assistente flutuante de ajuda · consolidação docs e QA · fechamento backlog segurança · docs e QA ×16 · Excluir no Scan · visão geral pipeline · reforço de segurança 2 · reforço do sanitizador · reforço de segurança · uso e custo de IA · logos de empresa · ferramentas CLI de IA · perguntar ao guia · adaptar CV + carta · autopreenchimento do two-pager · exportação DOCX · saúde dos portais · relator de bugs integrado · 16 locales · 6 provedores LLM · 46 adaptadores de scanner · orientação de carreira · plano de carreira · reformulação de estatísticas · camada de memória · CV Studio · planejador de networking · mock interview · encaixe de mercado com o two-pager · paridade com career-ops v1.16.0._
...
O [career-ops](https://career-ops.org) é um sistema open-source de busca de emprego que roda como slash commands dentro de qualquer CLI de IA para código (Claude Code, Cursor, Codex, OpenCode, Antigravity CLI, Grok Build CLI, Qwen Code, Kimi, GitHub Copilot CLI, Gemini CLI (legacy) — outras CLIs compatíveis com Claude também funcionam pela mesma superfície de slash-commands). É agnóstico em relação ao modelo. Avalia cada vaga contra o seu CV usando uma rubrica de seis dimensões na escala 0.0–5.0, gera currículos em PDF personalizados e registra cada candidatura localmente — sem contas em nuvem, sem telemetria, sem envio automático.
