...
[![tests](https://img.shields.io/badge/tests-2909%20passed-brightgreen)](#testes)
...
> **🆕 Última versão — v1.228.5** — **A versão informada descrevia o arquivo em disco, não o código em execução.** `/api/health` relia `package.json` a cada requisição, então um processo iniciado antes de um deploy informava a versão do arquivo novo enquanto servia o código antigo — o QA achou uma instância respondendo `1.228.4` e devolvendo 404 numa rota adicionada em 1.228.3. É assim que um deploy que copia arquivos sem reiniciar parece um sucesso. A versão agora é capturada no carregamento do módulo: um processo desatualizado informa a versão **antiga**, que é a verdade. **2909 testes.**
...
│  └─ reviews/REVIEW-*.md
└─ tests/                    # 2909 unit/integration + 101 Playwright e2e
...
```bash
npm test                       # 2909 testes unit/integration
...
| --------------------------- | ------- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **2909** | Cada endpoint, servidor efêmero, sem rede. Inclui parser, scanner (mockado), runner, anthropic, security headers, XSS entity-aware, JD sanitize, validação de URL, paridade i18n, mutex de tracker, rate-limit, path-traversal e DNS rebind. |
...
- Rode `npm test` antes do push — **2909 checks verdes** é a barra (mais 101 Playwright se você mexer na UI).
