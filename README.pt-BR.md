...
[![tests](https://img.shields.io/badge/tests-1824%20passed-brightgreen)](#testes)
...
│  └─ reviews/REVIEW-*.md
└─ tests/                    # 1824 unit/integration + 90 Playwright e2e
...
```bash
npm test                       # 1824 testes unit/integration
...
| --------------------------- | ------- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **1824** | Cada endpoint, servidor efêmero, sem rede. Inclui parser, scanner (mockado), runner, anthropic, security headers, XSS entity-aware, JD sanitize, validação de URL, paridade i18n, mutex de tracker, rate-limit, path-traversal e DNS rebind. |
...
- Rode `npm test` antes do push — **1824 checks verdes** é a barra (mais 90 Playwright se você mexer na UI).
