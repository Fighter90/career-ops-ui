...
> **🆕 Seneste udgivelse — v1.131.2**
>
> **`app.css`-opdeling (refaktorering).** v1.131.2 afdrager en file-size-contract-gæld: den ~1990-LOC store `app.css` opdeles i tre ordnede stylesheets — `app.css` (basis/chrome/knapper), **`components.css`** (kort, tabeller, formularer, badges, paginator, …), **`overlays.css`** (toast, modal, skuffe, `[dir="rtl"]`, docs-fab, usage-hud) — hver inden for den hårde grænse på 800 LOC. Snittet er sammenhængende og i rækkefølge, så cascaden er **byte-for-byte identisk**; `index.html` indlæser alle tre som ordnede `<link>`s. Ingen ændring af adfærd/markup/i18n; verificeret i browseren (alle tre sheets parses og anvendes, 0 konsolfejl). **2138 tests**. **Adapter-host-pinning-hærdning (patch).** v1.131.1 retter code-review-opfølgninger på v1.130.0-kilderne (forsvar-i-dybden, ingen adfærdsændring for gyldige input): **a16z Speedrun**-adapteren genvaliderer nu endpoint-overskrivningen (HTTPS + eksakt host) og falder tilbage til det kanoniske feed — paritet med **Cryptocurrency Jobs** — og Cryptocurrency Jobs-parserens `cleanUrl` bruger nu den samme eksakte host-vagt som dens SSRF-tjek (var `endsWith`, som accepterede subdomæner), så parseren aldrig er løsere end vagten. **2135 tests**. **CRM-fanebladstavle.** v1.131.0 porterer forælderens web-apps `/pipeline`-visning til `#/tracker`: funnel-chip-baren + status-dropdownen bliver til en **fanebladsstrimmel** — **Alle** plus én fane pr. kanonisk status (Evaluated · Applied · Responded · Interview · Offer · Rejected · Discarded · SKIP · Hired), hver med et live optalt tal, **inklusive nul-tælling-stadier**, så hele tragten altid er synlig. Fanerne kommer fra en ny read-only `GET /api/tracker/stages` (kanoniske etiketter + alias-foldning fra `templates/states.yml` — klienten hardkoder aldrig whitelisten), rækkerne får valgfrie brandlogoer, og fanernes tilgængelighedsnavne bærer tællingerne. **2133 tests**. **Forælder v1.24.0-paritet.** v1.130.0 porterer to nye scan-kilder — **a16z Speedrun** (a16z Speedruns boardbrede talent-network-feed) og **Cryptocurrency Jobs** (Web3-boardet `cryptocurrencyjobs.co`, via dets offentlige RSS) — og bringer registret op på **72 kilder (67 engelske + 5 russiske)**, synlige i både `#/scan`-Kildefilteret og på cvstart.org-landingssiden. Den spejler også to forælder-rettelser: **echojobs** holder nu **hybride** roller adskillelige fra remote (`By · Hybrid`, aldrig kollapset til `Remote`), og **radancy** lærer legacy TalentBrew-markup samt JSON-resultatfragment-transporten. **2123 tests**. **AI-review-opfølgninger (patch).** v1.129.1 retter rådgivende fund fra web-portene i v1.128/v1.129: niveau-præcedens i job-facets (`Senior Engineering Manager` → `senior`, ikke `lead`), `states.mjs` fastlåser ikke længere sit fallback ved et forælder-udfald ved boot (+advarer ved misdannet fil), tonen viser en række uden score neutralt (ikke rødt), og `domainFromName` springer ikke-ASCII-slugs over. **Niveaufacet + alderskolonne i scan.** v1.129.0 kobler `job-facets.js`-biblioteket fra v1.128.0 til `#/scan`: et nyt **Niveau**-filter (lead/staff/senior/mid/junior/praktikant, udfyldt fra resultaterne som Land-facetten), en Niveau-badgekolonne og en token-fri **Alder**-kolonne (`i dag` / `Nd`). **Fire ports fra forælderens web-app.** v1.128.0 genimplementerer i vanilla JS fire løsninger fra career-ops' egen Next.js-app: trackeren læser kanoniske statusser live fra `templates/states.yml` (ingen hardkodet whitelist), brandlogoer nu på ATS-hostede rækker via en navn→domæne-map, en finere 4-trins score-tone og et token-frit job-facets-bibliotek. **2066 tests**. **Forælder v1.23.0-paritet.** v1.127.0 porterer tre nye scan-kilder — **Flowxtra**, **VDAB**, **iCIMS** (registret nu **70 adaptere**, 65 EN + 5 RU) —, spejler agenticjobs' HTML→REST-skifte, Greenhouses by-genfinding og role-matcher-rettelser, og genindfører **Cursor** i CLI-rosteret (parent #2115 — `cli-detect` rapporterer 10 værktøjer). **CLI-roster-drift-patch.** v1.126.1 retter to steder som v1.126.0-fejningen missede: introen på API keys-fanen i `#/config` (i18n ×17) lister nu Antigravity + Grok Build, og en forældet `Cursor / Gemini CLI`-række i hjælpeguiden ×17 (og site-help) har nu det fulde 8-CLI-roster. **Docs- & CLI-roster-resync.** v1.126.0 afstemmer hver dokumentationsflade (help ×17, README ×17, wiki) med den live career-ops.org/docs (alle 31 sider læst) og lærer AI CLI tools-scanningen i `#/config` at registrere alle 8 førsteklasses CLIer — **Grok Build CLI** og **Kimi CLI** slutter sig til Claude Code, Cursor, Codex, OpenCode, Antigravity, Qwen og Copilot (nu **9 værktøjer**, stadig aldrig kørt). **Vedligeholdelsessync.** v1.125.4 samler dependabot-opdateringerne i site (`sharp` 0.35.3, `svgo` 4.0.2, `fast-uri` 3.1.4) og registrerer en paritetsgennemgang af forælderen (efter v1.22.0, #2108–#2168): forkert-række-garden i set-status, Risk Summary i lokaliserede tilstande og opdaterens manifest-tjek er alle CLI-side — intet at portere (**1969 tests** uændret). **Prompt-lokalitetsrettelse.** v1.125.3 gør dansk og hindi fuldgyldige i hver AI-prompt — deep research (live + manuel), tilstande, evaluering, interview, netværk og CV Studio udsender nu `# Output language`-direktivet på disse to sprog (svar kom før på engelsk), og sproggaten gennemgår nu alle 17 sprog (**1969 tests**). **Eksternt bidragydernes pack.** v1.125.2 fletter de første community-PR'er fra [@Alien10140](https://github.com/Alien10140) — rettelsen af HTTP 502 i live Deep research (headless prompts uden værktøjer til API-udbydere) og Gemini-standarder løftet fra det udfasede `gemini-2.0-flash` til `gemini-3.6-flash`, fastgjort af en ny drift-gate med 5 tests (**1957 tests**). v1.125.1 lader multi-brand SuccessFactors RMK-tenants beholde deres brand-sti (forælder #2099). **Jobkilder på landingssiden.** v1.125.0 tilføjer en cvstart.org-sektion, der lister alle **70 scanner-kilder** som klikbare chips — synkroniseret med registret ved build og beskyttet mod drift. Før det porterer v1.124.0 **fem scan-kilder** — Welcome to the Jungle, Agentic Engineering Jobs, Jobvite, Gem og Alibaba Group (**70 adaptere** nu) — plus Arbeitsagentur-fuldt-remote-verifikationen (`homeofficetyp=VOLLSTAENDIG`) og SmartRecruiters-public-URL-rettelsen.
>
> _app.css-opdeling (3 stylesheets) · file-size-contract-afdrag · byte-identisk cascade · adapter-host-pinning-hærdning · a16z-overskrivning-genvalidering · cryptocurrencyjobs cleanUrl eksakt host · CRM-fanebladstavle · tracker/pipeline-port · statusfaner + tællinger · GET /api/tracker/stages · brandlogoer i trackeren · forælder v1.24-paritet · +2 kilder (a16z Speedrun/Cryptocurrency Jobs) · 72 kilder · echojobs hybrid · radancy TalentBrew · AI-review-patch · niveau-præcedens · states-fallback · neutral tone · niveaufacet · alderskolonne · job-facets koblet · 4 web/-ports · states.yml live · navn→domæne-logoer · score-tone · facetter · forælder v1.23-paritet · +3 kilder · 70 adaptere · Cursor · CLI-roster-drift-patch · docs- & CLI-roster-resync · vedligeholdelsessync · da/hi prompt-sprogrettelse · eksternt bidrag-pack · deep research 502-rettelse · Gemini 3.6-standarder · SF brand-sti-rettelse · kildesektion på landingssiden · 5 kilder · 67 adaptere · paritet med forælderen v1.22 · Oracle Cloud · 62 adaptere · paritet med forælderen v1.21 · Hindi · 17 sprog · manifest · methodology/license/changelog-sider på cvstart.org · forældre-paritet v1.19 · 61 adaptere · live stjerner + bidragydere på landingen · forældre-paritet v1.18 · 54 adaptere · Hired-status · samlet statistik · paritetspakke · 50 adaptere · forbrugsmåler-omarbejdning · designfinpudsning · forbrugsmåler · flydende hjælpeassistent · docs- og QA-konsolidering · lukning af sikkerheds-backlog · docs & QA ×16 · Scan Udeluk · pipeline-overblik · sikkerhedshærdning 2 · sanitizer-hærdning · sikkerhedshærdning · AI-forbrug & -omkostning · firmalogoer · AI CLI-værktøjer · spørg guiden · tilpas CV + ansøgning · two-pager-autoudfyldning · DOCX-eksport · portalsundhed · indbygget fejlrapportør · 16 lokaliteter · 6 LLM-providere · 46 scanneradaptere · karriereorientering · karriereplan · statistik omarbejdet · hukommelseslag · CV Studio · networking-planlægger · jobsamtale-øvelse · markedsfit via two-pager · paritet med forælderens career-ops v1.16.0._
