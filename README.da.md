...
> **🆕 Seneste udgivelse — v1.228.3** — **Dokumentationsassistenten kunne ikke svare på engelsk.** Ikke søgningen: kontekstbyggeren lavede `break` på første for store afsnit og efterlod konteksten tom. Det engelske §5 var vokset til 16 081 bytes mod et budget på 14 336. Afsnit over 6 KB deles nu ved deres `###` (32 → 75 stykker). Desuden: Telegram-firmaetiketter gav sætningen, ikke navnet, og `GET /api/ping` er en probe, der intet lækker. **2907 tests.**
...
- Kør `npm test` før push — **2907 checks green** er barren (plus 101 Playwright, hvis du rører UI).
