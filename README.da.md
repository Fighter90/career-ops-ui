...
> **🆕 Seneste udgivelse — v1.228.5** — **Den rapporterede version beskrev filen på disken, ikke den kørende kode.** `/api/health` genlæste `package.json` ved hver forespørgsel: en proces startet før en udrulning rapporterede den nye fils version, mens den serverede den gamle kode — QA fandt en instans, der svarede `1.228.4` og gav 404 på en rute tilføjet i 1.228.3. Sådan ser en udrulning ud, der kopierer filer uden at genstarte. Versionen fanges nu ved modulets indlæsning: en forældet proces rapporterer den **gamle** version — sandheden. **2909 tests.**
...
| Suite                       | Tests | What                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **2909** | Every endpoint, ephemeral server, no network. 329 files: parsers, scanners (mocked), runners, anthropic/openai, security headers, XSS, JD sanitize, URL validation, i18n parity, + the v1.55→v1.56 UX-fix suites. |
...
- Kør `npm test` før push — **2909 checks green** er barren (plus 101 Playwright, hvis du rører UI).
