...
> **🆕 Seneste udgivelse — v1.228.2** — **En søgning på to ord søgte efter sætningen, ikke ordene.** `/api/scan-results?q=` lavede ét `includes(q)`: fandt 32 rækker, hvor 162 bærer begge ord. Sådan bliver et snævert match til et forkert svar. Termer sammenlignes nu hver for sig (OG). **2899 tests.**
...
- Kør `npm test` før push — **2899 checks green** er barren (plus 101 Playwright, hvis du rører UI).
