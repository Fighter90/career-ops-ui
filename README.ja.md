...
> **🆕 最新リリース — v1.111.0**
>
> **セキュリティ — CodeQLバックログの解消。** 3つの多層防御強化が残る静的解析の指摘を原因箇所で閉じます:CVマークダウンのサニタイザーは途中で切れた危険タグの開始をエスケープし、出力に生きた `<script`/`<iframe` が決して残らないようにし、CVインポートはバッファサイズを明示的な数値変換で読み取り、モードのロール行は保存された関数ではなく `String.replace` で補間されるテンプレート文字列となって最後の動的ディスパッチ呼び出しを排除します。ユーザーに見える変化なし。
>
> _セキュリティ backlog 解消 · ドキュメント & QA ×16 · Scan除外 · パイプライン概要 · セキュリティ強化2 · サニタイザー強化 · セキュリティ強化 · AI使用量 · 会社ロゴ · AI CLIツール · ガイドに質問 · CV最適化＋カバーレター · ツーページャー自動入力 · DOCXエクスポート · ポータルの状態 · アプリ内バグレポーター · 統計の刷新 · 16 ロケール · 6 LLM プロバイダ · 46 スキャナアダプタ · キャリアの方向性 · キャリアプラン · メモリレイヤー · CV Studio · ネットワーキングプランナー · mock interview · two-pager による市場フィット · 親 career-ops v1.16.0 パリティ。_
