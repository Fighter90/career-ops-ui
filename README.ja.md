...
> **🆕 最新リリース — v1.115.0**
>
> **デザインの磨き込み(保守的、コーラルのブランド維持)。** 共有デザインシステムの軽い洗練——構造・パレット変更なし:メトリクスカードはホバーでコーラルの枠線とともに浮き上がり、ボタンは静止時の影と穏やかなホバーリフトを得て、大きな数字は tabular-nums で揃い、コントロールは柔らかなコーラルのフォーカスハローを受けます。モーションは prefers-reduced-motion を尊重します。
>
> _デザインの磨き込み · 使用量メーター · 浮かぶヘルプアシスタント · ドキュメントとQAの統合 · セキュリティ backlog 解消 · ドキュメント & QA ×16 · Scan除外 · パイプライン概要 · セキュリティ強化2 · サニタイザー強化 · セキュリティ強化 · AI使用量 · 会社ロゴ · AI CLIツール · ガイドに質問 · CV最適化＋カバーレター · ツーページャー自動入力 · DOCXエクスポート · ポータルの状態 · アプリ内バグレポーター · 統計の刷新 · 16 ロケール · 6 LLM プロバイダ · 46 スキャナアダプタ · キャリアの方向性 · キャリアプラン · メモリレイヤー · CV Studio · ネットワーキングプランナー · mock interview · two-pager による市場フィット · 親 career-ops v1.16.0 パリティ。_
