...
> **🆕 最新リリース — v1.227.3** — **タイトルフィルターのプレフィックスが機能するようになりました** — `word:` / `stem:` が文字列そのものとして照合され、その行が無効化されていました（`word:intern` は CLI では効き、ここでは何も除外しませんでした）。親から移植、Unicode 境界込み。**テスト 2852 件。**
