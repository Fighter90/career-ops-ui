...
> **🆕 最新リリース — v1.144.0** — **設定とフィルター（フェーズ4）：追跡ポータルの有効化／無効化** — `#/portals` から監視企業をオン／オフ（`POST /api/portals/toggle`、portals.yml の外科的書き込み）。スキャナーは既に `enabled: false` の企業をスキップ。 **2368 テスト.**
>
