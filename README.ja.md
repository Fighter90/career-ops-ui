...
> **🆕 最新リリース — v1.106.0**
>
> **セキュリティ強化(CodeQLトリアージ)。** ルート描画のエラー経路がエラーメッセージをDOM到達前にエスケープするようになり(サーバーエラーがユーザー入力を反映しうる — XSS境界)、プロフィール/設定のプロパティ書き込みが `__proto__` / `constructor` / `prototype` キーを拒否します(プロトタイプ汚染ガード)。残りの静的解析アラートの大半はスキャナーの正当な `data/*` 入出力に対する誤検知で、根拠付きで却下しました。
>
> _セキュリティ強化 · AI使用量 · 会社ロゴ · AI CLIツール · ガイドに質問 · CV最適化＋カバーレター · ツーページャー自動入力 · DOCXエクスポート · ポータルの状態 · アプリ内バグレポーター · 統計の刷新 · 16 ロケール · 6 LLM プロバイダ · 46 スキャナアダプタ · キャリアの方向性 · キャリアプラン · メモリレイヤー · CV Studio · ネットワーキングプランナー · mock interview · two-pager による市場フィット · 親 career-ops v1.16.0 パリティ。_
