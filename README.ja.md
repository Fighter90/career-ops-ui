...
> **🆕 最新リリース — v1.103.0**
>
> **設定: AI CLIツール。** アプリ設定の新しい**AI CLIツール**タブは、Claude Code、Codex、Gemini CLI、OpenCode、GitHub Copilot CLI、Qwen、Antigravity のうちサーバーを実行するマシンに何がインストールされているかとそのパスを表示します。読み取り専用のPATHスキャン: 各バイナリの存在を確認するだけで**決して実行せず**(`--version`なし、実行なし)、何も書き込まず、ユーザーデータを読みません。
>
> _AI CLIツール · ガイドに質問 · CV最適化＋カバーレター · ツーページャー自動入力 · DOCXエクスポート · ポータルの状態 · アプリ内バグレポーター · 統計の刷新 · 16 ロケール · 6 LLM プロバイダ · 46 スキャナアダプタ · キャリアの方向性 · キャリアプラン · メモリレイヤー · CV Studio · ネットワーキングプランナー · mock interview · two-pager による市場フィット · 親 career-ops v1.16.0 パリティ。_
