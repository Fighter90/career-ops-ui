...
> **🆕 最新リリース — v1.90.0**
>
> **Mock Interview 2.0（Epic 15）:** 新しい **`#/mock-interview`** ページは、実際の職務を相手にターンごとの面接リハーサルを実行します。各回答には **STAR+R フィードバック**、**スコア**、**フォローアップの質問**が返され — あなたの CV、two-pager、ストーリーバンクに基づいています。任意の LLM キーでライブ実行できるほか、コピー＆ペースト用のプロンプトを返すこともでき、企業／職務ごとに**トランスクリプトを保存**できます。v1.89.0（two-pager による候補者と市場のフィット）、v1.88.0（Scan の i18n + API の衛生）、v1.87.0（新しいスキャンプロバイダ 4 つ）をベースにしています。
>
> _16 ロケール · 6 LLM プロバイダ · 45 スキャナアダプタ · mock interview · two-pager による市場フィット · ターゲット職種の市場統計 · 再掲載検出器 · 親 career-ops v1.16.0 パリティ。_
