...
> **🆕 最新リリース — v1.89.0**
>
> **候補者と市場のフィット — two-pager（Epic 14）:** 新しい **`#/two-pager`** ページは、あなたが次の職務に本当に求めるもの（好きなこと、必須条件、嫌なこと、deal-breaker、譲れない条件）を「Mnookin の two-pager」形式でとらえます。**AI 記入アシスタント**があなたの CV + プロフィールから下書きを作成し、`#/scan` のすべての求人には**あなたが望むものとの適合度 `◎` スコア**が付き、two-pager はすべての評価に反映されます。v1.88.0（Scan の i18n + API の衛生）、v1.87.0（新しいスキャンプロバイダ 4 つ）、v1.86.0（ターゲット職種の統計）をベースにしています。
>
> _16 ロケール · 6 LLM プロバイダ · 45 スキャナアダプタ · two-pager による市場フィット · ターゲット職種の市場統計 · 再掲載検出器 · 親 career-ops v1.16.0 パリティ。_
