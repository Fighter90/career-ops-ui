...
> **🆕 最新リリース — v1.131.2**
>
> **`app.css` の分割(リファクタリング)。** v1.131.2 はファイルサイズ規約の負債を解消します — 約1990 LOCの `app.css` を3つの順序付きスタイルシートに分割:`app.css`(ベース/クローム/ボタン)、**`components.css`**(カード、テーブル、フォーム、バッジ、ページネーターなど)、**`overlays.css`**(トースト、モーダル、ドロワー、`[dir="rtl"]`、docs-fab、usage-hud)— いずれも800-LOCのハード目標に収まります。分割は連続的かつ順序どおりのため、カスケードは**バイト単位で完全に同一**です。`index.html` はこの3つを順序どおりの `<link>` として読み込みます。挙動・マークアップ・i18nの変更はなく、ブラウザ上で検証済み(3つのシートすべてがパースされ適用、コンソールエラー0件)。**2138 テスト**。 **アダプターのホスト固定強化(パッチ)。** v1.131.1 は v1.130.0 の2つのソースに対するコードレビュー対応です(多層防御、有効な入力に対する挙動変更なし)。**a16z Speedrun** アダプターはエンドポイントのオーバーライドを再検証するようになり(HTTPS 限定・厳密なホスト)、失敗時は正規フィードへフォールバックします — **Cryptocurrency Jobs** とのパリティです。そして Cryptocurrency Jobs パーサーの `cleanUrl` は SSRF チェックと同じ厳密ホストガードを使うようになりました(以前は `endsWith` で、サブドメインも許容)ので、パーサーがガードより緩くなることはありません。**2135 テスト**。 **CRM ステージタブボード。** v1.131.0 は親 Web アプリの `/pipeline` ビューを `#/tracker` に移植します。ファネルチップバー+ステータスドロップダウンが**ステージタブ帯**になり — **All** に加え正規ステータスごとのタブ(Evaluated · Applied · Responded · Interview · Offer · Rejected · Discarded · SKIP · Hired)がそれぞれライブ件数を表示、**ゼロ件のステージも含む**のでファネル全体が常に見渡せます。タブは新しい読み取り専用の `GET /api/tracker/stages`(`templates/states.yml` からの正規ラベル+エイリアス畳み込み — クライアントはホワイトリストを一切ハードコードしません)から供給され、行はブランドロゴを任意で持てるようになり、アクセシブルネームには件数が含まれます。**2133 テスト**。 **親 v1.24.0 パリティ。** v1.130.0 は新スキャンソース2件 — **a16z Speedrun**(a16z Speedrun のタレントネットワークボード全体フィード)と **Cryptocurrency Jobs**(Web3 ボード `cryptocurrencyjobs.co`、公開 RSS 経由)— を移植し、レジストリを **72 ソース(67 EN + 5 RU)** にして、両方を `#/scan` の Source フィルターと cvstart.org のランディングページに表示します。また、親の修正 2 件をミラー：**echojobs** は **ハイブリッド** 勤務をリモートと区別できるようにし(`City · Hybrid`、`Remote` に丸め込まれない)、**radancy** はレガシー TalentBrew マークアップと JSON 結果フラグメント転送を学習します。**2123 テスト**。 **AI レビュー対応(パッチ).** v1.129.1 は v1.128/v1.129 の web 移植の助言事項を修正:job-facets のレベル優先順位(`Senior Engineering Manager` → `senior`、`lead` ではない)、`states.mjs` が起動時の親の不調でフォールバックをピン留めしなくなった(+不正ファイルで警告)、スコアトーンがスコアなしの行を中立表示(赤ではない)、`domainFromName` が非 ASCII スラッグをスキップ。 **スキャンのレベルファセット + 経過カラム。** v1.129.0 は v1.128.0 の `job-facets.js` ライブラリを `#/scan` に接続:新しい **レベル** フィルター(lead/staff/senior/mid/junior/intern、Country ファセット同様に結果から自動生成)、レベルバッジ列、無トークンの **経過** 列(`今日`/`N日`)。 **親 Web アプリから 4 つ移植。** v1.128.0 は career-ops の Next.js アプリの解決策 4 つをバニラ JS で再実装:トラッカーは `templates/states.yml` から正規ステータスをライブ読み取り(ハードコード whitelist を廃止)、ATS ホスト行に名前→ドメインマップでブランドロゴ、より細かい 4 段階スコアトーン、無トークンの job-facets ライブラリ。**2066 テスト**。 **親 v1.23.0 パリティ。** v1.127.0 は 3 つの新スキャンソース — **Flowxtra**、**VDAB**、**iCIMS**(レジストリは **70 アダプター**、65 EN + 5 RU)— を移植し、agenticjobs の HTML→REST 移行・Greenhouse の都市復元・role-matcher 修正をミラーし、**Cursor** を CLI ロスターに復帰(parent #2115 — `cli-detect` は 10 ツール報告)。 **CLI ロスターのドリフト修正パッチ。** v1.126.1 は v1.126.0 スイープが見落とした 2 箇所を修正: `#/config` の API keys タブ導入文(i18n ×17)が Antigravity + Grok Build を列挙し、ヘルプガイド ×17(と CI ビルドのサイト help)の旧 `Cursor / Gemini CLI` 行が全 8-CLI ロスターになりました。 **ドキュメント・CLI ロスター再同期。** v1.126.0 はすべてのドキュメント面(help ×17、README ×17、wiki)をライブの career-ops.org/docs(全 31 ページ読了)と突き合わせ、`#/config` の AI CLI ツールスキャンが 8 つの一級 CLI をすべて検出するようにします — **Grok Build CLI** と **Kimi CLI** が Claude Code・Codex・OpenCode・Antigravity・Qwen・Copilot に加わります(**9 ツール** 報告、実行はしない)。 **メンテナンス同期。** v1.125.4 は dependabot の site 更新(`sharp` 0.35.3、`svgo` 4.0.2、`fast-uri` 3.1.4)をまとめ、親プロジェクトのパリティスイープ(v1.22.0 以降、#2108–#2168)を記録: set-status の誤行ガード、ローカライズ済みモードの Risk Summary、アップデーターのマニフェスト検証はすべて CLI 側 — 移植対象なし(**1969 テスト** 変更なし)。 **プロンプトロケール修正。** v1.125.3 はデンマーク語とヒンディー語をすべての AI プロンプトで正式サポートに — ディープリサーチ(ライブ・手動)、モード実行、評価、面接、ネットワーキング、CV Studio がこの 2 ロケールで `# Output language` 指示を出すようになり(従来は英語で応答)、ロケールゲートは全 17 ロケールを走査します(**1969 テスト**)。 **外部コントリビューターパック。** v1.125.2 は [@Alien10140](https://github.com/Alien10140) による初のコミュニティ PR をマージ — ライブ Deep research の HTTP 502 修正(API プロバイダ向けツールなしヘッドレスプロンプト)と、非推奨の `gemini-2.0-flash` から `gemini-3.6-flash` への Gemini デフォルト更新。新しい 5 テストのドリフトゲートで固定(**1957 テスト**)。v1.125.1 はマルチブランド SuccessFactors RMK テナントのブランドパスを保持(親 #2099)。**ランディングページの求人ソース。** v1.125.0 は cvstart.org に **70 件のスキャナーソース** すべてをクリック可能なチップとして一覧表示するセクションを追加しました。ビルド時にレジストリと同期され、ドリフトも検知されます。これ以前は、**親 v1.22.0 パリティ。** v1.124.0 は **5 件のスキャンソース** — Welcome to the Jungle、Agentic Engineering Jobs、Jobvite、Gem、Alibaba Group(**70 個のアダプタ** に)— を移植し、さらに Arbeitsagentur の完全リモート判定強化(`homeofficetyp=VOLLSTAENDIG`)と SmartRecruiters の公開 URL 修正を加えました。
>
> _app.css 分割(3スタイルシート) · ファイルサイズ規約の負債解消 · バイト同一カスケード · アダプターのホスト固定強化 · a16z オーバーライド再検証 · cryptocurrencyjobs cleanUrl 厳密ホスト · CRM ステージタブボード · トラッカー /pipeline 移植 · ステージタブ+件数 · GET /api/tracker/stages · トラッカーのブランドロゴ · 親 v1.24 パリティ · +2 ソース(a16z Speedrun/Cryptocurrency Jobs) · 72 ソース · echojobs ハイブリッド · radancy TalentBrew · AI レビューパッチ · レベル優先順位 · states フォールバック · 中立トーン · スキャン レベルファセット · 経過カラム · job-facets 接続 · web/ 4 移植 · states.yml ライブ · 名前→ドメインロゴ · スコアトーン · ファセット · 親 v1.23 パリティ · +3 ソース · 70 アダプター · Cursor · CLI ロスターのドリフト修正 · ドキュメント・CLI ロスター再同期 · メンテナンス同期 · da/hi プロンプトロケール修正 · 外部コントリビューターパック · deep research 502 修正 · Gemini 3.6 デフォルト · SF ブランドパス修正 · ランディングのソースセクション · ソース5件 · アダプター67個 · 親 v1.22 パリティ · Oracle Cloud · アダプター62個 · 親 v1.21 パリティ · ヒンディー語 · 17言語 · マニフェスト · cvstart.org メソドロジー/ライセンス/変更履歴ページ · 親 v1.19 パリティ · 61 アダプタ · ランディングのライブスター + コントリビューター · 親 v1.18 パリティ · 54 アダプター · Hired ステータス · 累計統計 · 親パリティパック · アダプタ50 · 使用量メーター再作 · デザインの磨き込み · 使用量メーター · 浮かぶヘルプアシスタント · ドキュメントとQAの統合 · セキュリティ backlog 解消 · ドキュメント & QA ×16 · Scan除外 · パイプライン概要 · セキュリティ強化2 · サニタイザー強化 · セキュリティ強化 · AI使用量 · 会社ロゴ · AI CLIツール · ガイドに質問 · CV最適化＋カバーレター · ツーページャー自動入力 · DOCXエクスポート · ポータルの状態 · アプリ内バグレポーター · 統計の刷新 · 16 ロケール · 6 LLM プロバイダ · 46 スキャナアダプタ · キャリアの方向性 · キャリアプラン · メモリレイヤー · CV Studio · ネットワーキングプランナー · mock interview · two-pager による市場フィット · 親 career-ops v1.16.0 パリティ。_
