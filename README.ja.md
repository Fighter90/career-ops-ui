...
> **🆕 最新リリース — v1.91.0**
>
> **ネットワーキングと企業の詳細リサーチ（Epic 16）:** 新しい **`#/networking`** ページは、企業を実行可能なプランに変換します — **企業ドシエ**、**誰に連絡すべきか**（ペルソナ + LinkedIn 検索文字列）、**最も温かい紹介ルート**、そしてカスタマイズされた**アウトリーチのドラフト** — すべてあなたの CV と two-pager に基づいています。任意の LLM キーでライブ実行できるほか、コピー＆ペースト用のプロンプトを返すこともでき、企業ごとに**プランを保存**できます。v1.90.0（mock interview）、v1.89.0（two-pager による候補者と市場のフィット）、v1.88.0（Scan の i18n + API の衛生）をベースにしています。
>
> _16 ロケール · 6 LLM プロバイダ · 45 スキャナアダプタ · ネットワーキングプランナー · mock interview · two-pager による市場フィット · ターゲット職種の市場統計 · 再掲載検出器 · 親 career-ops v1.16.0 パリティ。_
