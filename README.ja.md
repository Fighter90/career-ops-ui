...
> **🆕 最新リリース — v1.174.0** — **ローカライズレポートのスコア + SEO 修正** — 非英語レポートが "Score not detected" と表示されなくなり（ラベル語を含む H1 をスコアと誤認）、正当性チップは残った `**` を除去、長いスコアはカードをはみ出さず、cvstart.org の説明は実レジストリを数えます（~75）。**2448 テスト。**
>
