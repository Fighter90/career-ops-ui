...
> **🆕 最新リリース — v1.114.0**
>
> **サイドバーのAI使用量・コストメーター。** すべてのページのサイドバー下部にあるコンパクトな使用量セクション(左下、RTLは右下)が、24h / 7d / 30d のLLMトークン使用量を緑のバーと24時間の推定コストで表示します — 読み取り専用のローカル使用ログ(#/usageページと同じソース)から。折りたたみ可能、コストは推定で手動モードの実行は無料。
>
> _使用量メーター · 浮かぶヘルプアシスタント · ドキュメントとQAの統合 · セキュリティ backlog 解消 · ドキュメント & QA ×16 · Scan除外 · パイプライン概要 · セキュリティ強化2 · サニタイザー強化 · セキュリティ強化 · AI使用量 · 会社ロゴ · AI CLIツール · ガイドに質問 · CV最適化＋カバーレター · ツーページャー自動入力 · DOCXエクスポート · ポータルの状態 · アプリ内バグレポーター · 統計の刷新 · 16 ロケール · 6 LLM プロバイダ · 46 スキャナアダプタ · キャリアの方向性 · キャリアプラン · メモリレイヤー · CV Studio · ネットワーキングプランナー · mock interview · two-pager による市場フィット · 親 career-ops v1.16.0 パリティ。_
