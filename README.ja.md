...
> **🆕 最新リリース — v1.177.0** — **csod クッキー再送（parent #2769）** — 検索 API をセッションクッキーで保護する Cornerstone テナント（401 → 0 件）が動作: ブートストラップの `Set-Cookie` を検索 POST に `Cookie` ヘッダーとして再送（同一オリジンのみ）。**2454 テスト。**
>
