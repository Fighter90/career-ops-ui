...
> **🆕 最新リリース — v1.93.0**
>
> **メモリレイヤー（Epic 24）:** 新しい **`#/memory`** ページは、短く編集可能な「私についてこれを覚えておいて」というメモを保存します。共有プロジェクトコンテキストにインライン展開されるため、評価・模擬面接・ネットワーキング・CV Studio といった **every** AI リクエストを、**all** プロバイダーを横断して方向づけます。あなたの好みと働き方（あなたに関する新しい事実ではありません）を捉え、**あなたのデータから提案する**ヘルパーがトラッカーからレビュー用の行動的な箇条書きを下書きします。v1.92.0（CV Studio）、v1.91.0（ネットワーキングプランナー）、v1.90.0（mock interview）をベースにしています。
>
