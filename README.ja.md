...
> **🆕 最新リリース — v1.86.0**
>
> **ターゲット職種別の統計**（`#/stats`）: 新しい Analytics ページが**プロフィールのターゲット職種**（ハードコードではない）と最新のスキャンを読み取り、**国別の求人件数と給与中央値**をチャート表示します。職種／国のフィルタと**保存済みスナップショットのトレンド**を備え、スキャナがすでに収集している疎なデータからクライアント側で集計（数値の捏造なし）し、16 言語すべてに対応します。v1.85.0（ドイツ語／イタリア語／トルコ語ロケール）、v1.84.0（再応募クールダウン）、v1.83.0（再掲載検出器）をベースにしています。
>
> _16 ロケール · 6 LLM プロバイダ · 41 スキャナアダプタ · ターゲット職種の市場統計 · 再掲載検出器 · 親 career-ops v1.16.0 パリティ。_
