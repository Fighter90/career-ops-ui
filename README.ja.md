...
> **🆕 最新リリース — v1.116.0**
>
> **使用量メーター再作 + 初のE2Eウィジェットテスト。** AI使用量メーターは左サイドバー下部に固定され(メニューを隠さず — ナビが上をスクロールできるよう余白を確保)、リアルタイム更新し、ウィンドウごとに実際のトークン · 推定コストを表示します。恒久的なtypeofバリアが再発するCodeQLの誤検知を閉じ、新しいPlaywrightのE2Eテストが2つの常駐ウィジェットを実ブラウザで検証します。
>
> _使用量メーター再作 · デザインの磨き込み · 使用量メーター · 浮かぶヘルプアシスタント · ドキュメントとQAの統合 · セキュリティ backlog 解消 · ドキュメント & QA ×16 · Scan除外 · パイプライン概要 · セキュリティ強化2 · サニタイザー強化 · セキュリティ強化 · AI使用量 · 会社ロゴ · AI CLIツール · ガイドに質問 · CV最適化＋カバーレター · ツーページャー自動入力 · DOCXエクスポート · ポータルの状態 · アプリ内バグレポーター · 統計の刷新 · 16 ロケール · 6 LLM プロバイダ · 46 スキャナアダプタ · キャリアの方向性 · キャリアプラン · メモリレイヤー · CV Studio · ネットワーキングプランナー · mock interview · two-pager による市場フィット · 親 career-ops v1.16.0 パリティ。_
