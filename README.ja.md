...
> **🆕 最新リリース — v1.92.0**
>
> **CV Studio（Epic 21）:** 新しい **`#/cv-studio`** ページは、あなたの CV に 3 つの正直なツールを提供します — **履歴書診断**（各チェックの説明付きの決定論的な 0–100 スコア）、**プライバシーマスク**（メール/電話/リンク/住所/名前をすべてブラウザ内で伏せ字に）、そして **make it human**（硬い一文を _あなたの_ 声で書き直し、あなたの voice-dna とライティングサンプルに基づき、事実を決して捏造しない）。v1.91.0（ネットワーキングプランナー）、v1.90.0（mock interview）、v1.89.0（two-pager による市場フィット）をベースにしています。
>
> _16 ロケール · 6 LLM プロバイダ · 45 スキャナアダプタ · CV Studio · ネットワーキングプランナー · mock interview · two-pager による市場フィット · ターゲット職種の市場統計 · 再掲載検出器 · 親 career-ops v1.16.0 パリティ。_
