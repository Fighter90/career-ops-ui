...
> **🆕 最新リリース — v1.101.0**
>
> **CV Studio: 求人に合わせて最適化＋カバーレター。** `#/cv-studio`の新しい**求人に合わせて最適化**カードは、貼り付けた求人内容を最適化した履歴書と対応するカバーレターに変え、両方をリクルーター水準の**チェックリストゲート**に通します(エラーはブロック、警告は助言)。キャリアコーチングの実践を汎用ルールに蒸留 — あなた自身の履歴書・プロフィール・ツーページャーのみに基づき、決して捏造しません。結果はMarkdown / PDF / DOCXでエクスポート。
>
> _CV最適化＋カバーレター · ツーページャー自動入力 · DOCXエクスポート · ポータルの状態 · アプリ内バグレポーター · 統計の刷新 · 16 ロケール · 6 LLM プロバイダ · 46 スキャナアダプタ · キャリアの方向性 · キャリアプラン · メモリレイヤー · CV Studio · ネットワーキングプランナー · mock interview · two-pager による市場フィット · 親 career-ops v1.16.0 パリティ。_
