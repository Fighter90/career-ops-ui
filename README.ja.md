...
> **🆕 最新リリース — v1.107.0**
>
> **サニタイザー強化(保存時XSS)。** 履歴書/求人マークダウンのサニタイザーが危険なHTMLを**不動点まで**除去し(再構成される除去を捕捉)、末尾にゴミが付いたscript/style終了タグ(`</script foo>`)と未クローズの実行オープナーを除去します。描画時エスケープのクライアントの背後にある多層防御で、対応するCodeQLの指摘を解消します。
>
> _サニタイザー強化 · セキュリティ強化 · AI使用量 · 会社ロゴ · AI CLIツール · ガイドに質問 · CV最適化＋カバーレター · ツーページャー自動入力 · DOCXエクスポート · ポータルの状態 · アプリ内バグレポーター · 統計の刷新 · 16 ロケール · 6 LLM プロバイダ · 46 スキャナアダプタ · キャリアの方向性 · キャリアプラン · メモリレイヤー · CV Studio · ネットワーキングプランナー · mock interview · two-pager による市場フィット · 親 career-ops v1.16.0 パリティ。_
