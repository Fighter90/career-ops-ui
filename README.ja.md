...
> **🆕 最新リリース — v1.109.0**
>
> **Scan除外フィルター + パイプライン概要。** 検索ボックスがカンマをORとして扱い(「探す役割」)、新しい除外フィールドがカンマ区切りの語に一致する行を隠します。両方とも保存済み検索に保持されます。パイプラインページにコンパクトな概要ストリップ — 受信N、追跡N、トラッカーのApplied / Responded / Interview / Offer数、各々 `#/tracker` にリンク。
>
> _Scan除外 · パイプライン概要 · セキュリティ強化2 · サニタイザー強化 · セキュリティ強化 · AI使用量 · 会社ロゴ · AI CLIツール · ガイドに質問 · CV最適化＋カバーレター · ツーページャー自動入力 · DOCXエクスポート · ポータルの状態 · アプリ内バグレポーター · 統計の刷新 · 16 ロケール · 6 LLM プロバイダ · 46 スキャナアダプタ · キャリアの方向性 · キャリアプラン · メモリレイヤー · CV Studio · ネットワーキングプランナー · mock interview · two-pager による市場フィット · 親 career-ops v1.16.0 パリティ。_
