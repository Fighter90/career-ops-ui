...
### 新增
**兩個掃描器來源，92 → 94（89 EN + 5 RU）。** **Python.org Jobs**（`pythonorg`）讀取該看板的公開動態消息；雇主歸屬為必要欄位，因此標題中未列出雇主名稱的項目會直接捨棄，而不會回填成看板項目本身的名稱。**Generalist World**（`generalist-world`）擷取該看板的卡片，**一旦遇到無法解析的版面標記就會大聲失敗**——卡片版面若有變動會直接拋出例外，而不是被讀成一個空看板，而後者正是讓一個擷取型來源變得不可信的失敗模式。兩者皆出現在 `#/scan` 的 **Source** 篩選器中，皆為主機固定（錨定正規表示式、僅限 HTTPS），並皆由端到端的假傳輸層測試套件驅動（分別為 43 與 36 個案例）。

### 修復
**一個在多個城市都有職缺開放的 Workday 職位，被地點篩選器捨棄。** Workday 的清單端點對多地點職缺回傳的是一個**數量**，而其他每則職缺帶的都是一個地點：`"53 Locations"`。這不是一個地點，而 `buildLocationFilter` 是以不分大小寫的子字串比對——於是一個在 Austin *以及*另外 52 個城市都開放的職位，不符合任何 `allow: [austin]` 項目而遭捨棄，而同一個職位若只單獨列出一個地點則會通過。對三個租戶的上游實測：**291 筆職缺中有 53 筆**是這種佔位文字，所以這是常見情況，而非邊緣案例。職缺真正的地點只存在於 CXS 的**明細**文件之中，因此每個佔位文字現在都要多花一次 GET 請求去查詢，並設有上限，**每筆最多 200 次**——超過上限時，佔位文字會維持可見，而不是被假造出來，因為一則尚未解析的職缺絕不能被讀成已解析。若明細查詢失敗或本身沒有地點，職缺會維持清單端點原本回傳的樣子：一個可見的 `"53 Locations"` 誠實地反映了無法解析的事實，而一個被清空的地點欄位卻會悄悄通過所有篩選。同一次 GET 請求也帶有 `startDate`，因此一則已豐富化的職缺會有**精確**的日期，而非清單端點的相對敘述文字（`"Posted 30+ Days Ago"` → `undefined`）；任何不是單純 `YYYY-MM-DD` 格式的值都會原樣保留，而不會交給 `Date.parse` 處理，因為它在非 ISO 格式下的回退行為屬於實作定義。
**標題中標明的層級，不再讓兩則職缺申請被合併成同一次重貼。** `"Insurance Specialist II"` 是雇主自己的聲明：這則職缺申請並非另一則的同胞——在同一個基礎職稱之下，它有不同的薪資級距、職責範圍與申請編號。分詞器原本看不見這一點：`w.length > 3` 會把所有到 VIII 為止的羅馬數字，以及所有個位數字都捨棄，於是 `"Insurance Specialist"` 與 `"Insurance Specialist II"` 分詞結果完全相同，Jaccard 相似度算出滿分，重貼偵測器便把第二則標記為第一則的重新刊登。web-ui 的 `role-matcher.mjs` 原本**完全沒有**層級處理機制，因此這次直接落地了父專案的整套機制：層級的擷取會把羅馬數字與阿拉伯數字摺疊為同一個數字（`"Nurse II"` 與 `"Nurse 2"` 是同一個聲明——而*這一對*先前恰好被誤判為不同），兩邊標明的層級必須一致，而僅有一邊標明層級時，則視為鬆散改寫，**除非**兩個標題各自還帶有對方沒有的詞——這代表兩處分歧，而不只是裝飾性差異。
**一個已釘選篩選條件的 Avature 看板，實際上仍會走遍整個全域看板。** Avature 的搜尋介面會產生不透明的分面（facet）參數（`?42386=[812132]` 即為一個國家篩選器），而在 `portals.yml` 中釘選了一個縮小範圍看板的操作者，理應預期掃描會走*那個*範圍。`resolveSearch` 把網址重建為 `${origin}${path}`，卻直接把查詢字串捨棄——於是每一次請求，包括第一次在內，都走遍了整個租戶看板：更多頁面、更多請求，以及操作者刻意篩掉的職缺。分面參數現在會隨每一次分頁請求一併送出，並讓我們自己的 `jobOffset` 最後套用，如此一來，一個恰巧同名的雜散分面參數就絕不可能挾持分頁。

### 安全
**`devalue` 5.8.1 → 5.9.2**（透過 `astro` 間接引入），解決了唯一一個未結的 Dependabot 告警——一個由格式錯誤輸入觸發的阻斷服務漏洞。僅限網站端相依套件；執行期程式碼未變動。

### 說明
**未移植項目，並各自附上理由。** 父專案把 **lever** 的逾時從共用的 10 秒預設值調高為 30 秒，把 **manfred** 調高為 25 秒——這是一個**不移植**項目：web-ui 的掃描傳輸層本來就允許 **60 秒**（`DEFAULT_SCAN_TIMEOUT_MS`），因此這兩個看板在這裡本來就比那邊修復後給予的餘裕還要更寬鬆。**iCIMS** 重複的 `careers-` 前綴問題在這裡也不可能發生：web-ui 直接從 `api`/`careers_url` 取得租戶來源（origin），從不會從租戶名稱組出主機名稱；它新增的備援主機走訪機制，是為了父專案較寬鬆的設定而設計的功能，而不是缺陷修復。**deutschebahn** 與 **manfred** 新增了暫時性中止的重試機制，而 web-ui 的 60 秒上限讓這一點顯得沒那麼迫切——因此只記錄而不移植，讓未來若出現長時間走訪逾時問題時，有一個已知的第一線索可查。128 個提交差異中其餘的一切都屬於 CLI、驗證器、儀表板或 `web/`（父專案自己的前端）：`liveness-browser`、`check-jd-archive`、`verify-ats`/`verify-portals`/`verify-cv-facts`、`update-system`、`set-status`、`audit-portals`、`discover-ats`、`scaffolder`、`test-all`、外掛程式清單檔案。
拉取之後重新驗證過的 fork 差異：`providers/telegram-channel.mjs` 中的西里爾字母 `\p{L}` 修正、fork 自身的 `providers/telegram.mjs`、`web/src/lib/clis.ts` 中的 `hermes`。上游 `career-ops-hq/main` 已完全合併（0 個提交缺漏）。**3066 → 3164 項測試**；說明文件現於全部 17 個語系套件中皆載明 **94** 個配接器（**89 EN + 5 RU**），標題結構維持不變，為 32 H2 / 122 H3。

## [1.235.0] — 2026-09-16

**父專案對齊 — career-ops `main` @ `68e6b94`（`VERSION` 為 1.33.0，42 個提交）。兩項鏡射修復；其中一項曾悄悄讓本專案遺失 MokaHR 職缺。**

### 修復
**同一個 MokaHR 職缺看板上的第二筆職缺會憑空消失，而不是被加入。** `url-key.mjs` 會把職缺網址化簡為一個比對用的鍵，而它先前會無條件捨棄網址片段——這在多數情況下合理，因為幾乎每個片段都只是呈現用途（`#apply`、`#section-2`）。MokaHR 卻是我們自家登記表裡的例外：其職缺網址形如 `…/social-recruitment/{tenant}/{id}#/job/{n}`，同一租戶下的每則職缺共用同一路徑，職缺 id **只**存在於片段之中。於是它們全都被正規化成同一把鍵。修復前實測：把兩筆不同的 MokaHR 職缺貼進管線，最後只會留下**一行**——沒有錯誤、沒有警告，毫無異狀可察；掃描器的新鮮度過濾器（`en-scanner.mjs`）讀的是同一把鍵，所以只要該租戶底下曾有任何一筆職缺進了掃描歷史，其餘的都會被當成「已經看過」而捨棄。`normalizeUrl` 現在會在捨棄片段之前，先把可辨識的 `#/job/{id}` 或 `#/jobs/{id}` 路由提升為一個僅供比對用的查詢鍵——MokaHR 沿用既有的 `mokahr_job_id` 名稱，讓先前已寫入的鍵仍可比對，其他任何主機則會得到一個內部用的 `_career_ops_fragment_job_id`。這個被提升的 id 是**附加，而非覆寫**：一個網址若已帶有該參數，代表的是另一筆職缺的 id，直接覆寫就會把這把鍵原本要區隔開的兩樣東西合而為一。所有無法辨識的片段仍會照舊被摺疊，這一半原本就是對的。
**`accept-encoding` 現在會在每一次來源請求中固定送出。** 若不設定，Node 會協商其 undici 版本所提供的任何選項；從 **Node ≥ 23** 起，這包含了 `zstd`，而 amazon.jobs 回傳的 zstd 回應會**恰好在第 1024 個位元組被截斷，狀態碼卻仍是 200**——於是這個失敗看起來像 `Unterminated string in JSON at position 1024`，彷彿是 API 格式錯誤，而不是傳輸層的問題。現在每個請求都會送出 `gzip, deflate, br`。在我們的執行環境上，這是**預防性措施，而非修補**：Node 18/20/22——CI 矩陣與伺服器（22.22.1）——送出的是 `gzip, deflate`，這一點經過直接驗證而非假設，所以這裡本來就沒有被截斷過；今天真正改變的是現在會提供 `br`，這些版本預設不會要求它，但能正確解碼。這個預設值是以**不分大小寫**的方式套用，而這正是值得留意之處：標頭名稱本就不分大小寫，`fetch` 又會把兩個同名項目合併成一個以逗號分隔的值，因此若呼叫方傳入 `Accept-Encoding: identity`，原本會變成送出 `gzip, deflate, br, identity`。

### 說明
在 `fetchImpl` 邊界上，標頭維持為**純物件**，而父專案在此處交給 `fetch` 的是一個 `Headers` 實例。這是刻意為之的分歧，並非疏漏：web-ui 的 fetch 輔助函式接受可注入的 `fetchImpl`，而 45 個來源測試套件會從擷取到的內容讀取 `opts.headers['User-Agent']`——`Headers` 實例不支援中括號存取，所以忠實移植的是*行為*（不分大小寫覆寫、呼叫方優先），而非實作本身。呼叫方的物件只會被複製，絕不會被就地修改，因為有多個來源共用同一個標頭常數。父專案那項配套修復——以不分大小寫的方式套用**預設 user-agent**——則是**不移植**：web-ui 完全不會注入預設 UA，每個來源都會設定其看板要求的那一個，因此呼叫方的 `User-Agent` 根本沒有預設值可供衝突。
兩個新測試套件與三個新案例釘住這一對修復：`tests/http-accept-encoding.test.mjs`（釘住確切的固定值、四個 fetch 輔助函式各自的行為，以及呼叫方在三種大小寫拼法下的覆寫——斷言只會留下*單一*一個 `accept-encoding` 鍵，而合併錯誤原本會產生兩個）以及 `tests/parsers-pipeline-hash-route.test.mjs`，它驗證的是使用者看得見的介面，而非那把鍵本身：兩筆 MokaHR 職缺都必須進到管線裡，同一筆職缺以四種拼法重貼一次仍必須摺疊成一行，而一個裝飾性片段不得被誤判為身分識別。所有案例移植前都已確認會失敗。 該固定位於**傳輸層**，而不僅僅在這些輔助函式中：92 個來源中有 25 個直接呼叫注入的 `fetchImpl`，根本不經過它們，因此由 `makeTimeoutFetch`——兩個掃描器實際注入的包裝器——來套用它，這是所有來源共享的唯一通過點。（父專案不需要等價物：`providers/_http.mjs` 是它唯一的傳輸層。）現在有一道結構性防護：若新來源直接呼叫 `fetch` 而非使用交給它的傳輸層，CI 就會失敗；另有一個行為案例，讓 25 個之一的 greenhouse 經由該包裝器執行，證明標頭確實送到了線上。
未移植，理由如下：`liveness-browser.mjs`（有頭 Playwright 重用防護——web-ui 不會驅動有頭瀏覽器）、`check-jd-archive.mjs`（web-ui 沒有對應的轉送）、`test-all.mjs`（父專案自己的測試線束；fork 自身的 `vpFixtureEnv` 差異維持不變）、`web/`（父專案自己的前端，不在鏡射範圍內）、外掛程式清單檔案與 `SIGNATURES.md`（僅存在於儲存庫層級）。`batch/batch-runner.sh`（在 `sed` 插值前先跳脫 `&` 符號）與 `jd-skill-gap.mjs`（依資料根目錄解析 `cv.md`）屬於**唯讀轉送**——web-ui 是以 shell 呼叫父專案的副本，因此這兩者在本專案都不需要改動程式碼，而 `jd-skill-gap` 這項還讓 `/api/jds/:id/skill-gap` 的轉送更加正確：`cv.md` 現在會依資料根目錄解析，而不是依處理程序被啟動時所在的任意目錄。拉取之後重新驗證過的 fork 差異：`providers/telegram-channel.mjs` 中的西里爾字母 `\p{L}` 修正、fork 自身的 `providers/telegram.mjs`、`web/src/lib/clis.ts` 中的 `hermes`。來源數維持在 **92** 個不變（87 EN + 5 RU），因此說明文件未受影響。**3042 → 3066 項測試。**
...
### 修復
把儲存值寫入 `process.env` 的迴圈原本遍歷由請求體建立的 `Object.entries(safe)`，內部帶著針對原型鍵的顯式防護。程式碼本身安全，因為 `safe` 僅從 `KNOWN_KEYS` 拷貝，但分析器無法跨兩個迴圈追蹤該約束。現在直接遍歷常量陣列，屬性名可證明是模組級字面量。**行為完全相同。**

### 說明
釘住舊防護的測試是對源碼文字的 grep。已換成結構檢查與行為檢查，後者以**原始 JSON** 發送 `__proto__`——物件字面量會設定原型而非建立屬性。**3021 → 3022 項測試。**

## [1.233.1] — 2026-09-10

**修復 — 指南從未描述 v1.233.0 新增的控制項，而同一節仍留著 200 個版本前就不再成立的說明。**

### 修復
**DOC-1** — 每個下拉框現以「使用預設值（…）」開頭，表達介面此前無法表示的狀態：**該金鑰未設定**。它與從清單選中同一個值的差別既看不見又要緊。已在既有的 `### Behavior` 清單補入三條 ×17。
**DOC-2** — 表格中有一行沒有指明任何金鑰：Key 欄位是 `(server uses default UA)`，描述 **v1.19.0** 已移除的 `HH_USER_AGENT`，並要讀者去 dev.hh.ru 註冊——自 **v1.65.0** 起毫無用處。三處陳舊說明已清除；排障行是**改寫**而非刪除，因為自 2026 年 7 月起 hh.ru 對非俄羅斯 IP 回傳 HTTP 451。
**金鑰表**只列出 45 個中的 6 個，標題卻承諾登記表。一句話 ×17 聲明它是精選並指向 `#/config`。

### 說明
`tests/help-recognized-keys.test.mjs` 將金鑰表第一列與實時 `KNOWN_KEYS` 比對。**3018 → 3021 項測試**；說明仍是 32 H2 / 122 H3。

## [1.233.0] — 2026-09-10

**修復 — `#/config` 儲存路徑上的三處發現；CONFIG-3 由 v1.232.1 的修復本身引起。**

### 修復
**CONFIG-3** — 與預置值恰好相同的有意選擇被當作「未碰過」丟棄。`initial` 混淆兩種來源：來自 `.env` 的預置與來自 `defaultValue` 的預置，後者的相同是**歧義**的。現在在 `initial` 旁記錄 **`seededFromFile`**。
**刪除鍵會顯示「· 0」**，因為伺服器只回傳寫入的內容。現在還會回傳 `removed`，提示兩者相加。
**下拉框能設不能清**：18 個中沒有一個有空選項。現在每個以**「使用預設值（…）」**開頭。新增一個 i18n 鍵 ×17。

### 說明
單元基線仍為 **3018**；瀏覽器 **111 → 116**。

## [1.232.1] — 2026-09-10

**修復 — `#/config` 的儲存會寫入使用者從未碰過的欄位。**

### 修復
**CONFIG-2.** v1.57.1 會為未設定的控制項填入其 `defaultValue`，讓欄位顯示伺服器真正使用的值；而 `save()` 只對**機密欄位**查詢 `dirty`，其餘一律照送。於是為**顯示**而放的值被當作使用者的選擇送出：改動一個下拉選單就送出 **28 個鍵並釘死 18 個**從未開啟的欄位。變的是每項設定的狀態：從「未設定，跟隨專案預設」變成「釘死在 `.env`」。

### 說明
與 `cfg.values` 比較並不夠：未設定的 select 顯示自己的 `defaultValue`，而儲存值是 `''`——請求體只從 27 個鍵降到 19 個。現在改為與每個控制項**初始化時的值**比較。提示中的鍵數也回來了。**105 → 111** 個瀏覽器測試。
...
### 安全
**自 Dependabot 併入：** `multer` 2.2.0 → 2.3.0（**高危** — 中止時檔案描述子洩漏導致阻斷服務），以及站台的 `svgo` 4.0.2 → 4.1.0（**高危**與中危 — `removeScripts` 放過了可執行連結）。
**隨後由這兩次併入觸發的重新掃描又暴露五項。** 受影響區間均已被現有 `^` 涵蓋，因此只需升級：站台 **`astro` 7.1.0 → 7.3.2** ——**嚴重**，經 AVIF 影像最佳化的遠端程式碼執行（7.2.8 修復），並附帶授權繞過（7.2.4）；**`sharp` 0.35.3 → 0.35.4** ——**高危**，libheif 漏洞；**兩份**清單中的 **`js-yaml` 4.3.1 → 4.3.2** ——**高危**。

### 說明
此處「僅相依」並不等於「無風險」。`js-yaml` 解析應用讀取的每一份設定，`multer` 接收每一次上傳，而 Astro 跨了一個**次版本**。已驗證：`test:ci` **3013/3013**、瀏覽器檢查 4/4，以及完整重建站台後仍是同樣的 **86 個頁面**與全部 **17** 份鏡像。`npm audit`：兩份清單皆**0 個漏洞**。

## [1.231.4] — 2026-09-07

**修復 — v1.231.3 修的是點擊 Doctor 之後的殘局，不是當時。行動端與桌面端再次回報。**

### 修復
**請求進行期間，`withSpinner` 仍在改寫按鈕。** v1.231.3 讓它快照子節點並在 `finally` 裡還原，節點確實回來了——但忙碌提示依舊執行 `replaceChildren`，內容是 `'⏳ ' + original`。於是在 `doctor.mjs` 執行的整段時間裡，36 px 的方塊裝著 `⏳ 🩺Doctor` 文字，脹成壓壞整列的寬膠囊；桌面端則擠成一團 `⏳🩺Doctor`。**擁有元素子節點的按鈕，現在完全不再被改寫。** 它的提示交給 `.is-loading` 類，該類如今在 CSS 裡把圖示換成沙漏，因此尺寸不可能變化：320 px 下**靜止 36 px、進行中 36 px、結束後 36 px**。

### 說明
教訓在斷言，不在程式碼。v1.231.3 的閘門等待 `aria-busy` 清除才測量——按其構造，只能看到 `finally` 已經修好的狀態。新的瀏覽器檢查**自己把 promise 攥在手裡**，在飛行途中取樣。**3012 → 3013 項。**

## [1.231.3] — 2026-09-07

**修復 — 點擊 Doctor 會把行動版頂欄撐壞。v1.231.2 發布當天由手機端回報。**

### 修復
**`UI.withSpinner` 壓平了它正在裝飾的那個按鈕的子節點。** 它的忙碌提示走的是 `textContent`：儲存 `button.textContent`，寫入 `'⏳ ' + original`，再寫回去。而對 `textContent` 賦值會把**所有**子節點替換成一個文字節點——偏偏 v1.231.2 剛剛把頂欄的每個操作拆成 `.btn-ico` 與 `.btn-label`，並在 `max-width: 900px` 下隱藏標籤。於是第一次點擊 Doctor 就永久摧毀了這兩個 `<span>`：行動版規則再無可匹配之物，標籤以裸文字回歸，36 px 方塊膨脹成壓住主題切換的 `🩺Doctor` 膠囊。它不會自癒——只有重新整理頁面才能恢復標記。修復改為快照子**節點**並以 `replaceChildren` 還原；純文字按鈕，也就是 24 處呼叫中的絕大多數，行為與此前完全一致。
- **FIND-3 — 在 320 px 的手機上，搜尋框被壓縮到 8 px。** 把頂欄併成一列後，searchbar 成了唯一可伸縮的元素，而上個版本的變更日誌自己就寫著——*「可以縮到什麼都不剩」*。它真的縮到什麼都不剩：**320 px 下輸入框 8 px，340 px 下 28 px**，21 個字元的預留文字只露出一個。可點可輸入，但已經不像一個輸入框。`min-width` 救不了：320 px 的列裡有 32 px 內距、40 px 選單和 162 px 操作，拿不出 120 px。於是在 **420 px** 以下，它藏到放大鏡後面，輕點即展開佔滿整條列：**320 px 下 182 px，360 px 下 222 px，390 px 下 252 px**，任何寬度都不溢位。展開按鈕保持唯一的可存取名稱，狀態由 `aria-expanded` 傳達；🔍/✕ 的切換是純 CSS。420 px 以上一切照舊。由針對線上建置的瀏覽器迴歸發現。

### 說明
這與 v1.231.2 記下的 `applyI18n()` 陷阱同屬一類缺陷——把 `textContent` 賦給一個如今擁有元素子節點的元素——只是經由另一個呼叫方觸發。教訓可以推廣：**一旦把某元素的內容拆進子 `<span>`，該元素上每一個既有的 `textContent` 寫入方都成了潛伏的缺陷**。
雙重鎖定：在 `node:vm` 中以假按鈕跑真實的 `withSpinner`，以及在 320 px 的真實瀏覽器裡點擊 Doctor。兩者都已確認在舊程式碼上失敗。**3009 → 3012 項測試**。

## [1.231.2] — 2026-09-06

**修復 — 行動版頂欄折成兩列、伺服器執行較舊的上游、以及 cvstart.ru 的轉址對任何已含語言的路徑都回傳 404。**

### 修復
- **行動版頂欄重新回到一列，操作按鈕改為純圖示。** 操作按鈕此前被強制放到整寬的第二列，手機上呈現為 `[☰ · 搜尋]` 上方、`[🔔 🌙 Doctor 開啟 Scan]` 下方——兩枚寬膠囊獨佔一列。現在它們是鈴鐺與主題切換旁的 36 px 方塊（🩺 / ⚡），在 320 px 下全部裝進一列。是實測而非目測：文件寬度在 320、360、390、430 px 以及按鈕從前會蓋住搜尋框的 560–760 px 整個區間內都與視區相等。**語言不再對行動版版面產生任何影響**，因為標籤被隱藏、按鈕是固定方塊；超過 900 px 時標籤原樣回歸。
- **發行版是依上游 1.32.0 建置的，伺服器卻執行上游 1.31.0。** resumecraft.ru 的 `/api/health` 回傳 `parentVersion: 1.31.0`。已同步 95 個執行期檔案並刪除 2 個。由針對線上環境的瀏覽器 QA 發現。
- **`cvstart.ru` 會為任何路徑前置 `/ru/`，完全不看裡面已經有什麼。** `/ru/help` 變成 `/ru/ru/help`，`/en/help` 變成 `/ru/en/help`，兩者皆 404。前者是從主站複製網址就會遇到的；後者更嚴重，直接讓英語訪客無法抵達英文版。現在已帶有 16 個語言前綴之一的路徑原樣通過，`en` 被移除（英文位於根目錄），其餘仍然取得 `/ru/`。

### 變更
- **98 份被取代的 QA 提示已移入 `qa/archive/superseded-prompts/`。** `qa/README.md` 自 v1.137.0 起就寫明頂層只保留當前發行版的提示——但該規則連續 95 個版本未被執行。
- **又有五份提示離開頂層——它們自稱當前，實則不是。** `REGRESSION-FINAL.md` 自稱「唯一權威、與版本無關的回歸提示」，但其 §§11–15 是五份已關閉的週期台帳（v1.55.x → v1.59.7），所宣告的說明基線為 **16 個語言下 28 H2 / 103 H3**，而今天是 32 / 122 與 17：照它執行的 QA 代理會在 §9 第一步就失敗。`QA-REGRESSION-PROMPT.md` 頂著無版本的名字，內容卻停在 v1.226.0。`QA-REGRESSION-PROMPT-v1.76.0-FULL.md` 被釘在 155 個版本之前，`REGRESSION-PROMPT-FINAL.md` 收束的是 v1.58.52 → v1.59.10 週期，而 `QA-FULL-REGRESSION.md` 以 v1.131.1 基線把它們統統重複了一遍。`PARENT-SYNC-WORKLIST-v1.26.0.md` 一併歸檔（上游已到 1.32.0），帶日期的 `UX-AUDIT-2026-07-06.md` 移入 `reports/`。留下四份驅動，其自身基線已更新為 v1.231.2 的實測值。`qa/README.md` 重寫為描述真實存在的目錄。

### 說明
- 頂欄修復過程中踩到的兩個優先序陷阱現已在 `app.css` 中註明：桌面端規則宣告在 `max-width: 900px` 區塊**之後**，因此在優先序相同時勝出。
- `applyI18n()` 會指派 `el.textContent`，因此 i18n 鍵被移到內層 `<span>`；留在按鈕上會在每次切換語言時抹掉圖示。
- 上游的第一次 rsync 在一個本地已不存在的檔案處**中途中斷**，卻仍回報成功。上游部署現在以總和檢查碼驗證，絕不依據結束碼。

## [1.231.1] — 2026-09-06

**修正 — v1.231.0 的計數替換改寫了所有連結中的帳號名。**

### 修復
- **每一個 `Fighter90` 連結都變成了 `Fighter92`。** v1.231.0 的來源計數替換在數字邊界保護下把 `90` 換成 `92`；該保護能正確跳過 `290` 與 `901`，卻會比對到 **`Fighter90`** 裡的 `90`。**17 個 README、17 套說明文件以及網站說明鏡像中的 562 處連結**，加上**維基裡的 138 處**，都指向不存在的帳號。是一位使用者因為圖片沒顯示而發現的。
- **五處歷史版本引用發生偏移。** 同一版本的 `85 → 87` 替換把 `v1.85.0` 變成 `v1.87.0`，憑空造出一個從未發布的版本。

### 說明
- 數字邊界並不夠：它保護數字不受相鄰**數字**影響，卻不能防止它是**識別碼**或**版本字串**的一部分。計數本身正確：92 個來源、87 個 EN 轉接器、3009 項測試。
- 上游儲存庫未受影響。

## [1.231.0] — 2026-09-05

**新增 — 來自上游 career-ops v1.32.0 的兩個掃描來源（Collage、Telegram 嚴格）、Gem 的 REST 模式，以及移植過程在上游自身程式碼中發現的一處缺陷。**

### 新增
- **兩個新來源，90 → 92**（87 EN + 5 RU），`ALL_ADAPTERS` **85 → 87**。皆無需權杖。
  - **Collage**（`provider: collage`）— Collage HR 的公開職缺網站 API。路徑片段是**租戶自己選定的網站位址**，而非由公司名產生的 slug，因此只從 `api:` 或 `secure.collage.co/jobs/<位址>` 的職缺頁位址讀取，**絕不猜測**：猜出的位址會去掃描別人的職缺板。
  - **Telegram（嚴格）**（`provider: telegram-channel`）— 讀取與現有 `telegram` 來源相同的 `t.me/s/<頻道>` 預覽，但做的是**相反的取捨**：只有當貼文**點名雇主並且連結到職缺頁**時才成為一列。摘要貼文、隱藏雇主的貼文與僅留聯絡方式的貼文都會被丟棄。涵蓋率低是刻意為之——上游量得 HR 經營頻道有 32%–77% 的貼文通過——但每一列都帶著真實的雇主與真實的連結。兩者與上游一樣同時提供。
- **Gem 新增了選用的 REST 模式**，與 GraphQL 路徑並存（`api: https://api.gem.com/job_board/v0/<board>/job_posts`）。它絕不由職缺板 id 推導，GraphQL 路徑仍為預設。移植自上游 #3783。

### 修復
- **`telegram-channel` 的地點篩選看不見西里爾字母，會把城市當作雇主。** 上游的 `LOCATIONISH_RE` 使用了僅適用於 ASCII 的 `\b`，它在西里爾字母旁邊永遠不會觸發，因此其中所有俄語備選項都不可達，`Senior Engineer | Москва` 會把「Москва」**當作雇主**回傳。這正是整套歸屬策略要防止的「把錯誤欄位當作事實」，而且恰好打中這個來源的主要受眾。web-ui 的移植改用 Unicode 判定，與上游在下一個常數 `ROLE_WORD_RE` 中已有的做法一致。以一個在 ASCII 形式下會失敗的測試固定。**上游尚未修正。**

### 說明
- **上游的 Workday 分面復原工作（#3851、#3874）無需移植** — web-ui 根本不對 Workday 分頁，只有 `offset: 0` 的一次 POST。
- **`providers/_types.js`** 僅文件變更。
- 測試：**2962 → 3009**（+47）。

## [1.230.0] — 2026-09-05

**變更 — 來自上游 career-ops v1.32.0 的三項提供方修正，以及移植過程本身暴露的一處缺陷。**

### 新增
- **Welcome to the Jungle 接受伺服器端 Algolia `filters` 運算式**（`wttj: { filters: "offices.country_code:FR AND contract_type:full_time" }`）。單靠關鍵字無法收窄一個全球職缺板：決定哪 `max_hits` 筆結果回傳的是 Algolia 的相關度排序，而不是掃描器的篩選條件——我們的篩選在其後才生效，作用於已經抵達的內容，因此超出上限的部分無論多麼契合都看不見。篩選器縮小的是結果**集合**本身而非重新排序，所以單次查詢的上限從 200 提高到 1000。設定篩選器後 `queries` 變為選用：空查詢意味著「凡通過篩選的一切」。移植自上游 #3757。

### 修復
- **當租戶只是提供的職缺比其自家橫幅所宣稱的更少時，Radancy 不再警告「truncated」。** 上游將 `data-total-results` 與九個真實租戶逐一核對：五個吻合，但四個——既有小租戶也有超大租戶——在沒有任何重複列的情況下，交付的唯一職缺比公告值少 10%–56%。橫幅誇大了租戶自身搜尋索引實際提供的數量，因此拿累計計數與之比較，在多數職缺板上都是誤報。警告現在只在**我們自己的** `max_jobs`/`max_pages` 終止走訪時出現，那是呼叫方唯一能採取行動的情形；`totalResults` 作為脈絡保留在訊息中，永不參與判斷。移植自 #3839。
- **Radancy 為每一次 JSON 片段請求繞過快取。** 該路由前的快取層在部分租戶上重播過期回應：同一筆職缺在數頁之後再次出現，而另一筆從未被回傳——上游九次執行中九次重現。`Cache-Control`/`Pragma` 請求標頭毫無作用，而每次請求附帶一個隨機參數解決了問題。它必須按**呼叫**唯一，絕不能由頁碼推導：確定性的參數仍會給快取一個穩定的鍵，從而保留同樣穩定而錯誤的對應。移植自 #3839。
- **Greenhouse 的辦公地點清單具有確定順序。** Greenhouse 並不保證兩次回應之間 `offices[]` 的順序，而從 `/offices` 樹中還原出的集合遵循其走訪順序。兩者都會進入職缺的 `location` 欄位，因此職缺板重排辦公地點就會改寫這個字串——於是毫無變動的職缺被讀成新職缺。上游修正了集合（#3839）；**web-ui 還存在同一缺陷的第二處實例，而上游沒有**——當職缺自身沒有地點時啟用的 `offices[0]` 回退——現在兩處都已排序。

### 說明
- **上游的 iCIMS 修正（#3728）無需移植。** 它從詳細頁的 JSON-LD 中讀取每一個 `jobLocation` 項目，因為部分租戶會在真實地址之前放置一筆全為 `UNAVAILABLE` 的項目。web-ui 的 iCIMS 從**列表頁**的 HTML 中取得地點，從不開啟詳細頁。
- **上游對 `_http.mjs` 的改動無需移植。** 它抽出 `isRefusedRedirectError` 供第二個使用者共享判斷，而 web-ui 沒有那個使用者；`isRetryableError` 的行為不變。
- **位置感知去重（#3751）未做鏡像**——它位於上游 CLI 的 `scan.mjs` 管線中。
- 登錄檔不變：**90 個來源**（85 EN + 5 RU）。測試：**2956 → 2962**（+6）。

## [1.229.0] — 2026-09-03

**新增 —— 來自父專案 career-ops v1.31.0 的四個掃描來源：Built In、Feishu Jobs、Garena 與 MokaHR。**

### 新增
- **四個新來源，86 → 90**（EN 85 + RU 5），`ALL_ADAPTERS` **81 → 85**，四者皆無需權杖。**Built In** 涵蓋美國科技職缺板的九個市場，且**不攜帶預設查詢**：既無 `queries:` 也無 `categories:` 的項目什麼都不掃描，並直說這一點 —— 共用來源不該背負某個人的搜尋詞；不在白名單上的市場會被**拒絕**，絕不悄悄換成全國板。**Feishu Jobs** 只接受兩種主機形式，而兩者通往職缺頁的**路徑不同**。**Garena** 是單一公司：`office` 塑造的是**連結**，不是列表。**MokaHR** 回傳**加密回應**（金鑰隨行的 AES-128-CBC 信封）—— 這是混淆而非安全，但不解密就什麼也讀不到。

### 修復
- **`htmlToText` 可能把不完整的標籤開頭洩漏進內文。** `safe &lt;img src=x onerror=1` 會輸出為 `safe <img src=x onerror=1`。現在每次解碼之後都跟一次清除，末尾的開頭字元會失去角括號。移植自父專案（#3491）。

### 說明
- **父專案的 Workday 修復一點也沒有移植。** 它拯救的是後端在位移 2000 處卡住分頁的租戶 —— 而 web-ui 根本不分頁 Workday（`offset: 0` 上一次 POST，100 列）。
- **`verify-cv-facts` 與 `merge-tracker` 的修復無需移植**：web-ui 從父專案檢出中執行這些指令碼。
- 測試：**2909 → 2956**（+47）。


## [1.228.5] — 2026-08-30

**修復 —— 上報的版本描述的是磁碟上的檔案，而不是正在執行的程式碼。**

### 修復
- **`/api/health` 在每個請求上重新讀取 `package.json`，因此陳舊的行程會上報一個它並未執行的版本。** QA 發現一個本機實例回答 `version: 1.228.4`，同時對 1.228.3 新增的 `/api/ping` 回傳 404：它在部署之前啟動，提供著舊程式碼，卻上報新檔案的版本。一次只複製檔案、從未重啟的部署，正是這樣看起來像成功的 —— 維運唯一會檢查的那個字串，恰恰是看不見問題的那個。這與 v1.228.1 的符號連結同屬一類，當時 rsync 在傳輸中途中斷，卻依然回報成功。

  版本現在在模組載入時擷取，因而描述正在執行的程式碼。陳舊的行程會上報**舊**版本 —— 那是真相，第一個請求就能看見。`parentVersion` 刻意保留每請求讀取：它描述的是父專案檢出，本行程並不載入，且在執行中的伺服器下合法地變化。

## [1.228.4] — 2026-08-30

**修復 —— 無需認證的存活探針在每個請求上都做磁碟工作。**

### 修復
- **`GET /api/ping` 每次命中都讀取並解析 `package.json`。** 它是唯一無需憑證即可到達的端點，因此每請求一次磁碟讀取加一次 JSON 解析，等於把拒絕服務的槓桿遞給任何人 —— CodeQL 將其標記為缺少速率限制。版本現在只在註冊路由時讀取一次；不重啟本就無法改變，因此處理函式完全不做 I/O。速率限制能給傷害封頂；移除這項工作則移除了槓桿本身。
- **迴環之外對個人資料擁有者真實姓名的遮罩現在由測試固定。** `/api/health` 早已將其替換為 `hidden`——但只是因為那一行與專案根目錄共用 `hidden ?? value` 這道防線。沒有任何測試驗證它，因此一次給該行賦予自身值的改動，會把真實人名悄無聲息地洩漏到區域網路。
...
**新增 —— Telegram 頻道作為掃描來源，以及 `/api/scan-results` 的分頁查詢模式。**

### 新增
- **Telegram 頻道現在可被掃描。** 在 `portals.yml` 的 `telegram_channels:` 區塊中列出，每個頻道透過公開網頁預覽 `https://t.me/s/<頻道>` 讀取。註冊表：**85 → 86 個來源**（EN 81 + RU 5），`ALL_ADAPTERS` **80 → 81**。

  Telegram 不提供 RSS，Bot API 也無法讀取機器人未管理的頻道 —— 兩條顯而易見的路都被堵死。`/s/` 預覽是伺服器渲染的純 HTML：無需登入、無需 JS、無需 Cookie。已在 16 個頻道驗證：15 個在裸 GET 下各回傳 20 則。

  `channel:` 接受任意寫法。解析器錨定在 Telegram 改版最不可能挪動的三處：`data-post`、內文容器、ISO 日期 `<time datetime>`。
- **`GET /api/scan-results` 新增分頁查詢模式。** `?q=<文字>&limit=50[&region=ru|en][&set=filtered|fresh][&offset=N]` 回傳 `{ total, returned, offset, limit, set, rows }`，其中 **`total` 是分頁之前的計數**。不帶參數時該路由仍回傳完整快照 —— `#/scan` 表格依賴那個形狀。

### 修復
- **Telegram 助理只憑一小部分資料作答。** 它的技能早已指向 `/api/scan-results`，但該路由只會回傳整份快照 —— 實機上約 2 MB —— 塞不進代理的上下文。面對「找出所有 Продуктовый менеджер」，它回報 **9** 筆，而快照裡有數百筆，並說自己「在用一個受限的本地索引」。這是實話，原因是它看得見端點卻消化不了。有了分頁模式，同一個問題只花 2 KB，並回傳誠實的 `total`。技能也一併重寫：原文要求總結*新增*貼文，也就是指向 `fresh`（僅上次執行）而非 `filtered`（全部保留項）。

### 備註
- **頻道貼文是散文，不是職缺記錄。** 沒有結構化欄位：標題取自第一行實質內容，公司／地點／薪資取自明確標籤，內文放入描述。**把真實職缺與廣告、摘要區分開的工作交給現有的 `title_filter`** —— 就是你已為其他來源調好的那個。這些是線索，不是清單。
...
**修復 —— 區域掃描耗盡伺服器記憶體，另有一個 Playwright 套件因自身拆解而隨機失敗。**

### 修復
- **區域掃描以 OOM 拖垮了伺服器。** `ru-scanner` 會累積**所有查詢的全部原始結果**，直到最後才去重與過濾。查詢清單刻意由近義詞構成（`Golang`、`Go разработчик`、`Golang разработчик`、`Senior Go` ……），因此同一職缺每個查詢都會回傳一次，陣列持有的物件是唯一數量的數倍 —— 還帶著職缺描述，而描述佔物件的絕大部分體積，且幾乎全會被過濾器丟棄。實測：21 個查詢的真實執行佔用堆積 **742 MB**。伺服器將 Node 堆積上限設為 **490 MB**（V8 依機器 956 MB 記憶體推算），於是掃描以 `FATAL ERROR: Reached heap limit` 拖垮服務 —— **一天四次**。現在去重與過濾按查詢進行，同一次執行降至 **177 MB**，減少 76%。所回報的數字不變：唯一總數由 URL 字串的 `Set` 承載。
- **`net::ERR_SOCKET_NOT_CONNECTED` 隨機拖垮 CI。** 三個 Playwright 套件在斷言無主控台錯誤前呼叫 `window.stop()` 中止在途請求。Chromium 會依 socket 所處狀態，用**三種**不同名稱回報這同一個動作：取消時為 `ERR_ABORTED`，在途時為 `ERR_EMPTY_RESPONSE`，socket 已消失時為 `ERR_SOCKET_NOT_CONNECTED`。共用過濾器只列了前兩種。它在語言巡檢的 `tr` 上暴露，而同樣呼叫 `window.stop()` 的 `playwright-forms` 與 `playwright-smoke` 也只差一次倒楣的執行。

### 備註
- **為何現在才失敗。** 加入產品經理相關搜尋後，查詢從 14 條增至 21 條。舊實作的記憶體隨結果總數成長，查詢 +50% 便衝破了機器上限。
- **去重改動保持行為不變，這一點是測過的而非宣稱的。** `tests/ru-scanner-dedup-order.test.mjs` 鎖定了那個彆扭的方向 —— 後來的重複項若**未**通過過濾，必須把先前通過的那條擠掉 —— 並對舊實作做了 200 次隨機等價校驗。
- **主控台過濾器仍然狹窄。** 只有被取消的請求這一族是良性的；連線被拒、DNS 失敗、連線重設、任何 5xx 和未捕捉的 JS 例外仍會使斷言失敗。
- 兩個假設經檢驗後被否決而非草率發布：Caddy 逾時（設定乾淨，SSE 所需的 `flush_interval -1` 正確）與 Node 預設的 5 分鐘 `requestTimeout`（縮比重現表明它不會中斷長時間串流回應）。真正的證據是日誌裡的 `Reached heap limit`。測試：**2860 → 2865**。

## [1.227.4] — 2026-08-29

**修復 —— `content_filter` 仍忽略 `word:` / `stem:`；v1.227.3 只修了標題過濾器。**

### 修復
- **`content_filter` 未遵循這兩個前綴。** v1.227.3 讓 `title_filter` 學會了上游的 `word:` / `stem:` 前綴，卻把 `content_filter` 留在它自己的裸 `lower.includes(k)` 上，因此那裡帶前綴的條目仍被當成字面文字 `"word:java"` 來比對 —— 同一個靜默空操作，只是挪到了下一個過濾器。`content_filter` 讀取職缺**描述**，其預設一直是簡單子字串比對：正因如此，一個裸的否定項 `java` 會排除所有僅僅提到「JavaScript」的職缺；前綴就是讓單一條目跳脫該預設的手段。已移植上游的 `compileContentKeyword`：它共用前綴機制，但刻意**不**自動錨定短關鍵字 —— 標題過濾器錨定 2–3 字母縮寫，是因為「Coordinator」中的「COO」永遠是錯的；而描述段落裡的短字串通常是刻意為之（`aws`、`gcp`、`sql`、`go`）。

### 備註
- **在 v1.227.3 PR 的 AI 審查中發現**：審查注意到程式碼並未涵蓋其自身註解所宣稱的範圍 —— 註解寫著「`title_filter` / `content_filter`」，但接上的只有標題路徑。現在註解已明確區分：兩個過濾器都遵循前綴，只有標題過濾器會自動錨定縮寫。
- **同樣依該審查核實且未變：** 把 2–3 字母縮寫規則從 `\b` 換成 Unicode 邊界，在 ASCII 上行為完全一致，僅在應當從嚴處更嚴 —— `COO_lead` 與 `coo1` 仍保留（底線與數字在兩種寫法下都是單字字元），`Coordinator` 保留，「Директор COO」與「VP «Продукт»」被正確排除。已核對 14 個邊界案例。
- 不帶前綴的 `content_filter` 條目逐位元組保持原有比對，空的或缺失的描述仍會通過，空的 `word:` / `stem:` 不比對任何內容而非全部比對。`tests/content-filter.test.mjs` 新增 6 個案例。測試：**2852 → 2858**。

## [1.227.3] — 2026-08-29

**修復 —— 標題過濾器的 `word:` / `stem:` 前綴被靜默忽略，使那些過濾行形同虛設。**

### 修復
- **web-ui 未實作上游的 `word:` / `stem:` 前綴。** `title_filter`（以及 `content_filter`）預設以不分大小寫的**子字串**比對，這正是為何一個裸的否定項 `intern` 連「International Product Manager」與「Internal Tools Engineer」也一併排除。與其翻轉該預設值（會破壞所有已設定的安裝），上游選擇讓精確比對逐條選用：`word:intern` 只比對完整單字，`stem:agent` 必須位於詞首且可延續（藉此區分「Agentforce」與「Reagents」）。web-ui 兩者皆無，於是把該條目當成**字面文字** `"word:intern"` 來比對 —— 而沒有任何職缺標題含有這串字元。該行因而成為靜默的空操作：同一份 `portals.yml` 在 CLI 中過濾正常，在這裡完全不過濾，使用者排除掉的實習職缺依舊全數出現在 `#/scan` 表中。已自上游 `title-keywords.mjs` 移植，包含支援 Unicode 的詞邊界（`\p{L}\p{M}\p{N}_`），而非僅支援 ASCII、會在帶重音或西里爾字母標題中於詞中命中的 `\b`。

### 備註
- **在檢查一份真實 `portals.yml` 時發現**，其中剛引入 `word:intern`，**正是**為了讓裸的 `intern` 不再丟棄「International Product Manager」。該前綴在 CLI 中生效，web-ui 卻悄悄保留了所有實習職缺。現在兩者行為一致。
- **預設行為未變。** 不帶前綴的關鍵字維持原有比對方式：片語以及任何含非字母字元的條目（`.NET`、`SAP `、`L&D`）仍按子字串比對，2–3 個字母的縮寫仍錨定詞邊界。空的 `word:` / `stem:` 視為筆誤，**不比對任何內容** —— 作為否定項，空樣式會比對所有標題，僅因一個多餘的冒號就否決整次掃描。
- 2–3 字母縮寫規則也改用同一 Unicode 邊界：先前使用 `\b`，導致 `coo` 與 `word:coo` 在非 ASCII 標題上表現不一致 —— 同一條規則卻有兩種寫法。
- `tests/title-filter.test.mjs` 新增 6 個案例。測試：**2846 → 2852**。

## [1.227.2] — 2026-08-29

**修復 —— 窄螢幕手機上 `#/config` 會橫向溢出。**

### 修復
- **MOBILE-1：在 ≤ 352 px 的視口下 `#/config` 橫向溢出。** 每個 select 類欄位都帶著**行內**的 `min-width: 300px`。行內樣式優先於樣式表，因此控制項無法隨容器收縮：在 320 px 視口下 select 的右邊緣落在 353 px，頁面出現橫向捲軸，控制項被擠出右邊界。受影響的是全部 18 個供應商/模型下拉選單（`cfg-llm-provider` 及所有 `cfg-*-model`）。現改為 `min-width: min(300px, 100%)` + `max-width: 100%`，只有空間足夠時才套用 300 px 的舒適寬度。處於臨界值及以下的裝置：iPhone SE（一代，320 px）、老舊 Android、Galaxy Fold 外螢幕。

### 備註
- **先前所有行動端檢查為何都沒發現。** 360 px 與 375 px 恰好有足夠餘裕掩蓋溢出，而 Playwright 套件執行在 1280 px 寬的視窗中。CI 裡從未有任何測試量測過窄於 375 px 的視口。新增的 `tests/playwright-narrow-viewport.mjs` 在 **320 px** 下巡覽帶表單的路由，出現任何橫向溢出即失敗，並在斷言訊息中指出最寬的元素；第二個案例鎖定 `#/config` 的任何下拉選單都不得越出視口。提交前已先驗證它能重現該缺陷（`scrollWidth 353 > clientWidth 320`，18 個右邊緣為 353 的下拉選單）。Playwright：**99 → 101**。
- **已知且刻意暫未納入門檻：280 px**（Galaxy Fold 外螢幕）仍會溢出約 5 px，但成因不同 —— 是設定卡片 `<details>` 區塊內的長內容，以及 256 px 側邊欄對 280 px 視口，而非本次修復的行內 `min-width`。為一個主流手機都不會回報的寬度重新排版屬於另一項工作；套件的 `WIDTHS` 清單已為此就緒。
- 同時更正了 v1.227.1 的 QA 提示：其中把跟進路由寫成 `#/modes/followup`。mode 頁面以裸 slug 註冊（`Router.register(cfg.slug)`），因此路由是 `#/followup`，而 `#/modes/followup` 正確回傳 404。僅文件變更。
- 單元測試套件維持 **2846**（新增涵蓋屬瀏覽器層級）。

## [1.227.1] — 2026-08-29

**修復 —— 說明中的來源拆分與自身對不上，且 GitHub 橫幅在應用內被當成原始標記印出。**

### 修復
- **§17 給出的總數與拆分不相加。** 所有說明包都寫著「`server/lib/sources/` 註冊表內建 **85** 個 adapter —— **78 個英文 + 5 個俄文**」。78 + 5 = 83，而非 85；ja/ko 偏得更遠，為 77（= 82）。每加一個來源只更新總數、不動加數，兩個數字已經錯開好幾個版本。現在 17 種語言的拆分統一為 **80 EN + 5 RU**。這不只是觀感問題：`docs/help/<lang>.md` 是「Ask the docs」唯一的依據語料，因此詢問支援多少來源的使用者會從同一句話得到兩個互相矛盾的數字。
- **同一句話釘死了一個過期的版本錨點。** 「截至 **v1.213.0**，註冊表 …」在 16 種語言中存活，而計數已變動兩次（v1.219.0 為 83，v1.226.0 為 85）。該錨點在 v1.210.1 已手工修過一次，隨後又漂移。現改為不含版本的措辭並移除 —— 計數本身對照活的註冊表校驗，不會過期。
- **供應商橫幅在 `#/help` 頂部被當成跳脫後的標記印出。** v1.225.0/v1.225.1 記錄稱它「因 `UI.md()` 不支援圖片而在應用內被剝離」。這混淆了兩件事：`UI.md()` 是跳脫優先的 —— 它在任何 markdown 轉換之前跳脫來源的每個位元組，這是正確的 XSS 邊界，但與移除恰恰相反。沒有人移除橫幅，因為從來沒有程式碼嘗試過；於是 17 種語言的 `#/help` 都以 268 個字元的字面 `<p align="center"><img src="https://…` 開頭。新的共用入口（`server/lib/help-markdown.mjs::stripGithubOnlyBlocks`）在讀取時丟棄獨立的 HTML 圖片區塊 —— `GET /api/help/:lang` 與 docs-assistant 語料兩者皆然。檔案中仍保留橫幅，因此 GitHub 上的展示不受影響。
...
**新增 —— SEEK 香港（JobsDB）、跟進的僅待辦篩選，以及保密雇主顯示代為投遞的仲介機構。**

### 新增
- **透過既有 Jobstreet/SEEK 來源接入 JobsDB 香港。** 香港以 JobsDB 品牌營運，但運行在同一 v5 端點背後的同一 SEEK 平台上，因此無需獨立來源——只需在主機白名單中加入 `hk.jobsdb.com` 並設定 `siteKey: HK-Main`。登錄檔維持不變：**85 個來源**（80 EN + 5 RU），`ALL_ADAPTERS` **80**——這是既有配接器上的新市場，而非新配接器。
- **節奏看板的僅待辦篩選**（`#/modes/followup`）。新增核取方塊把看板收窄到*此刻*需要行動的項目。是否待辦取自上游 `followup-cadence.mjs` 已算出的 `urgency`（`urgent` / `overdue`），絕不取自追蹤表的 `status`——後者是 `applied`/`responded`/`interview`，不會比對到任何內容。當沒有待辦時，看板會指出最近一筆已排程的跟進，而非顯示空白。邏輯位於純模組 `public/js/lib/followup-view.js`，對應上游的 `followup-view.mjs`。
- **保密雇主歸因到代為投遞的仲介機構。** 隱去雇主的職缺在追蹤表中記為 `?`，招募方則寫入 `Via` 欄。`#/tracker` 先前只顯示一個孤零零的 `?`，多筆保密紀錄因此無法區分，標誌解析也無從比對。現在這類紀錄顯示為 **「保密 · 經由 \<仲介機構\>」**，標誌依仲介機構名稱解析，並可在搜尋框中用該名稱找到。這只是呈現層變更——規範欄位 `company` 絕不會被招募方覆寫。新增純模組 `public/js/lib/company-presentation.js`，對應上游的 `company-presentation.mjs`。

### 修復
- **Jobstreet/SEEK 先前在呼叫已淘汰的端點。** 該來源仍指向 SEEK 已下線的 chalice-search v4（`/api/chalice-search/v4/search`）；上游已遷移到 `/api/jobsearch/v5/search`，而 web-ui 未跟進。現在所有 Jobstreet/SEEK 項目都查詢 v5。v5 的項目結構被原生讀取（由 `id` 建構職缺 URL、`locations[0].label`、`advertiser.description`、`salaryLabel`），v4 專有的投影參數 `solrFields` 已移除；若某項目的 `portals.yml` 仍固定著失效的 v4 路徑，其市場主機名會保留，路徑則在 v5 上重建——無需使用者改動。

### 備註
- 與 career-ops 1.30.0 對齊（上游 HEAD `8d64f65` → `6cc46a4`；其 `VERSION` 仍為 1.30.0——release-please 落後於 `main`）。未移植及理由：**iCIMS 的 JSON-LD 地點回填**——web-ui 的 iCIMS 來源根本沒有 `enrichDate()` 詳情頁掛鉤（僅列表頁），無處可改；**非致命 CLI stderr（#1974）**——web-ui 的執行器僅憑結束代碼判定失敗，從未使用該啟發式；**Turbopack 路徑追蹤**——web-ui 沒有打包器；**`cv-sync-check` 修復**——以唯讀、fail-soft 方式轉送；**`scan.mjs` 中 Playwright 串行化、Block H 答案、updater/doctor/`update-system` 防護、遞迴語法檢查、Go 版儀表板，以及 `jd-skill-gap`/`verify-cv-facts` 的自我測試新增**——web-ui 不會呼叫的純 CLI 面，或不改變行為的自我測試。測試套件：**2818 → 2837**。
...
### 安全
- 掃描器 HTTP 核心(`server/lib/http-json.mjs` 的 `fetchJson`/`fetchText`)現在會在真實網路路徑上解析每個來源的主機,並**在其解析到私有/回送/連結本地/CGNAT/雲端中繼資料位址時拒絕連線** —— 封堵 DNS 重新繫結向量。掃描器主機本已鎖定到公共網域,故為縱深防禦;使用者 URL 路徑已有更強的連線鎖定(`safe-fetch.mjs`)。守衛**僅**在真實 `fetch` 傳輸上執行 —— 測試注入的 `fetchImpl` 從不被解析。

### 備註
- 正常掃描行為不變。掃描來源不變:**83**。測試:**2775**。
...
### 新增
- **Torre**(`provider: torre`)—— torre.ai 的泛拉美人才市集,一個免驗證的公開職缺搜尋,包含 Greenhouse/Lever/Ashby 上沒有的遠端職缺;每個項目一次受限請求。註冊表現在提供 **83** 個來源(英文 78 + 俄文 5)。此外,歡迎橫幅上顯示目前供應方的字母標。

### 修復
- a16z Speedrun 掃描在會輪換的看板上過早停止(把掃描中途的短頁當成最後一頁,截斷為約 300 中的 149);現在信任資料來源本身的 `total_pages`。歡迎橫幅原先從 4 條對應解析供應方(其餘 14 個顯示原始 slug),現改用 18 個的標籤+字母標;設定裡的供應方說明從 7 改為 18。

### 備註
- Get on Board 每項目多分類的新選項尚未接入(需要來源層重構)。測試:**2768**。

## [1.218.0] — 2026-08-26

**新增 —— 在顯示供應方的每個位置都加上標誌(與正確名稱)。**

### 新增
- 儀表板小標籤、設定「目前」摘要、⚡ 評估結果標題處的供應方字母標。

### 修復
- 過時標籤:三處介面從 5 條對應解析名稱(評估結果把所有非 Anthropic 供應方誤標為「Gemini」);現改用共享的 `ProviderStatus.label`(18)。金鑰計數顯示「/ 7」→18。`LLM_PROVIDER` 提示的 slug 清單在全部 17 種語言中新增 `ark` / `arkcn`。

### 備註
- 僅用戶端,CSP 安全。無伺服器改動。掃描來源:**82**。測試:**2758**。
...
### 新增
- **九個 OpenAI 相容供應方。** DeepSeek、GLM (Z.ai)、Kimi (Moonshot)、MiniMax、Mistral、Grok (xAI)、Together AI、Fireworks AI 與 **Ollama**（完全本機、免金鑰）。在**設定**中填入金鑰（Ollama 則填 `OLLAMA_BASE_URL`）,即會加入 Hermes 之後的 `auto` 順序;用 `LLM_PROVIDER` 可固定任一個。Together 亦託管 Thinking Machines 的 **Inkling**（`thinkingmachines/Inkling`）。Web UI 現已涵蓋 **16 個**即時供應方。
- **供應方字母標。** 一個 CSP 安全、品牌配色的首字母方塊在設定的金鑰欄位旁及用量頁面的列內標記每個供應方——內聯 SVG,無外部標誌,任何資料都不離開你的裝置。

### 備註
- 供應方的基礎 URL 屬受信任設定,經由 `runOpenAICompatible()` 的 http(s) 協定檢查(允許 Ollama 回送),絕不經過職缺 URL 的 SSRF 驗證器。掃描來源不變:**82**。測試:**2752**。
...
### 修復
- **屬性內含 `>` 的 HTML 描述現在被乾淨地剝離。** Greenhouse、Recruitee 描述背後的標籤剝離器用的是在第一個 `>` 就停下的樸素比對,於是像 `<a title="salary > 100k">` 這樣的標籤會把尾部屬性文字(`100k">`)留在描述裡。現在它尊重帶引號的屬性值(單引號或雙引號),在解碼實體*之前*剝離標記(這樣屬性內被編碼的引號不會成為假分隔符),並且仍在純文字中保留字面的 `<>`。

### 說明
- 測試套件:**2742**。掃描來源維持 **82** 不變。本輪父專案沒有其他新增:其餘更改以唯讀方式中繼(follow-up / weekly-digest / rejection-latency / salary-gap 修復無需 web-ui 改動)或不屬於 web-ui 範疇(一個外部 X/Xquik 自帶金鑰外掛、CLI/verify 修復)。可選項 —— SmartRecruiters 詳情描述、detect-reposts 聚合器跳過、`stem:` / `word:` 標題前綴、DNS 重繫結加固 —— 仍在排隊。



## [1.214.1] — 2026-08-25

**修復 — Ashby 的職缺描述現在與 Greenhouse、Recruitee 截斷到相同長度,因此內容篩選在三個板塊上表現一致,且單一板塊無法撐大掃描快取。**

### 修復
- **Ashby 描述現在遵守與其他板塊相同的長度上限。** v1.214.0 把 Ashby 的職缺內文送入內容篩選,但與截斷到 4000 字元的 Greenhouse、Recruitee 不同,Ashby 的內文是無上限直接映射(有些長 4 倍)。於是同一個 `content_filter` 關鍵字僅因它落在文字的位置不同,就可能在 Ashby 比對到而在 Greenhouse 比對不到,而單一板塊可能給 `#/scan` 載入的掃描快取增加數百 KB。Ashby 現在也截斷到相同的 4000 字元(其文字已是純文字,因此只截斷,絕不剝離標籤)。

### 說明
- 測試套件:**2738**。掃描來源維持 **82** 不變。
