...
### Rettet
**Jobstreet / SEEK — hvert australsk, newzealandsk, singaporeansk, malaysisk og Hongkong-opslag pegede på en død side.** `parseJobstreetItem` byggede detalje-URL'en som `${baseUrl}/id/job/${id}` betingelsesløst. `/id/`-segmentet er ikke en sti — det er det **indonesiske lokale præfiks**, og det findes kun på de indonesiske værter. Enhver anden SEEK-platform-vært svarer **404** på `/id/job/<id>`. web-ui havde allerede tilføjet `www.seek.com.au`, `www.seek.co.nz`, `sg.jobstreet.com`, `my.jobstreet.com` og `hk.jobsdb.com` til `ALLOWED_JOBSTREET_HOSTS`, så scanningen returnerede disse opslag uden videre — hvert eneste pegende på en 404. Stien er nu nøglet på værten: `ID_LOCALE_HOSTS` (`id.jobstreet.com`, `www.jobstreet.co.id`, `jobstreet.co.id`) beholder `/id/job/`, alt andet får `/job/`, og en ikke-parsebar oprindelse falder nu igennem til den fælles sti i stedet for at kaste en fejl. To af vores egne test havde **indkodet fejlen** — de påstod `hk.jobsdb.com/id/job/…` og `www.jobstreet.com/id/job/…` — og blev rettet.
**Oracle Cloud — en kort side midt i listen afsluttede gennemgangen og droppede resten af boardet.** Stopbetingelsen var `listLen === 0 || listLen < PAGE_SIZE`, men ORC leverer færre rækker end grænsen *midt i en liste*, fordi visse rækker filtreres server-side. Målt opstrøms hos American Express: `TotalJobsCount` **454**, sider på **200 / 199 / 54** — de 199 blev læst som slutningen, og de sidste **54 opslag, 12% af boardet**, blev aldrig hentet. Nu afslutter en tom side altid gennemgangen; når tenanten rapporterer et samlet antal, fortsætter pagineringen, indtil `offset + PAGE_SIZE >= total`; kun en tenant uden et samlet antal at holde op imod afsluttes på en kort side. Dette følger den bredere konvention om, at et API »kan returnere færre resultater end det anmodede antal … selv når det ikke er ved slutningen af samlingen« (Google AIP-158). `hasMore` ignoreres fortsat bevidst — nogle tenants returnerer `hasMore:false` på hver eneste side af et board med 7000 job. Nu hvor korte sider ikke længere afslutter gennemgangen, er `MAX_PAGES` det eneste bagstop mod en tenant, der rapporterer et falsk samlet antal, så det er dækket af sin egen test.
**Liveness — `"This role is closed"` blev ikke genkendt, og en falsk udløbsstatus er permanent.** Mønsteret var `/this job (listing )?is closed/i`, som overser de lige så almindelige formuleringer med `role` og `position`; på ét board opstrøms blev **111 af 111** usikre opslag målt med `"role"`. Udvidet til `/this (?:job|role|position)(?: listing)? is closed\b(?!-)/i`. Den afsluttende `\b(?!-)` er en vagt mod sammensatte tillægsord: `\b` alene afviser `closedown`, men matcher stadig ægte prosa som *"This role is closed-loop control of the platform"*, og det er lookahead'en, der stopper det.
**Liveness — `job expired` ligger nu i et blødt lag, der ikke kan overtrumfe en synlig Apply-knap.** `HARD_EXPIRED_PATTERNS` tjekkes *før* apply-kontrol-testen, så alt, der er placeret der, vinder over et aktivt opslag. `/\bjob expired\b/i` udløses fejlagtigt af fire ægte live-sideformer — en karruselpost under `"Similar jobs"`, en filterknap `"Hide job expired"`, en FAQ i bunden, og closed-loop-prosaen nævnt ovenfor — fordi klassifikatoren modtager hele sidens tekst. Mønsteret ligger nu i `SOFT_EXPIRED_PATTERNS`, tjekket efter apply-kontrollen og før heuristikken for opslagssider, og returnerer `expired_body_soft`; et rent `"JOB EXPIRED"`-banner uden apply-kontrol udløber stadig. Denne rækkefølge betyder noget, fordi en falsk `expired` skrives til scan-historikken som `skipped_expired` og derefter **dedup-filtrerer et ægte job ud af hver senere scanning, på ubestemt tid**, medmindre `scan_history.recheck_after_days` er sat.

### Tilføjet
**Personio — en eksplicit `personio: <slug>` fastgør tenanten.** Mange virksomheder indlejrer deres Personio-board som en **iframe** på en brandet karriereside, så `careers_url` peger på virksomhedens eget domæne, mens feedet ligger på `<slug>.jobs.personio.de`; uden en fastgørelse løser disse boards sig til intet. Slug'en er tegnbegrænset af en forankret tilladelsesliste (`/^[a-z0-9][a-z0-9-]{0,62}$/i`), og den resulterende URL routes stadig gennem den eksisterende `new URL()`-parsing, HTTPS-tjek og `PERSONIO_HOST_RE`-værtsspærre — fastgørelsen er aldrig en vej til at nå en anden vært. En slug, der bærer `.`, `/`, `@`, `:`, `..`, mellemrum, en indledende `-`, eller overskrider DNS-labelgrænsen på 63 tegn, falder igennem til dagens `api`/`careers_url`-adfærd. I modsætning til forælderen er web-ui's værts-regex versalfølsom, så fastgørelsen normaliseres gennem `new URL()` — `AcmeGroup` resolves til `acmegroup.jobs.personio.de` i stedet for at opbygge en vært, som dens egen tilladelsesliste derefter ville afvise.
**Jobstreet — tilvalgt `appendWorkType`** tilføjer opslagets arbejdstyper som suffiks til titlen, f.eks. `Strategy Consultant [Part time]`, så titelfiltre og triage kan se ansættelsestypen allerede ved scanningstidspunktet. Slået fra som standard; en manglende eller tom `workTypes` er en no-op.

### Noter
Kilde- og adapterantal er uændret: **94** kilder (89 EN + 5 RU), 89 EN-adaptere. Denne udgivelse flytter kun testantallet, **3164 → 3201** (+13 jobstreet, +7 Oracle Cloud, +9 liveness, +8 Personio).
Forælderen **slettede `providers/telegram.mjs`**, og konsoliderede på `telegram-channel`. web-ui registrerer begge som brugervendte kilder, så sletningen følges **ikke**: den ville sænke registret fra 94 til 93 og stiltiende ødelægge det for enhver, der har `telegram` konfigureret. Genovervejes kun som en bevidst, annonceret fjernelse.
`jd-skill-gap.mjs` fik en ikke-sammentrukket overskriftsform for `"what we are looking for"` og en strip af fed-markører. web-ui **relayer** dette script i stedet for at spejle det, så rettelsen ankommer sammen med den udrullede forælder og kræver ingen ændring her.
Ikke porteret, med begrundelser: `lib/latex-escape.mjs`, `browser-extract.mjs`, `openai-eval.mjs`, `update-system.mjs` og scaffolderen (web-ui hverken spejler eller kalder ud til dem); Go-dashboardets TUI og `web/`-konfigurationsformularen (separate flader); `doctor.mjs` og `analyze-patterns.mjs` (skrivebeskyttede relæer, hvis fail-soft-adfærd absorberer ændringen).
Forkets kyrilliske `\p{L}`-afvigelse i `providers/telegram-channel.mjs` overlevede upstream-sammenfletningen intakt og blev verificeret byte-for-byte mod en kopi fra før sammenfletningen.

## [1.236.0] — 2026-09-18

**Forælder-paritet — career-ops `main` @ `6a9c84c` (VERSION 1.33.0, 128 commits). To nye kilder, og tre spejlede rettelser — to af dem mistede opslag her, i stilhed.**

### Tilføjet
**To scanner-kilder, 92 → 94 (89 EN + 5 RU).** **Python.org Jobs** (`pythonorg`) læser boardets offentlige feed; arbejdsgiverattribution er obligatorisk, så en række, hvis titel ikke navngiver nogen arbejdsgiver, droppes i stedet for at blive udfyldt bagudrettet med portalindgangens navn. **Generalist World** (`generalist-world`) skraber boardets kort og **fejler højlydt ved markup, den ikke kan parse** — en kortform, der har ændret sig, kaster en fejl i stedet for at blive læst som et tomt board, hvilket er den fejltilstand, der gør en skrabet kilde utroværdig. Begge optræder i `#/scan`s Kilde-filter, begge er vært-fastgjort (forankret regex, kun HTTPS), og begge er drevet ende-til-ende af testpakker med falsk transport (43 og 36 tilfælde).

### Rettet
**En Workday-stilling åben i flere byer blev droppet af et lokationsfilter.** Workdays liste-endpoint besvarer et opslag med flere lokationer med et **antal**, hvor hvert andet opslag bærer et sted: `"53 Locations"`. Det er ikke en lokation, og `buildLocationFilter` matcher ved understreng uden hensyn til store/små bogstaver — så en stilling åben i Austin *og* 52 andre byer matchede ingen `allow: [austin]`-post og blev kasseret, mens den samme stilling opført enkeltvis slap igennem. Målt opstrøms på tværs af tre tenants: **53 af 291 opslag** var pladsholdere, så dette er standardtilfældet, ikke et randtilfælde. Opslagets reelle steder findes kun i CXS-**detalje**-dokumentet, så hver pladsholder koster nu én ekstra GET mod det, med et loft på **200 pr. post** — over loftet forbliver pladsholderen synlig i stedet for at blive forfalsket, fordi et uafklaret opslag aldrig må læses som et afklaret. En fejlet eller stedløs detalje efterlader opslaget præcis, som listen returnerede det: en synlig `"53 Locations"` er ærlig om det, der ikke kunne afklares, hvorimod en tømt lokation stiltiende ville bestå ethvert filter. Den samme GET bærer også `startDate`, så et beriget opslag dateres **præcist** i stedet for via liste-endpointets relative prosa (`"Posted 30+ Days Ago"` → `undefined`); alt, der ikke er en ren `YYYY-MM-DD`, lades urørt i stedet for at blive kørt gennem `Date.parse`, hvis ikke-ISO-fallback er implementeringsafhængig.
**Et angivet niveau kollapser ikke længere to rekvisitioner til ét genopslag.** `"Insurance Specialist II"` er arbejdsgiverens egen udtalelse om, at denne rekvisition ikke er søskende med den anden — et andet lønbånd, omfang og rekvisitionsnummer under én grundtitel. Tokenizeren kunne ikke se det: `w.length > 3` dropper hvert romertal op til VIII og hvert enkeltciffer, så `"Insurance Specialist"` og `"Insurance Specialist II"` blev tokeniseret identisk, scorede et perfekt Jaccard-forhold, og genopslagsdetektoren markerede den anden som en genudbydelse af den første. web-ui's `role-matcher.mjs` havde **slet ingen** niveauhåndtering, så dette bringer forælderens hele mekanisme med: niveauer udtrækkes med romerske og arabiske tal foldet til ét tal (`"Nurse II"` og `"Nurse 2"` er samme udtalelse — og *det* par adskilte sig forkert før), to angivne niveauer skal stemme overens, og et niveau kun på den ene side behandles som en løs omskrivning **medmindre** hver titel også bærer et ord, den anden mangler, hvilket er to uoverensstemmelser snarere end en udsmykning.
**Et Avature-board fastgjort til et filter gennemgik alligevel hele det globale board.** Avatures søge-UI genererer uigennemsigtige facet-parametre (`?42386=[812132]` er et landefilter), og en operatør, der fastgør et indsnævret board i `portals.yml`, forventer, at scanningen gennemgår *det*. `resolveSearch` genopbyggede URL'en som `${origin}${path}` og droppede forespørgselsstrengen fuldstændigt — så hver forespørgsel, inklusive den første, gennemgik tenantens hele board: flere sider, flere forespørgsler og opslag, som operatøren bevidst havde filtreret fra. Facetter følger nu med i hver pagineret forespørgsel, med vores egen `jobOffset` anvendt sidst, så en tilfældig facet med samme navn aldrig kan kapre pagineringen.

### Sikkerhed
**`devalue` 5.8.1 → 5.9.2** (transitivt via `astro`), som lukker den ene åbne Dependabot-advarsel — et lammelsesangreb ved fejlformet input. Kun en site-afhængighed; ingen runtime-kode ændret.

### Noter
**Ikke porteret, hver med sin begrundelse.** Forælderen gav **lever** en timeout på 30 sek. og **manfred** 25 sek. mod dens fælles standard på 10 sek. — en **ikke-portering**: web-ui's scan-transport tillader allerede **60 sek.** (`DEFAULT_SCAN_TIMEOUT_MS`), så begge boards har mere spillerum her, end rettelsen giver der. **iCIMS'** fordoblede `careers-`-præfiks kan heller ikke opstå her: web-ui tager tenant-origin direkte fra `api`/`careers_url` og konstruerer aldrig en vært ud fra et tenant-navn; dens nye fallback-vært-gennemgang er en funktion til forælderens løsere konfiguration, ikke en fejlrettelse. **deutschebahn** og **manfred** fik retry-ved-forbigående-afbrydelse, hvilket web-ui's loft på 60 sek. gør langt mindre presserende — noteret snarere end porteret, så en fremtidig timeout ved lang gennemgang har et kendt første sted at prøve. Alt andet i de 128 commits' delta er CLI, verifier, dashboard eller `web/` (forælderens eget frontend): `liveness-browser`, `check-jd-archive`, `verify-ats`/`verify-portals`/`verify-cv-facts`, `update-system`, `set-status`, `audit-portals`, `discover-ats`, `scaffolder`, `test-all`, plugin-manifester.
Forkets afvigelser genbekræftet efter pull'et: den kyrilliske `\p{L}`-rettelse i `providers/telegram-channel.mjs`, forkets egen `providers/telegram.mjs`, `hermes` i `web/src/lib/clis.ts`. Upstream `career-ops-hq/main` er fuldt flettet (0 commits mangler). **3066 → 3164 test**; hjælpen angiver nu **94** adaptere (**89 EN + 5 RU**) i alle 17 bundler, og overskriftsstrukturen er uændret på 32 H2 / 122 H3.

## [1.235.0] — 2026-09-16

**Forælder-paritet — career-ops `main` @ `68e6b94` (VERSION 1.33.0, 42 commits). To spejlede rettelser; den ene mistede MokaHR-opslag her, i stilhed.**

### Rettet
**Et andet job fra samme MokaHR-board forsvandt i stedet for at blive tilføjet.** `url-key.mjs` reducerer en opslags-URL til en sammenligningsnøgle, og den droppede fragmentet betingelsesløst — rimeligt nok, siden næsten hvert fragment kun er præsentation (`#apply`, `#section-2`). MokaHR er undtagelsen i vores eget register: dens opslag er `…/social-recruitment/{tenant}/{id}#/job/{n}`, hvor hvert job hos en tenant deler stien, og opslags-id'et findes **kun** i fragmentet. Så de blev alle normaliseret til én nøgle. Målt før rettelsen: indsæt to forskellige MokaHR-job i pipelinen, og du ender med **én linje** — ingen fejl, ingen advarsel, intet at bemærke; scannerens friskhedsfilter (`en-scanner.mjs`) læser samme nøgle, så så snart ét job fra den tenant var i scan-historikken, blev alle de andre kasseret som »allerede set«. `normalizeUrl` løfter nu en genkendt `#/job/{id}`- eller `#/jobs/{id}`-rute op i en ren sammenligningsnøgle i forespørgslen, før fragmentet droppes — MokaHR beholder sit etablerede navn `mokahr_job_id`, så tidligere skrevne nøgler forbliver sammenlignelige, mens enhver anden vært får et internt `_career_ops_fragment_job_id`. Det løftede id bliver **tilføjet, ikke sat**: en URL, der allerede bærer den parameter, bærer et andet opslags id, og at overskrive den ville lægge sammen præcis det, nøglen findes for at holde adskilt. Ethvert ikke-genkendt fragment kollapser stadig, hvilket er den halvdel, der allerede var korrekt.
**`accept-encoding` er nu fastlåst på hver kilde-forespørgsel.** Uspecificeret forhandler Node, hvad dens undici-build tilbyder; fra **Node ≥ 23** inkluderer det `zstd`, og amazon.jobs' zstd-svar kommer tilbage **afkortet ved præcis 1024 byte med status 200** — så fejlen læses som `Unterminated string in JSON at position 1024`, som et misdannet API snarere end en transportfejl. Hver forespørgsel sender nu `gzip, deflate, br`. På vores runtimes er dette **forebyggende, ikke en reparation**: Node 18/20/22 — CI-matrixen og serveren (22.22.1) — sender `gzip, deflate`, verificeret direkte frem for antaget, så intet blev afkortet her; det, der ændrer sig i dag, er, at `br` bliver tilbudt, hvilket de versioner ikke anmoder om som standard, men afkoder korrekt. Standarden anvendes **uden hensyn til store/små bogstaver**, hvilket er den del, der er værd at gennemgå: header-navne skelner ikke mellem store og små bogstaver, og `fetch` sammenføjer to enslydende felter til én kommasepareret værdi, så en kaldende part, der sender `Accept-Encoding: identity`, ellers ville have fået sendt `gzip, deflate, br, identity`.

### Noter
Headers forbliver et **almindeligt objekt** ved `fetchImpl`-grænsen, hvor forælderen giver `fetch` en `Headers`-instans. Det er en bevidst afvigelse, ikke en forglemmelse: web-ui's fetch-hjælpefunktioner tager en injicerbar `fetchImpl`, og 45 kilde-testpakker læser `opts.headers['User-Agent']` af det, de opfanger — en `Headers`-instans har ingen adgang med klammer, så den tro portering er af *adfærden* (tilsidesættelse uden hensyn til store/små bogstaver, kaldende part vinder) snarere end af implementeringen. Den kaldende parts objekt kopieres, aldrig muteres, da flere kilder deler én header-konstant. Forælderens ledsagende rettelse — at anvende en **standard-user-agent** uden hensyn til store/små bogstaver — er en **ikke-portering**: web-ui injicerer slet ingen standard-UA, hver kilde sætter den, dens board kræver, så der er ingen standard for en kaldende parts `User-Agent` at kollidere med.
To nye testpakker og tre nye tilfælde fastlåser parret: `tests/http-accept-encoding.test.mjs` (den præcise fastlåste værdi, hver af de fire fetch-hjælpefunktioner, og en tilsidesættelse fra kaldende part under tre stavemåder med store/små bogstaver — der bekræfter, at *én enkelt* `accept-encoding`-nøgle overlever, hvilket er, hvad sammenfletningsfejlen producerede to af) og `tests/parsers-pipeline-hash-route.test.mjs`, som arbejder med den brugersynlige sømlinje snarere end nøglen: to MokaHR-job skal begge nå pipelinen, en gen-indsættelse af det samme under fire stavemåder skal stadig kollapse til én linje, og et kosmetisk fragment må ikke blive til en identitet. Alle blev bekræftet fejlende først. Selve fastlåsningen bor i **transporten**, ikke kun i de hjælpere: 25 af de 92 kilder kalder den indsprøjtede `fetchImpl` direkte og passerer dem aldrig, så `makeTimeoutFetch` — den wrapper begge scannere faktisk indsprøjter — anvender den, og det er det ene gennemgangspunkt alle kilder deler. (Forælderen har ikke brug for en pendant: `providers/_http.mjs` er dens eneste transport.) En strukturel vagt fælder nu CI, hvis en ny kilde kalder `fetch` direkte i stedet for at bruge den transport, den fik rakt, og et adfærdstilfælde kører greenhouse — en af de 25 — gennem wrapperen og beviser, at headeren når ud på ledningen.
Ikke porteret, med begrundelsen: `liveness-browser.mjs` (genbrugsvagt for headed Playwright — web-ui kører ikke en headed browser), `check-jd-archive.mjs` (ingen web-ui-relæ), `test-all.mjs` (forælderens eget testrammeværk; forkets egen `vpFixtureEnv`-afvigelse er intakt), `web/` (forælderens eget frontend, spejles ikke), plugin-manifester og `SIGNATURES.md` (kun repository). `batch/batch-runner.sh` (og-tegn escaped før `sed`-interpolation) og `jd-skill-gap.mjs` (løser `cv.md` op mod dataroden) er **relæet skrivebeskyttet** — web-ui shell'er ud til forælderens egne kopier, så begge lander uden kodeændring her, og `jd-skill-gap`-ændringen gør `/api/jds/:id/skill-gap`-relæet mere korrekt: `cv.md` løses nu op fra dataroden i stedet for hvilken som helst mappe, processen blev startet fra. Forkets afvigelser genbekræftet efter pull'et: den kyrilliske `\p{L}`-rettelse i `providers/telegram-channel.mjs`, forkets egen `providers/telegram.mjs`, `hermes` i `web/src/lib/clis.ts`. Antallet af kilder er uændret på **92** (87 EN + 5 RU), så hjælpen er urørt. **3042 → 3066 test.**

## [1.234.0] — 2026-09-15

**Forælder-paritet — career-ops `main` @ `56cce8f` (VERSION stadig 1.32.0, 58 commits forbi 1.32.0). Én spejlet rettelse på tværs af 15 kilder, plus en vagt, så den ikke kan komme tilbage.**

### Rettet
**Ét enkelt fejlformet job-id tømmer ikke længere hele siden.** `encodeURIComponent` kaster `URIError` ved en enlig UTF-16-surrogat, og et JSON-payload kan bære én — `JSON.parse('"\\uD800"')` bevarer den. Femten kilder byggede hvert opslags URL ud fra et vært-styret id eller slug *inde i* parse-løkken, så ét dårligt opslag afbrød løkken og mistede alle job på den side; intet blev logget som en kilde-fejl, fordi undtagelsen rullede tilbage, før virksomhedens egen catch havde noget at rapportere. Forælderen rettede det i `providers/_safe-url.mjs` (`safeEncodeURIComponent` → `null` ved `URIError`, kaster alt andet videre) og lod hver løkkes id gå igennem den, så kun det ene opslag droppes ved `null`. Den hjælpefunktion er nu `server/lib/sources/_safe-url.mjs`, og samme ændring er anvendt på de tretten spejlede kilder — alibaba, arbeitsagentur, bamboohr, feishu-jobs, garena, jibeapply, manfred, meituan, mokahr, phenom, thehub, tkms, vdab — og på **to kilder, der kun findes i web-ui, og som den nye vagt afslørede ved sin første kørsel, jobstreet og trudvsem**, som havde samme løkkeform og samme latente kast. Omfanget er bevidst: kun API-hentede id'er går gennem hjælpefunktionen. Konfigurationsafledte værdier beholder deres strenge encode og fejler højlydt — garenas `office` (et `portals.yml`-segment) bliver på den kastende `urlSegment`, mens dens API-`id` får en null-returnerende `idUrlSegment`; csods `corpName` og 4dayweeks `SLUG_RE`-validerede slug er navngivet på en tilladelsesliste med begrundelsen.
**rheinmetalls titel-fallback kunne kaste en fejl ved en fejlformet href.** Når et kort ikke har nogen overskrift, genopbygges titlen fra URL-sluggen via `decodeURIComponent`, som kaster en fejl ved en ugyldig procent-sekvens i det skrabede `href` — samme mønster, hvor én dårlig række dræber hele siden, blot på afkodningssiden. Den afkoder nu defensivt og falder tilbage til den rå slug.
**Kilderegistret springer filer med `_`-præfiks over.** Auto-opdagelsen importerede hver `.mjs`-fil i `server/lib/sources/` undtagen sig selv og advarede om alt uden `export const meta` — så hjælpefunktionen udløste en advarsel ved opstart. Forælderens konvention er nu overtaget: et indledende underscore betyder *ikke en kilde*; sådanne filer bliver hverken importeret eller udløser en advarsel. En ikke-deklareret fil uden `meta` udløser stadig en advarsel, og opdagelsestesten bekræfter nu begge halvdele (bekræftet rød mod det gamle filter). Sitets build-script `sync-assets.mjs` tæller adapterfiler på disken efter samme regel, så hjælperen ikke længere udløser dets vagt mod stille frafald.

### Noter
`tests/sources-url-encoding-surrogate.test.mjs` spejler forælderens testpakke i tre dele: hjælpefunktionens kontrakt (et parret surrogat — en almindelig emoji — encoder stadig; et kast fra værdiens egen `toString` er den kaldendes egen fejl og forplanter sig), pr.-kilde-adfærd for alle femten (ét dårligt id + ét rent id → intet kast, det rene bevares, det dårlige droppes, hver drevet gennem kildens eksporterede rene parser eller, for thehub, dens fetcher med en falsk transport), og en **kilde-niveau-vagt**: hver konverteret fil skal både importere hjælpefunktionen *og* kalde den, ingen fil må referere til den uden at importere den, og ingen ikke-gennemgået fil må bygge en job-URL med et bart `encodeURIComponent` — en ny kilde med den gamle form fejler i CI med linjenummeret på den pågældende linje. Alle femten pr.-kilde-tilfælde blev bekræftet fejlende før porteringen.
Alt andet i forælderens delta er ikke porteret, med begrundelsen: `batch/batch-runner.sh --cli` er et rent CLI-flag (web-ui shell'er ikke ud til batch-runneren); `invite-match.mjs` finder nu `set-status.mjs` fra kodens rod — web-ui relayer den ikke; `company-funded.mjs` fik den samme safe-url-ændring internt, og dens `--json`-outputform, som `/api/company-funded` parser, er uændret; `test-all.mjs` flyttede kun et timeout-budget (forkets egen `vpFixtureEnv`-afvigelse er intakt); `web/` er forælderens eget frontend og spejles ikke; `AGENTS.md`/`SIGNATURES.md` er kun repository. Forkets afvigelser genbekræftet efter pull'et: den kyrilliske `\p{L}`-rettelse i `providers/telegram-channel.mjs`, forkets egen `providers/telegram.mjs`, `hermes` i `web/src/lib/clis.ts`. Antallet af kilder er uændret på **92** (87 EN + 5 RU), så hjælpen er urørt. **3022 → 3042 test.**
