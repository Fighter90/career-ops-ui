...
**v1.237.0 पर एक बाहरी QA पास से पैच।** Jobstreet सुधार पहली बार बाहर से पुष्टि किया गया: AU, NZ, HK और MY होस्ट पर पुराना रूप `/id/job/<id>` एक स्वच्छ 404 लौटाता है और नया रूप `/job/<id>` एक असली, Cloudflare-सुरक्षित रूट तक पहुँचता है; `id.jobstreet.com` पर उल्टा सच है। खोई हुई लिंकें असली थीं। दो दोष मिले, दोनों उसी में जो v1.237.0 ने ख़ुद जोड़ा था।

### ठीक किया गया

- **किताब के लिंक का लेबल स्थानीयकृत था, पर उसका लक्ष्य नहीं।** सभी 17 SPA लोकेल और सभी 17 cvstart.org लोकेल, *Agentic Coding Design Patterns* footer लिंक को अंग्रेज़ी संस्करण की ओर इंगित करते थे, जबकि इकलौता अनूदित लेबल — रूसी, «Паттерны агентного кодинга» — एक रूसी किताब का वादा करता था। एक नया `data-i18n-href` अप्लायर (`data-i18n-title` जैसे ही रूप में, केवल पूर्ण `https://` मानों तक सीमित) और एक `footer.patternsUrl` कुंजी, `ru` को रूसी संस्करण की ओर और बाक़ी हर लोकेल को अंग्रेज़ी संस्करण की ओर भेजते हैं। रिग्रेशन टेस्ट इस नियम को दोनों दिशाओं में जाँचता है: एक अनूदित शीर्षक का मतलब एक स्थानीयकृत URL भी है।
- **SEEK होस्ट बदल रहा है, और ट्रांसपोर्ट रीडायरेक्ट को अस्वीकार करता है।** `www.seek.com.au` → `au.seek.com` और `www.seek.co.nz` → `nz.seek.com`: विवरण पृष्ठ पहले से ही 301 देते हैं, और दोनों होस्ट-जोड़े आज v5 सर्च API को 200 पर परोसते हैं। चूँकि `http-json.mjs`, `redirect:'error'` सेट करता है (एक SSRF बचाव जिसे बनाए रखना उचित है), जिस दिन API पथ रीडायरेक्ट करना शुरू करेगा, पुराने होस्ट पर एक AU/NZ प्रविष्टि विफल हो जाएगी — और केवल `www.` होस्ट allowlist में होने से कोई भी माइग्रेट नहीं कर पाता था। `au.seek.com` और `nz.seek.com` अब `ALLOWED_JOBSTREET_HOSTS` में हैं; पुराने होस्ट बने हुए हैं, इसलिए मौजूदा कॉन्फ़िग और पहले से बनी लिंकें काम करती रहती हैं। नक़लची डोमेन (`au.seek.com.evil.com`, `au-seek.com`) अब भी अस्वीकृत हैं।
- **Wiki:** Home डॉक्यूमेंटेशन मैप में *Agentic Practices* वाली पंक्ति बारह में से इकलौती थी जो `[[…|…]]` के भीतर `|` को एस्केप नहीं करती थी, इसलिए GitHub ने सेल काट दी — मृत लिंक, विवरण छिपा। एक ही अक्षर।

### टिप्पणियाँ

- गिनती अपरिवर्तित: **94** स्रोत (89 EN + 5 RU), 89 अडैप्टर। टेस्ट **3201 → 3210** (+5 footer-लिंक लोकेल, +4 SEEK होस्ट)।
- बाहर से बंद न हो सकने वाला, दावा किए जाने के बजाय दर्ज किया गया: Oracle Cloud (Akamai ने दोनों REST जाँचों को रोक दिया), Personio पिन और `appendWorkType` (एक `portals.yml` प्रविष्टि चाहिए, और UI में किसी एक को हटाने का कोई तरीक़ा नहीं है), और liveness स्ट्रिंग मामले (कोई नियंत्रित करने योग्य पृष्ठ नहीं; 40 लाइव जाँचों में से 12 लाइव मिलीं, 28 अनिश्चित, **0 ग़लत expiry**)।
- सहायता ×17 अब `AU-Main` और `NZ-Main` के लिए दोनों होस्ट रूप सूचीबद्ध करती है। शीर्षक-संरचना 32 H2 / 122 H3 पर अपरिवर्तित है।

## [1.237.0] — 2026-09-21

**मूल परियोजना समानता — career-ops की `main` शाखा @ `93c4302b` (VERSION 1.33.0, `career-ops-hq/main` से खींचे गए 75 कमिट)। इस बार कोई नया स्रोत नहीं: चार मिरर किए गए सुधार, और इनमें से दो यहाँ चुपचाप पोस्टिंग खो रहे थे।**

### ठीक किया गया
**Jobstreet / SEEK — हर ऑस्ट्रेलियाई, न्यूज़ीलैंड, सिंगापुर, मलेशियाई और हांगकांग पोस्टिंग एक मृत पृष्ठ से जुड़ी थी।** `parseJobstreetItem` विवरण URL को बिना शर्त `${baseUrl}/id/job/${id}` के रूप में बनाता था। `/id/` खंड कोई पथ नहीं है — यह **इंडोनेशियाई स्थानीय उपसर्ग** है, और यह केवल इंडोनेशियाई होस्ट पर मौजूद होता है। हर दूसरा SEEK-प्लेटफ़ॉर्म होस्ट `/id/job/<id>` पर **404** देता है। web-ui पहले ही `www.seek.com.au`, `www.seek.co.nz`, `sg.jobstreet.com`, `my.jobstreet.com` और `hk.jobsdb.com` को `ALLOWED_JOBSTREET_HOSTS` में जोड़ चुका था, इसलिए स्कैन ख़ुशी-ख़ुशी वे पोस्टिंग लौटाता रहा — हर एक 404 की ओर इशारा करते हुए। पथ अब होस्ट पर कुंजीबद्ध है: `ID_LOCALE_HOSTS` (`id.jobstreet.com`, `www.jobstreet.co.id`, `jobstreet.co.id`) `/id/job/` बनाए रखता है, बाक़ी सबको `/job/` मिलता है, और कोई अपार्स-योग्य origin फेंकने के बजाय सामान्य पथ पर गिर जाता है। हमारे अपने दो टेस्ट ने **यह बग एनकोड कर दिया था** — उन्होंने `hk.jobsdb.com/id/job/…` और `www.jobstreet.com/id/job/…` का दावा किया था — और उन्हें ठीक किया गया।
**Oracle Cloud — सूची के बीच में आया एक छोटा पेज चाल ख़त्म कर देता था और बोर्ड की पूँछ गिरा देता था।** रुकने की शर्त थी `listLen === 0 || listLen < PAGE_SIZE`, लेकिन ORC सीमा से कम पंक्तियाँ *सूची के बीच में* भी देता है क्योंकि कुछ पंक्तियाँ सर्वर-साइड फ़िल्टर हो जाती हैं। American Express पर अपस्ट्रीम नापा गया: `TotalJobsCount` **454**, पेज **200 / 199 / 54** के — 199 को अंत समझ लिया गया और आख़िरी **54 पोस्टिंग, बोर्ड का 12%**, कभी नहीं लाई गईं। अब कोई ख़ाली पेज ही हमेशा चाल ख़त्म करता है; जब टेनेंट कोई कुल संख्या बताता है, तो पेजिंग तब तक जारी रहती है जब तक `offset + PAGE_SIZE >= total`; केवल वही टेनेंट जिसके पास जाँचने के लिए कोई कुल संख्या नहीं है, किसी छोटे पेज पर ख़त्म होता है। यह उस व्यापक परंपरा से मेल खाता है कि कोई API "अनुरोधित संख्या से कम परिणाम लौटा सकता है … भले ही वह संग्रह के अंत में न हो" (Google AIP-158)। `hasMore` को जानबूझकर अनदेखा किया जाना जारी है — कुछ टेनेंट 7000-नौकरी वाले बोर्ड के हर पेज पर `hasMore:false` लौटाते हैं। छोटे पेज अब चाल ख़त्म न करने के साथ, `MAX_PAGES` अब किसी टेनेंट द्वारा बताई गई झूठी कुल संख्या के ख़िलाफ़ इकलौता बचाव है, इसलिए इसे अपने ख़ुद के टेस्ट से ढका गया है।
**Liveness — "This role is closed" को पहचाना नहीं जाता था, और एक ग़लत expiry स्थायी होती है।** पैटर्न था `/this job (listing )?is closed/i`, जो उतने ही आम `role` और `position` वाक्यांशों को छोड़ देता है; एक बोर्ड पर अपस्ट्रीम नापा गया कि "role" का उपयोग करने वाली **111 में से 111** अनिश्चित पोस्टिंग छूट गईं। इसे बढ़ाकर `/this (?:job|role|position)(?: listing)? is closed\b(?!-)/i` किया गया। अंत का `\b(?!-)` एक यौगिक-विशेषण गार्ड है: अकेला `\b` `closedown` को अस्वीकार कर देता है लेकिन फिर भी *"This role is closed-loop control of the platform"* जैसे असली गद्य से मेल खाता रहता है, और यही lookahead है जो इसे रोकता है।
**Liveness — `job expired` अब एक नरम स्तर में रहता है जो किसी दृश्यमान Apply बटन को मात नहीं दे सकता।** `HARD_EXPIRED_PATTERNS` की जाँच apply-control टेस्ट *से पहले* होती है, इसलिए वहाँ रखी गई कोई भी चीज़ किसी जीवित पोस्टिंग पर जीत जाती है। `/\bjob expired\b/i` चार असली जीवित-पृष्ठ रूपों पर ग़लती से चल पड़ता है — एक "Similar jobs" कैरोसेल प्रविष्टि, एक "Hide job expired" फ़िल्टर चिप, एक फ़ुटर FAQ, और ऊपर वाला closed-loop गद्य — क्योंकि classifier को पूरे पृष्ठ का पाठ मिलता है। यह अब `SOFT_EXPIRED_PATTERNS` में रहता है, जिसकी जाँच apply control के बाद और लिस्टिंग-पृष्ठ heuristic से पहले होती है, और यह `expired_body_soft` लौटाता है; बिना किसी apply control वाला नंगा "JOB EXPIRED" बैनर अब भी expire होता है। यह दिशा इसलिए मायने रखती है क्योंकि एक ग़लत `expired` scan history में `skipped_expired` के रूप में लिखा जाता है और फिर **किसी असली नौकरी को हर बाद के स्कैन से हमेशा के लिए dedup-फ़िल्टर कर देता है**, जब तक कि `scan_history.recheck_after_days` सेट न हो।

### जोड़ा गया
**Personio — एक स्पष्ट `personio: <slug>` टेनेंट को पिन करता है।** कई कंपनियाँ अपने Personio बोर्ड को अपने ब्रांडेड करियर पृष्ठ पर एक **iframe** के रूप में एम्बेड करती हैं, इसलिए `careers_url` कंपनी के अपने डोमेन की ओर इशारा करता है जबकि फ़ीड `<slug>.jobs.personio.de` पर रहती है; बिना किसी पिन के वे बोर्ड कुछ भी हल नहीं होते। स्लग को एक एंकर्ड allowlist (`/^[a-z0-9][a-z0-9-]{0,62}$/i`) से चरित्र-सेट तक सीमित किया गया है और परिणामी URL अब भी मौजूदा `new URL()` पार्स, HTTPS जाँच और `PERSONIO_HOST_RE` होस्ट-गेट से होकर गुज़रता है — पिन कभी किसी दूसरे होस्ट तक पहुँचने का रास्ता नहीं बनता। कोई स्लग जिसमें `.`, `/`, `@`, `:`, `..`, ख़ाली स्थान, आगे लगा `-` हो, या जो 63-अक्षर की DNS लेबल सीमा से आगे निकल जाए, आज के `api`/`careers_url` व्यवहार पर गिर जाता है। मूल परियोजना के विपरीत, web-ui का होस्ट रीजेक्स केस-संवेदनशील है, इसलिए पिन को `new URL()` से सामान्यीकृत किया जाता है — `AcmeGroup`, `acmegroup.jobs.personio.de` पर हल होता है, न कि ऐसा कोई होस्ट बनाता है जिसे उसकी अपनी allowlist बाद में अस्वीकार कर दे।
**Jobstreet — वैकल्पिक `appendWorkType`** शीर्षक के आगे लिस्टिंग के work types जोड़ देता है, जैसे `Strategy Consultant [Part time]`, ताकि शीर्षक फ़िल्टर और triage स्कैन के समय रोज़गार-प्रकार देख सकें। डिफ़ॉल्ट रूप से बंद; कोई ग़ायब या ख़ाली `workTypes` कुछ नहीं करता है।

### टिप्पणियाँ
स्रोत और अडैप्टर की गिनती अपरिवर्तित है: **94** स्रोत (89 EN + 5 RU), 89 EN अडैप्टर। यह रिलीज़ केवल परीक्षण गिनती बदलती है, **3164 → 3201** (+13 jobstreet, +7 Oracle Cloud, +9 liveness, +8 Personio)।
मूल परियोजना ने **`providers/telegram.mjs` हटा दिया**, और `telegram-channel` पर समेकित हो गई। web-ui दोनों को उपयोगकर्ता-सामने वाले स्रोत के रूप में पंजीकृत करता है, इसलिए यह हटाना **नहीं** अपनाया गया: इससे रजिस्ट्री 94 से 93 पर गिर जाती और जिसने भी `telegram` कॉन्फ़िगर किया है वह चुपचाप टूट जाता। इसे तभी दोबारा देखा जाए जब यह एक जान-बूझकर, घोषित हटाना हो।
`jd-skill-gap.mjs` को एक बिना-संकुचित "what we are looking for" शीर्षक-रूप और एक बोल्ड-मार्कर स्ट्रिप मिली। web-ui उस स्क्रिप्ट को मिरर करने के बजाय **रिले** करता है, इसलिए यह सुधार तैनात मूल परियोजना के साथ ख़ुद-ब-ख़ुद आ जाता है और यहाँ कोई बदलाव नहीं चाहिए।
पोर्ट नहीं किया गया, कारण सहित: `lib/latex-escape.mjs`, `browser-extract.mjs`, `openai-eval.mjs`, `update-system.mjs` और scaffolder (web-ui न तो इन्हें मिरर करता है न ही इनमें shell करता है); Go डैशबोर्ड TUI और `web/` कॉन्फ़िग फ़ॉर्म (अलग सतहें); `doctor.mjs` और `analyze-patterns.mjs` (केवल-पठन रिले जिनका fail-soft बदलाव सोख लेता है)।
`providers/telegram-channel.mjs` में फ़ोर्क का सिरिलिक `\p{L}` विचलन अपस्ट्रीम मर्ज के बाद बरक़रार रहा और एक प्री-मर्ज प्रति के विरुद्ध बाइट-दर-बाइट सत्यापित किया गया।
