...
**변경됨 (낮음, 스캐너) — Getro 패리티 포트 (상위 #2640): 급여, 전체 위치, `work_mode` 원격 감지.**

### 변경됨
- 상위 `providers/getro.mjs` 확장(#2640)의 안전한 절반을 `server/lib/sources/getro.mjs`로 포팅했습니다. 이제 Getro 채용은 **급여** 표시 문자열(`compensation_amount_min/max_cents` + 통화에서, 연봉만 — 클라이언트 급여 필터가 다시 파싱하는 remotli/lever 문자열 규약을 따름)을 담고, **위치**는 `locations` **전체**를 결합하며(이전엔 첫 번째만), `work_mode: 'remote'`가 기존 `remote` 플래그 / 위치 텍스트와 함께 원격 신호로 인정됩니다. web-ui가 이미 가져오는 API 응답의 순수 데이터 매핑 — 새 호스트 없음, SSRF 표면 변경 없음.

### 참고
- 의도적으로 포팅하지 않음: 상위의 `collection_id` **자동 해석**(보드 자체의 `careers_url` — 임의 호스트 — 를 가져오므로 web-ui의 `safeGet` SSRF 경계를 통해 별도 변경으로 처리해야 함)과 상향된 1500페이지 하드 캡(web-ui는 브라우저 트리거 스캔을 위해 200페이지 / 4000건 캡 유지). 이 동기화의 나머지 상위 커밋 11개는 web-ui 미러가 없습니다. 테스트: `tests/sources-getro.test.mjs` (+5). 전체: **2470** (+5).
