...
**Змінено (НИЗЬКО, сканер) — порт паритету Getro (батько #2640): зарплата, усі локації та визначення віддаленої роботи за `work_mode`.**

### Змінено
- Перенесено безпечну частину розширення батьківського `providers/getro.mjs` (#2640) до `server/lib/sources/getro.mjs`. Вакансія Getro тепер несе рядок **зарплати** (з `compensation_amount_min/max_cents` + валюта, лише річна — за рядковою угодою remotli/lever, яку клієнтський фільтр зарплати розбирає заново), її **локація** об'єднує **всі** `locations` (раніше лише першу), а `work_mode: 'remote'` враховується як сигнал віддаленої роботи поряд з наявним прапорцем `remote` / текстом локації. Чисте відображення даних з відповіді API, яку web-ui і так запитує — без нового хоста, без зміни SSRF-поверхні.

### Примітки
- Навмисно НЕ перенесено: **авторозв'язання** `collection_id` батька (воно запитує власний `careers_url` дошки — довільний хост — тож це окрема зміна через SSRF-межу `safeGet` web-ui) та піднятий ліміт у 1500 сторінок (web-ui зберігає ліміт 200 сторінок / 4000 вакансій для сканів із браузера). Решта 11 батьківських комітів цієї синхронізації не мають дзеркала в web-ui. Тести: `tests/sources-getro.test.mjs` (+5). Набір: **2470** (+5).
