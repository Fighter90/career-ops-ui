...
### Corrigido
- **Jobstreet / SEEK — toda vaga australiana, neozelandesa, cingapuriana, malaia e de Hong Kong apontava para uma página morta.** `parseJobstreetItem` construía a URL de detalhe como `${baseUrl}/id/job/${id}` incondicionalmente. O segmento `/id/` não é um caminho — é o **prefixo de localidade indonésio**, e só existe nos hosts indonésios. Todo outro host da plataforma SEEK responde **404** em `/id/job/<id>`. O web-ui já havia adicionado `www.seek.com.au`, `www.seek.co.nz`, `sg.jobstreet.com`, `my.jobstreet.com` e `hk.jobsdb.com` a `ALLOWED_JOBSTREET_HOSTS`, então a varredura retornava essas vagas tranquilamente — cada uma apontando para um 404. O caminho agora é definido pelo host: `ID_LOCALE_HOSTS` (`id.jobstreet.com`, `www.jobstreet.co.id`, `jobstreet.co.id`) mantém `/id/job/`, todo o resto recebe `/job/`, e uma origem não analisável cai para o caminho comum em vez de lançar exceção. Dois de nossos próprios testes tinham **codificado o bug** — eles afirmavam `hk.jobsdb.com/id/job/…` e `www.jobstreet.com/id/job/…` — e foram corrigidos.
- **Oracle Cloud — uma página curta no meio da lista encerrava a varredura e derrubava a cauda do board.** A condição de parada era `listLen === 0 || listLen < PAGE_SIZE`, mas o ORC serve menos linhas que o limite *no meio de uma lista* porque algumas linhas são filtradas no servidor. Medido a montante na American Express: `TotalJobsCount` **454**, páginas de **200 / 199 / 54** — o 199 era lido como o fim, e as últimas **54 vagas, 12% do board**, nunca eram buscadas. Agora uma página vazia sempre encerra a varredura; quando o tenant informa um total, a paginação continua até `offset + PAGE_SIZE >= total`; só um tenant sem total contra o qual conferir encerra numa página curta. Isso segue a convenção mais ampla de que uma API «pode retornar menos resultados do que o número solicitado … mesmo que não esteja no fim da coleção» (Google AIP-158). `hasMore` permanece deliberadamente ignorado — alguns tenants retornam `hasMore:false` em toda página de um board de 7000 vagas. Com páginas curtas não encerrando mais a varredura, `MAX_PAGES` agora é a única salvaguarda contra um tenant informando um total falso, por isso está coberto por seu próprio teste.
- **Liveness — "This role is closed" não era reconhecido, e uma expiração falsa é permanente.** O padrão era `/this job (listing )?is closed/i`, que não captura as formulações igualmente comuns com `role` e `position`; em um board a montante, **111 de 111** vagas incertas usavam "role". Ampliado para `/this (?:job|role|position)(?: listing)? is closed\b(?!-)/i`. O `\b(?!-)` final é uma guarda contra adjetivo composto: `\b` sozinho rejeita `closedown`, mas ainda casa com uma prosa real como *"This role is closed-loop control of the platform"*, e é o lookahead que impede isso.
- **Liveness — `job expired` agora fica em um nível brando que não pode superar um botão Apply visível.** `HARD_EXPIRED_PATTERNS` é verificado *antes* do teste de controle de candidatura, então qualquer coisa colocada ali vence sobre uma vaga ativa. `/\bjob expired\b/i` disparava falsamente em quatro formatos reais de página ativa — um item de carrossel "Similar jobs", um chip de filtro "Hide job expired", um FAQ de rodapé e a prosa de closed-loop acima — porque o classificador recebe o texto da página inteira. Agora ele vive em `SOFT_EXPIRED_PATTERNS`, verificado depois do controle de candidatura e antes da heurística de página de listagem, retornando `expired_body_soft`; um banner isolado "JOB EXPIRED" sem controle de candidatura ainda expira. Essa direção importa porque um `expired` falso é gravado no histórico de varredura como `skipped_expired` e então **filtra por deduplicação uma vaga real para fora de toda varredura posterior, indefinidamente**, a menos que `scan_history.recheck_after_days` esteja definido.

### Adicionado
- **Personio — um `personio: <slug>` explícito fixa o tenant.** Muitas empresas incorporam seu board do Personio como um **iframe** em uma página de carreiras com marca própria, então `careers_url` aponta para o próprio domínio da empresa enquanto o feed vive em `<slug>.jobs.personio.de`; sem uma fixação, esses boards não resolvem a nada. O slug tem o conjunto de caracteres restrito por uma allowlist ancorada (`/^[a-z0-9][a-z0-9-]{0,62}$/i`), e a URL resultante ainda passa pela análise existente de `new URL()`, pela checagem de HTTPS e pelo portão de host `PERSONIO_HOST_RE` — a fixação nunca é uma forma de alcançar outro host. Um slug contendo `.`, `/`, `@`, `:`, `..`, espaço em branco, um `-` inicial, ou que exceda o limite de 63 caracteres de um rótulo DNS, cai para o comportamento atual de `api`/`careers_url`. Diferente do pai, a regex de host do web-ui diferencia maiúsculas de minúsculas, então a fixação é normalizada por `new URL()` — `AcmeGroup` resolve para `acmegroup.jobs.personio.de` em vez de construir um host que sua própria allowlist rejeitaria depois.
- **Jobstreet — `appendWorkType`, opcional (opt-in)** adiciona ao título os tipos de trabalho da vaga como sufixo, por exemplo `Strategy Consultant [Part time]`, para que filtros de título e a triagem enxerguem o tipo de contratação no momento da varredura. Desativado por padrão; um `workTypes` ausente ou vazio é uma operação nula.

### Notas
- As contagens de fontes e adaptadores permanecem inalteradas: **94** fontes (89 EN + 5 RU), 89 adaptadores EN. Esta versão move apenas a contagem de testes, **3164 → 3201** (+13 jobstreet, +7 Oracle Cloud, +9 liveness, +8 Personio).
- O pai **excluiu `providers/telegram.mjs`**, consolidando em `telegram-channel`. O web-ui registra ambos como fontes voltadas ao usuário, então a exclusão **não** é seguida: ela derrubaria o registro de 94 para 93 e quebraria silenciosamente quem tiver `telegram` configurado. Revisitar somente como uma remoção deliberada e anunciada.
- `jd-skill-gap.mjs` ganhou uma forma de cabeçalho "what we are looking for" sem contração e uma remoção de marcador em negrito. O web-ui **repassa** esse script em vez de espelhá-lo, então a correção chega junto com o pai implantado e não exige mudança nenhuma aqui.
- Não portado, com os motivos: `lib/latex-escape.mjs`, `browser-extract.mjs`, `openai-eval.mjs`, `update-system.mjs` e o scaffolder (o web-ui não os espelha nem os invoca via shell); a TUI do dashboard em Go e o formulário de configuração de `web/` (superfícies separadas); `doctor.mjs` e `analyze-patterns.mjs` (repasses somente leitura cuja falha branda absorve a mudança).
- A divergência cirílica `\p{L}` do fork em `providers/telegram-channel.mjs` sobreviveu intacta à mesclagem do upstream e foi verificada byte a byte contra uma cópia anterior à mesclagem.

## [1.236.0] — 2026-09-18

**Paridade com o pai — career-ops `main` @ `6a9c84c` (VERSION 1.33.0, 128 commits). Duas novas fontes, e três correções espelhadas — duas das quais estavam perdendo vagas aqui, silenciosamente.**

### Adicionado
**Duas fontes de varredura, 92 → 94 (89 EN + 5 RU).** **Python.org Jobs** (`pythonorg`) lê o feed público do board; a atribuição de empregador é obrigatória, então uma linha cujo título não nomeia nenhum empregador é descartada em vez de ser preenchida retroativamente com o nome da entrada do portal. **Generalist World** (`generalist-world`) faz scraping dos cards do board e **falha ruidosamente diante de um markup que não consegue analisar** — um formato de card que mudou lança exceção em vez de ser lido como um board vazio, que é o modo de falha que torna uma fonte via scraping não confiável. Ambas aparecem no filtro **Fonte** de `#/scan`, ambas têm o host fixado (regex ancorada, somente HTTPS), e ambas são conduzidas de ponta a ponta por suítes com transporte falso (43 e 36 casos).

### Corrigido
**Uma vaga do Workday aberta em várias cidades era descartada por um filtro de localização.** O endpoint de listagem do Workday responde a uma vaga multi-localização com uma **contagem**, onde toda outra vaga carrega um lugar: `"53 Locations"`. Isso não é uma localização, e `buildLocationFilter` casa por substring sem diferenciar maiúsculas de minúsculas — então uma vaga aberta em Austin *e* em outras 52 cidades não casava com nenhuma entrada `allow: [austin]` e era descartada, enquanto a mesma vaga listada isoladamente passava. Medido a montante, em três tenants: **53 de 291 vagas** eram placeholders, então este é o caso comum, não um caso de borda. Os lugares reais da vaga existem só no documento de **detalhe** do CXS, então cada placeholder agora custa um GET extra contra ele, limitado a **200** por entrada — além do limite, o placeholder é deixado visível em vez de ser forjado, porque uma vaga não resolvida nunca deve parecer resolvida. Um detalhe que falha ou que não traz lugar algum deixa a vaga exatamente como a listagem a retornou: um `"53 Locations"` visível é honesto sobre o que não pôde ser resolvido, enquanto uma localização em branco passaria silenciosamente por todo filtro. O mesmo GET também traz `startDate`, então uma vaga enriquecida é datada **com exatidão**, em vez de pela prosa relativa do endpoint de listagem (`"Posted 30+ Days Ago"` → `undefined`); tudo o que não for um `YYYY-MM-DD` puro é deixado como está, em vez de passar por `Date.parse`, cujo fallback não-ISO é definido pela implementação.
**Um nível declarado não colapsa mais duas requisições em uma única repostagem.** `"Insurance Specialist II"` é a própria declaração do empregador de que esta requisição não é irmã da outra — uma faixa salarial, escopo e número de requisição diferentes sob um mesmo título-base. O tokenizador não conseguia enxergar isso: `w.length > 3` descarta todo numeral romano até VIII e todo dígito isolado, então `"Insurance Specialist"` e `"Insurance Specialist II"` eram tokenizados de forma idêntica, pontuavam um índice de Jaccard perfeito, e o detector de repostagem sinalizava a segunda como uma repostagem da primeira. O `role-matcher.mjs` do web-ui não tinha tratamento de nível **nenhum**, então isto traz o mecanismo inteiro do pai: níveis são extraídos com romano e arábico dobrados em um único número (`"Nurse II"` e `"Nurse 2"` são a mesma declaração — e *esse* par diferia erroneamente antes), dois níveis declarados precisam concordar, e um nível presente em apenas um dos lados é tratado como uma reescrita solta, **a menos que** cada título também carregue uma palavra que falte ao outro, o que são duas discordâncias, e não um enfeite.
**Um board do Avature fixado em um filtro percorria o board global inteiro mesmo assim.** A UI de busca do Avature gera parâmetros de faceta opacos (`?42386=[812132]` é um filtro de país), e um operador que fixa um board restrito em `portals.yml` espera que a varredura percorra *esse*. `resolveSearch` reconstruía a URL como `${origin}${path}` e descartava a query string por completo — então toda requisição, incluindo a primeira, percorria o board inteiro do tenant: mais páginas, mais requisições, e vagas que o operador havia deliberadamente filtrado. As facetas agora acompanham toda requisição paginada, com nosso próprio `jobOffset` aplicado por último, de modo que uma faceta perdida de mesmo nome jamais possa sequestrar a paginação.

### Segurança
**`devalue` 5.8.1 → 5.9.2** (transitivamente via `astro`), fechando o único alerta aberto do Dependabot — uma negação de serviço diante de entrada malformada. Dependência exclusiva do site; nenhum código de runtime mudou.

### Notas
**Não portado, cada um com o motivo.** O pai deu ao **lever** um timeout de 30s e ao **manfred** um de 25s, contra seu padrão compartilhado de 10s — um **não portado**: o transporte de varredura do web-ui já permite **60s** (`DEFAULT_SCAN_TIMEOUT_MS`), então ambos os boards já têm mais margem aqui do que a correção concede lá. O prefixo `careers-` duplicado do **iCIMS** também não pode ocorrer aqui: o web-ui obtém a origem do tenant diretamente de `api`/`careers_url` e nunca constrói um host a partir de um nome de tenant; seu novo percurso de host alternativo é um recurso para a configuração mais permissiva do pai, não a correção de um defeito. **deutschebahn** e **manfred** ganharam nova tentativa em abortos transitórios, o que o teto de 60s do web-ui torna bem menos urgente — anotado em vez de portado, para que um futuro timeout de percurso longo já tenha uma primeira coisa conhecida a tentar. Todo o resto no delta de 128 commits é CLI, verificador, dashboard ou `web/` (o próprio frontend do pai): `liveness-browser`, `check-jd-archive`, `verify-ats`/`verify-portals`/`verify-cv-facts`, `update-system`, `set-status`, `audit-portals`, `discover-ats`, `scaffolder`, `test-all`, manifestos de plugin.
Divergências do fork reverificadas após o pull: a correção cirílica de `\p{L}` em `providers/telegram-channel.mjs`, o `providers/telegram.mjs` próprio do fork, `hermes` em `web/src/lib/clis.ts`. O `career-ops-hq/main` upstream está totalmente mesclado (0 commits faltando). **3066 → 3164 testes**; a ajuda agora declara **94** adaptadores (**89 EN + 5 RU**) em todos os 17 pacotes, e a estrutura de cabeçalhos permanece inalterada em 32 H2 / 122 H3.

## [1.235.0] — 2026-09-16

**Paridade com o pai — career-ops `main` @ `68e6b94` (VERSION 1.33.0, 42 commits). Duas correções espelhadas; uma delas estava perdendo vagas do MokaHR aqui, silenciosamente.**

### Corrigido
**Uma segunda vaga do mesmo board do MokaHR desaparecia em vez de ser adicionada.** `url-key.mjs` reduz a URL de uma vaga a uma chave de comparação, e descartava o fragmento incondicionalmente — razoável, já que quase todo fragmento é só apresentação (`#apply`, `#section-2`). O MokaHR é a exceção em nosso próprio registro: suas vagas são `…/social-recruitment/{tenant}/{id}#/job/{n}`, onde toda vaga de um tenant compartilha o mesmo caminho e o id da vaga existe **somente** no fragmento. Então todas normalizavam para uma única chave. Medido antes da correção: colar duas vagas diferentes do MokaHR no pipeline resultava em **uma única linha** — sem erro, sem aviso, nada perceptível; o filtro de novidade do scanner (`en-scanner.mjs`) lê essa mesma chave, então, uma vez que qualquer vaga daquele tenant estivesse no histórico de scan, todas as demais eram descartadas como "já vista". `normalizeUrl` agora promove uma rota reconhecida `#/job/{id}` ou `#/jobs/{id}` a uma chave de consulta só para comparação antes de o fragmento ser descartado — o MokaHR mantém seu nome já estabelecido `mokahr_job_id`, para que chaves gravadas anteriormente continuem comparáveis; qualquer outro host recebe um `_career_ops_fragment_job_id` interno. O id promovido é **anexado, não definido**: uma URL que já carregue esse parâmetro contém o id de uma vaga diferente, e sobrescrevê-lo fundiria exatamente as duas coisas que a chave existe para manter separadas. Todo fragmento não reconhecido ainda colapsa, que é a metade que já estava certa.
**`accept-encoding` agora é fixado em toda requisição a uma fonte.** Sem definição, o Node negocia o que sua build do undici oferecer; a partir do **Node ≥ 23** isso inclui `zstd`, e a resposta zstd do amazon.jobs volta **truncada em exatamente 1024 bytes com status 200** — de modo que a falha aparece como `Unterminated string in JSON at position 1024`, como se fosse uma API malformada, e não um bug de transporte. Toda requisição agora envia `gzip, deflate, br`. Em nossos runtimes isso é **preventivo, não um reparo**: Node 18/20/22 — a matriz de CI e o servidor (22.22.1) — enviam `gzip, deflate`, verificado diretamente, não presumido, então nada foi truncado aqui; o que muda hoje é que `br` passa a ser oferecido, o que essas versões não solicitam por padrão, mas decodificam corretamente. O padrão é aplicado **sem diferenciar maiúsculas de minúsculas**, que é a parte que vale a pena revisar: nomes de cabeçalho não diferenciam maiúsculas de minúsculas, e `fetch` une duas entradas de mesmo nome em um único valor separado por vírgula, de modo que um chamador passando `Accept-Encoding: identity` teria recebido, de outra forma, `gzip, deflate, br, identity`.

### Notas
Os cabeçalhos permanecem um **objeto simples** na fronteira do `fetchImpl`, onde o pai entrega ao `fetch` uma instância de `Headers`. Essa é uma divergência deliberada, não um descuido: os helpers de fetch do web-ui recebem um `fetchImpl` injetável, e 45 suítes de fontes leem `opts.headers['User-Agent']` a partir do que capturam — uma instância de `Headers` não tem acesso por colchetes, então a portagem fiel é a do *comportamento* (sobrescrita sem diferenciar maiúsculas de minúsculas, o chamador vence), e não da implementação. O objeto do chamador é copiado, nunca alterado, já que várias fontes compartilham uma mesma constante de cabeçalho. A correção complementar do pai — aplicar um **user-agent padrão** sem diferenciar maiúsculas de minúsculas — é um **não portado**: o web-ui não injeta nenhum UA padrão; cada fonte define o que seu board exige, então não há padrão algum para colidir com o `User-Agent` de um chamador.
Duas novas suítes e três novos casos fixam o par: `tests/http-accept-encoding.test.mjs` (o valor exato fixado, cada um dos quatro helpers de fetch, e uma sobrescrita do chamador em três variações de capitalização — verificando que sobrevive uma *única* chave `accept-encoding`, que é o que o bug de mesclagem produzia em dobro) e `tests/parsers-pipeline-hash-route.test.mjs`, que testa a costura visível ao usuário, e não a chave: duas vagas do MokaHR precisam chegar ao pipeline, colar novamente a mesma vaga sob quatro grafias ainda precisa colapsar em uma linha, e um fragmento cosmético não pode virar identidade. Todos foram confirmados falhando antes. A fixação vive no **transporte**, não apenas nesses helpers: 25 das 92 fontes chamam o `fetchImpl` injetado diretamente e nunca passam por eles, então `makeTimeoutFetch` — o wrapper que ambos os scanners de fato injetam — a aplica, e esse é o único ponto de passagem comum a todas as fontes. (O pai não precisa de equivalente: `providers/_http.mjs` é seu único transporte.) Uma guarda estrutural agora quebra o CI se uma fonte nova chamar `fetch` diretamente em vez de usar o transporte que recebeu, e um caso comportamental conduz o greenhouse, uma das 25, pelo wrapper para provar que o cabeçalho chega ao fio.
Não portado, com o motivo: `liveness-browser.mjs` (proteção de reuso do Playwright em modo com interface — o web-ui não conduz um navegador com interface gráfica), `check-jd-archive.mjs` (sem repasse no web-ui), `test-all.mjs` (harness do pai; a divergência `vpFixtureEnv` do fork permanece intacta), `web/` (o próprio frontend do pai, não espelhado), manifestos de plugin e `SIGNATURES.md` (exclusivos do repositório). `batch/batch-runner.sh` (os `&` escapados antes da interpolação do `sed`) e `jd-skill-gap.mjs` (resolve `cv.md` a partir da raiz de dados) são **repassados somente leitura** — o web-ui aciona via shell as cópias do pai, então ambos chegam sem mudança de código aqui, e o de `jd-skill-gap` deixa o repasse de `/api/jds/:id/skill-gap` mais correto: `cv.md` agora é resolvido a partir da raiz de dados, e não do diretório em que o processo foi iniciado. Divergências do fork reverificadas após o pull: a correção cirílica de `\p{L}` em `providers/telegram-channel.mjs`, o `providers/telegram.mjs` próprio do fork, `hermes` em `web/src/lib/clis.ts`. Contagem de fontes inalterada em **92** (87 EN + 5 RU), então a ajuda permanece intocada. **3042 → 3066 testes.**

## [1.234.0] — 2026-09-15

**Paridade com o pai — career-ops `main` @ `56cce8f` (VERSION ainda 1.32.0, 58 commits além da 1.32.0). Uma correção espelhada em 15 fontes, mais uma proteção para que não volte.**

### Corrigido
**Um único id de vaga malformado não esvazia mais a página inteira.** `encodeURIComponent` lança `URIError` diante de um substituto UTF-16 (surrogate) solitário, e um payload JSON pode carregar um — `JSON.parse('"\\uD800"')` o preserva. Quinze fontes construíam a URL de cada vaga a partir de um id ou slug controlado pelo host *dentro* do laço de parse, então uma vaga malformada abortava o laço e perdia todas as vagas daquela página; nada era registrado como falha da fonte, pois a exceção se propagava antes que o catch por empresa tivesse algo a relatar. O projeto pai corrigiu isso em `providers/_safe-url.mjs` (`safeEncodeURIComponent` → `null` em `URIError`, relança qualquer outro erro) e passou o id de cada laço por ele, descartando apenas a vaga problemática quando o retorno é `null`. Esse helper agora é `server/lib/sources/_safe-url.mjs`, e a mesma mudança é aplicada às treze fontes espelhadas — alibaba, arbeitsagentur, bamboohr, feishu-jobs, garena, jibeapply, manfred, meituan, mokahr, phenom, thehub, tkms, vdab — e a **duas fontes exclusivas do web-ui que a nova proteção revelou logo na primeira execução, jobstreet e trudvsem**, que tinham o mesmo formato de laço e o mesmo lançamento latente. O escopo é deliberado: só ids vindos de API passam pelo helper. Valores derivados de configuração mantêm seu encode estrito e falham ruidosamente — o `office` do garena (um segmento de `portals.yml`) permanece no `urlSegment` que lança exceção, enquanto seu `id` de API recebe um `idUrlSegment` que retorna `null`; o `corpName` do csod e o slug do 4dayweek validado por `SLUG_RE` são colocados em lista de permissão nominalmente, com o motivo registrado.
**O fallback de título do rheinmetall podia lançar exceção diante de um `href` malformado.** Quando um card não tem título, o título é reconstruído a partir do slug da URL via `decodeURIComponent`, que lança exceção diante de uma sequência percentual inválida no `href` obtido por scraping — o mesmo formato de «uma linha ruim mata a página inteira», agora do lado da decodificação. Agora ele decodifica de forma defensiva e recorre ao slug bruto como alternativa.
**O registro de fontes agora ignora arquivos com prefixo `_`.** A descoberta automática importava todo `.mjs` em `server/lib/sources/`, exceto a si mesma, e avisava sobre qualquer arquivo sem `export const meta` — então o helper gerava um aviso na inicialização. Foi adotada a convenção do pai: um underscore inicial significa *não é uma fonte*; esses arquivos não são importados nem geram aviso. Um arquivo não declarado e sem `meta` continua avisando, e o teste de descoberta agora verifica as duas metades (confirmado falhando com o filtro antigo). O script de build do site `sync-assets.mjs` conta os arquivos de adaptadores em disco pela mesma regra, então o helper não aciona mais sua guarda contra quedas silenciosas.

### Notas
`tests/sources-url-encoding-surrogate.test.mjs` espelha a suíte do pai em três partes: o contrato do helper (um substituto pareado — um emoji comum — ainda é codificado; um lançamento vindo do próprio `toString` do valor é um bug do chamador e se propaga), o comportamento por fonte para as quinze (um id ruim + um id limpo → sem exceção, o limpo é mantido, o ruim é descartado, cada um exercitado através do parser puro exportado pela fonte ou, no caso do thehub, seu fetcher com um transporte falso), e uma **proteção no nível da fonte**: todo arquivo convertido precisa importar o helper *e* chamá-lo, nenhum arquivo pode referenciá-lo sem importá-lo, e nenhum arquivo não revisado pode montar uma URL de vaga com um `encodeURIComponent` isolado — uma fonte nova com o formato antigo falha no CI com o número da linha responsável. Os quinze casos por fonte foram confirmados falhando antes da portagem.
Tudo o mais no delta do pai não foi portado, com o motivo: `batch/batch-runner.sh --cli` é uma flag exclusiva de CLI (o web-ui não invoca o batch runner via shell); `invite-match.mjs` agora resolve `set-status.mjs` a partir da raiz do código — o web-ui não faz esse repasse; `company-funded.mjs` recebeu a mesma mudança de safe-url internamente, e o formato de saída do seu `--json`, que `/api/company-funded` interpreta, permanece inalterado; `test-all.mjs` só moveu um orçamento de timeout (a divergência `vpFixtureEnv` do fork permanece intacta); `web/` é o frontend do próprio pai e não é espelhado; `AGENTS.md`/`SIGNATURES.md` são exclusivos do repositório. Divergências do fork reverificadas após o pull: a correção cirílica de `\p{L}` em `providers/telegram-channel.mjs`, o `providers/telegram.mjs` próprio do fork, `hermes` em `web/src/lib/clis.ts`. Contagem de fontes inalterada em **92** (87 EN + 5 RU), então a ajuda permanece intocada. **3022 → 3042 testes.**

## [1.233.2] — 2026-09-10

**Corrigido — dois alertas CodeQL `js/remote-property-injection` de alta severidade.**

### Corrigido
O laço que copia os valores salvos para `process.env` percorria `Object.entries(safe)` — um objeto construído a partir do corpo da requisição — com uma guarda explícita contra chaves de protótipo. O código era seguro, pois `safe` só é preenchido a partir de `KNOWN_KEYS`, mas o analisador não consegue seguir essa restrição entre dois laços. Agora percorre o array constante, então o nome da propriedade é comprovadamente um literal de módulo. **O comportamento é idêntico.**

### Notas
O teste que fixava a guarda era um grep do texto-fonte. Foi trocado por um estrutural e um comportamental que envia `__proto__` como **JSON cru** — um literal de objeto definiria o protótipo em vez de criar a propriedade. **3021 → 3022 testes.**
...
- **FIND-3 — o campo de busca encolhia até 8 px num telefone de 320 px.** Ao pôr a barra em uma linha só, o searchbar ficou como o único item flexível, e o changelog anterior dizia isso: *«pode encolher até nada»*. E encolheu: **8 px de campo a 320 px, 28 px a 340 px**, um caractere de um placeholder de 21. Clicável e digitável, mas ilegível como campo. Nenhum `min-width` resolve: a 320 px a barra tem 32 px de padding, um menu de 40 px e 162 px de ações, sem 120 px sobrando. Então abaixo de **420 px** — a largura em que o campo enfim passa de 100 px sozinho — ele se esconde atrás de uma lupa e se expande por toda a barra ao toque: **182 px a 320 px, 222 px a 360 px, 252 px a 390 px**, sem overflow em largura alguma. O botão mantém um único nome acessível e informa o estado por `aria-expanded`, e a troca 🔍/✕ é CSS puro. Acima de 420 px nada muda. Encontrado por uma passada de regressão em navegador contra a build ao vivo.
...
**Adicionado — canais do Telegram como fonte de varredura e um modo de consulta paginado em `/api/scan-results`.**

### Adicionado
- **Canais do Telegram agora são uma fonte varrível.** Um bloco `telegram_channels:` no `portals.yml` os lista; cada um é lido pela prévia pública `https://t.me/s/<canal>`. Registro: **85 → 86 fontes** (81 EN + 5 RU), `ALL_ADAPTERS` **80 → 81**.

  O Telegram não publica RSS e a Bot API não lê um canal que o bot não administra — nenhuma das rotas óbvias serve. A prévia `/s/` é HTML puro renderizado no servidor: sem autenticação, sem JS, sem cookie. Verificado em 16 canais: 15 devolveram 20 posts cada num GET simples.

  `channel:` aceita qualquer forma de escrever. O parser se ancora nas três coisas que um redesign do Telegram moveria por último: `data-post`, o contêiner do texto e a data ISO `<time datetime>`.
- **`GET /api/scan-results` ganhou um modo de consulta paginado.** `?q=<texto>&limit=50[&region=ru|en][&set=filtered|fresh][&offset=N]` devolve `{ total, returned, offset, limit, set, rows }`, onde **`total` é a contagem ANTES da paginação**. Sem parâmetros a rota continua devolvendo o snapshot inteiro: a tabela `#/scan` depende dessa forma.

### Corrigido
- **O assistente do Telegram respondia com uma fração dos dados.** Seu skill já apontava para `/api/scan-results`, mas a rota só sabia devolver o snapshot inteiro — cerca de 2 MB — que não cabe no contexto de um agente. Diante de «encontre todos os Product Manager» relatou **9** correspondências onde o snapshot tinha centenas, e disse que «trabalhava com um índice local limitado». Era verdade: via o endpoint mas não conseguia consumi-lo. Com o modo paginado a mesma pergunta custa 2 KB e devolve um `total` honesto. O skill também foi reescrito: pedia resumir os posts *novos*, ou seja apontava para `fresh` em vez de `filtered`.

### Notas
- **Um post de canal é prosa, não um registro de vaga.** Não há campos estruturados: o título vem da primeira linha substantiva, e empresa/local/faixa de rótulos explícitos. **Separar vagas reais de anúncios e digests fica com o `title_filter` existente**, o mesmo já ajustado para outras fontes. São pistas, não uma listagem.
- **A empresa nunca é adivinhada.** Sem rótulo, a linha é atribuída ao canal (`@rabotaphp`): um empregador errado entra no tracker como fato.
- **Uma análise vazia é um erro, não «sem vagas».** O t.me responde com redirecionamento a um canal privado ou inexistente; devolver zero linhas leria como um dia calmo e esconderia um erro de digitação na configuração para sempre. `jobGeeks` faz exatamente isso (HTTP 302) e deliberadamente **não** está configurado.
- A detecção de remoto usa uma fronteira de palavra Unicode em vez de `\b`, que é só ASCII — a mesma armadilha que o filtro de títulos sofreu na v1.227.3, aqui pega por um teste antes de publicar.
- 15 canais configurados e verificados ao vivo: **299 posts, 15/15 canais**. 30 testes novos. Testes: **2865 → 2895**.
...
### Adicionado
- **JobsDB Hong Kong pela fonte Jobstreet/SEEK existente.** Hong Kong opera sob a marca JobsDB, mas roda na mesma plataforma SEEK atrás do mesmo endpoint v5, então não precisa de fonte separada: basta `hk.jobsdb.com` na lista de hosts permitidos e `siteKey: HK-Main`. O registro não muda: **85 fontes** (80 EN + 5 RU) e `ALL_ADAPTERS` **80** — é um MERCADO novo sobre um adaptador existente, não um adaptador novo.
- **Filtro de somente pendentes no painel de cadência** (`#/modes/followup`). Uma nova caixa de seleção reduz o painel ao que é acionável *agora*. A condição vem da `urgency` que o `followup-cadence.mjs` do projeto pai já calcula (`urgent` / `overdue`), nunca do `status` do tracker, que é `applied`/`responded`/`interview` e não casaria com nada. Quando nada está pendente, o painel nomeia o follow-up agendado mais próximo em vez de exibir um vazio. A lógica fica no módulo puro `public/js/lib/followup-view.js`, espelho do `followup-view.mjs` do pai.
- **Empregadores confidenciais são atribuídos à agência que os intermediou.** Uma vaga com o empregador omitido é registrada como `?` no tracker, com o recrutador na coluna `Via`. O `#/tracker` mostrava um `?` cru, então várias linhas confidenciais ficavam indistinguíveis e o resolvedor de logos não tinha o que buscar. Agora a linha lê **“Confidencial · via \<agência\>”**, resolve o logo pelo nome da agência e é encontrável por esse nome na busca. Apenas apresentação: o campo canônico `company` nunca é sobrescrito com o recrutador. Novo módulo puro `public/js/lib/company-presentation.js`, espelho do `company-presentation.mjs` do pai.

### Corrigido
- **Jobstreet/SEEK chamava um endpoint obsoleto.** A fonte ainda apontava para chalice-search v4 (`/api/chalice-search/v4/search`), que a SEEK aposentou; o pai migrou para `/api/jobsearch/v5/search` e o web-ui nunca acompanhou. Agora toda entrada Jobstreet/SEEK consulta v5. O formato do item v5 é lido nativamente (URL construída a partir de `id`, `locations[0].label`, `advertiser.description`, `salaryLabel`), o parâmetro de projeção `solrFields` (só v4) some, e uma entrada cujo `portals.yml` ainda fixe o caminho v4 morto mantém o hostname do mercado enquanto o caminho é reconstruído em v5 — sem edição do usuário.

### Notas
- Paridade com career-ops 1.30.0 (HEAD do pai `8d64f65` → `6cc46a4`; o `VERSION` dele ainda marca 1.30.0 — o release-please fica atrás da `main`). Não portado, com motivos: **preenchimento de localização JSON-LD do iCIMS** — a fonte iCIMS do web-ui não tem gancho `enrichDate()` (só páginas de listagem), então não há caminho a corrigir; **stderr não fatal do CLI (#1974)** — o runner do web-ui decide falha só pelo código de saída e nunca teve essa heurística; **rastreio de caminhos do Turbopack** — o web-ui não usa bundler; **correções do `cv-sync-check`** — repassadas somente-leitura e fail-soft; **sequenciamento do Playwright em `scan.mjs`, respostas do Block H, guardas de updater/doctor/`update-system`, o verificador de sintaxe recursivo, o dashboard em Go e as autotestagens de `jd-skill-gap`/`verify-cv-facts`** — superfícies só-CLI que o web-ui não invoca, ou autotestes sem mudança de comportamento. Testes: **2818 → 2837**.

## [1.226.0] — 2026-08-27

**Adicionado — duas fontes de vagas vietnamitas: ITviec e CareerViet.**

### Adicionado
- **ITviec + CareerViet (Vietnã).** Dois portais vietnamitas com host fixado entram no scanner via entradas `provider: itviec` / `provider: careerviet`: **ITviec** (itviec.com, tecnologia) e **CareerViet** (careerviet.vn, ampla cobertura). Ambos são scrapers HTML sem tokens, com host-pin anti-SSRF, só-HTTPS, fail-soft por empresa e limites de página. O registro agora traz **85 fontes** (80 EN + 5 RU), `ALL_ADAPTERS` **80** — ambos aparecem no filtro Fonte do `#/scan`.

### Notas
- Paridade com career-ops 1.30.0 (parentVersion 1.29.0 → 1.30.0). Não portado (só CLI/comunidade ou já coberto): o Hired Wall, `/calibrate`, clean-markers, packs de templates, doctor, pdf/LaTeX e as CLIs de eval/tracker/scan que o web-ui não invoca; o ajuste de `_html-to-text` e `roleFuzzyMatch` já existiam; o prefixo `title-filter stem:` fica adiado. Conjunto de testes: **2818**.
...
### Adicionado
- **Banner de logotipos de provedores.** Um único `images/providers.svg` autônomo — os **18 provedores de LLM** com o **logotipo real da marca** ([simple-icons](https://simpleicons.org), CC0) para os 11 que publicam um e um monograma com a cor da marca para os 7 que não — agora encabeça o README (×17) e o guia de ajuda integrado. (O app renderiza os blocos com o logotipo real; o banner é renderizado no GitHub e removido no app pelo renderizador seguro contra XSS.)

### Notas
- Somente docs — sem mudança de código ou testes; o SVG é embutido (sem recurso remoto). Fontes de varredura sem mudança: **83**. Conjunto de testes: **2784**.

## [1.224.0] — 2026-08-26

**Alterado — os blocos de provedores LLM agora mostram os logotipos reais da marca.**

### Alterado
- **Logotipos reais dos provedores.** Os blocos de provedor — campos de chave API nas Configurações, as linhas de Uso, o chip do painel, o resumo "Ativo" das Configurações, o resultado da ⚡ eval e o banner de boas-vindas — agora mostram o **logotipo real da marca** dos 11 provedores que publicam um ícone de código aberto (Anthropic · Gemini · OpenAI · Qwen · OpenRouter · GitHub · DeepSeek · Kimi · MiniMax · Mistral · Ollama; SVGs de caminho único do [simple-icons](https://simpleicons.org), CC0). Os 7 sem ícone publicado (Hermes · GLM/Z.ai · Grok · Together · Fireworks · BytePlus Ark · Volcengine Ark) mantêm o monograma com a cor da marca. Continua **seguro para a CSP por construção**: o caminho do logo é uma constante estática embutida — sem recurso remoto, sem `innerHTML`.

### Notas
- Sem mudança de rota ou comportamento; a mesma API `ProviderLogo.el(slug)`. `provider-logo.js` cresceu (os caminhos de logo embutidos). Fontes de varredura sem mudança: **83**. Conjunto de testes: **2784**.
...
### Alterado
- **Dicas de campo de provedores localizadas.** Cada `config.<slug>Hint` dos 11 provedores compatíveis com OpenAI adicionados em v1.216.0–v1.217.0 (DeepSeek · GLM/Z.ai · Kimi · MiniMax · Mistral · Grok · Together · Fireworks · Ollama · BytePlus Ark · Volcengine Ark) — as dicas de chave API, modelo e URL base em **Configurações → Chaves API** — agora é traduzida nos 17 idiomas em vez de recorrer ao inglês. URLs de cadastro, ids de modelo e o marcador ⚡ de eval ao vivo são preservados na íntegra.

### Notas
- Sem mudança de comportamento — o `hintFallback` em inglês em `field-specs.js` ainda protege uma chave ausente, e um novo teste de contrato garante que cada `hintKey` dos descritores de campo resolve nos 17 idiomas. Fontes de varredura sem mudança: **83**. Conjunto de testes: **2779**.
...
### Adicionado
- Uma entrada `getonbrd` pode definir `categories: [programming, operations-management, machine-learning-ai]` (ou um único `category:`) em vez de apenas o feed `programming`; as vagas são deduplicadas por URL entre categorias, com limite de 12. Entradas existentes ficam idênticas (o padrão continua `programming`).

### Notas
- Apenas servidor; o host-pin SSRF (www.getonbrd.com) e `redirect:'error'` não mudam; um slug de categoria inválido é rejeitado antes de qualquer requisição. Fontes de varredura inalteradas: **83**. Testes: **2771**.
...
### Adicionado
- Um teste parametrizado de `run<Provider>` que verifica endpoint, modelo padrão e autenticação Bearer para **Kimi, MiniMax, Mistral e Fireworks**.

## [1.217.0] — 2026-08-26

**Adicionado — Ark entra na lista de LLM (18 provedores).** BytePlus Ark e Volcengine Ark — os modelos Doubao da ByteDance — agora são provedores ⚡ ao vivo, pelo mesmo núcleo compatível com OpenAI e uma URL base por região.

### Adicionado
- **BytePlus Ark** (`ARK_API_KEY`, internacional) e **Volcengine Ark** (`ARK_CN_API_KEY`, China) — Chat Completions compatível com OpenAI via `runOpenAICompatible()`. Defina `ARK_MODEL` / `ARK_CN_MODEL` (um modelo Doubao ou seu id de endpoint `ep-…`); `ARK_BASE_URL` / `ARK_CN_BASE_URL` trocam de região. Ambos entram na ordem `auto` (por último), com seu monograma em Configurações e Uso. A interface web abrange agora **18 provedores** ao vivo.

### Notas
- Mesmo caminho de configuração confiável + verificação de esquema http(s) do restante (nunca o validador SSRF de URL de vaga). Fontes de varredura inalteradas: **82**. Testes: **2755**.

## [1.216.0] — 2026-08-26

**Adicionado — mais nove provedores de LLM, cada um com seu próprio emblema de marca.** Suas avaliações ⚡ ao vivo agora podem rodar via DeepSeek, GLM (Z.ai), Kimi (Moonshot), MiniMax, Mistral, Grok (xAI), Together, Fireworks ou um Ollama totalmente local — a uma chave de distância, cada um marcado por um monograma ao lado do seu campo em Configurações.

### Adicionado
- **Nove provedores compatíveis com OpenAI.** DeepSeek, GLM (Z.ai), Kimi (Moonshot), MiniMax, Mistral, Grok (xAI), Together AI, Fireworks AI e **Ollama** (totalmente local, sem chave). Defina uma chave — ou `OLLAMA_BASE_URL` para o Ollama — em **Configurações** e ele entra na ordem `auto` após o Hermes; fixe qualquer um com `LLM_PROVIDER`. O Together também hospeda o **Inkling** da Thinking Machines (`thinkingmachines/Inkling`). A interface web já abrange **16 provedores** ao vivo.
- **Emblemas de provedor.** Um mosaico de inicial na cor da marca, seguro para CSP, marca cada provedor ao lado do seu campo de chave em Configurações e na sua linha da página de Uso — SVG inline, sem logos externos, nada sai da sua máquina.

### Notas
- As URLs base dos provedores são configuração confiável que passa por `runOpenAICompatible()` com verificação de esquema http(s) (o loopback do Ollama é permitido), nunca o validador SSRF de URL de vaga. Fontes de varredura inalteradas: **82**. Testes: **2752**.
