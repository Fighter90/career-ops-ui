...
**变更(低,扫描器)——Getro 对齐移植(父 #2640):薪资、全部地点、`work_mode` 远程识别。**

### 变更
- 将父级 `providers/getro.mjs` 扩展(#2640)的安全部分移植到 `server/lib/sources/getro.mjs`。Getro 职位现在带有**薪资**显示字符串(来自 `compensation_amount_min/max_cents` + 币种,仅年薪——沿用客户端薪资过滤器会重新解析的 remotli/lever 字符串约定),其**地点**合并**全部** `locations`(此前仅第一个),且 `work_mode: 'remote'` 与现有的 `remote` 标志 / 地点文本一同被视为远程信号。纯粹是对 web-ui 已获取的 API 响应的数据映射——无新主机,无 SSRF 面变更。

### 说明
- 有意未移植:父级的 `collection_id` **自动解析**(它会抓取看板自身的 `careers_url`——任意主机——因此应作为单独变更经由 web-ui 的 `safeGet` SSRF 边界)以及提升到 1500 页的硬上限(web-ui 为浏览器触发的扫描保持 200 页 / 4000 个职位上限)。本次同步中其余 11 个父级提交在 web-ui 无镜像。测试:`tests/sources-getro.test.mjs`(+5)。套件:**2470**(+5)。
