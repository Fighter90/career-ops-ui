...
### 修复
**书籍链接 —— 文案被翻译了，目标网址却没有。** 全部 17 个 SPA 语言与全部 17 个 cvstart.org 语言，都把页脚指向 *Agentic Coding Design Patterns* 的链接指向了英文版，而唯一被翻译的文案——俄语 «Паттерны агентного кодинга»——承诺的却是一本俄语版的书。新增的 `data-i18n-href` 应用器（与 `data-i18n-title` 同形，仅限绝对的 `https://` 取值）与 `footer.patternsUrl` 键，现在会把 `ru` 指向俄语版，其余每个语言都指向英文版。回归测试从两个方向断言这一不变式：被翻译的标题意味着本地化的网址。
**SEEK —— 正在迁移主机，而传输层拒绝跟随重定向。** `www.seek.com.au` → `au.seek.com`、`www.seek.co.nz` → `nz.seek.com`：详情页面如今已经返回 301，而这两对主机眼下都以 200 提供 v5 搜索 API。由于 `http-json.mjs` 设置了 `redirect:'error'`（一项值得保留的 SSRF 防御），一旦 API 路径开始对旧主机上的 AU/NZ 条目发出重定向，抓取就会失败——而由于白名单里此前只有 `www.` 开头的主机，也就没有人能够完成迁移。`au.seek.com` 与 `nz.seek.com` 现已加入 `ALLOWED_JOBSTREET_HOSTS`；旧主机仍然保留，因此既有配置与已经生成的链接照常可用。仿冒主机（`au.seek.com.evil.com`、`au-seek.com`）依旧会被拒绝。
**Wiki：** Home 文档地图中的 *Agentic Practices* 一行，是十二行里唯一没有在 `[[…|…]]` 内转义 `|` 的一行，导致 GitHub 截断了该单元格——链接失效，描述被隐藏。只差一个字符。

### 说明
来源数量维持 **94** 个不变（89 EN + 5 RU），89 个适配器。测试 **3201 → 3210**（+5 页脚链接语言，+4 SEEK 主机）。
无法从外部关闭，只能记录而非断言：Oracle Cloud（Akamai 拦截了两次 REST 探测）、Personio 的租户固定与 `appendWorkType`（需要在 `portals.yml` 中新增一项，而 UI 里没有针对其中一项的删除功能），以及 liveness 的字符串用例（没有可控页面；40 次存活探测得到 12 次存活、28 次不确定、**0** 次误判过期）。
Help ×17 现在同时列出 `AU-Main` 与 `NZ-Main` 的两种主机形式。标题结构维持不变：32 个 H2 / 122 个 H3。
...
### 新增
**两个新增的扫描器来源,92 → 94(89 EN + 5 RU)。** **Python.org Jobs**(`pythonorg`)读取该职位板的公开 feed;雇主署名是强制性的,因此标题中未指明雇主的行会被丢弃,而不是回填门户条目自身的名称。**Generalist World**(`generalist-world`)抓取该职位板的卡片,并且**在遇到无法解析的标记时会大声失败**——卡片结构一旦发生变化就会抛出异常,而不是被当作一个空白职位板来读取,这正是让一个抓取式来源变得不可信的失败模式。两者都出现在 `#/scan` 的来源过滤器中,都以主机锁定(锚定正则、仅限 HTTPS),并且都由端到端的伪造传输层测试套件驱动(分别为 43 项与 36 项用例)。

### 修复
**一个在多个城市均有开放的 Workday 职位,被地点过滤器丢弃了。** Workday 的列表端点对一个多地点职位发布给出的是一个**计数**,而其他每一条职位发布携带的都是一个地点:`"53 Locations"`。这不是一个地点,而 `buildLocationFilter` 是按大小写不敏感的子串匹配——于是一个在 Austin *以及*另外 52 个城市均有开放的职位,匹配不到任何 `allow: [austin]` 条目,因而被丢弃,而同一个职位若单独列出反而能通过。在上游对三个租户的实测中:**291 条职位发布中有 53 条**是占位符,可见这是常规情形,而非边缘情形。该职位发布的真实地点只存在于 CXS 的**详情**文档中,因此现在每个占位符都要额外付出一次针对该文档的 GET 请求,上限为**每条 200 次**——超出上限后,占位符会保持可见,而不会被伪造成已解析的样子,因为一个未解析的职位发布绝不能读起来像是已经解析过的。一次失败的或没有地点信息的详情请求,会让职位发布原样保留列表端点返回的内容:一个可见的 `"53 Locations"` 如实反映了无法解析的部分,而一个被清空的地点则会悄无声息地通过每一道过滤器。同一次 GET 请求还携带 `startDate`,因此一条被充实过的职位发布现在能被**精确**标注日期,而不再依赖列表端点的相对性文字描述(`"Posted 30+ Days Ago"` → `undefined`);任何不是纯粹 `YYYY-MM-DD` 格式的值都会被原样保留,而不会交给 `Date.parse` 处理,因为它对非 ISO 格式的回退行为是由具体实现决定的。
**一个明示的级别,不会再被错误地把两条招聘需求合并成一条重复发布。** `"Insurance Specialist II"` 是雇主自己给出的声明:这条招聘需求并非其「同胞」——同一个基础职位名称之下,是不同的薪级、职责范围与需求编号。分词器此前根本看不到这一点:`w.length > 3` 会丢弃所有到 VIII 为止的罗马数字以及所有单个数字,于是 `"Insurance Specialist"` 与 `"Insurance Specialist II"` 被分词为完全相同的结果,Jaccard 相似度打出满分,重复发布检测器便把第二条标记为对第一条的重新刊登。web-ui 的 `role-matcher.mjs` **完全没有**任何级别处理逻辑,因此这次改动整体移植了父项目的机制:级别提取时将罗马数字与阿拉伯数字折叠为同一个数字(`"Nurse II"` 与 `"Nurse 2"` 是同一个声明——而*这一对*此前恰恰被错误地判定为不同),两侧明示的级别必须一致,而只有一侧带级别时会被视为一次松散的改写,**除非**两个标题各自还带有对方没有的词——那就是两处分歧,而不是一处装饰性差异。
**一个被固定到某个过滤器的 Avature 职位板,实际上却遍历了整个全局职位板。** Avature 的搜索界面生成的是不透明的分面参数(`?42386=[812132]` 是一个国家过滤器),而在 `portals.yml` 中固定了一个缩小范围职位板的操作员,期望扫描的正是*那个*范围。`resolveSearch` 把 URL 重建为 `${origin}${path}`,却把查询字符串彻底丢弃——于是每一次请求,包括第一次在内,都遍历了该租户的整个职位板:更多页面、更多请求,以及操作员本已刻意过滤掉的职位发布。分面参数现在会随每一次分页请求一同携带,并且我们自己的 `jobOffset` 被放在最后应用,这样任何偶然同名的分面参数都绝不可能劫持分页逻辑。

### 安全
**`devalue` 5.8.1 → 5.9.2**(通过 `astro` 间接引入),关闭了唯一一条待处理的 Dependabot 告警——一个针对畸形输入的拒绝服务漏洞。仅为站点依赖;运行时代码未发生变化。

### 说明
**以下内容未移植,并附上各自理由。** 父项目把 **lever** 的超时从共享的 10 秒默认值调整为 30 秒,把 **manfred** 调整为 25 秒——这属于**不移植**:web-ui 的扫描传输层已经允许**60 秒**(`DEFAULT_SCAN_TIMEOUT_MS`),因此这两个职位板在这里拥有的余量,比那边这次修复所给出的还要大。**iCIMS** 的 `careers-` 前缀重复问题在这里同样不可能发生:web-ui 直接从 `api`/`careers_url` 取得租户源,从不会用租户名称拼出主机名;它新增的回退主机遍历逻辑,是针对父项目那边更宽松配置的一项特性,而不是缺陷修复。**deutschebahn** 与 **manfred** 获得了「瞬时中断重试」能力,而 web-ui 60 秒的上限让这一点远没有那么迫切——因此这里只作记录而不移植,以便日后若真的出现长时间遍历超时,能有一个已知的首选排查方向。128 个提交差量中的其余部分,全部属于 CLI、校验器、仪表板或 `web/`(父项目自己的前端):`liveness-browser`、`check-jd-archive`、`verify-ats`/`verify-portals`/`verify-cv-facts`、`update-system`、`set-status`、`audit-portals`、`discover-ats`、`scaffolder`、`test-all`、插件清单。
拉取之后重新核实了 fork 的分歧点:`providers/telegram-channel.mjs` 中的西里尔字母 `\p{L}` 修复、fork 自有的 `providers/telegram.mjs`,以及 `web/src/lib/clis.ts` 中的 `hermes`。上游 `career-ops-hq/main` 已完全合并(缺失 0 个提交)。**3066 → 3164 项测试**;帮助内容现在在全部 17 个语言包中都声明了 **94** 个适配器(**89 EN + 5 RU**),标题结构维持不变,仍是 32 H2 / 122 H3。

## [1.235.0] — 2026-09-16

**父项目对齐 — career-ops `main` @ `68e6b94`（`VERSION` 为 1.33.0，42 个提交）。两处镜像修复；其中一处会悄无声息地在这里丢失 MokaHR 的职位发布。**

### 修复
**同一个 MokaHR 职位板上的第二条职位没有被新增，而是彻底消失了。** `url-key.mjs` 会把一条职位发布 URL 归约成一个用于比较的键，此前它会无条件丢弃片段——这本是合理的做法，因为几乎每个片段都只是展示性的（`#apply`、`#section-2`）。MokaHR 是我们自家注册表里的例外：它的职位发布形如 `…/social-recruitment/{tenant}/{id}#/job/{n}`，同一租户下的每条职位共享同一路径，职位 id **只**存在于片段之中。于是它们全都被归一化成了同一个键。修复前实测：把两条不同的 MokaHR 职位粘贴进流水线，最终只剩**一行**——没有报错，没有警告，也没有任何异常可察觉；扫描器的新鲜度过滤器（`en-scanner.mjs`）读取的是同一个键，因此只要该租户下有任意一条职位曾进入扫描历史，其余全部职位都会被当作「已见过」而丢弃。`normalizeUrl` 现在会在片段被丢弃之前，把一个可识别的 `#/job/{id}` 或 `#/jobs/{id}` 路由提升为一个仅用于比较的查询键——MokaHR 沿用其既有的 `mokahr_job_id` 名称，以便此前写入的键仍可比较；其余任何站点则得到一个内部的 `_career_ops_fragment_job_id`。提升后的 id 是**追加而非设置**：一个已经带有该参数的 URL 中留存的是另一条职位的 id，覆盖它就会把这个键本该分开的两样东西合并到一起。每一个无法识别的片段依旧照常合并，这一半此前就是对的。
**现在每一次来源请求都会固定 `accept-encoding`。** 若不设置，Node 会协商其 undici 构建所提供的任何编码；从 **Node ≥ 23** 起，这包括 `zstd`，而 amazon.jobs 的 zstd 响应会**恰好在 1024 字节处被截断，却仍带着 200 状态码**返回——于是这次失败读起来像 `Unterminated string in JSON at position 1024`，更像是一个畸形的 API，而不是传输层的问题。现在每一次请求都会发送 `gzip, deflate, br`。在我们的运行环境里，这是**预防性的，而非修复**：Node 18/20/22——CI 矩阵与服务器（22.22.1）——发送的是 `gzip, deflate`，这一点已直接验证而非假设，因此这里从未发生过截断；今天真正改变的是提供了 `br`，这些版本默认不会请求它，但能正确解码。这个默认值是**以大小写不敏感的方式**应用的，这才是值得细看的地方：请求头名称本就大小写不敏感，而 `fetch` 会把两个同名条目合并成一个以逗号分隔的值，因此调用方若传入 `Accept-Encoding: identity`，原本会变成发送 `gzip, deflate, br, identity`。

### 说明
在 `fetchImpl` 边界处，请求头始终保持为一个**普通对象**，而父项目在这里交给 `fetch` 的是一个 `Headers` 实例。这是刻意的分歧，而非疏漏：web-ui 的 fetch 辅助函数接受一个可注入的 `fetchImpl`，而 45 个来源测试套件读取的是它们捕获到的 `opts.headers['User-Agent']`——`Headers` 实例没有方括号访问方式，因此忠实移植的是这份*行为*（大小写不敏感的覆盖、调用方优先），而不是具体实现。调用方传入的对象会被复制，从不会被就地修改，因为多个来源共用同一个请求头常量。父项目那处配套修复——以大小写不敏感的方式应用**默认 user-agent**——属于**不移植**：web-ui 根本不会注入任何默认 UA，每个来源都自行设置其招聘板所要求的 UA，因此不存在会与调用方的 `User-Agent` 相冲突的默认值。
两个新增的测试套件与三个新增用例钉住了这一对修复：`tests/http-accept-encoding.test.mjs`（精确的固定值、四个 fetch 辅助函数中的每一个，以及调用方在三种大小写写法下的覆盖——断言最终只存活*一个* `accept-encoding` 键，而合并缺陷此前恰恰会产生两个）；以及 `tests/parsers-pipeline-hash-route.test.mjs`，它验证的是用户可见的接缝而非键本身：两条 MokaHR 职位都必须到达流水线，同一条职位以四种拼写重新粘贴仍必须合并为一行，而一个装饰性片段绝不能变成一个身份标识。全部用例均已确认先为失败状态。 该固定位于**传输层**，而不仅仅在这些辅助函数中：92 个来源中有 25 个直接调用注入的 `fetchImpl`，根本不经过它们，因此由 `makeTimeoutFetch`——两个扫描器实际注入的包装器——来应用它，这是所有来源共享的唯一通过点。（父项目不需要等价物：`providers/_http.mjs` 是它唯一的传输层。）现在有一道结构性防护：若新来源直接调用 `fetch` 而不使用交给它的传输层，CI 就会失败；另有一个行为用例，让 25 个之一的 greenhouse 经由该包装器运行，证明请求头确实发到了线上。
以下内容未移植，并附上理由：`liveness-browser.mjs`（有头 Playwright 复用守卫——web-ui 并不驱动有头浏览器）、`check-jd-archive.mjs`（web-ui 没有对应的转发）、`test-all.mjs`（父项目自己的测试驱动；fork 自有的 `vpFixtureEnv` 分歧完好无损）、`web/`（父项目自己的前端，不做镜像）、插件清单以及 `SIGNATURES.md`（仅存在于仓库层面）。`batch/batch-runner.sh`（在 `sed` 插值之前转义了 & 符号）与 `jd-skill-gap.mjs`（相对数据根目录解析 `cv.md`）属于**只读转发**——web-ui 直接外壳调用父项目的副本，因此这两处在这里都无需改动代码即可落地；其中 `jd-skill-gap` 这一处让 `/api/jds/:id/skill-gap` 的转发更加正确：`cv.md` 现在相对数据根目录解析，而不是相对进程启动时所在的任意目录。拉取之后重新核实了 fork 的分歧点：`providers/telegram-channel.mjs` 中的西里尔字母 `\p{L}` 修复、fork 自有的 `providers/telegram.mjs`，以及 `web/src/lib/clis.ts` 中的 `hermes`。来源数量维持 **92** 个不变（87 EN + 5 RU），因此帮助未受影响。**3042 → 3066 项测试。**
...
### 修复
`POST /api/config` 会把保存的值写入 `process.env` 以免重启，而那个循环遍历的是由请求体构建的 `Object.entries(safe)`，内部还带着针对 `__proto__` / `constructor` / `prototype` 的显式防护。代码本身是安全的：`safe` 在二十行前只从 `KNOWN_KEYS` 拷贝，属性名从不受用户控制。但孤立地读，这个循环说的是*给一个名字来自请求体所构建对象的属性赋值*，而 CodeQL 无法跨两个循环追踪该约束。现在改为直接遍历常量数组并从 `safe` 取值，属性名可证明是模块级字面量。原型防护也随之删去——大写蛇形命名的硬编码列表里不可能出现它，对不可能情形的运行时检查只是噪音。**行为完全相同**；告警日期为 9 月 9 日，仅因 v1.232/v1.233 挪动了行号。

### 说明
把两条当作误报驳回也是一条路，本仓库亦有此惯例。这里选择修复：注释里的断言，比不上分析器与下一位读者无需跨二十行追踪即可验证的形状。
钉住旧防护的测试是**对源码文本的 grep**，只能证明字符串存在，证明不了端点安全。已换成两项更强的检查：结构检查断言到达 `process.env[k]` 的路径是遍历 `KNOWN_KEYS`（且*不*遍历请求派生的对象），已确认在旧形态下变红；行为检查在 `config-endpoint.test.mjs` 中以**原始 JSON** 发送 `__proto__`、`constructor`、`prototype` 及未登记名称——对象字面量不行，因为源码中的 `{ __proto__: … }` 设置的是原型而非自有属性，`JSON.stringify` 会发出 `{}`，测试便什么也证明不了。**3021 → 3022 项测试。**

## [1.233.1] — 2026-09-10

**修复 — 指南从未描述 v1.233.0 新增的控件，而同一节里仍留着 200 个版本前就不再成立的说明。**

### 修复
**DOC-1 — 新增的控件在指南里无从查起。** 现在每个下拉框都以「使用默认值（…）」开头，表达一种界面此前无法表示的状态：**该键未设置**。它与从下方列表里选中同一个值的差别既看不见又要紧——「使用默认值（llama3.2）」什么都不写，项目改动默认值时你会一直拿到新的；而显式选择 `llama3.2` 会写入 `OLLAMA_MODEL=llama3.2`，下次默认值变动时把你无声地留在旧值上。这正是 CONFIG-2 与 CONFIG-3 所针对的静默漂移：机制在代码里修好了，却无处可读。现已在既有的 `### Behavior` 列表中补入三条，覆盖全部 17 个语言包。标题闸门抓不到这类问题——它们数的是 H2/H3，而覆盖并非计数。
**DOC-2 — 表格中有一行没有指明任何键，且其唯一的具体指示是错的。** Key 单元格里是 `(server uses default UA)` 这样一句括号注释，描述的是 **v1.19.0** 已从 `KNOWN_KEYS` 和界面移除的 `HH_USER_AGENT`；随后它让读者去 dev.hh.ru 注册应用并提供 UA 字符串，而自 **v1.65.0** 起 hh.ru 适配器改为用自带的浏览器 UA 抓取公开搜索页，这一步毫无用处。另有两处同样陈旧：描述 **v1.28.1** 已删除的 health 行的「可选检查」条目，以及重复 dev.hh.ru 建议的排障行。三处已从全部 17 个语言包中清除。排障行是**改写**而非删除，因为它有一半仍然成立：自 2026 年 7 月起 hh.ru 对非俄罗斯 IP 返回 HTTP 451，所以确实需要俄罗斯住宅 IP；错的是注册步骤和对一个不存在的键的引用。
**键表现在说明了自己是什么。** `### Recognized keys` 只列出 45 个键中的 6 个，而标题却承诺是一份登记表；各提供方家族已在同节正文中叙述。现以一句话 ×17 声明它是精选，并指向 `#/config` 获取完整的分组列表。没有新增键——这是消除歧义，而不是撑大表格。

### 说明
`tests/help-recognized-keys.test.mjs` 正是此前缺席的闸门。它将真正列举配置键的帮助表格第一列与实时 `KNOWN_KEYS` 比对，任何仍提及 dev.hh.ru 的语言包都会失败，任何未说明「使用默认值」条目的语言包也会失败。范围界定很关键：若干语言包还有别的表格，其第一列同样是反引号包裹的大写词（导出矩阵中的 `PDF`），因此只有当表格自身至少有一行指明真实键时才算键表。已在修复前的语言包上确认三项全红。**3018 → 3021 项测试**；由于全部内容都落在既有标题之内，帮助在 17 个语言包中仍是 **32 H2 / 122 H3**。

## [1.233.0] — 2026-09-10

**修复 — `#/config` 保存路径上的三处发现；CONFIG-3 由 v1.232.1 的修复本身引起。**

### 修复
**CONFIG-3 — 与预置值恰好相同的有意选择，被当作「未触碰」丢弃。** v1.232.1 中我写的注释声称代码能「区分用于显示的默认值与特意选了同一值的用户」，实际并不能。`initial` 混淆了两种来源：来自 `.env` 的预置（相同意味着未编辑，跳过是对的）与来自 `defaultValue` 的预置（相同是**歧义**的——从未打开，或打开后表示同意）。`HOST` 缺失时控件显示 `127.0.0.1`；输入这同一个值再保存会发出 `{}`，键仍不存在。唯一能钉住它的路径是先填错值、保存、再改回正确值——一条没人会发现的死路。现在在 `initial` 旁记录 **`seededFromFile`**：来自文件的预置一切照旧，来自默认值的预置则让 `dirty` 成为有效的*第二*信号。
**删除键会显示「· 0」。** 服务器只返回*写入*的内容，删除便无从计数，于是键其实已被移除，提示却读作「什么都没发生」。`POST /api/config` 现在还返回 `removed`，提示两者相加。
**下拉框能设不能清。** 18 个中没有一个提供空选项，表单无法为其发送 `''`。现在每个下拉框以**「使用默认值（…）」**开头，未设置的键选中的是该项而非默认值本身。v1.57.1 的意图得以保留，但它现在如实说明而非冒充一个选择。`select-remote` 需要小心：实时目录调用 `replaceChildren` 会抹掉该项，故在那里重建。新增一个 i18n 键 ×17。

### 说明
本次一并查看并有意保留的：**`lang` 是字面允许项而非前缀规则**（`langx`、`lang_evil`、`LANG` 均为 400）。**`facts.json` 按设计不在部署产物中**——22 个文件在构建时导入，值内联进 HTML。**把无扩展名上传当作文本是有文档记载的刻意取舍**；该路由只转换并返回，不写入也不执行，`.exe`/`.zip` 被明确拒绝。
单元基线仍为 **3018**。闸门在浏览器侧：`playwright-config-save-scope.mjs` 从 6 例增至 **11 例**（浏览器合计 **111 → 116**），五个新例都先在旧代码上确认失败，其中一个专为防止 CONFIG-2 复发。

## [1.232.1] — 2026-09-10

**修复 — `#/config` 的保存会写入用户从未触碰的字段。**

### 修复
**CONFIG-2.** v1.57.1 会给未设置的控件填入其 `defaultValue`，让字段显示服务器真正使用的值（这是对的，未作改动）；而 `save()` 只对**机密字段**查询 `dirty`，其余字段一律照发。于是为**显示**而放的值，被当作用户的选择送到了服务器。在已设两个键的夹具上实测：改动一个下拉框会发出 **28 个键并钉死 18 个**从未打开过的字段，各自都是其默认值；什么都不改也照发 27 个。当天行为毫无变化——写入的正是默认值。变的是每项设置的**状态**：从「未设置，跟随项目默认」变成「钉死在 `.env`」。精选列表会在版本之间变动，于是下次默认值一改，凡按过保存的人都会无声地留在旧值上。这与 CONFIG-1 同属一类：那次写的是垃圾，这次写的是看起来合理的东西，因而更难察觉。

### 说明
**值得细看的是比较基准。** 与载入的 `cfg.values` 比较——最先尝试的显然做法——并不够：未设置的 select 显示自己的 `defaultValue`，而存储值是 `''`，二者确实不同，十七个模型下拉框照旧被写入。请求体只从 27 个键降到 19 个，而非 1 个。现在改为与每个控件**被初始化时的值**比较，这是唯一能把「为显示而放的值」与「用户特意选了同一个值」区分开的表述。`dirty` 也不能作基准——输入后又改回原值时它同样会触发。
另修复：提示中的键数写在 `t()` 的**回退**参数里，而 `config.saved` 在全部 17 本词典中都有，于是这个数字从未被渲染——保存 12 个键与保存 1 个键看起来一模一样。
其余未动。v1.57.1 的预填保留，机密字段仍只在被触碰时发送，v1.232.0 的服务端宽限继续守护不经表单的直接 API 调用。修复在客户端，故测试基线仍为 **3018**；其闸门是 `tests/playwright-config-save-scope.mjs` 的 6 个浏览器用例，已登记进 `test:e2e:browser`（**105 → 111**），六个都先在旧代码上确认失败。

## [1.232.0] — 2026-09-09

**修复 — 浏览器 QA 发现的两处缺陷，均非来自 v1.231.5。**

### 修复
**PROFILE-1 — `#/profile` 会让整页横向滚动。** 字段值是一个仅带内联字体样式的匿名 `<div>`，其计算后的 `overflow-wrap` 仍为 `normal`。LinkedIn 的 URL 不给 CSS 任何合法断点，于是**元素的 min-content 宽度就是整条字符串**；网格项默认 `min-width: auto`，拒绝收缩到其下，卡片便撑破轨道，继而撑破行、主列直至文档（1280 px 下**实测 23 px 横向滚动**）。已用共享类 `.card-value` 加 `overflow-wrap: anywhere` 修复，并以 `.card-row > * { min-width: 0 }` 兜底。**是 `anywhere` 而非 `break-word`**：只有 `anywhere` 会计入 min-content，`break-word` 换了行却仍留下同样宽的轨道。
**CONFIG-1 — `POST /api/config` 对 select 字段来者不拒。** `{"LLM_PROVIDER":"not-a-provider"}` 返回 **200** 并写入 `.env`；此后一切照常，因为解析器悄悄退回备选，用户所选的设置形同虚设。**恰好持有这 19 个值的 `LLM_PROVIDERS` 一直由 `env-config.mjs` 导出**，只是校验器从未查阅。

### 说明
**只校验 `LLM_PROVIDER`，这是对报告的有意收窄。** 模型列表的真实域属于厂商，会在我们两次发布之间变动；强制套用我们的精选列表，就会挡住厂商已在提供的模型——本仓库自己的用例便配置了被精选列表剔除的 `gemini-2.0-flash`。危害分析同样指向此处：错误的模型在调用时大声失败，只有错误的提供方是无声失败。
**已保存的值予以豁免，因为严格版本被浏览器测试证明不可用。** 配置表单每次保存都会重发全部非机密字段，故拒绝既有的坏值会冻结整页，用户在修好一个自己从未碰过的字段前无法更改任何设置。现在只拒绝**变更**。
另修复：`OLLAMA_API_KEY` 可经 API 写入却在 `#/config` 中无字段，位于鉴权代理之后的 Ollama 用户根本无法在应用内设置。现已补上描述符（机密，新增 2 个 i18n 键 ×17），并以测试保证服务端 45 个键与界面 45 个描述符始终一致。**3013 → 3018 项测试。**

## [1.231.5] — 2026-09-09

**安全 — 关闭七项公告，其中一项为严重的远程代码执行。** 未改动任何源码文件。

### 安全
**自 Dependabot 合入：** `multer` 2.2.0 → 2.3.0（**高危** — 中止时文件描述符泄漏导致拒绝服务），以及站点的 `svgo` 4.0.2 → 4.1.0（**高危**与中危 — `removeScripts` 经命名空间与内容处理放过了可执行链接，且对 `foreignObject` 内可执行 HTML 的净化不完整）。
**随后由这两次合入触发的重新扫描又暴露五项。** 受影响区间均已被现有 `^` 覆盖，因此只需升级而无须改动清单：站点 **`astro` 7.1.0 → 7.3.2** ——**严重**，经 AVIF 图像优化的远程代码执行（7.2.8 修复），并附带剥离所配置 base 时缺少路径段边界检查导致的授权绕过（7.2.4）；**`sharp` 0.35.3 → 0.35.4** ——**高危**，两项 libheif 漏洞；**两份**清单中的 **`js-yaml` 4.3.1 → 4.3.2** ——**高危**，`maxTotalMergeKeys` 未对空合并源限制 CPU 占用。

### 说明
此处「仅依赖」并不等于「无风险」，所以没有仅凭审计结果就发布。`js-yaml` 解析应用读取的每一份配置，`multer` 接收每一次上传，而 Astro 跨了一个**次版本**。已验证：`npm run test:ci` **3013/3013**、浏览器检查 4/4，以及完整重建站点后仍是同样的 **86 个页面**、全部 **17** 份变更日志镜像和正确的 `facts.json`。`npm audit` 在两份清单中均报告**0 个漏洞**。源码未变，故测试基准数不变。

## [1.231.4] — 2026-09-07

**修复 — v1.231.3 修的是点击 Doctor 之后的残局，不是当时。移动端和桌面端再次报告。**

### 修复
**请求进行期间，`withSpinner` 仍在改写按钮。** v1.231.3 让它快照子节点并在 `finally` 里还原，节点确实回来了——但忙碌提示依旧执行 `button.replaceChildren(document.createTextNode('⏳ ' + original))`。于是在 `doctor.mjs` 运行的整段时间里，36 px 的方块装着 `⏳ 🩺Doctor` 文本，胀成压坏整行的宽胶囊；桌面端则挤成一团 `⏳🩺Doctor`。还原子节点修的是残局而非当时，而只看结束状态的断言正好放过它。**拥有元素子节点的按钮，现在完全不再被改写。** 它的提示交给 `.is-loading` 类，该类如今在 CSS 里把图标换成沙漏，因此尺寸不可能变化：320 px 下**静止 36 px、进行中 36 px、结束后 36 px**。纯文本按钮——24 处调用中的 23 处——保持原有的前置沙漏。

### 说明
教训在断言，不在代码。v1.231.3 的闸门点击 Doctor 后等待 `aria-busy` 清除才测量——按其构造，只能看到 `finally` 已经修好的状态。新的浏览器检查**自己把 promise 攥在手里**，不与真实请求赛跑，在飞行途中取样，要求宽度不变、两个 `<span>` 仍在、没有溢出。初稿是在真实点击后取样，请求跑得快就跳过断言——那是靠运气通过的锁。**3012 → 3013 项。** 两项新检查都已确认在 v1.231.3 的代码上失败。

## [1.231.3] — 2026-09-07

**修复 — 点击 Doctor 会把移动端顶栏撑坏。v1.231.2 发布当天由手机端报告。**

### 修复
**`UI.withSpinner` 压平了它正在装饰的那个按钮的子节点。** 它的忙碌提示走的是 `textContent`：保存 `button.textContent`，写入 `'⏳ ' + original`，再写回去。而给 `textContent` 赋值会把**所有**子节点替换成一个文本节点——偏偏 v1.231.2 刚刚把顶栏的每个操作拆成 `.btn-ico` 与 `.btn-label`，并在 `max-width: 900px` 下隐藏标签。于是第一次点击 Doctor 就永久摧毁了这两个 `<span>`：移动端规则再无可匹配之物，标签以裸文本回归，36 px 方块膨胀成压住主题开关的 `🩺Doctor` 胶囊。它不会自愈——只有刷新页面才能恢复标记。修复改为快照子**节点**并用 `replaceChildren` 还原；纯文本按钮，也就是 24 处调用中的绝大多数，行为与此前完全一致。
- **FIND-3 — 在 320 px 的手机上，搜索框被压缩到 8 px。** 把顶栏并成一行后，searchbar 成了唯一可伸缩的元素，而上个版本的变更日志自己就写着——*“可以缩到什么都不剩”*。它真的缩到什么都不剩：**320 px 下输入框 8 px，340 px 下 28 px**，21 个字符的占位符只露出一个。可点可输入，但已经不像一个输入框。`min-width` 救不了：320 px 的栏里有 32 px 内边距、40 px 菜单和 162 px 操作，拿不出 120 px。于是在 **420 px** 以下——输入框自己终于超过 100 px 的宽度——它藏到放大镜后面，轻点即展开占满整条栏：**320 px 下 182 px，360 px 下 222 px，390 px 下 252 px**，任何宽度都不溢出。展开按钮保持唯一的可访问名称，状态由 `aria-expanded` 传达；🔍/✕ 的切换是纯 CSS——没有 JS 往这个按钮里写，而正是这种写入在同一版本里毁了 Doctor。420 px 以上一切照旧。由针对线上构建的浏览器回归发现。

### 说明
这与 v1.231.2 记下的 `applyI18n()` 陷阱同属一类缺陷——把 `textContent` 赋给一个如今拥有元素子节点的元素——只是经由另一个调用方触发。教训可以推广：**一旦把某元素的内容拆进子 `<span>`，该元素上每一个既有的 `textContent` 写入方都成了潜伏的缺陷**，而这些写入方未必躺在你正在编辑的文件里。
双重锁定：在 `node:vm` 中以假按钮跑真实的 `withSpinner`，以及在 320 px 的真实浏览器里点击 Doctor。两者都已确认在旧代码上失败。**3009 → 3012 项测试**（浏览器那项为可选，不计入 `test:ci`）。

## [1.231.2] — 2026-09-06

**修复 — 移动端顶栏折成两行、服务器运行着较旧的上游、以及 cvstart.ru 的跳转对任何已包含语言的路径都返回 404。**

### 修复
- **移动端顶栏重新回到一行，操作按钮改为纯图标。** 操作按钮此前被强制放到整宽的第二行，手机上呈现为 `[☰ · 搜索]` 上方、`[🔔 🌙 Doctor 打开 Scan]` 下方——两枚宽胶囊独占一行。现在它们是铃铛和主题开关旁的 36 px 方块（🩺 / ⚡），在 320 px 下全部装进一行。是实测而非目测：文档宽度在 320、360、390、430 px 以及按钮从前会盖住搜索框的 560–760 px 整个区间内都与视口相等。**语言不再对移动端布局产生任何影响**，因为标签被隐藏、按钮是固定方块；超过 900 px 时标签原样回归。无障碍名称取自 `aria-label`，因为 `display:none` 的标签不参与名称计算。
- **发行版是按上游 1.32.0 构建的，服务器却运行着上游 1.31.0。** resumecraft.ru 的 `/api/health` 返回 `parentVersion: 1.31.0`。已同步 95 个运行时文件并删除 2 个。由针对线上环境的浏览器 QA 发现。
- **`cvstart.ru` 会给任何路径前置 `/ru/`，完全不看里面已经有什么。** `/ru/help` 变成 `/ru/ru/help`，`/en/help` 变成 `/ru/en/help`，两者皆 404。前者是从主站复制网址就会遇到的；后者更严重，直接让英语访客无法抵达英文版。现在已经带有 16 个语言前缀之一的路径原样通过，`en` 被去掉（cvstart.org 在根目录提供英文，没有 `/en/` 目录），其余仍然获得 `/ru/`。语言判断带锚定，因此 `/enterprise/` 不会被误认为 `en`。

### 变更
- **98 份被取代的 QA 提示已移入 `qa/archive/superseded-prompts/`。** `qa/README.md` 自 v1.137.0 起就写明顶层只保留当前发行版的提示——但该规则连续 95 个版本未被执行。
- **又有五份提示离开顶层——它们自称当前，实则不是。** `REGRESSION-FINAL.md` 自称“唯一权威、与版本无关的回归提示”，但其 §§11–15 是五份已关闭的周期台账（v1.55.x → v1.59.7），所声明的帮助基线为 **16 个语言下 28 H2 / 103 H3**，而今天是 32 / 122 与 17：照它执行的 QA 代理会在 §9 第一步就失败。`QA-REGRESSION-PROMPT.md` 顶着无版本的名字，内容却停在 v1.226.0。`QA-REGRESSION-PROMPT-v1.76.0-FULL.md` 被钉在 155 个版本之前，`REGRESSION-PROMPT-FINAL.md` 收束的是 v1.58.52 → v1.59.10 周期，而 `QA-FULL-REGRESSION.md` 以 v1.131.1 基线把它们统统重复了一遍。`PARENT-SYNC-WORKLIST-v1.26.0.md` 一并归档（上游已到 1.32.0），带日期的 `UX-AUDIT-2026-07-06.md` 移入 `reports/`。留下四份驱动，其自身基线已更新为 v1.231.2 的实测值。`qa/README.md` 重写为描述真实存在的目录。

### 说明
- 顶栏修复过程中踩到的两个优先级陷阱现已在 `app.css` 中注明：桌面端规则声明在 `max-width: 900px` 区块**之后**，因此在优先级相同时胜出——移动端的 `gap` 悄悄停留在 24 px（这正是 320 px 溢出的原因），图标也仍被隐藏（按钮成了空方块）。
- `applyI18n()` 会赋值 `el.textContent`，因此 i18n 键被移到内层 `<span>`；留在按钮上会在每次切换语言时抹掉图标。
- 上游的第一次 rsync 在一个本地已不存在的文件处**中途中断**，却仍报告成功。上游部署现在以校验和验证，绝不依据退出码。

## [1.231.1] — 2026-09-06

**修复 — v1.231.0 的计数替换改写了所有链接中的账号名。**

### 修复
- **每一个 `Fighter90` 链接都变成了 `Fighter92`。** v1.231.0 的来源计数替换在数字边界保护下把 `90` 换成 `92`；该保护能正确跳过 `290` 和 `901`，却会匹配 **`Fighter90`** 里的 `90`——它前面是字母，后面正好是词尾。**17 个 README、17 套帮助文档以及站点帮助镜像中的 562 处链接**，加上**维基里的 138 处**，都指向一个不存在的账号；`providers.png` 横幅和所有“在 GitHub 上编辑”的链接都是 404。是一位用户因为图片没显示而发现的。
- **五处历史版本引用发生偏移。** 同一版本的 `85 → 87` 替换把 `v1.85.0` 变成了 `v1.87.0`，凭空造出一个从未发布的版本。

### 说明
- 数字边界并不够：它保护一个数字不受相邻**数字**影响，却不能防止它是**标识符**或**版本字符串**的一部分。计数本身正确且未变动：92 个来源，87 个 EN 适配器，3009 项测试。
- 上游仓库未受影响：从未对其做过计数替换。

## [1.231.0] — 2026-09-05

**新增 — 来自上游 career-ops v1.32.0 的两个扫描来源（Collage、Telegram 严格）、Gem 的 REST 模式，以及移植过程在上游自身代码中发现的一处缺陷。**

### 新增
- **两个新来源，90 → 92**（87 EN + 5 RU），`ALL_ADAPTERS` **85 → 87**。均无需令牌。
  - **Collage**（`provider: collage`）— Collage HR 的公开招聘站点 API。路径片段是**租户自己选定的站点地址**，而不是从公司名生成的 slug，因此只从 `api:` 或 `secure.collage.co/jobs/<地址>` 的招聘页地址读取，**绝不猜测**：猜出的地址会去扫描别人的招聘板。
  - **Telegram（严格）**（`provider: telegram-channel`）— 读取与现有 `telegram` 来源相同的 `t.me/s/<频道>` 预览，但做的是**相反的取舍**：只有当帖子**点名雇主并且链接到职位页**时才成为一行。摘要帖、隐藏雇主的帖子和仅留联系方式的帖子都会被丢弃。覆盖率低是有意为之——上游测得 HR 运营频道有 32%–77% 的帖子通过——但每一行都带着真实的雇主和真实的链接。两者与上游一样同时提供。
- **Gem 新增了可选的 REST 模式**，与 GraphQL 路径并存（`api: https://api.gem.com/job_board/v0/<board>/job_posts`）。它绝不由招聘板 id 推导，GraphQL 路径仍为默认。移植自上游 #3783。

### 修复
- **`telegram-channel` 的地点过滤看不见西里尔字母，会把城市当作雇主。** 上游的 `LOCATIONISH_RE` 使用了仅适用于 ASCII 的 `\b`，它在西里尔字母旁边永远不会触发，因此其中所有俄语备选项（`москва`、`спб`、`офис`、`удалёнк*`）都不可达，`Senior Engineer | Москва` 会把**“Москва”当作雇主**返回。这正是整套归属策略要防止的“把错误字段当作事实”，而且恰好打中这个来源的主要受众：俄语频道用西里尔字母写地点。web-ui 的移植改用 Unicode 判定（`\p{L}` / `\p{N}`），与上游在下一个常量 `ROLE_WORD_RE` 中已有的做法一致。以一个在 ASCII 形式下会失败的测试固定。**上游尚未修复。**

### 说明
- **上游的 Workday 分面恢复工作（#3851、#3874）无需移植。** web-ui 根本不对 Workday 分页或分面拆分——只有 `offset: 0` 的一次 POST。与 v1.229.0 结论相同，且经过重新核实而非假定。
- **`providers/_types.js`** 仅文档变更（可选的 `Job.salary` 字段）。
- 测试：**2962 → 3009**（+47）。

## [1.230.0] — 2026-09-05

**变更 — 来自上游 career-ops v1.32.0 的三项提供方修复，以及移植过程本身暴露的一处缺陷。**

### 新增
- **Welcome to the Jungle 接受服务端 Algolia `filters` 表达式**（`wttj: { filters: "offices.country_code:FR AND contract_type:full_time" }`）。单靠关键词无法收窄一个全球招聘板：决定哪 `max_hits` 条结果返回的是 Algolia 的相关度排序，而不是扫描器的过滤条件——我们的过滤在其后才生效，作用于已经到达的内容，因此超出上限的部分无论多么契合都不可见。过滤器缩小的是结果**集合**本身而非重新排序，所以单次查询的上限从 200 提高到 1000。设置过滤器后 `queries` 变为可选：空查询意味着“凡通过过滤的一切”。移植自上游 #3757。

### 修复
- **当租户只是提供的职位比其自家横幅所声称的更少时，Radancy 不再警告“truncated”。** 上游将 `data-total-results` 与九个真实租户逐一核对：五个吻合，但四个——既有小租户也有超大租户——在没有任何重复行的情况下，交付的唯一职位比公告值少 10%–56%。横幅夸大了租户自身搜索索引实际提供的数量，因此拿累计计数与之比较，在多数招聘板上都是误报。警告现在只在**我们自己的** `max_jobs`/`max_pages` 终止遍历时出现，那是调用方唯一能采取行动的情形；`totalResults` 作为上下文保留在消息中，永不参与判断。移植自 #3839。
- **Radancy 为每一次 JSON 片段请求绕过缓存。** 该路由前的缓存层在部分租户上重放过期响应：同一条职位在数页之后再次出现，而另一条从未被返回——上游九次运行中九次复现。`Cache-Control`/`Pragma` 请求头毫无作用，而每次请求附带一个随机参数解决了问题。它必须按**调用**唯一，绝不能由页码推导：确定性的参数仍会给缓存一个稳定的键，从而保留同样稳定而错误的映射。移植自 #3839。
- **Greenhouse 的办公地点列表具有确定顺序。** Greenhouse 并不保证两次响应之间 `offices[]` 的顺序，而从 `/offices` 树中还原出的集合遵循其遍历顺序。两者都会进入职位的 `location` 字段，因此招聘板重排办公地点就会改写这个字符串——于是毫无变动的职位被读成新职位。上游修复了集合（#3839）；**web-ui 还存在同一缺陷的第二处实例，而上游没有**——当职位自身没有地点时启用的 `offices[0]` 回退——现在两处都已排序。

### 说明
- **上游的 iCIMS 修复（#3728）无需移植。** 它从详情页的 JSON-LD 中读取每一个 `jobLocation` 条目，因为部分租户会在真实地址之前放置一条全为 `UNAVAILABLE` 的条目。web-ui 的 iCIMS 从**列表页**的 HTML 中取地点，从不打开详情页。
- **上游对 `_http.mjs` 的改动无需移植。** 它抽出 `isRefusedRedirectError` 供第二个使用者共享判断，而 web-ui 没有那个使用者；`isRetryableError` 的行为不变。
- **位置感知去重（#3751）未做镜像**——它位于上游 CLI 的 `scan.mjs` 流水线中。
- 注册表不变：**90 个来源**（85 EN + 5 RU）。测试：**2956 → 2962**（+6）。

## [1.229.0] — 2026-09-03

**新增 —— 来自父项目 career-ops v1.31.0 的四个扫描来源：Built In、Feishu Jobs、Garena 和 MokaHR。**

### 新增
- **四个新来源，86 → 90**（EN 85 + RU 5），`ALL_ADAPTERS` **81 → 85**，四者皆无需令牌。**Built In** 覆盖美国科技招聘板的九个市场，且**不携带默认查询**：既无 `queries:` 也无 `categories:` 的条目什么都不扫描，并直说这一点 —— 共享来源不该背负某个人的搜索词；不在白名单上的市场会被**拒绝**，绝不悄悄换成全国板。**Feishu Jobs** 只接受两种主机形式，而两者通往职位页的**路径不同**。**Garena** 是单一公司：`office` 塑造的是**链接**，不是列表。**MokaHR** 返回**加密响应**（密钥随行的 AES-128-CBC 信封）—— 这是混淆而非安全，但不解密就什么也读不到。

### 修复
- **`htmlToText` 可能把不完整的标签开头泄漏进正文。** `safe &lt;img src=x onerror=1` 会输出为 `safe <img src=x onerror=1`。现在每次解码之后都跟一次清除，末尾的开头字符会失去尖括号。移植自父项目（#3491）。

### 说明
- **父项目的 Workday 修复一点也没有移植。** 它拯救的是后端在偏移 2000 处卡住分页的租户 —— 而 web-ui 根本不分页 Workday（`offset: 0` 上一次 POST，100 行）。
- **`verify-cv-facts` 与 `merge-tracker` 的修复无需移植**：web-ui 从父项目检出中运行这些脚本。
- 测试：**2909 → 2956**（+47）。
...
**新增 —— Telegram 频道作为扫描来源，以及 `/api/scan-results` 的分页查询模式。**

### 新增
- **Telegram 频道现在可被扫描。** 在 `portals.yml` 的 `telegram_channels:` 区块中列出，每个频道通过公开网页预览 `https://t.me/s/<频道>` 读取。注册表：**85 → 86 个来源**（EN 81 + RU 5），`ALL_ADAPTERS` **80 → 81**。

  Telegram 不提供 RSS，Bot API 也无法读取机器人未管理的频道 —— 两条显而易见的路都被堵死。`/s/` 预览是服务端渲染的纯 HTML：无需登录、无需 JS、无需 Cookie。已在 16 个频道验证：15 个在裸 GET 下各返回 20 条。

  `channel:` 接受任意写法。解析器锚定在 Telegram 改版最不可能挪动的三处：`data-post`、正文容器、ISO 日期 `<time datetime>`。
- **`GET /api/scan-results` 新增分页查询模式。** `?q=<文本>&limit=50[&region=ru|en][&set=filtered|fresh][&offset=N]` 返回 `{ total, returned, offset, limit, set, rows }`，其中 **`total` 是分页之前的计数**。不带参数时该路由仍返回完整快照 —— `#/scan` 表格依赖那个形状。

### 修复
- **Telegram 助手只凭一小部分数据作答。** 它的技能早已指向 `/api/scan-results`，但该路由只会返回整份快照 —— 真机上约 2 MB —— 塞不进智能体的上下文。面对「找出所有 Продуктовый менеджер」，它报出 **9** 条，而快照里有数百条，并说自己「在用一个受限的本地索引」。这是实话，原因是它看得见端点却消费不了。有了分页模式，同一个问题只花 2 KB，并返回诚实的 `total`。技能也一并重写：原文要求总结*新增*帖子，也就是指向 `fresh`（仅上次运行）而非 `filtered`（全部保留项）。

### 说明
- **频道帖子是散文，不是职位记录。** 没有结构化字段：标题取自第一行实质内容，公司／地点／薪资取自显式标签，正文放入描述。**把真实职位与广告、摘要区分开的工作交给现有的 `title_filter`** —— 就是你已为其他来源调好的那个。这些是线索，不是清单。
- **公司名绝不猜测。** 没有标签时该行归属于频道（`@rabotaphp`）：错误的雇主会作为事实进入追踪表。
- **解析为空是错误，而不是「没有职位」。** 对于私有或不存在的频道，t.me 以重定向作答；此时返回零行会被读成风平浪静的一天，并永久掩盖配置里的拼写错误。原列表中的 `jobGeeks` 正是如此（HTTP 302），因此刻意**未**配置。
- 远程判定使用 Unicode 词边界而非 `\b`：`\b` 仅支持 ASCII，西里尔词前的空格并不构成边界 —— 与 v1.227.3 标题过滤器踩中的是同一个陷阱，这次在发布前被测试逮住。
- 已配置并实测 15 个频道：**299 条帖子，15/15 频道**。新增 28 个测试。测试：**2865 → 2895**。
...
**修复 —— 区域扫描耗尽服务器内存，另有一个 Playwright 套件因自身拆解而随机失败。**

### 修复
- **区域扫描以 OOM 拖垮了服务器。** `ru-scanner` 会累积**所有查询的全部原始结果**，直到最后才做去重与过滤。查询列表刻意由近义词构成（`Golang`、`Go разработчик`、`Golang разработчик`、`Senior Go` ……），因此同一职位每个查询都会返回一次，数组持有的对象是唯一数量的数倍 —— 还带着职位描述，而描述占对象的绝大部分体积，且几乎全部会被过滤器丢弃。实测：21 个查询的真实运行占用堆 **742 MB**。服务器将 Node 堆上限设为 **490 MB**（V8 依据机器 956 MB 内存推算），于是扫描以 `FATAL ERROR: Reached heap limit` 拖垮服务 —— **一天四次**。现在去重与过滤按查询进行，同一次运行降至 **177 MB**，减少 76%。所报告的数字不变：唯一总数由 URL 字符串的 `Set` 承载。
- **`net::ERR_SOCKET_NOT_CONNECTED` 随机拖垮 CI。** 三个 Playwright 套件在断言无控制台错误前调用 `window.stop()` 中止在途请求。Chromium 会依 socket 所处状态，用**三种**不同名称报告这同一个动作：取消时为 `ERR_ABORTED`，在途时为 `ERR_EMPTY_RESPONSE`，socket 已消失时为 `ERR_SOCKET_NOT_CONNECTED`。共享过滤器只列了前两种。它在语言巡检的 `tr` 上暴露，而同样调用 `window.stop()` 的 `playwright-forms` 与 `playwright-smoke` 也只差一次倒霉的运行。

### 说明
- **为何现在才失败。** 加入产品经理相关检索后，查询从 14 条增至 21 条。旧实现的内存随结果总数增长，查询 +50% 便冲破了机器上限。
- **去重改动保持行为不变，这一点是测过的而非声称的。** `tests/ru-scanner-dedup-order.test.mjs` 锁定了那个别扭的方向 —— 后来的重复项若**未**通过过滤，必须把先前通过的那条挤掉 —— 并对旧实现做了 200 次随机等价校验。
- **控制台过滤器仍然狭窄。** 只有被取消的请求这一族是良性的；连接被拒、DNS 失败、连接重置、任何 5xx 和未捕获的 JS 异常仍会使断言失败。
- 两个假设经检验后被否决而非草率发布：Caddy 超时（配置干净，SSE 所需的 `flush_interval -1` 正确）与 Node 默认的 5 分钟 `requestTimeout`（缩比复现表明它不会中断长时间流式响应）。真正的证据是日志里的 `Reached heap limit`。测试：**2860 → 2865**。

## [1.227.4] — 2026-08-29

**修复 —— `content_filter` 仍然忽略 `word:` / `stem:`；v1.227.3 只修了标题过滤器。**

### 修复
- **`content_filter` 未遵循这两个前缀。** v1.227.3 让 `title_filter` 学会了上游的 `word:` / `stem:` 前缀，却把 `content_filter` 留在它自己的裸 `lower.includes(k)` 上，因此那里带前缀的条目仍被当作字面文本 `"word:java"` 来匹配 —— 同一个静默空操作，只是挪到了下一个过滤器。`content_filter` 读取职位**描述**，其默认一直是简单子串匹配：正因如此，一个裸的否定项 `java` 会排除掉所有仅仅提到「JavaScript」的职位；前缀就是让单个条目跳出该默认的手段。已移植上游的 `compileContentKeyword`：它共用前缀机制，但刻意**不**自动锚定短关键词 —— 标题过滤器锚定 2–3 字母缩写，是因为「Coordinator」中的「COO」永远是错的；而描述段落里的短字符串通常是有意为之（`aws`、`gcp`、`sql`、`go`）。

### 说明
- **在 v1.227.3 PR 的 AI 评审中发现**：评审注意到代码并未覆盖其自身注释所宣称的范围 —— 注释写着「`title_filter` / `content_filter`」，但接通的只有标题路径。现在注释已明确区分：两个过滤器都遵循前缀，只有标题过滤器会自动锚定缩写。
- **同样依据该评审核实且未变：** 把 2–3 字母缩写规则从 `\b` 换成 Unicode 边界，在 ASCII 上行为完全一致，仅在应当从严处更严 —— `COO_lead` 与 `coo1` 仍保留（下划线与数字在两种写法下都是单词字符），`Coordinator` 保留，「Директор COO」与「VP «Продукт»」被正确排除。已核对 14 个边界用例。
- 不带前缀的 `content_filter` 条目逐字节保持原有匹配，空的或缺失的描述仍然通过，空的 `word:` / `stem:` 不匹配任何内容而非全部匹配。`tests/content-filter.test.mjs` 新增 6 个用例。测试：**2852 → 2858**。

## [1.227.3] — 2026-08-29

**修复 —— 标题过滤器的 `word:` / `stem:` 前缀被静默忽略，使那些过滤行形同虚设。**

### 修复
- **web-ui 未实现上游的 `word:` / `stem:` 前缀。** `title_filter`（以及 `content_filter`）默认按不区分大小写的**子串**匹配，这正是为什么一个裸的否定项 `intern` 连「International Product Manager」和「Internal Tools Engineer」也一并排除。与其翻转该默认值（会破坏所有已配置的安装），上游选择让精确匹配按条目选择启用：`word:intern` 只匹配完整单词，`stem:agent` 必须位于词首且可继续（从而区分「Agentforce」与「Reagents」）。web-ui 两者皆无，于是把该条目当作**字面文本** `"word:intern"` 来匹配 —— 而没有任何职位标题含有这串字符。该行遂成为静默的空操作：同一份 `portals.yml` 在 CLI 中过滤正常，在这里完全不过滤，用户排除掉的实习岗位依旧全部出现在 `#/scan` 表中。已从上游 `title-keywords.mjs` 移植，包括支持 Unicode 的词边界（`\p{L}\p{M}\p{N}_`），而非仅支持 ASCII、会在带重音或西里尔字母的标题中于词中命中的 `\b`。

### 说明
- **在检查一份真实 `portals.yml` 时发现**，其中刚刚引入 `word:intern`，**正是**为了让裸的 `intern` 不再丢弃「International Product Manager」。该前缀在 CLI 中生效，web-ui 却悄悄保留了所有实习岗位。现在两者行为一致。
- **默认行为未变。** 不带前缀的关键词保持原有匹配方式：短语以及任何含非字母字符的条目（`.NET`、`SAP `、`L&D`）仍按子串匹配，2–3 个字母的缩写仍锚定词边界。空的 `word:` / `stem:` 视为笔误，**不匹配任何内容** —— 作为否定项，空模式会匹配所有标题，仅因一个多余的冒号就否决整次扫描。
- 2–3 字母缩写规则也改用同一 Unicode 边界：此前使用 `\b`，导致 `coo` 与 `word:coo` 在非 ASCII 标题上表现不一致 —— 同一条规则却有两种写法。
- `tests/title-filter.test.mjs` 新增 6 个用例。测试：**2846 → 2852**。

## [1.227.2] — 2026-08-29

**修复 —— 窄屏手机上 `#/config` 会横向溢出。**

### 修复
- **MOBILE-1：在 ≤ 352 px 的视口下 `#/config` 横向溢出。** 每个 select 类字段都带着**内联**的 `min-width: 300px`。内联样式优先于样式表，因此控件无法随容器收缩：在 320 px 视口下 select 的右边缘落在 353 px，页面出现横向滚动条，控件被挤出右边界。受影响的是全部 18 个提供商/模型下拉框（`cfg-llm-provider` 及所有 `cfg-*-model`）。现改为 `min-width: min(300px, 100%)` + `max-width: 100%`，只有空间足够时才应用 300 px 的舒适宽度。处于阈值及以下的设备：iPhone SE（一代，320 px）、老旧 Android、Galaxy Fold 外屏。

### 说明
- **此前所有移动端检查为何都没发现。** 360 px 和 375 px 恰好有足够余量掩盖溢出，而 Playwright 套件运行在 1280 px 宽的窗口中。CI 里从未有任何测试测量过窄于 375 px 的视口。新增的 `tests/playwright-narrow-viewport.mjs` 在 **320 px** 下遍历带表单的路由，出现任何横向溢出即失败，并在断言消息中指明最宽的元素；第二个用例锁定 `#/config` 的任何下拉框都不得越出视口。提交前已先验证它能复现该缺陷（`scrollWidth 353 > clientWidth 320`，18 个右边缘为 353 的下拉框）。Playwright：**99 → 101**。
- **已知且刻意暂未纳入门禁：280 px**（Galaxy Fold 外屏）仍会溢出约 5 px，但成因不同 —— 是配置卡片 `<details>` 区块内的长内容，以及 256 px 侧边栏对 280 px 视口，而非本次修复的内联 `min-width`。为一个主流手机都不会上报的宽度重新排版属于另一项工作；套件的 `WIDTHS` 列表已为此就绪。
- 同时更正了 v1.227.1 的 QA 提示：其中把跟进路由写成 `#/modes/followup`。mode 页面以裸 slug 注册（`Router.register(cfg.slug)`），因此路由是 `#/followup`，而 `#/modes/followup` 正确返回 404。仅文档改动。
- 单元测试套件维持 **2846**（新增覆盖属浏览器层面）。

## [1.227.1] — 2026-08-29

**修复 —— 帮助中的来源拆分与自身对不上，且 GitHub 横幅在应用内被当作原始标记打印出来。**

### 修复
- **§17 给出的总数与拆分不相加。** 所有帮助包都写着「`server/lib/sources/` 注册表自带 **85** 个 adapter —— **78 个英文 + 5 个俄文**」。78 + 5 = 83，而非 85；ja/ko 偏得更远，为 77（= 82）。每加一个来源只更新总数、不动加数，两个数字已经错开了好几个版本。现在 17 种语言的拆分统一为 **80 EN + 5 RU**。这不只是观感问题：`docs/help/<lang>.md` 是「Ask the docs」唯一的依据语料，因此询问支持多少来源的用户会从同一句话里得到两个互相矛盾的数字。
- **同一句话钉死了一个过期的版本锚点。** 「截至 **v1.213.0**，注册表 …」在 16 种语言中存活，而计数已经变动过两次（v1.219.0 为 83，v1.226.0 为 85）。该锚点在 v1.210.1 已被手工修过一次，随后又漂移。现改为不含版本的措辞并移除 —— 计数本身对照活的注册表校验，不会过期。
- **供应商横幅在 `#/help` 顶部被当作转义后的标记打印。** v1.225.0/v1.225.1 记录称它「因 `UI.md()` 不支持图片而在应用内被剥离」。这混淆了两件事：`UI.md()` 是转义优先的 —— 它在任何 markdown 变换之前转义源的每个字节，这是正确的 XSS 边界，但与移除恰恰相反。没有人移除横幅，因为从来没有代码尝试过；于是 17 种语言的 `#/help` 都以 268 个字符的字面 `<p align="center"><img src="https://…` 开头。新的共享入口（`server/lib/help-markdown.mjs::stripGithubOnlyBlocks`）在读取时丢弃独立的 HTML 图片块 —— `GET /api/help/:lang` 与 docs-assistant 语料两者皆然。文件中仍保留横幅，因此 GitHub 上的展示不受影响。
- **节奏看板的「#」列自 v1.117.0 起一直是空的。** 它读取 `entry.appNum`，而 `followup-cadence.mjs` 输出的是 `num` —— `appNum` 从未出现在负载中。现在两者都会读取。v1.227.0 的「Next up」行继承了同一处错误，一并修复。

### 说明
- **四处问题都因为无人把关而长期未被发现。** 帮助的门禁（`canonical-docs-coverage`, `help-ui`, `help-ru-config-section`）统计的是**标题** —— 32 H2 / 121 H3 —— 从来没有测试读过段落内部的数字，也没有测试看过帮助入口实际返回什么。两个新测试套件补上了这一环：`tests/help-source-counts.test.mjs`（所声明的总数等于 `SOURCES.length`；加数等于活的 EN/RU 划分且相加等于总数；段落不钉死版本）与 `tests/help-banner-strip.test.mjs`（横幅为 GitHub 保留在磁盘上、绝不在剥离后存活、每个帮助包的首个渲染行是其标题、剥离范围保持狭窄、两个消费方都经过它）。两者在提交前都已验证会针对这些具体缺陷失败。测试套件：**2837 → 2846**。
- 注册表、扫描器与路由契约均未改动；来源仍为 **85** 个（80 EN + 5 RU），`ALL_ADAPTERS` 仍为 **80**。
...
### 修复
- **属性内含 `>` 的 HTML 描述现在被干净地剥离。** Greenhouse、Recruitee 描述背后的标签剥离器用的是在第一个 `>` 就停下的朴素匹配,于是像 `<a title="salary > 100k">` 这样的标签会把尾部属性文本(`100k">`)留在描述里。现在它尊重带引号的属性值(单引号或双引号),在解码实体*之前*剥离标记(这样属性内被编码的引号不会成为假分隔符),并且仍在纯文本中保留字面的 `<>`。

### 说明
- 测试套件:**2742**。扫描来源保持 **82** 不变。本轮父项目没有其他新增:其余更改以只读方式中继(follow-up / weekly-digest / rejection-latency / salary-gap 修复无需 web-ui 改动)或不属于 web-ui 范畴(一个外部 X/Xquik 自带密钥插件、CLI/verify 修复)。可选项 —— SmartRecruiters 详情描述、detect-reposts 聚合器跳过、`stem:` / `word:` 标题前缀、DNS 重绑定加固 —— 仍在排队。



## [1.214.1] — 2026-08-25

**修复 — Ashby 的职位描述现在与 Greenhouse、Recruitee 截断到相同长度,因此内容筛选在三个板块上表现一致,且单个板块无法撑大扫描缓存。**

### 修复
- **Ashby 描述现在遵守与其他板块相同的长度上限。** v1.214.0 把 Ashby 的职位正文送入内容筛选,但与截断到 4000 字符的 Greenhouse、Recruitee 不同,Ashby 的正文是无上限直接映射(有些长 4 倍)。于是同一个 `content_filter` 关键词仅因它落在文本的位置不同,就可能在 Ashby 匹配而在 Greenhouse 不匹配,而单个板块可能给 `#/scan` 加载的扫描缓存增加数百 KB。Ashby 现在也截断到相同的 4000 字符(其文本已是纯文本,因此只截断,绝不剥离标签)。

### 说明
- 测试套件:**2738**。扫描来源保持 **82** 不变。
...
**fix(i18n): I-2 — Saved-research 卡片日期改用 `Intl.RelativeTimeFormat` 按语言本地化。** [public/js/views/deep.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/deep.js#L57-L82) 的 `formatRelative()` 之前在任何语言下都硬编码英文 `today` / `1d ago` / `Nd ago`。改为 `Intl.RelativeTimeFormat(I18n.getLang(), { numeric: 'auto' })` — 浏览器原生本地化字符串("今天/昨天/N 天前", "сегодня/вчера" 等)。超过 7 天的日期回退到 `Intl.DateTimeFormat(locale, { dateStyle: 'medium' })`。912 → **913** 单元。(I-2)

---

## [1.58.16] — 2026-05-20

**fix(ui): 品牌按钮悬停闪烁(用户反馈)。** 原因:`.btn-primary` / `.btn-danger` 默认背景为 `linear-gradient(...)`,`:hover` 把它换成纯色 `var(--rausch-dark)`。CSS 无法在渐变↔纯色之间补间,180ms `transition: background` 会"卡顿",用户看到白/粉色的瞬闪。修复 [public/css/app.css](https://github.com/Fighter90/career-ops-ui/blob/main/public/css/app.css):悬停时保留渐变,改用 `filter: brightness(0.92)` 减暗 — `filter` 在所有浏览器中都能平滑补间。`.btn` 的 `transition` 列表新增 `filter var(--transition)`,让减暗带动画效果。911 → **912** 单元。(用户反馈)

---

## [1.58.15] — 2026-05-20

**fix(a11y/i18n): I-1 — 顶栏搜索的 `aria-label` 和视觉隐藏 `<label>` 现已本地化。** 之前所有 8 种语言下,屏幕阅读器听到的都是英文 aria-label。[public/js/app.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/app.js#L4-L29) 新增通用 `data-i18n-aria-label` 钩子 — `applyI18n()` 在每次语言切换时更新 `aria-label`,与 `data-i18n` / `data-i18n-placeholder` 对称。新增 2 个 i18n 键(`top.search.aria`, `top.search.label`)覆盖 8 种语言。钩子可被任何未来控件复用。910 → **911** 单元。(I-1)
...
**fix(ux): M-7 — 成本提示跟随当前活跃提供商(OpenRouter 不再回退到伪造数字)。** `UI.providerCostHint()` 已经通过 `/api/status/providers` 实现 provider-aware,但 [public/js/api.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/api.js#L623-L676) 中的映射只列出 `anthropic`/`gemini`/`openai`/`qwen`。v1.57.0 加入 OpenRouter 为第 5 个提供商后,它会落到通用回退 0.03 并把名字显示为小写字面值 `openrouter`。现在 EST 加入 `openrouter: null`(由路由选择模型,费用因此而异),`=== null` 分支输出本地化的"cost varies (router picks)",而不是误导性的 `~$0.03/eval`。NAME 加入 `openrouter: 'OpenRouter'`。新增 i18n 键 `cost.varies` 覆盖 8 种语言。907 → **908** 单元。(M-7)

---

## [1.58.11] — 2026-05-20

**fix(ux): M-4 — 已保存研究卡片的标题↔日期间距改为结构化 CSS(原先为内联 margin)。** v1.58.3 MASTER 回归确认部分卡片显示为 `software-engineer-generaltoday`(标题与日期之间无空格),而另一些正常。原因:旧代码依赖两个裸 `<span>` 间的 `style="margin-left: 8px"`,在某些条目中折叠。修复:[public/js/views/deep.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/deep.js#L34-L55) — 将两个 `<span>` 替换为 `.saved-card__title` + 语义化 `<time class="saved-card__date" datetime="…">`,外层包裹 `.saved-card` flex 容器。间距由 `gap: var(--space-2, 8px)` 控制 → 不会再折叠,同时获得 `<time>` 的 a11y/SEO 语义。906 → **907** 单元。(M-4)

---

## [1.58.10] — 2026-05-20

**fix(ux): M-2 — 在打开任何结果模态框前先清空进度 toast。** 在 `#/cv` 点击 `sync-check` 时,"Running cv-sync-check.mjs…" toast 仍保留在右下角,而结果模态框已经打开 — 二者争夺注意,在窄屏上视觉重叠。Health 页面的 Doctor / verify-pipeline 按钮原本就在 `UI.modal()` 之前显式调用了 `UI.dismissToast()`;cv.js 的 sync-check 是唯一遗漏的入口。修复:[public/js/api.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/api.js#L272) — `UI.modal()` 现在将 `dismissToast()` 作为第一条可执行语句调用(边界处的纵深防御)。同时把 cv.js 中硬编码的英文字符串改为 `t('cv.syncCheckRunning')` / `t('cv.syncCheck')`,满足 BUG-008 不变量(模态框标题 == 本地化按钮标签)。在 8 种语言中新增两个 i18n 键。905 → **906** 单元。(M-2)

---

## [1.58.9] — 2026-05-20

**fix(a11y): M-1 — 在表单字段上恢复可见的 `:focus-visible` 焦点环(WCAG 2.4.7 Level AA)。** v1.58.3 MASTER 回归确认 `getComputedStyle(focusedInput)` 返回 `outline: rgb(255,255,255) none 1.5px` — `none` 关键字将每个字段的焦点环宽度坍缩为 0 px。根因:`.input, .textarea, .select { outline: none }` 与 `.searchbar input { outline: none }` 的基础规则比全局 `*:focus-visible` 优先级更高,悄悄移除了每页 88 个可聚焦元素的键盘焦点环。修复在 [public/css/app.css](https://github.com/Fighter90/career-ops-ui/blob/main/public/css/app.css) — 显式添加 `.input:focus-visible/.textarea:focus-visible/.select:focus-visible` 与 `.searchbar input:focus-visible` 规则,带 `outline: 2px solid var(--rausch)` + 半透明 box-shadow;鼠标焦点(`:focus`)保持干净。904 → **905** 单元(静态契约守卫);Playwright **60 → 61**(Tab 遍历)。(M-1)

---

## [1.58.8] — 2026-05-20

**feat(health): 在 `#/health` 显示 `OPENAI_API_KEY` / `QWEN_API_KEY` / `OPENROUTER_API_KEY`(与 `GEMINI_API_KEY` 类似)。** v1.57.0 引入 OpenRouter 作为第 5 个 headless live-eval 供应商;v1.55.3(UX-2)上屏 4 供应商引导。但 `#/health` 页面仅显示 `GEMINI_API_KEY` 与 `ANTHROPIC_API_KEY` — 其余三个尽管 `/api/status/providers` 已路由,却在 Health 上不可见。用户要求:将"set / unset (manual mode)"行模式扩展到全部 headless 供应商。[server/lib/routes/health.mjs](https://github.com/Fighter90/career-ops-ui/blob/main/server/lib/routes/health.mjs#L57-L71) 新增 3 个可选检查行,接入与 `/api/status/providers` 相同的 `isUsableKey` 闸。Health 视图迭代 `body.checks`,因此无需新增 8 语言字符串。903 → **904** 单元。(用户请求)

---

## [1.58.7] — 2026-05-20

**fix(security): NEW-2 — `isValidJobUrl` 现在拒绝成对的模板占位符语法(`${…}`、`{{…}}`),与错误消息一致。** `POST /api/pipeline` 的 400 响应声称 *"contain no script or template characters"*,但 v1.58.3 MASTER 回归确认:实际仅 ASP/EJS 形式的 `<%…%>` 被 `[<>"'`\\\s]` 守卫顺带拦截。JS 模板字面量 `${TEST}` 与 Mustache/Handlebars `{{TEST}}` 直接通过 — 正则与错误消息的语义不一致。fix-prompt 选项 A(把正则收紧以匹配消息):在 [server/lib/security.mjs](https://github.com/Fighter90/career-ops-ui/blob/main/server/lib/security.mjs) 新增 `TEMPLATE_PATTERNS` 数组,经 `hasTemplatePlaceholder(url)` 在 `new URL(…)` 前检查。**只拒绝成对** 的占位符(`{normal}` 等单括号 ATS 路径继续接受)。901 → **903** 单元。(NEW-2)

---

## [1.58.6] — 2026-05-20

**fix(a11y/i18n): BUG-008-tb — 顶栏 `Doctor` 模态框标题现在与本地化按钮标签一致。** 在 v1.58.0 关闭的台账 BUG-008(*"模态框标题 == 本地化按钮标签"*)只覆盖了 Health 页面入口。v1.58.3 MASTER 回归发现**顶栏**入口仍违反不变量:无论 UI 语言为何,点击顶栏 `Doctor` 打开的模态框标题始终是 `doctor`(小写英文)。修复:[public/js/app.js:118](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/app.js#L118) 将字面量 `'doctor'` 替换为 `I18n.t('top.doctor', 'Doctor')`。`top.doctor` 键在 8 种语言中均已存在(EN `Doctor` · ES/pt-BR `Diagnóstico` · KO `진단` · JA `診断` · RU `Диагностика` · zh-CN `诊断` · zh-TW `診斷`),与按钮通过 `data-i18n="top.doctor"` 声明的键相同。`tests/qa-report-fixes.test.mjs` 新增静态契约守卫。900 → **901** 单元;Playwright 60/60。(BUG-008-tb)
...
SSE 客户端(`public/js/api.js:156`)在第一个 `done` 事件就关闭了 `EventSource`,而服务端在 `source=both` 模式下每阶段各发一个 `done`。RU 阶段刚启动就被取消。修复:服务端在每个 `done` 上标记 `final: true|false`,客户端仅在 `final !== false` 时关闭。向后兼容 — 不设置 `final` 的单阶段生产者继续保持原行为。**547 → 558** 单元 + 验收测试(+11 新增)。完整细节见 [`CHANGELOG.md`](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.md)。
...
在 §5(Portals & sources)内新增 ### 子节「配置俄文门户 — 详细设置指南」:5 个来源的清单表(含认证与地理限制)、定位与编辑 `portals.yml` 的分步说明、完整的 5 来源 YAML 示例、与 negative 列表的冲突及其修复示例、临时禁用某个来源的方法、通过 🌐 Scan 与 SSE 日志验证设置的方法。§17(v1.29.0 上线)覆盖开发者流程,§5 v1.29.1 覆盖最终用户流程。**540 → 547** 单元 + 验收测试(+7 新增)。完整细节见 [`CHANGELOG.md`](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.md)。
...
- 测试:**520 → 540**(+20)。完整细节见 [`CHANGELOG.md`](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.md)。
...
v1.28.1 之前,`Router.go('/evaluate?url=…')` 产生的 hash 经 `split('/')` 后第一段是字面量 `"evaluate?url=…"`,永远不会匹配已注册的路由 → `__not_found__`(404)。一行修复:在按名称拆分前先 `hash.split('?')[0]`。覆盖两个已报告点击:`#/pipeline → ▶` 与「App settings → Modes」。`/api/health` 中可选的 `HH_USER_AGENT` 行被移除(俄国外 403 提示仍保留在 help-bundle §16 中,扫描时 stderr 也仍会提示)。**515 → 520** 单元 + 验收测试(+5 新增)。完整细节见 [`CHANGELOG.md`](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.md)。
...
- **506 → 515** 单元 + 验收测试(+9 新增)。Playwright 32/32 无变化。完整细节见 [`CHANGELOG.md`](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.md)。
...
侧边栏中，品牌徽标（`<a class="logo" href="#/dashboard">`）和第一个导航项指向同一路由。屏幕阅读器会重复念出「Dashboard」两次，键盘用户多出一个无意义的 tab 焦点。徽标块现在是普通的 `<div class="logo">`，仅导航项保留为 `#/dashboard` 的唯一链接。**506 / 506** 单元测试 + **32 / 32** Playwright — 无变化。完整细节见 [`CHANGELOG.md`](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.md)。
...
v1.26.0 中 `.btn` 的 `min-height: 44px` 声明缺失,头部按钮渲染为 39-41 px(违反 WCAG 2.5.5)。v1.26.1 恢复 44 px 下限 + `flex-shrink: 0` + `line-height: 1.2`。**502 → 506** unit,Playwright 32/32 不变。详细见 [`CHANGELOG.md`](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.md)。
...
按 v1.25 待办事项采用四级测试金字塔(unit → functional → acceptance → e2e)。新增 22 个测试,覆盖 v1.25 的最大空白(jds.mjs 61.64 % → 100 %,auto-pipeline 拒绝路径)。新建 `tests/acceptance/` 目录用于跨端点用户旅程测试。**480 → 502** unit + acceptance,Playwright 32/32 不变。完整细节见 [`CHANGELOG.md`](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.md) 和 [`docs/architecture/TESTING.md`](https://github.com/Fighter90/career-ops-ui/blob/main/docs/architecture/TESTING.md)。
...
- **`fix(auto-pipeline): G-014 — honour mode:'manual' short-circuit`** ([`server/lib/routes/auto-pipeline.mjs:158-195`](https://github.com/Fighter90/career-ops-ui/blob/main/server/lib/routes/auto-pipeline.mjs#L158-L195)) — v1.25 之前,该路由总是调用一次 LLM。传入 `mode: 'manual'`(自 v1.10.2 起对齐 `/api/evaluate` 的约定)会被静默忽略,请求会在 Anthropic 端口阻塞 1–3 分钟。新版处理器:
...
- **测试** — [`tests/auto-pipeline-manual-mode.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/auto-pipeline-manual-mode.test.mjs) 中 3 个用例分别验证:(1) `mode: 'manual'` 在 2 s 内返回并完整下发 5 个 step 键;(2) 即便设置了 `ANTHROPIC_API_KEY`(原始症状),短路仍会触发;(3) 旧版 `evalMode: 'manual'` 调用方继续正常工作。
...
- **`fix(dashboard): dedup ✨ glyph in auto-pipeline button label`** ([`public/js/lib/i18n-dict.js:219`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/lib/i18n-dict.js#L219)) — `dash.autoPipeline` 在每种语言的字符串中均以 `✨` 起头,而 `public/js/views/dashboard.js:58` 又在视图层再次前置一个 `✨`,导致按钮渲染为 `✨ ✨ Auto-pipeline …`。v1.25 在每种语言的 DICT 条目中去除前导字形,视图层的前缀成为唯一来源。同一次审计扫了整套 i18n 资源包,未发现其他双字形模式。
...
- **`fix(config): G-015 — replace removed Element.prototype.also call in config.js`** ([`public/js/views/config.js:371`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/config.js#L371)) — v1.22.0 N-2 移除了 `Element.prototype.also` 全局猴子补丁,并将 `cv.js` 迁移为自由语句模式,**但漏掉了 `config.js`**。结果是任一语言版本下 `#/config` 首次调用即崩溃并抛出 `c(...).also is not a function`。v1.24.1 沿用 `cv.js:188-201` 的同款迁移模式 — 将树根抽取为 `const root = c(...)`,在其后独立执行激活语句块,最后 `return root;`。
...
- **`docs(help): refresh en.md from 5 canonical career-ops.org/docs URLs`** ([`docs/help/en.md`](https://github.com/Fighter90/career-ops-ui/blob/main/docs/help/en.md)) — v1.24 之前 EN 资源包为 1113 行,虽在 front-matter 中列出 5 个规范 URL,但正文未做展开。v1.24 经 WebFetch 抓取全部 5 个 URL,并对对应的 H2 区段加深内容:
...
- **`docs(translate): CHANGELOG.ru.md retry agent — full body translation`** ([`CHANGELOG.ru.md`](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.ru.md)) — v1.23.0 交付时 RU CHANGELOG 重试代理仍在执行(首次曾因 socket 错误失败,经重新调度)。v1.24 接收该代理 1542 行的完整译文:从 v1.23.0 到 v1.6.0 的每一条目均落地为出版级俄语正文,EN 原文性质的占位说明全部清除。文体纪律对齐 v1.22.0 README 质量复核:以 "функциональность" / "возможности" / "поведение" 替换生硬的 "функционал";以 "через" / "с помощью" 替换 "при помощи";主动语态优先;"эндпоинт"、"лимит запросов"、"состояние гонки"、"санитайзинг" 为规范术语;英文缩写(TOCTOU、CSP、SSRF、WCAG、ATS、JD、SSE、REST、API)按原文保留。
...
- **`fix(client): reset health-poll cadence + visibilitychange eager re-check`** ([`public/js/api.js:21-91`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/api.js#L21-L91)) — v1.22.0 的 M-6 指数退避方向正确(3 s → 6 s → 12 s → cap 15 s,自原 60 s 上限下调),但在飞中的 `setTimeout` 仍锁定了上一次设置的延迟。若服务器在 t=0.1 被杀且首次 ping 落在 t=3,该次会失败,延迟翻倍到 6,下一次恢复探测要拖到 t=9 才发出。烟囱 E2E 中 "Flow 2a:服务器宕机时连接横幅出现、恢复后隐藏" 仅等 4 s,因此在 `main` 上转红。
...
- **`refactor(client): split DICT into i18n-dict.js (data) + i18n.js (logic)`** — v1.23 之前 `public/js/lib/i18n.js` 共 639 行。其中绝大部分(23–586 行)是 `DICT` 翻译表 — 纯结构化数据。v1.23.0 将其抽出为 [`public/js/lib/i18n-dict.js`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/lib/i18n-dict.js)(578 行,按 CLAUDE.md "Exempt from these limits: generated files, migrations, test fixtures, lock files, vendored code" 条款豁免行数约束 — 翻译表归入 fixtures),余下 [`public/js/lib/i18n.js`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/lib/i18n.js) 缩至 86 行的纯模块逻辑(远低于 400 行目标)。
- **加载契约:**`i18n-dict.js` 向 `window.__I18N_DICT = { … }` 写入数据,随后 `i18n.js` 在既有 IIFE 中读取。[`public/index.html`](https://github.com/Fighter90/career-ops-ui/blob/main/public/index.html) 按顺序加载二者 — `i18n-dict.js` 先于 `i18n.js` — 确保 IIFE 构造时 DICT 已完全填充。缺失字典的兜底:任一 `t()` 调用回退至内联 fallback 或原始 key,将配置异常显式暴露而不导致 SPA 崩溃。
- **测试管道同步更新:**[`tests/i18n-coverage.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/i18n-coverage.test.mjs)、[`tests/help-ui.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/help-ui.test.mjs)、[`tests/canonical-docs-coverage.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/canonical-docs-coverage.test.mjs) 现在将两份文件一同载入测试 VM 上下文(或拼接源文本供正则扫描),保留全部既有断言。
...
- **`docs(readme): wire each locale README at its locale-specific PNG`** — v1.23 之前仅 `README.pt-BR.md` 引用了 `dashboard-pt-BR.png`,其余 6 个非英文 README 仍指向 `dashboard-en.png`。截图已由 v1.22.0 周期中的 [`scripts/capture-dashboard-screenshots.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/scripts/capture-dashboard-screenshots.mjs) 生成并落于 `images/`,但未投入使用。v1.23.0 将每份 `README.{es,ja,ko-KR,ru,zh-CN,zh-TW}.md` 第 14 行指向其本地化 `dashboard-<locale>.png`。
...
- **`fix(security): M-4 — 支持 HTML 实体识别的 stripDangerousMarkdown`** ([`server/lib/security.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/server/lib/security.mjs)) — v1.22 之前的正则将 `<script>`、`javascript:`、`on*=` 作为字面子串匹配,因此 `&lt;script&gt;`、`java&#115;cript:` 以及 `<img src="data:image/svg+xml,<svg onload=…>">` 可以绕过。新版本会在执行剥离正则之前,先解码 `&lt;`、`&gt;`、`&amp;`、`&quot;`,以及十进制(`&#NN;`)和十六进制(`&#xHH;`)字符引用。[`tests/cv-xss-bypasses.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/cv-xss-bypasses.test.mjs) 中 11 个用例验证此行为。真正的防线仍然是客户端 `UI.md` 先转义再渲染的管道;此项强化的是静态文件层。

- **`fix(security): L-2 — 批处理运行器使用 bash --noprofile --norc`** ([`server/lib/routes/batch.mjs:108`](https://github.com/Fighter90/career-ops-ui/blob/main/server/lib/routes/batch.mjs#L108)) — `spawn('bash', [PATHS.batchRunner, ...])` 此前会继承用户的 `~/.bashrc`。恶意 rc 文件可能影响执行。改为 `spawn('bash', ['--noprofile', '--norc', PATHS.batchRunner, ...])`。

### 🔒 韧性

- **`fix(client): M-6 — 健康探测使用指数退避`** ([`public/js/api.js:22-48`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/api.js#L22-L48)) — 断连状态下的轮询此前会在一夜之间对死掉的服务器发起 28,800 次请求。现改为 3s → 6s → 12s → 24s → 60s,首次返回 2xx 后重置为 3s。实现采用 `setTimeout` 链(而非 `setInterval`),以便每一步都能采用新的延迟。

- **`fix(client): M-5 — Safari 隐私模式 localStorage 守卫`** ([`public/js/lib/i18n.js:572-583`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/lib/i18n.js#L572-L583)) — Safari 隐私模式会对每次 `localStorage.getItem/setItem` 抛出 `SecurityError`。加载期间的 IIFE 此前会让整个 i18n 模块崩溃,导致 SPA 渲染原始键名。现已为两处调用都包了 try/catch,并回落到 `detect()` 浏览器语言检测。

- **`fix(server): M-2 — 预览出站请求的响应体大小上限(测试 + 验证)`** — v1.21.0 的 `safeGet` 已经流式读取分块并在 `opts.maxBytes` 处截断。v1.22 在 [`tests/ssrf-redirect-rebind.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/ssrf-redirect-rebind.test.mjs) 中新增一条回归测试以锁定契约:上游 100 KB + 上限 4 KB → 响应 ≤ 4 KB。

- **`fix(client): L-5 — scan.js 在 hashchange 时清除 setTimeout`** ([`public/js/views/scan.js:6-22, :113-120`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/scan.js#L6-L22)) — 扫描完成后 300 ms 的 `refreshResults()` 计时器此前会在用户于该窗口期内离开 `#/scan` 时泄漏。现在句柄已被捕获并在 `__cancelActiveScanPoll` 中清理。

- **`fix(client): L-4 — 多行 SSE data: 拼接器`** ([`public/js/lib/auto-pipeline.js:158-176`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/lib/auto-pipeline.js#L158-L176)) — SSE 解析器此前使用 `match()`(单行)。根据规范,一个事件可携带多行 `data:`,消费方需用 `\n` 拼接。服务器当前发送的是单行 JSON,所以旧代码尚能工作 — 但对未来任何多行负载都是脆弱的。

### ♿ 无障碍

- **`feat(a11y): M-3 — WCAG 1.4.1 在分数胶囊与连接横幅上补充冗余视觉提示`** ([`public/css/app.css:602-625, :812-822`](https://github.com/Fighter90/career-ops-ui/blob/main/public/css/app.css#L602-L625)) — score-high / score-mid / score-low 此前仅靠色相(红/琥珀/绿)传达状态,无法感知色相的用户没有备用提示。每个分级现在通过 `::before` 获得冗余字形(✓ / ◐ / ○)。连接横幅在离线状态下增加前导 `⚠` 字形。渲染位置未动 — 纯 CSS 加固。

- **`feat(a11y): M-1 — 每个 mode-page 字段都有内联提示段落`** ([`public/js/views/mode-page.js`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/mode-page.js)、[`public/js/lib/i18n.js`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/lib/i18n.js)) — v1.20.0 为每个 mode-page 字段接通了 `htmlFor → id`,但没有携带内联提示文案;仅 README 教程说明了字段意图。v1.22.0 新增 19 个提示 i18n 键 × 8 个语言 = **152 条新译文**,并让 `field()` 构造器为每个字段渲染一个 `<p id="…-hint">` 并通过 `aria-describedby` 关联。屏幕阅读器用户在输入聚焦时能听到提示。

- **`fix(a11y): M-7 — UI.el() 的 htmlFor 别名空值守卫`** ([`public/js/api.js:194-198`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/api.js#L194-L198)) — `htmlFor: null` 此前会渲染成字面量 `for="null"`。一行修复,镜像缺省分支的 `v != null && v !== false` 守卫。
...
- **`fix(server): L-3 — Windows 安全的入口点检查`** ([`server/index.mjs:159-163`](https://github.com/Fighter90/career-ops-ui/blob/main/server/index.mjs#L159-L163)) — `import.meta.url === \`file://${process.argv[1]}\`` 在 Windows 上对盘符和反斜杠处理有误。替换为 `fileURLToPath(import.meta.url) === path.resolve(process.argv[1])`。

- **`refactor(client): N-2 — 移除 Element.prototype.also 猴子补丁`** ([`public/js/views/cv.js:188-201`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/cv.js#L188-L201)) — 全局 DOM 原型污染。替换为局部变量持有树根。

- **`test(canary): M-8 — 已退役 /api/scan-ru/config 的 404 回归测试`** ([`tests/scan-consolidated.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/scan-consolidated.test.mjs)) — v1.20.0 退役了该别名但未加守护测试。新增三行,与 v1.18 退役测试保持一致。
...
- **`docs(qa): scenario 31 — career-ops.org/docs 对齐`** ([`qa/claude-cowork-browser-test-prompt.md`](https://github.com/Fighter90/career-ops-ui/blob/main/qa/claude-cowork-browser-test-prompt.md)) — 6 个新子测试(31.1–31.6)验证 UI 与 career-ops.org/docs 五份官方指南所述行为一致:分数阈值、扫描流程(单按钮)、申请流程(清单而非自动提交)、批量流程(TSV 编辑器)、Playwright 安装(优雅降级)、帮助文档覆盖(5 个 URL × 8 个语言)。
...
**两次独立代码评审带来的安全 + 并发 + 无障碍打磨。** [`docs/specs/V1.20.1-BACKLOG.md`](https://github.com/Fighter90/career-ops-ui/blob/main/docs/specs/V1.20.1-BACKLOG.md) 中的 7 个发现一次性发布:1 个阻塞项(DNS-rebind TOCTOU)、6 个高严重度缺陷(路径遍历净化分散、LAN 部署的流控空缺、并发写入竞态、i18n 覆盖漏洞、悬空的 aria-describedby、缺失的 label 关联)。新增 34 个测试;基线从 427 → 461 单元 + 32/32 Playwright。每项修复都附带一条命名的回归测试。

### 🛡️ 安全

- **`fix(security): B-1 — 通过 safe-fetch.mjs 关闭 DNS-rebind TOCTOU`** ([`server/lib/safe-fetch.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/server/lib/safe-fetch.mjs)) — 此前的模式是做一次显式 `dnsLookup` 用于校验,然后让 `fetch()` 自己再做一次独立的解析。掌握 TTL=0 的 DNS-rebind 攻击者可以在第 1 次解析返回公网 IP、第 2 次解析返回 `127.0.0.1` / `169.254.169.254` 或某个 LAN 地址,从而绕过 `isPrivateOrLoopbackHost`。新的 `safeGet` 只解析一次,通过 node:http(s) 把 TCP 连接锁定到那个具体 IP,并设置 SNI/Host 让证书校验仍指向原始主机名。被 `/api/pipeline/preview` 和 `/api/auto-pipeline` 使用。解析失败时 fail-CLOSE(逆转了此前 `try { … } catch { /* fall through */ }` 的语义)。由 [`tests/ssrf-redirect-rebind.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/ssrf-redirect-rebind.test.mjs) 中 8 个新测试验证。

- **`fix(security): H-4 — 在 10 条路由间统一 sanitizePathName`** ([`server/lib/security.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/server/lib/security.mjs)) — 裸正则 `replace(/[^\w\-.]/g, '')` 在 `jds.mjs`、`content.mjs`、`reports.mjs`、`llm.mjs`、`runners.mjs` 中被复制了多份且保留了 `.` 字符,所以 `..pdf`、`....md`、以点开头的文件名都能存活。只有 `reports.mjs::sanitizeSlug` 是正确的。v1.21.0 将正确版本(`sanitizePathName`)提升到 `security.mjs`,删除了 10 处错误副本,并对空结果返回 400。由 [`tests/path-traversal.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/path-traversal.test.mjs) 中 12 个测试验证。

- **`fix(security): H-5 — 在公开绑定时对 LLM 端点进行速率限制`** ([`server/lib/rate-limit.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/server/lib/rate-limit.mjs)) — `/api/evaluate`、`/api/deep`、`/api/mode/:slug`、`/api/auto-pipeline` 之前没有按 IP 的限流。Loopback 用户不受影响;LAN 暴露的部署(`HOST=0.0.0.0`)每 IP 每分钟 10 次请求,溢出时携带 `Retry-After` 与 `X-RateLimit-*` 头。通过 `LLM_RATE_LIMIT="N/Ws"` 配置。这是 v2.0 P-12 鉴权门之前廉价的过渡防御。由 [`tests/rate-limit.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/rate-limit.test.mjs) 中 6 个测试验证。
...
- **`fix(a11y): H-1 — batch.js 提示段落补上 id="batch-tsv-hint"`** ([`public/js/views/batch.js`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/batch.js)) — v1.20.0 给 TSV 文本框加了 `aria-describedby="batch-tsv-hint"`,但从未给提示 `<p>` 配上对应的 `id`。屏幕阅读器无可朗读。已修复。

- **`fix(a11y): H-2 — batch-parallel / batch-min-score 标签的 htmlFor`** ([`public/js/views/batch.js`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/batch.js)) — v1.20.0 给 4 个输入新增了 id,但 label 与之并未以编程方式关联。WCAG 3.3.2 现已满足。

- 在 [`tests/a11y-form-wires.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/a11y-form-wires.test.mjs) 中新增静态分析守护测试 — 遍历所有视图文件,断言每个 `aria-describedby` / `htmlFor` IDREF 都指向同级的 `id:` 声明。CI 期可捕获笔误级别的回归。

### 🌐 i18n

- **`fix(i18n): H-3 — v1.20.0 引入的 13 个键对 7 种语言静默回退到 EN`** ([`public/js/lib/i18n.js`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/lib/i18n.js)) — `pipe.filter`、`pipe.count`、`pipe.preview*`、`pipe.openTab`、`pipe.evaluateAll*`、`eval.jdHint`、`batch.parallelAria`、`batch.minScoreAria`,以及 `common.delete`、`config.group{Core,Runtime,Regional}`、`config.profileEmpty`、`config.viewProfile`、`scan.atsBadge`、`scan.regionalBadge` 通过 `t('key', 'EN fallback')` 引用却从未加入 DICT。俄语、日语、中文屏幕阅读器用户听到的 `aria-label` 是英文 — 直接抵消了 v1.20.0 宣称的 WCAG 3.3.2 收益。v1.21.0 添加了全部 19 个键 × 8 个语言(约 150 条新译文),并在 [`tests/i18n-coverage.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/i18n-coverage.test.mjs) 中扩展静态分析,扫描 `public/js/**/*.js` 中每一次 `t('key', …)` 调用并断言键存在于 DICT。未来漂移在 CI 期捕获。
