...
### Исправлено
- Значение `api:` Workday в `portals.yml` теперь принимается только если его **имя хоста** — `myworkdayjobs.com` (или поддомен `.myworkdayjobs.com`). Старая проверка была поиском подстроки, поэтому любой URL, просто содержащий строку — напр. `https://evil.com/?x=myworkdayjobs.com` — проходил и мог быть использован как эндпоинт. Реальные эндпоинты Workday не затронуты. (Сообщено CodeQL, #443.)
