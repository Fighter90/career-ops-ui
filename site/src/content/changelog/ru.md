...
Переводы: [🇬🇧 English](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.md) · [🇪🇸 Español](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.es.md) · [🇧🇷 Português](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.pt-BR.md) · [🇰🇷 한국어](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.ko-KR.md) · [🇯🇵 日本語](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.ja.md) · [🇨🇳 简体中文](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.zh-CN.md) · [🇹🇼 繁體中文](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.zh-TW.md) · [🇫🇷 Français](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.fr.md) · [🇵🇱 Polski](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.pl.md) · [🇺🇦 Українська](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.uk.md) · [🇩🇰 Dansk](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.da.md) · [🇸🇦 العربية](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.ar.md) · [🇩🇪 Deutsch](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.de.md) · [🇮🇹 Italiano](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.it.md) · [🇹🇷 Türkçe](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.tr.md) · [🇮🇳 हिन्दी](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.hi.md)
...
**Патч по итогам внешней QA-проверки v1.237.0.** Исправление Jobstreet впервые подтверждено извне: на хостах AU, NZ, HK и MY старая форма `/id/job/<id>` отдаёт чистый 404, а новая `/job/<id>` доходит до настоящего маршрута под защитой Cloudflare; на `id.jobstreet.com` верно обратное. Потерянные ссылки были реальными. Обнаружены два дефекта — оба в том, что добавил сам v1.237.0.

### Исправлено
**Подпись ссылки на книгу была переведена, а её цель — нет.** Все 17 локалей SPA и все 17 локалей cvstart.org указывали ссылку в футере на *Agentic Coding Design Patterns* на английское издание, хотя единственная переведённая подпись — русская, «Паттерны агентного кодинга», — обещала русскую книгу. Новый применитель `data-i18n-href` (той же формы, что `data-i18n-title`, ограниченный абсолютными значениями `https://`) и ключ `footer.patternsUrl` теперь отправляют `ru` на русское издание, а любую другую локаль — на английское. Регрессионный тест проверяет инвариант в обе стороны: переведённый заголовок подразумевает локализованный URL.
**SEEK переезжает на новые хосты, а транспорт отказывается следовать редиректам.** `www.seek.com.au` → `au.seek.com` и `www.seek.co.nz` → `nz.seek.com`: страницы деталей уже отдают 301, и обе пары хостов сегодня обслуживают поисковый API v5 с кодом 200. Поскольку `http-json.mjs` задаёт `redirect:'error'` (защита от SSRF, которую стоит сохранить), в день, когда путь API начнёт редиректить, запись AU/NZ на старом хосте начала бы падать — а поскольку в allowlist были только хосты `www.`, мигрировать было бы некому. Теперь `au.seek.com` и `nz.seek.com` в `ALLOWED_JOBSTREET_HOSTS`; старые хосты остаются, поэтому существующая конфигурация и уже собранные ссылки продолжают работать. Похожие на них хосты (`au.seek.com.evil.com`, `au-seek.com`) по-прежнему отклоняются.
**Вики:** строка *Agentic Practices* в карте документации на Home была единственной из двенадцати, не экранировавшей `|` внутри `[[…|…]]`, из-за чего GitHub обрезал ячейку — мёртвая ссылка, скрытое описание. Один символ.

### Примечания
Число источников не изменилось: **94** источника (89 EN + 5 RU), 89 адаптеров. Тесты **3201 → 3210** (+5 локаль ссылки в футере, +4 хосты SEEK).
Не закрыто извне, а зафиксировано без претензии на подтверждение: Oracle Cloud (Akamai заблокировал оба REST-зонда), закрепление Personio и `appendWorkType` (нужна запись в `portals.yml`, а в UI нет удаления для одной из них), и строковые кейсы liveness (нет управляемой страницы; 40 живых зондов дали 12 живых, 28 неопределённых, **0** ложных истечений).
Help ×17 теперь перечисляют обе формы хоста для `AU-Main` и `NZ-Main`. Структура заголовков не изменилась: 32 H2 / 122 H3.

## [1.237.0] — 2026-09-21

**Паритет с родителем — career-ops `main` @ `93c4302b` (VERSION 1.33.0, 75 коммитов, взятых из `career-ops-hq/main`). Новых источников в этот раз нет: четыре зеркальных исправления, и два из них здесь незаметно теряли вакансии.**

### Исправлено
**Jobstreet / SEEK — каждая австралийская, новозеландская, сингапурская, малайзийская и гонконгская вакансия вела на мёртвую страницу.** `parseJobstreetItem` безусловно строил URL детали как `${baseUrl}/id/job/${id}`. Сегмент `/id/` — это не путь, а **индонезийский языковой префикс**, и он существует только на индонезийских хостах. Любой другой хост платформы SEEK отвечает **404** на `/id/job/<id>`. web-ui уже добавил `www.seek.com.au`, `www.seek.co.nz`, `sg.jobstreet.com`, `my.jobstreet.com` и `hk.jobsdb.com` в `ALLOWED_JOBSTREET_HOSTS`, поэтому скан спокойно возвращал эти вакансии — и каждая указывала на 404. Теперь путь определяется хостом: `ID_LOCALE_HOSTS` (`id.jobstreet.com`, `www.jobstreet.co.id`, `jobstreet.co.id`) сохраняет `/id/job/`, всё остальное получает `/job/`, а неразбираемый origin проваливается в общий путь вместо выброса исключения. Два собственных теста **закодировали баг** — они утверждали `hk.jobsdb.com/id/job/…` и `www.jobstreet.com/id/job/…` — и были исправлены.
**Oracle Cloud — короткая страница в середине списка обрывала обход и отбрасывала хвост доски.** Условие остановки было `listLen === 0 || listLen < PAGE_SIZE`, но ORC отдаёт меньше строк, чем лимит, *в середине списка*, поскольку часть строк отфильтровывается на сервере. Замерено выше по течению на American Express: `TotalJobsCount` **454**, страницы по **200 / 199 / 54** — 199 читалось как конец, и последние **54 вакансии, 12% доски**, так и не были получены. Теперь пустая страница всегда завершает обход; когда тенант сообщает итог, постраничный обход продолжается, пока `offset + PAGE_SIZE >= total`; только тенант без итога для сверки завершается на короткой странице. Это согласуется с более широким соглашением о том, что API «может возвращать меньше результатов, чем запрошено… даже если это не конец коллекции» (Google AIP-158). `hasMore` по-прежнему намеренно игнорируется — некоторые тенанты возвращают `hasMore:false` на каждой странице доски из 7000 вакансий. Раз короткие страницы больше не обрывают обход, `MAX_PAGES` теперь единственный барьер против тенанта, сообщающего фиктивный итог, поэтому он покрыт отдельным тестом.
**Liveness — «This role is closed» не распознавалось, а ложное истечение — навсегда.** Паттерн был `/this job (listing )?is closed/i`, который пропускает не менее частые формулировки `role` и `position`; на одной доске выше по течению было замерено **111 из 111** неопределённых вакансий со словом «role». Расширено до `/this (?:job|role|position)(?: listing)? is closed\b(?!-)/i`. Завершающий `\b(?!-)` — это защита от составных прилагательных: один `\b` отклоняет `closedown`, но всё ещё совпадает с реальным текстом вроде *«This role is closed-loop control of the platform»*, и именно опережающая проверка это останавливает.
**Liveness — `job expired` теперь в мягком уровне, который не может перебить видимую кнопку отклика.** `HARD_EXPIRED_PATTERNS` проверяется *до* теста на элемент отклика, поэтому всё, что туда помещено, побеждает даже живую вакансию. `/\bjob expired\b/i` ложно срабатывает на четырёх реальных формах живой страницы — запись карусели «Similar jobs», чип фильтра «Hide job expired», футер FAQ и вышеупомянутый текст про `closed-loop` — потому что классификатор получает текст всей страницы. Теперь он живёт в `SOFT_EXPIRED_PATTERNS`, проверяется после элемента отклика и до эвристики страницы списка, возвращая `expired_body_soft`; голый баннер «JOB EXPIRED» без элемента отклика по-прежнему считается истёкшим. Это направление важно, потому что ложный `expired` записывается в историю сканирования как `skipped_expired` и затем **дедуп-фильтр исключает настоящую вакансию из каждого последующего скана, бессрочно**, если не задан `scan_history.recheck_after_days`.

### Добавлено
**Personio — явный `personio: <slug>` закрепляет тенанта.** Многие компании встраивают свою доску Personio как **iframe** на брендированной странице карьеры, поэтому `careers_url` указывает на собственный домен компании, а лента живёт на `<slug>.jobs.personio.de`; без закрепления такие доски не разрешаются ни во что. Слаг ограничен по набору символов заякоренным allowlist (`/^[a-z0-9][a-z0-9-]{0,62}$/i`), а получившийся URL всё равно проходит через существующий разбор `new URL()`, проверку HTTPS и хост-фильтр `PERSONIO_HOST_RE` — закрепление никогда не становится способом дотянуться до другого хоста. Слаг с `.`, `/`, `@`, `:`, `..`, пробелом, ведущим `-` или превышающий лимит DNS-метки в 63 символа проваливается в сегодняшнее поведение `api`/`careers_url`. В отличие от родителя, хост-регулярка web-ui регистрозависима, поэтому закрепление нормализуется через `new URL()` — `AcmeGroup` разрешается в `acmegroup.jobs.personio.de`, а не строит хост, который его же allowlist затем отклонил бы.
**Jobstreet — опциональный `appendWorkType`** добавляет к заголовку типы занятости вакансии, например `Strategy Consultant [Part time]`, чтобы фильтры по заголовку и триаж видели тип занятости уже на этапе скана. По умолчанию выключено; отсутствующий или пустой `workTypes` не меняет ничего.

### Примечания
Число источников и адаптеров не изменилось: **94** источника (89 EN + 5 RU), 89 EN-адаптеров. Этот релиз меняет только число тестов, **3164 → 3201** (+13 jobstreet, +7 Oracle Cloud, +9 liveness, +8 Personio).
Родитель **удалил `providers/telegram.mjs`**, консолидировавшись на `telegram-channel`. web-ui регистрирует оба как пользовательские источники, поэтому удаление **не** переносится: оно уменьшило бы реестр с 94 до 93 и тихо сломало бы всех, у кого настроен `telegram`. Возвращаться к этому только как к осознанному, объявленному удалению.
`jd-skill-gap.mjs` получил нераспознанную ранее форму заголовка «what we are looking for» без сокращений и удаление жирных маркеров. web-ui **ретранслирует** этот скрипт, а не зеркалирует его, поэтому исправление приходит вместе с задеплоенным родителем и не требует изменений здесь.
Не перенесено, с указанием причин: `lib/latex-escape.mjs`, `browser-extract.mjs`, `openai-eval.mjs`, `update-system.mjs` и скаффолдер (web-ui их не зеркалирует и не оборачивает шеллом); TUI Go-дашборда и форма конфигурации `web/` (отдельные поверхности); `doctor.mjs` и `analyze-patterns.mjs` (ретрансляторы только для чтения, чей fail-soft поглощает изменение).
Кириллическое расхождение форка `\p{L}` в `providers/telegram-channel.mjs` пережило слияние с апстримом в неизменном виде и было сверено побайтово с копией до слияния.

## [1.236.0] — 2026-09-18

**Паритет с родителем — career-ops `main` @ `6a9c84c` (VERSION 1.33.0, 128 коммитов). Два новых источника и три зеркальных исправления — два из них здесь незаметно теряли вакансии.**

### Добавлено
**Два источника сканирования, 92 → 94 (89 EN + 5 RU).** **Python.org Jobs** (`pythonorg`) читает публичную ленту доски; атрибуция работодателя обязательна, поэтому строка, в заголовке которой не назван работодатель, отбрасывается, а не восполняется именем записи портала. **Generalist World** (`generalist-world`) скрейпит карточки доски и **громко падает на разметке, которую не может разобрать** — карточка изменившейся формы выбрасывает исключение вместо того, чтобы читаться как пустая доска, а именно этот отказ и делает скрейпинговый источник ненадёжным. Оба появляются в фильтре Source на `#/scan`, оба закреплены за хостом (заякоренное регулярное выражение, только HTTPS), и оба насквозь проверены наборами с фейковым транспортом (43 и 36 случаев).

### Исправлено
**Вакансия Workday, открытая в нескольких городах, отбрасывалась фильтром локации.** Списочный endpoint Workday отвечает на многолокационную вакансию **счётчиком** там, где любая другая вакансия несёт место: `"53 Locations"`. Это не локация, а `buildLocationFilter` сопоставляет по регистронезависимой подстроке — поэтому вакансия, открытая в Остине *и* ещё в 52 городах, не совпадала ни с одной записью `allow: [austin]` и отбрасывалась, тогда как та же вакансия, указанная в одном городе, проходила. Замерено выше по течению на трёх тенантах: **53 из 291 вакансии** были такими заглушками — то есть это обычный случай, а не краевой. Настоящие места вакансии существуют только в CXS-документе **detail**, поэтому каждая заглушка теперь стоит один дополнительный GET к нему, с потолком **200 на запись** — сверх потолка заглушка остаётся видимой, а не подделывается, поскольку неразрешённая вакансия не должна читаться как разрешённая. Неудачный или безместный detail оставляет вакансию ровно такой, какой её вернул список: видимая `"53 Locations"` честно говорит о том, что не удалось разрешить, тогда как обнулённая локация молча прошла бы любой фильтр. Тот же GET несёт и `startDate`, поэтому обогащённая вакансия датируется **точно**, а не относительной прозой списочного endpoint (`"Posted 30+ Days Ago"` → `undefined`); всё, что не является голым `YYYY-MM-DD`, оставляется как есть, а не прогоняется через `Date.parse`, чей неISO-фолбэк определяется реализацией.
**Указанный уровень больше не схлопывает две заявки в один репост.** `"Insurance Specialist II"` — это собственное заявление работодателя о том, что эта заявка не сиблинг другой: другая вилка оплаты, объём и номер заявки под одним базовым названием. Токенизатор этого не видел: `w.length > 3` отбрасывает каждую римскую цифру вплоть до VIII и каждую одиночную цифру, поэтому `"Insurance Specialist"` и `"Insurance Specialist II"` токенизировались одинаково, давали идеальный коэффициент Жаккара, и детектор репостов помечал вторую как переразмещение первой. В `role-matcher.mjs` web-ui обработки уровня не было **вовсе**, поэтому переносится весь механизм родителя целиком: уровни извлекаются со свёрткой римских и арабских цифр в одно число (`"Nurse II"` и `"Nurse 2"` — одно и то же заявление, а именно эта пара прежде ошибочно различалась), два указанных уровня обязаны совпасть, а уровень, указанный только на одной стороне, трактуется как вольный рерайт **если только** каждое название не несёт ещё и слово, которого нет у другого, — это уже два расхождения, а не одно украшение.
**Доска Avature, закреплённая за фильтром, всё равно обходила всю глобальную доску.** Поисковый интерфейс Avature генерирует непрозрачные параметры фасетов (`?42386=[812132]` — это фильтр по стране), и оператор, закрепивший в `portals.yml` суженную доску, ожидает, что скан обойдёт именно её. `resolveSearch` пересобирал URL как `${origin}${path}` и напрочь отбрасывал строку запроса — поэтому каждый запрос, включая первый, обходил всю доску тенанта целиком: больше страниц, больше запросов и вакансии, которые оператор сознательно отфильтровал. Теперь фасеты едут в каждом постраничном запросе, а собственный `jobOffset` применяется последним, чтобы случайный одноимённый фасет никогда не мог перехватить пагинацию.

### Безопасность
**`devalue` 5.8.1 → 5.9.2** (транзитивно через `astro`), закрывает единственное открытое уведомление Dependabot — отказ в обслуживании на некорректном вводе. Зависимость только сайта; рантайм-код не менялся.

### Примечания
**Не перенесено, каждое — с указанием причины.** Родитель выдал **lever** таймаут 30с, а **manfred** — 25с против общего умолчания в 10с — это **не-порт**: транспорт сканирования web-ui и так допускает **60с** (`DEFAULT_SCAN_TIMEOUT_MS`), так что у обеих досок здесь запаса больше, чем даёт та правка там. Удвоенный префикс `careers-` у **iCIMS** здесь тоже невозможен: web-ui берёт origin тенанта прямо из `api`/`careers_url` и никогда не строит хост из имени тенанта; новый обход запасных хостов у родителя — это функциональность под его более рыхлую конфигурацию, а не починка дефекта. **deutschebahn** и **manfred** получили повтор при временном обрыве — на фоне потолка web-ui в 60с это гораздо менее насущно, поэтому взято на заметку, а не перенесено: у будущего таймаута на долгом обходе будет известное первое, что стоит попробовать. Всё остальное в дельте из 128 коммитов — это CLI, верификаторы, дашборд или `web/` (собственный фронтенд родителя): `liveness-browser`, `check-jd-archive`, `verify-ats`/`verify-portals`/`verify-cv-facts`, `update-system`, `set-status`, `audit-portals`, `discover-ats`, `scaffolder`, `test-all`, манифесты плагинов.
Расхождения форка перепроверены после пула: кириллическое исправление `\p{L}` в `providers/telegram-channel.mjs`, собственный `providers/telegram.mjs` форка, `hermes` в `web/src/lib/clis.ts`. Апстрим `career-ops-hq/main` смержен полностью (0 недостающих коммитов). **3066 → 3164 тестов**; справка теперь указывает **94** адаптера (**89 EN + 5 RU**) во всех 17 бандлах, а структура заголовков не изменилась — **32 H2 / 122 H3**.

## [1.235.0] — 2026-09-16

**Паритет с родителем — career-ops `main` @ `68e6b94` (VERSION 1.33.0, 42 коммита). Два зеркальных исправления; одно из них здесь незаметно теряло вакансии MokaHR.**

### Исправлено
**Вторая вакансия с той же доски MokaHR исчезала вместо того, чтобы добавиться.** `url-key.mjs` сводит URL вакансии к ключу сравнения и безусловно отбрасывал фрагмент — разумно, поскольку почти каждый фрагмент носит чисто оформительский характер (`#apply`, `#section-2`). MokaHR — исключение в нашем собственном реестре: её вакансии имеют вид `…/social-recruitment/{tenant}/{id}#/job/{n}`, где все вакансии одного тенанта делят один и тот же путь, а идентификатор вакансии существует **только** во фрагменте. Поэтому все они нормализовались к одному ключу. Замер до исправления: вставьте в конвейер две разные вакансии MokaHR — и на выходе останется **одна строка** — без ошибки, без предупреждения, незаметно; фильтр свежести сканера (`en-scanner.mjs`) читает тот же ключ, поэтому стоило одной вакансии этого тенанта попасть в историю сканирования, как все остальные отбрасывались как «уже виденные». Теперь `normalizeUrl` продвигает распознанный маршрут `#/job/{id}` или `#/jobs/{id}` в ключ сравнения (только для запроса) до того, как фрагмент отбрасывается — MokaHR сохраняет своё устоявшееся имя `mokahr_job_id`, чтобы ранее записанные ключи оставались сравнимыми, а любой другой хост получает внутреннее `_career_ops_fragment_job_id`. Продвинутый идентификатор **добавляется, а не устанавливается**: URL, уже несущий этот параметр, хранит идентификатор другой вакансии, и перезапись слила бы ровно то, ради разделения чего этот ключ существует. Каждый нераспознанный фрагмент по-прежнему схлопывается — и это та половина, что и так была верна.
**`accept-encoding` теперь фиксируется на каждом запросе к источнику.** Если заголовок не задан, Node согласует всё, что предлагает его сборка undici; начиная с **Node ≥ 23** в это входит `zstd`, а zstd-ответ amazon.jobs возвращается **обрезанным ровно на 1024 байтах со статусом 200** — так что сбой выглядит как `Unterminated string in JSON at position 1024`, будто это испорченный API, а не проблема транспорта. Теперь каждый запрос отправляет `gzip, deflate, br`. На наших рантаймах это **превентивная мера, а не починка**: Node 18/20/22 — матрица CI и сервер (22.22.1) — отправляют `gzip, deflate`, что проверено напрямую, а не предположено, так что здесь ничего не обрезалось; сегодня меняется то, что теперь предлагается `br`, который эти версии по умолчанию не запрашивают, но корректно декодируют. Умолчание применяется **регистронезависимо**, и именно эту часть стоило проверить: имена заголовков регистронезависимы, а `fetch` объединяет две одноимённые записи в одно значение через запятую, поэтому вызывающий код, передающий `Accept-Encoding: identity`, иначе получил бы отправленным `gzip, deflate, br, identity`.

### Примечания
На границе `fetchImpl` заголовки остаются **обычным объектом**, тогда как родитель передаёт в `fetch` экземпляр `Headers`. Это осознанное расхождение, а не недосмотр: fetch-хелперы web-ui принимают инжектируемый `fetchImpl`, а 45 наборов тестов источников читают `opts.headers['User-Agent']` из того, что они перехватывают, — у экземпляра `Headers` нет доступа по скобкам, поэтому добросовестно перенесено именно *поведение* (регистронезависимая перезапись, приоритет вызывающего кода), а не реализация. Объект вызывающего кода копируется, а не мутируется, поскольку несколько источников делят одну и ту же константу заголовков. Сопутствующее исправление родителя — применение **User-Agent по умолчанию** регистронезависимо — здесь **не переносится**: web-ui вообще не подставляет UA по умолчанию, каждый источник задаёт тот, что требует его доска, поэтому нет умолчания, с которым мог бы столкнуться `User-Agent` вызывающего кода.
Два новых набора тестов и три новых случая фиксируют эту пару: `tests/http-accept-encoding.test.mjs` (точное зафиксированное значение, каждый из четырёх fetch-хелперов и переопределение вызывающим кодом в трёх вариантах регистра — подтверждая, что выживает *единственный* ключ `accept-encoding`, тогда как баг слияния порождал два) и `tests/parsers-pipeline-hash-route.test.mjs`, который проверяет видимый пользователю стык, а не сам ключ: обе вакансии MokaHR должны дойти до конвейера, повторная вставка одной и той же вакансии в четырёх вариантах написания по-прежнему должна схлопываться в одну строку, а оформительский фрагмент не должен становиться идентификатором. Все они были подтверждённо красными до исправления. Сама фиксация живёт на уровне **транспорта**, а не только в этих хелперах: 25 из 92 источников вызывают внедрённый `fetchImpl` напрямую и через хелперы вообще не проходят, поэтому её применяет `makeTimeoutFetch` — обёртка, которую оба сканера действительно внедряют, и единственная точка прохода, общая для всех источников. (Родителю эквивалент не нужен: `providers/_http.mjs` — его единственный транспорт.) Структурная проверка теперь роняет CI, если новый источник вызывает `fetch` напрямую вместо переданного ему транспорта, а поведенческий кейс прогоняет greenhouse — один из этих 25 — через обёртку и доказывает, что заголовок доходит до провода.
Не перенесено, с указанием причины: `liveness-browser.mjs` (защита от повторного использования headed-Playwright — web-ui не управляет headed-браузером), `check-jd-archive.mjs` (в web-ui нет соответствующего relay), `test-all.mjs` (harness родителя; расхождение форка `vpFixtureEnv` сохранено нетронутым), `web/` (собственный фронтенд родителя, не зеркалируется), манифесты плагинов и `SIGNATURES.md` (существуют только в репозитории). `batch/batch-runner.sh` (амперсанды экранированы до интерполяции `sed`) и `jd-skill-gap.mjs` (резолвит `cv.md` относительно корня данных) являются **ретрансляцией только для чтения**: web-ui обращается через шелл к копиям родителя, поэтому оба ложатся без изменений кода здесь, а правка в `jd-skill-gap` делает релей `/api/jds/:id/skill-gap` более точным: `cv.md` теперь резолвится от корня данных, а не от той директории, в которой был запущен процесс. Расхождения форка перепроверены после пула: кириллическое исправление `\p{L}` в `providers/telegram-channel.mjs`, собственный `providers/telegram.mjs` форка, `hermes` в `web/src/lib/clis.ts`. Число источников не изменилось, **92** (87 EN + 5 RU), поэтому справка не тронута. **3042 → 3066 тест.**

## [1.234.0] — 2026-09-15

**Паритет с родителем — career-ops `main` @ `56cce8f` (VERSION всё ещё 1.32.0, что на 58 коммитов позже 1.32.0). Одно зеркальное исправление, применённое к 15 источникам, плюс защита от его повторения.**

### Исправлено
**Один-единственный некорректный идентификатор вакансии больше не опустошает всю страницу.** `encodeURIComponent` выбрасывает `URIError` на одиночном суррогате UTF-16, а JSON-полезная нагрузка может его нести — `JSON.parse('"\\uD800"')` сохраняет его как есть. Пятнадцать источников собирали URL каждой вакансии из id или slug, полученных от хоста, *внутри* цикла разбора, поэтому одна плохая вакансия прерывала весь цикл и теряла все вакансии на этой странице; ничего не попадало в лог как отказ источника, потому что исключение разворачивало стек ещё до того, как catch на уровне компании успевал что-либо зафиксировать. Родитель исправил это в `providers/_safe-url.mjs` (`safeEncodeURIComponent` → `null` при `URIError`, в остальных случаях — повторный выброс) и пропустил через него id в каждом цикле, отбрасывая при `null` лишь одну вакансию. Этот хелпер теперь называется `server/lib/sources/_safe-url.mjs`, и то же изменение внесено в тринадцать зеркалированных источников — alibaba, arbeitsagentur, bamboohr, feishu-jobs, garena, jibeapply, manfred, meituan, mokahr, phenom, thehub, tkms, vdab — а также в **два источника, существующих только в web-ui, которые новая защита выявила при первом же прогоне, jobstreet и trudvsem**, — у них была та же форма цикла и тот же скрытый throw. Область применения выбрана осознанно: через хелпер проходят только id, полученные из API. Значения, производные от конфигурации, сохраняют строгое кодирование и падают громко — `office` у garena (сегмент из `portals.yml`) остаётся на выбрасывающем исключение `urlSegment`, тогда как её же API-`id` получает возвращающий `null` `idUrlSegment`; `corpName` у csod и провалидированный `SLUG_RE` slug у 4dayweek внесены в список исключений поимённо, с указанием причины.
**Резервное восстановление заголовка у rheinmetall могло упасть на некорректном href.** Когда у карточки нет заголовка, он восстанавливается из slug в URL через `decodeURIComponent`, который выбрасывает исключение на некорректной percent-последовательности в спарсенном `href` — та же форма «одна плохая строка убивает всю страницу», только на стороне декодирования. Теперь декодирование выполняется защищённо, с откатом на исходный slug.
**Реестр источников пропускает файлы с префиксом `_`.** Автообнаружение импортировало каждый `.mjs` в `server/lib/sources/`, кроме себя самого, и предупреждало о любом файле без `export const meta` — так что хелпер порождал предупреждение при старте. Принята конвенция родителя: ведущее подчёркивание означает *не источник*; такие файлы не импортируются, и предупреждений по ним не возникает. Необъявленный файл без `meta` по-прежнему получает предупреждение, и тест обнаружения теперь проверяет обе половины (подтверждённо красный на старом фильтре). Скрипт сборки сайта `sync-assets.mjs` считает файлы адаптеров на диске по тому же правилу, так что хелпер больше не срабатывает на его защиту от тихой потери источника.

### Примечания
`tests/sources-url-encoding-surrogate.test.mjs` зеркалирует набор родителя в трёх частях: контракт хелпера (парный суррогат — обычный эмодзи — по-прежнему кодируется; throw из собственного `toString` значения — это баг вызывающего кода, и он пробрасывается дальше), поведение каждого из пятнадцати источников по отдельности (один плохой id + один чистый id → без throw, чистый сохраняется, плохой отбрасывается, каждый прогоняется через экспортируемый чистый парсер источника либо, для thehub, через его fetcher с фейковым транспортом), и **защита на уровне источников**: каждый переведённый файл обязан импортировать хелпер *и* вызывать его, ни один файл не может ссылаться на него без импорта, и ни один непроверенный файл не может строить URL вакансии голым `encodeURIComponent` — новый источник со старой формой падает в CI с указанием номера строки-нарушителя. Все пятнадцать посистемных случаев были подтверждённо красными до переноса.
Всё остальное в дельте родителя не перенесено, с указанием причины: `batch/batch-runner.sh --cli` — флаг только для CLI (web-ui не вызывает batch-runner через шелл); `invite-match.mjs` теперь резолвит `set-status.mjs` от корня кода — web-ui его не ретранслирует; `company-funded.mjs` получил то же изменение safe-url внутри себя, а форма его вывода `--json`, которую парсит `/api/company-funded`, не изменилась; `test-all.mjs` лишь сдвинул бюджет таймаута (расхождение форка `vpFixtureEnv` сохранено нетронутым); `web/` — это собственный фронтенд родителя, он не зеркалируется; `AGENTS.md`/`SIGNATURES.md` существуют только в репозитории. Расхождения форка перепроверены после пула: кириллическое исправление `\p{L}` в `providers/telegram-channel.mjs`, собственный `providers/telegram.mjs` форка, `hermes` в `web/src/lib/clis.ts`. Число источников не изменилось, **92** (87 EN + 5 RU), поэтому справка не тронута. **3022 → 3042 теста.**

## [1.233.2] — 2026-09-10

**Исправлено — две находки CodeQL высокой серьёзности в пути сохранения `#/config`. Поведение не менялось.**

### Исправлено
**Две находки CodeQL `js/remote-property-injection` высокой серьёзности, закрытые тем, что существовавший инвариант сделан структурным, а не декларируемым.** `POST /api/config` переносит сохранённые значения в `process.env`, чтобы не требовался перезапуск, и этот цикл шёл по `Object.entries(safe)` — по объекту, собранному из тела запроса, — с явной защитой от `__proto__` / `constructor` / `prototype` внутри. Код был безопасен: `safe` наполняется двадцатью строками выше обходом `KNOWN_KEYS` и копированием только их, так что имя свойства никогда не контролировалось пользователем. Но сам по себе цикл читается как «присвоить свойству, имя которого пришло из объекта, собранного по телу запроса», а перенести это ограничение через два цикла анализатор не может. Теперь цикл идёт прямо по константному массиву, доставая значение из `safe`, и имя свойства доказуемо является литералом уровня модуля. Защита от прототипных ключей ушла вместе с ним: в жёстко заданном списке имён в верхнем регистре её быть не может, а проверка невозможного случая — шум. **Поведение идентично**; даты находок (9 сентября) объясняются лишь тем, что v1.232/v1.233 сдвинули номера строк, а не новизной цикла.

### Примечания
Альтернативой было отклонить обе как ложноположительные — в репозитории для этого есть заведённая практика. Здесь предпочтительнее починить: утверждение в комментарии стоит меньше, чем форма, которую и анализатор, и следующий читатель могут проверить, не прослеживая ограничение через двадцать строк.
Тест, закреплявший прежнюю защиту, был **грепом по тексту исходника** в поисках буквального выражения `k === '__proto__' || …`, а это доказывает наличие строки, а не безопасность маршрута. Он заменён двумя более сильными: структурным — что цикл доходит до `process.env[k]`, обходя `KNOWN_KEYS`, и что он **не** обходит объект, построенный по запросу (подтверждённо падает на старой форме); и поведенческим в `config-endpoint.test.mjs`, который отправляет `__proto__`, `constructor`, `prototype` и незарегистрированное имя **сырым JSON** — литерал объекта не годится, поскольку `{ __proto__: … }` в исходнике задаёт прототип, а не создаёт свойство, так что `JSON.stringify` отправил бы `{}` и тест ничего бы не доказал. **3021 → 3022 теста.**

## [1.233.1] — 2026-09-10

**Исправлено — руководство не описывало элемент, добавленный в v1.233.0, а тот же раздел нёс указания, переставшие быть верными 200 релизов назад.**

### Исправлено
**DOC-1 — элемент, появившийся в v1.233.0, нигде в руководстве не описан.** Каждый выпадающий список теперь начинается с пункта *«Использовать умолчание (…)»*, который выражает состояние, невыразимое в интерфейсе прежде: **ключ не задан**. Отличие от выбора того же значения из списка ниже невидимо и существенно — «Использовать умолчание (llama3.2)» не пишет ничего и продолжает отдавать вам умолчание проекта по мере его изменения, тогда как явный выбор `llama3.2` записывает `OLLAMA_MODEL=llama3.2` и молча оставляет вас на старом значении, когда умолчание сдвинется. Это ровно тот тихий дрейф, ради которого делались CONFIG-2 и CONFIG-3: механику починили в коде, а прочитать о ней было негде. Три пункта добавлены в существующий список `### Behavior` во всех 17 бандлах — про сам пункт, про правило «поле, которого вы не касались, не записывается», и про то, что тост считает и удаления. Гейты по заголовкам этого поймать не могли: они считают H2/H3, а покрытие — не счётчик.
**DOC-2 — строка таблицы `### Recognized keys` не называла ключа, а единственное конкретное указание в ней было неверным.** В колонке «Key» стояло `(server uses default UA)` — пояснение в скобках там, где место имени ключа, — и описывало `HH_USER_AGENT`, который **v1.19.0** убрал из `KNOWN_KEYS` и из интерфейса. Дальше строка предлагала зарегистрировать приложение на dev.hh.ru и подставить его UA, что не даёт ничего с **v1.65.0**, когда адаптер hh.ru перешёл на чтение публичного сайта поиска с собственным браузерным User-Agent. Ещё два упоминания были столь же устаревшими: пункт в *Optional checks*, описывающий health-строку, удалённую в **v1.28.1**, и troubleshooting-строка, повторяющая совет про dev.hh.ru. Все три убраны из всех 17 бандлов. Troubleshooting-строка при этом **переписана, а не удалена**, потому что половина её верна: с июля 2026 hh.ru отдаёт HTTP 451 не-российским адресам, так что российский домашний IP действительно нужен; неверными были шаг с регистрацией и ссылка на несуществующий ключ.
**Таблица ключей теперь говорит, чем она является.** `### Recognized keys` перечисляет 6 ключей из 45, тогда как заголовок обещает реестр; провайдерские семейства при этом описаны прозой в том же разделе. Одна фраза ×17 теперь прямо называет её выборкой и указывает на `#/config` как на полный сгруппированный список. Ключи не добавлялись — это снимает двусмысленность, а не раздувает таблицу.

### Примечания
`tests/help-recognized-keys.test.mjs` — тот самый гейт, которого не хватало. Он сверяет первую колонку каждой справочной таблицы, действительно перечисляющей ключи, с живым `KNOWN_KEYS`, падает на любом бандле, всё ещё упоминающем dev.hh.ru, и на любом, где не описан пункт *«Использовать умолчание»*. Сужение области важно: в бандлах есть и другие таблицы, у которых первая колонка — заглавное слово в обратных кавычках (`PDF` в матрице экспорта), поэтому таблица считается ключевой, только если хотя бы одна её собственная строка называет настоящий ключ. Проверено на бандлах до правки: все три случая красные. **3018 → 3021 тестов**; справка остаётся **32 H2 / 122 H3** во всех 17, потому что всё легло внутрь существующих заголовков.

## [1.233.0] — 2026-09-10

**Исправлено — три находки в пути сохранения `#/config`. CONFIG-3 порождён моей же правкой v1.232.1.**

### Исправлено
**CONFIG-3 — осознанный выбор, совпавший с засеянным значением, отбрасывался как «не трогали».** Порождён моей же правкой v1.232.1, комментарий к которой утверждал, что код различает «значение для показа» и «пользователь сознательно выбрал то же самое». Он их не различал. `initial` смешивает два происхождения: засев из `.env` (равенство значит, что поле не редактировали, — пропустить верно) и засев из `defaultValue` (равенство **неоднозначно**: либо не открывали, либо открыли и согласились). При снятом `HOST` контрол показывает `127.0.0.1`; ввод ровно этого значения и Save отправляли `{}`, и ключ так и не появлялся. Единственный путь закрепить его — ввести заведомо неверное значение, сохранить и вернуть правильное; тупик, которого никто не найдёт. Поведение приложения при этом не ломалось — отсутствующий ключ и ключ, равный умолчанию, дают один результат, — но интерфейс показывал состояние, которого нет в файле, и повторный Save это рассогласование не устранял. Исправлено флагом **`seededFromFile`** рядом с `initial`: где засев пришёл из файла, ничего не меняется; где из `defaultValue`, вторым признаком становится `dirty`. Общим основанием он быть не может — срабатывает и при «ввёл и вернул», — но здесь это единственная оставшаяся улика, и цена ей самое большее одно поле, в котором человек работал, а не восемнадцать из CONFIG-2, для которых `dirty` пуст, потому что их никто не открывал.
**Снятие ключа показывалось как «· 0».** `updateEnvFile` всегда возвращал только *записанное*, поэтому удалению нечего было считать, и тост читался как «Настройки сохранены · 0» — что разумно понять как «ничего не произошло», хотя ключ был удалён. Теперь `POST /api/config` отдаёт и `removed`, вычисленный по снимку до записи, так что он называет ключи, которые действительно были, а не все, которые просили очистить; тост считает и то и другое.
**Select-поле можно было задать, но нельзя очистить.** Ни у одного из 18 дропдаунов не было пустого пункта, поэтому форма не могла отправить для него `''` — текстовые поля очищаются, выпадающие списки нет. Вместе с CONFIG-3 это замыкало круг для незаданного селекта: закрепить нельзя, потому что значение совпадает с засеянным, а закрепив иным путём — не снять. Теперь каждый селект начинается с явного пункта **«Использовать умолчание (…)»**, и незаданный ключ выбирает именно его, а не значение умолчания. Замысел v1.57.1 сохранён — подпись по-прежнему называет значение, которое использует сервер, — но теперь она это говорит, а не выдаёт себя за выбор, чем и был возможен CONFIG-3. С `select-remote` потребовалась аккуратность: живой каталог OpenRouter вызывает `replaceChildren` и стирал пункт, поэтому он там пересоздаётся. Один новый ключ i18n, `config.useDefault`, во всех 17 словарях.

### Примечания
Проверено в этом же проходе и намеренно оставлено как есть, чтобы запись была полной. **`lang` в теле каждого POST — точечная запись в allowlist, а не префиксное правило**: проверено поведением, `lang` → 200, тогда как `langx`, `lang_evil` и `LANG` дают 400, как любой неизвестный ключ. **`facts.json` отсутствует на задеплоенном сайте по замыслу** — его импортируют 22 файла на этапе сборки, значения инлайнятся в HTML, поэтому проверять надо версию на странице, а не файл; §2.3 промпта v1.231.5 в этом вводил в заблуждение. **Трактовка файла без расширения как текста в `/api/cv/import` документирована и осознанна**, чтобы вставка из буфера без расширения продолжала работать; маршрут только конвертирует и возвращает, не пишет и не исполняет, а `.exe` и `.zip` отвергаются явно.
База юнит-тестов не изменилась и равна **3018** — все правки клиентские либо в форме ответа одного маршрута. Гейты браузерные: `tests/playwright-config-save-scope.mjs` вырос с шести случаев до **одиннадцати** (**111 → 116** браузерных), и все пять новых подтверждённо падали на старом коде. Один из них существует ровно затем, чтобы CONFIG-2 не вернулся: сохранение без единой правки по-прежнему обязано не отправлять ничего.

## [1.232.1] — 2026-09-10

**Исправлено — сохранение на `#/config` записывало поля, которых пользователь не касался. Это не регресс v1.232.0, дефект старше.**

### Исправлено
**CONFIG-2 — сохранение на `#/config` записывало поля, которых пользователь не касался.** Два по отдельности разумных решения вместе давали запись: v1.57.1 засевает незаданный контрол его `defaultValue`, чтобы поле показывало значение, которое сервер и вправду использует (это верно и не тронуто), а `save()` спрашивал `dirty` **только для секретов** и отправлял все прочие поля независимо ни от чего. Значение, подставленное для ПОКАЗА, уходило на сервер как выбор пользователя. Замерено на фикстуре с двумя заданными ключами: смена одного дропдауна отправляла **28 ключей и закрепляла 18**, которых человек не открывал, каждый — своим умолчанием; а Save вообще без изменений отправлял 27. В день записи ничего не меняется, потому что записанное и есть умолчания. Меняется статус настройки: из «**не задана — следуй умолчанию проекта**» в «**закреплена в `.env`**». Курируемые списки двигаются между релизами — в примечаниях к v1.232.0 сам приведён выпавший `gemini-2.0-flash`, — поэтому при следующей смене умолчания каждый, кто хоть раз нажал Save, молча останется на старом значении. Это тот же класс, что и CONFIG-1: там писался мусор, здесь пишется правдоподобное, и потому оно незаметнее.

### Примечания
**Основание сравнения — та часть, которую стоит смотреть внимательно.** Сравнения с загруженным `cfg.values` — очевидная правка, и первая испробованная — недостаточно: незаданный select показывает свой `defaultValue`, тогда как сохранённое значение пустое, они честно различаются, и все семнадцать модельных дропдаунов по-прежнему пишутся. Тело запроса сократилось с 27 ключей до 19, а не до одного. Теперь сохранение сравнивает с тем, **чем контрол был проинициализирован**: это единственное выражение «не трогали», которое отличает значение, поставленное для показа, от пользователя, сознательно выбравшего то же самое. `dirty` основанием тоже не годится — он срабатывает, когда человек ввёл значение и вернул исходное.
Заодно исправлено: число записанных ключей жило внутри **fallback**-аргумента `t()`, а ключ `config.saved` есть во всех 17 словарях — поэтому число не показывалось никогда, и сохранение двенадцати ключей выглядело так же, как сохранение одного. Теперь тост читается как «Настройки сохранены · 1».
Больше ничего не сдвинулось. Префилл v1.57.1 на месте, секреты по-прежнему уходят только после касания, а серверная поблажка из v1.232.0 остаётся защитой для прямых вызовов API, которые через форму не идут. База тестов не изменилась и равна **3018**, потому что правка клиентская; её гейт — `tests/playwright-config-save-scope.mjs`, шесть браузерных случаев, зарегистрированных в `test:e2e:browser` (**105 → 111**); все шесть подтверждённо падали на старом коде.

## [1.232.0] — 2026-09-09

**Исправлено — два дефекта, найденные браузерным прогоном; ни один из них не пришёл из v1.231.5.**

### Исправлено
**PROFILE-1 — `#/profile` прокручивал вбок всю страницу.** Каждое поле профиля — это `.card` в сетке `.card-row` с `repeat(auto-fit, minmax(220px, 1fr))`, а значение было безымянным `<div>` с одними лишь инлайновыми шрифтовыми стилями, поэтому вычисленный `overflow-wrap` оставался `normal`. URL LinkedIn не даёт CSS ни одной допустимой точки переноса, а значит **min-content ширина элемента равна всей строке**; grid-элементы по умолчанию имеют `min-width: auto` и отказываются ужиматься ниже неё — карточка распирала свою колонку, затем строку, главную колонку и наконец документ. На фикстуре карточки переполнялись на **768, 1024, 1280 и 1440 px**, а на 1280 px это доходило до страницы как **23 px настоящего горизонтального скролла**. Зависимость от ширины и объясняет, почему дефект так долго жил: до 420 px медиазапрос даёт одну достаточно широкую колонку, а между 768 и 1024 px текст лишь вылезает из карточки, не сдвигая документ. Исправлено общим классом `.card-value` с `overflow-wrap: anywhere` и подстраховкой `.card-row > * { min-width: 0 }`. **Именно `anywhere`, а не `break-word`** — только `anywhere` учитывается при расчёте min-content, поэтому `break-word` перенёс бы текст, оставив колонку ровно такой же распёртой. `dashboard.js` переведён на тот же класс; в `health.js` уже стоял `word-break: break-all`, и он не потребовал ничего.
**CONFIG-1 — `POST /api/config` принимал любое значение для select-поля.** `{"LLM_PROVIDER":"not-a-provider"}` возвращал **200** и записывался в ваш `.env`. Дальше ничего не падало: резолвер уходил в фолбэк на тот ключ, что был настроен, `/api/status/providers` показывал рабочего провайдера, а выбранная пользователем настройка молча не действовала — и обнаружилась бы много позже, когда фолбэк перестал бы совпадать с намерением. Валидатор проверял имена ключей, тип, длину, переводы строк и формат трёх конкретных полей, но не принадлежность значения домену. **`LLM_PROVIDERS` — ровно этот список из 19 значений — всё это время экспортировался из `env-config.mjs`**; валидатор просто к нему не обращался. Теперь обращается.

### Примечания
**Проверяется только `LLM_PROVIDER`, и это осознанное сужение отчёта.** Шестнадцать списков моделей оставлены без проверки: их настоящий домен — каталог вендора, который меняется между нашими релизами, поэтому применение курируемого списка запретило бы выбрать модель, которую провайдер уже отдаёт. В самом репозитории тесты используют `gemini-2.0-flash`, а курируемый список её не содержит. Анализ вреда ведёт туда же: неверная модель падает громко, в момент вызова, и только неверный провайдер падает молча. `select-remote` исключён по той же причине, а сверка дрейфа по-прежнему доказывает, что сервер и браузер не расходятся в том, что предлагает выпадающий список.
**Уже сохранённое значение получает поблажку — строгий вариант оказался неработоспособным, и это показал браузерный тест.** `public/js/views/config.js` отправляет все несекретные поля при каждом сохранении, тронутые и нет, поэтому отказ по ранее сохранённому значению замораживал всю страницу: пользователь не мог изменить ничего, пока не исправит поле, которого не касался. Теперь отвергаются только **изменения** — заявленный дефект по-прежнему закрыт, новое плохое значение не пройдёт, а старое чинится через выпадающий список, где есть лишь допустимые варианты.
Заодно закрыто: `OLLAMA_API_KEY` принимался на запись через API, но не имел поля в `#/config`, то есть за проксирующей аутентификацией Ollama его нельзя было выставить из приложения вовсе. Дескриптор добавлен (секретный, два новых ключа i18n ×17), а тест следит, чтобы 45 записываемых ключей сервера и 45 дескрипторов интерфейса совпадали. **Ни один из дефектов не пришёл из v1.231.5** — в том релизе менялись только `package.json`, локфайлы и документация. **3013 → 3018 тестов**, плюс два браузерных случая в новом зарегистрированном `tests/playwright-card-overflow.mjs`.

## [1.231.5] — 2026-09-09

**Безопасность — закрыто семь уведомлений, одно из них — критическое удалённое выполнение кода.** Ни один файл исходников не изменён.

### Безопасность
**Смержено из Dependabot:** `multer` 2.2.0 → 2.3.0 (**high** — отказ в обслуживании из-за утечки файловых дескрипторов при обрыве) и `svgo` 4.0.2 → 4.1.0 в сайте (**high** и medium — `removeScripts` пропускал исполняемые ссылки через пространства имён и обработку содержимого, а также не до конца обеззараживал исполняемый HTML внутри `foreignObject`).
**Затем ещё пять, вскрытых пересканированием, которое эти мержи и запустили.** Каждый затронутый диапазон уже допускался существующим спецификатором `^`, поэтому потребовалось обновление, а не правка манифеста: **`astro` 7.1.0 → 7.3.2** в сайте — **критично**, удалённое выполнение кода через оптимизацию изображений AVIF (исправлено в 7.2.8), а заодно обход авторизации из-за отсутствия проверки границы сегмента пути при отсечении настроенного base (7.2.4); **`sharp` 0.35.3 → 0.35.4** — **high**, две уязвимости libheif; и **`js-yaml` 4.3.1 → 4.3.2** в **обоих** манифестах — **high**, `maxTotalMergeKeys` не ограничивал расход процессора на пустых источниках слияния.

### Примечания
«Только зависимости» здесь не равно «без риска», поэтому релиз не выпускался по одному лишь результату аудита. `js-yaml` разбирает каждый конфиг, который читает приложение, `multer` принимает каждую загрузку, а Astro сместился на **минорную** версию. Проверено: `npm run test:ci` **3013/3013**, браузерный набор 4/4 и полная пересборка сайта, давшая те же **86 страниц**, все **17** зеркал changelog и корректный `facts.json`. `npm audit` сообщает о **0** уязвимостях в обоих манифестах. База тестов не изменилась, потому что не изменился ни один файл исходников.

## [1.231.4] — 2026-09-07

**Исправлено — v1.231.3 починил последствие клика по Doctor, а не сам момент. Сообщено снова, с телефона и с десктопа.**

### Исправлено
**`withSpinner` всё ещё переписывал кнопку на время запроса.** v1.231.3 научил его сохранять узлы-детей и возвращать их в `finally` — и они действительно возвращались, но индикатор занятости по-прежнему выполнял `button.replaceChildren(document.createTextNode('⏳ ' + original))`. Поэтому на всё время работы `doctor.mjs` квадрат 36 px держал текст `⏳ 🩺Doctor` и превращался в широкую «таблетку», ломавшую строку; на десктопе это выглядело как слипшееся `⏳🩺Doctor`. Восстановление детей чинило последствие, а не момент, и проверка, смотревшая только на конечное состояние, это пропускала. **Кнопка, у которой есть дети-элементы, теперь не переписывается вовсе.** Её индикатор — класс `.is-loading`, который таблица стилей и так приглушала и который теперь подменяет иконку песочными часами средствами CSS, так что геометрия измениться не может: **36 px в покое, 36 px во время, 36 px после** на 320 px. Кнопки с одним текстом — 23 из 24 мест вызова — сохраняют прежние песочные часы перед подписью. На десктопе кнопка на время работы читается как `⏳Doctor`, расширяясь на ширину значка и возвращаясь обратно — ровно так же ведут себя все прочие кнопки приложения уже много лет.

### Примечания
Урок здесь про проверку, а не про код. Гейт v1.231.3 кликал по Doctor, ждал снятия `aria-busy` и только потом мерил — то есть по построению видел лишь то состояние, которое блок `finally` уже привёл в порядок. Новая браузерная проверка **сама удерживает промис открытым**, а не гонится за настоящим запросом: она снимает замер в середине полёта и требует, чтобы ширина не изменилась, оба `<span>` были на месте и ничего не переполнялось. Первый черновик замерял после настоящего клика и пропускал проверку, если запрос успевал раньше, — такой замок срабатывал бы по удаче. **3012 → 3013 тестов.** Обе новые проверки подтверждённо падали на коде v1.231.3 до внесения правки.

## [1.231.3] — 2026-09-07

**Исправлено — клик по Doctor ломал верхнюю панель на телефоне. Сообщено с телефона в день выхода v1.231.2.**

### Исправлено
**`UI.withSpinner` уничтожал детей той кнопки, которую оформлял.** Индикатор занятости работал через `textContent`: сохранить `button.textContent`, записать `'⏳ ' + original`, записать обратно. Присваивание `textContent` заменяет **всех** детей одним текстовым узлом — а v1.231.2 как раз разделил каждую кнопку верхней панели на `.btn-ico` и `.btn-label`, скрыв подпись под `max-width: 900px`. Поэтому первый же тап по Doctor уничтожал оба `<span>` навсегда: мобильным правилам нечего стало сопоставлять, подпись возвращалась голым текстом, а квадрат 36 px разрастался в «таблетку» `🩺Doctor`, наезжавшую на переключатель темы. Само не восстанавливалось — только перезагрузка страницы возвращала разметку. Исправление сохраняет **узлы**-детей и возвращает их через `replaceChildren`; кнопки с одним текстом, а это большинство из 24 мест вызова, ведут себя ровно как прежде. Флаг `dataset.spinnerBusy` не даёт повторному вызову принять песочные часы за исходное содержимое.
- **FIND-3 — поле поиска схлопывалось до 8 px на экране 320 px.** Приведя панель к одной строке, я оставил searchbar единственным гибким элементом — и прошлый выпуск прямо это записал: он *«может сжаться до нуля, поэтому отрицательного свободного места не возникает»*. Он и сжался до нуля: **8 px ввода на 320 px, 28 px на 340 px** — один символ из двадцати одного в подсказке. Кликается и печатается, но как поле больше не читается. Никакой `min-width` тут не помогает: на 320 px панель держит 32 px отступов, 40 px меню и 162 px действий, и взять 120 px попросту неоткуда. Поэтому ниже **420 px** — ширины, на которой поле само наконец превышает 100 px — оно прячется за лупу и по тапу разворачивается на всю панель: **182 px на 320 px, 222 px на 360 px, 252 px на 390 px**, меню и остальные действия уступают место, переполнения нет ни на одной ширине. Кнопка-раскрытие сохраняет одно доступное имя и сообщает состояние через `aria-expanded`, так что ничто не переименовывается под пользователем, а смена 🔍/✕ сделана чистым CSS — JS в эту кнопку не пишет, ведь именно запись сломала Doctor в этом же выпуске. Выше 420 px не меняется ничего: 109 px на 421 px, 447 px на 1280 px. Найдено браузерным регрессом по живой сборке.

### Примечания
Это тот же класс дефекта, что и ловушка с `applyI18n()`, отмеченная в v1.231.2 — присваивание `textContent` элементу, у которого теперь есть дети-элементы, — но через другого вызывающего. v1.231.2 починил того вызывающего, о котором знал, и проехал мимо этого. Урок обобщается: **разделив содержимое элемента на дочерние `<span>`, вы превращаете каждого существующего писателя `textContent` по этому элементу в скрытый баг**, и писатели не всегда лежат в том файле, который вы правите.
Закрыто двумя замками. `tests/with-spinner-preserves-children.test.mjs` исполняет настоящий `withSpinner` над поддельной кнопкой в `node:vm` и проверяет, что `<span>` переживают полный цикл, а заодно контракт занятости (возвращает результат, блокирует кнопку на время запроса, ставит и снимает `aria-busy`, восстанавливает прежнее `disabled`). `tests/playwright-narrow-viewport.mjs` кликает по Doctor в настоящем браузере на 320 px и проверяет, что ширина кнопки не изменилась, подпись по-прежнему `display:none`, действия в одной строке и ничего не прокручивается. Оба подтверждённо падали на старом коде до внесения правки. **3009 → 3012 тестов** (браузерный — opt-in, в `test:ci` не входит).

## [1.231.2] — 2026-09-06

**Исправлено — мобильная верхняя панель переносилась на вторую строку, сервер работал на старом родителе, а редирект cvstart.ru давал 404 на любом пути, где уже указан язык.**

### Исправлено
- **Мобильная панель снова в одну строку, кнопки — иконками.** Кнопки действий принудительно уходили на отдельную строку во всю ширину, и на телефоне получалось `[☰ · поиск]` над `[🔔 🌙 Диагностика Открыть Scan]` — две широкие пилюли на своей строке. Теперь «Диагностика» и «Открыть Scan» — квадраты 36 px (🩺 / ⚡) рядом с колокольчиком и переключателем темы, и всё помещается в одну строку при 320 px. Замерено, а не на глаз: ширина документа равна вьюпорту при 320, 360, 390 и 430 px и на всей полосе 560–760 px — там, где кнопки прежде наезжали на поиск. **Локаль больше не влияет на мобильную раскладку вовсе**, потому что подпись скрыта, а кнопка фиксированного размера; выше 900 px подписи возвращаются без изменений. Доступное имя берётся из `aria-label`, так как подпись с `display:none` выпадает из вычисления имени.
- **Сервер работал на родителе 1.31.0, хотя релиз собран под 1.32.0.** v1.231.0 был паритетным, но в `/opt/career-ops/src` лежали файлы 1.31.0, и `/api/health` на resumecraft.ru отдавал `parentVersion: 1.31.0`. Выложено 95 файлов рантайма и удалены два (`plugins-registry/theirstack.json` — снят выше по течению; `lib/context-budget.test.mjs` — переехал в `tests/`). Найдено браузерным регрессом на живой поверхности.
- **`cvstart.ru` приклеивал `/ru/` к любому пути, не глядя на то, что там уже есть.** `cvstart.ru/ru/help` превращался в `cvstart.org/ru/ru/help`, а `cvstart.ru/en/help` — в `cvstart.org/ru/en/help`; оба 404. Первое получается, если скопировать адрес с основного сайта; второе полностью закрывало англоязычному посетителю доступ к английской версии, потому что любой языковой префикс перезаписывался. Теперь путь, уже называющий одну из 16 локалей, проходит как есть, `en` отбрасывается (на cvstart.org английский живёт в корне, каталога `/en/` нет), остальное по-прежнему получает `/ru/`. Проверка локали заякорена, поэтому `/enterprise/` не принимается за `en`.

### Изменено
- **98 устаревших QA-промптов перенесены в `qa/archive/superseded-prompts/`.** В `qa/README.md` с версии v1.137.0 записано, что в корне остаётся только текущий релизный промпт, — и правило не применялось 95 релизов. Теперь в README сказано, что архивация происходит при отгрузке релиза, и исправлены его собственные устаревшие показатели (`≥1238` → `≥3009` тестов, 13 → 17 локалей, 19 H2 / 75 H3 → 32 H2 / 122 H3).
- **С верхнего уровня ушли ещё пять промптов — они называли себя актуальными, не будучи таковыми.** `REGRESSION-FINAL.md` объявлял себя «единственным авторитетным, версионно-независимым регрессионным промптом», но его §§11–15 — пять закрытых журналов циклов (v1.55.x → v1.59.7), а заявленная база хелпа — **28 H2 / 103 H3 на 16 локалей** против 32 / 122 и 17 сегодня: QA-агент, идущий по нему, упал бы на §9 первым же шагом. У `QA-REGRESSION-PROMPT.md` имя без версии, а содержимое от v1.226.0. `QA-REGRESSION-PROMPT-v1.76.0-FULL.md` закреплён на 155 релизов назад, `REGRESSION-PROMPT-FINAL.md` закрывает цикл v1.58.52 → v1.59.10, а `QA-FULL-REGRESSION.md` дублировал их всех на базе v1.131.1. Туда же ушёл `PARENT-SYNC-WORKLIST-v1.26.0.md` (родитель уже 1.32.0), а датированный `UX-AUDIT-2026-07-06.md` переехал в `reports/`. Осталось четыре драйвера: промпт этого релиза плюс `FUNCTIONALITY-CHECK.md`, `UX-AUDIT-PROMPT.md` и `DESIGNER-EXPORT-PROMPT.md`, чьи собственные базы обновлены до фактических значений v1.231.2. `qa/README.md` переписан под ту папку, которая существует.

### Примечания
- Две ловушки специфичности, в которые я попал при правке панели, теперь описаны в `app.css`: десктопные правила `.topbar` / `.topbar-actions` / `.btn-ico` объявлены **ниже** блока `max-width: 900px`, поэтому при равной специфичности выигрывают они — мобильный `gap` молча оставался 24 px (это и переполняло 320 px), а иконка оставалась скрытой (кнопки выходили пустыми квадратами). Мобильные правила на класс глубже намеренно.
- `applyI18n()` присваивает `el.textContent`, поэтому ключ i18n переехал с `<button>` на внутренний `<span class="btn-label">`: на кнопке он стирал бы иконку при каждой смене языка. Тест BUG-008-tb теперь проверяет, что ключ находится **внутри** `#btn-doctor`, а не является его атрибутом — тот же инвариант, проверенный там, где он теперь живёт.
- Первый rsync родителя **оборвался на полпути** на файле, которого больше нет локально, и оставил частичную выкладку, отрапортовавшую успех, — та же форма отказа, что записана про висячий симлинк в `QA-REGRESSION-PROMPT-v1.228.3.md`. Выкладки родителя теперь проверяются контрольными суммами, а не кодом возврата.

## [1.231.1] — 2026-09-06

**Исправлено — свип счётчиков из v1.231.0 переписал имя аккаунта во всех ссылках.**

### Исправлено
- **Каждая ссылка `Fighter90` стала `Fighter92`.** Свип счётчика источников в v1.231.0 заменял `90` на `92` под защитой цифровой границы — `(?<![0-9])90(?![0-9])`, — которая верно пропускает `290` и `901`, но совпадает с `90` внутри **`Fighter90`**: перед ним буква, а после токен просто кончается. **562 ссылки во всех 17 README, всех 17 наборах справки и зеркалах справки на сайте**, плюс **138 в вики**, вели на несуществующий аккаунт; баннер `providers.png` и все ссылки «править на GitHub» отдавали 404. Заметил пользователь — по пропавшей картинке.
- **Пять исторических отсылок к релизам сдвинулись.** Свип `85 → 87` в том же релизе превратил `v1.85.0` в `v1.87.0` в README и справке, придумав версию, которой не выпускали.

### Примечания
- Цифровой границы недостаточно. Она защищает число от соседних **цифр**, но не от того, что оно часть **идентификатора** или **номера версии**, — а обоих в этих файлах много. Сами счётчики верны и не менялись: 92 источника, 87 EN-адаптеров, 3009 тестов.
- Родительский репозиторий не пострадал: его счётчики я не свипал.

## [1.231.0] — 2026-09-05

**Добавлено — два источника сканирования из родительского career-ops v1.32.0 (Collage, Telegram строгий), режим REST у Gem и один дефект, который порт вскрыл в коде самого родителя.**

### Добавлено
- **Два новых источника, 90 → 92** (87 EN + 5 RU), `ALL_ADAPTERS` **85 → 87**. Оба без токенов.
  - **Collage** (`provider: collage`) — публичный API джоб-сайтов Collage HR. Сегмент пути — это **адрес джоб-сайта, который арендатор выбрал сам**, а не слаг из названия компании, поэтому он читается из `api:` либо из careers-адреса `secure.collage.co/jobs/<адрес>` и **никогда не угадывается**: угаданный адрес привёл бы к сканированию чужой доски. Запись, не называющая ни того ни другого, падает громко.
  - **Telegram (строгий)** (`provider: telegram-channel`) — читает те же превью `t.me/s/<канал>`, что и существующий источник `telegram`, но с **обратным разменом**. `telegram` берёт каждый пост и приписывает неподписанный хэндлу канала; `telegram-channel` выдаёт строку **только если пост называет работодателя И ссылается на страницу вакансии**. Дайджесты, посты со скрытым работодателем («название скрыто», «our client») и посты «пишите в личку» отбрасываются. Низкая полнота по замыслу — выше по течению измерено 32–77 % проходящих постов на HR-каналах — но каждая строка несёт настоящего работодателя и настоящую ссылку. Оба остаются, как и у родителя: это разные продукты, а не дубликаты.
- **У Gem появился режим REST по выбору** рядом с путём GraphQL (`api: https://api.gem.com/job_board/v0/<board>/job_posts`). Gem документирует этот неаутентифицированный интерфейс для собственных карьерных страниц. Он никогда не выводится из идентификатора доски — выведенный адрес был бы догадкой о том, какую поверхность открыл арендатор, — а путь GraphQL остаётся умолчанием, так что для записи без `api:` ничего не меняется. Перенесено из родительского #3783.

### Исправлено
- **Фильтр локаций у `telegram-channel` не видел кириллицу и называл города работодателями.** В родительском `LOCATIONISH_RE` стояла `\b` — граница только по ASCII, рядом с кириллицей она не срабатывает никогда. Поэтому все русские варианты в нём (`москва`, `спб`, `офис`, `удалёнк*`) были недостижимы, и `Senior Engineer | Москва` возвращал **«Москва» как работодателя**. Это ровно та подстановка неверного поля, ради предотвращения которой вся политика атрибуции и написана, и бьёт она по основной аудитории провайдера: русскоязычный канал пишет локацию кириллицей. В порте web-ui использованы юникодные проверки (`\p{L}` / `\p{N}`) — то же самое, что родитель уже делает строкой ниже в `ROLE_WORD_RE`. Закреплено тестом, который падает на ASCII-форме. **Выше по течению пока не исправлено.**

### Примечания
- **Работа родителя по восстановлению Workday через фасеты (#3851, #3874) переноса не потребовала.** Она отдаёт приоритет настроенным локациям при выборе разбиения и помечает разбиение, чей фасет не покрывает доску. **web-ui вообще не листает и не разбивает Workday по фасетам** — один POST при `offset: 0` — так что ни того ни другого пути кода здесь нет. Тот же вывод, что и в v1.229.0, перепроверен, а не принят на веру.
- **`providers/_types.js`** изменился только в документации (необязательное поле `Job.salary`).
- Тесты: **2962 → 3009** (+47).

## [1.230.0] — 2026-09-05

**Изменено — три исправления провайдеров из родительского career-ops v1.32.0 и один дефект, который вскрыл сам перенос.**

### Добавлено
- **Welcome to the Jungle принимает серверное выражение `filters` для Algolia** (`wttj: { filters: "offices.country_code:FR AND contract_type:full_time" }`). Ключевое слово само по себе глобальную доску не сужает: какие именно `max_hits` результатов вернутся, решает ранжирование Algolia, а не фильтры сканера — они срабатывают позже, уже по пришедшему, поэтому всё за пределами предела невидимо, как бы хорошо оно ни подходило. Фильтр сокращает сам **набор**, а не переупорядочивает его, и потому предел на запрос поднимается с 200 до 1000. При заданном фильтре `queries` становится необязательным: пустой запрос означает «всё, что проходит фильтр». Перенесено из родительского #3757.

### Исправлено
- **Radancy больше не пишет «truncated», когда у арендатора просто меньше вакансий, чем заявляет его собственный баннер.** Родитель сверил `data-total-results` с девятью живыми арендаторами: пять совпали, а четыре — и маленький, и очень крупный — отдали на 10–56 % меньше уникальных вакансий, чем объявлено, без единой дублирующейся строки. Баннер завышает то, что реально отдаёт поисковый индекс арендатора, поэтому сравнение с накопленным счётчиком давало ложное срабатывание на большинстве досок. Теперь предупреждение возникает только когда walk остановил **наш собственный** `max_jobs`/`max_pages` — единственный случай, с которым вызывающая сторона может что-то сделать; `totalResults` остаётся в тексте как контекст и ничего не решает. Перенесено из родительского #3839.
- **Radancy обходит кеш на каждом JSON-запросе фрагмента.** Кеширующий слой перед этим маршрутом у части арендаторов отдаёт устаревшие ответы: одна и та же вакансия появляется страницами позже, а другая не приходит вовсе — воспроизведено выше по течению девять прогонов из девяти. Заголовки `Cache-Control`/`Pragma` не изменили ничего, случайный параметр на запрос — исправил. Он обязан быть уникальным на **вызов**, а не выводиться из номера страницы: детерминированный параметр всё равно даёт кешу устойчивый ключ, а значит то же устойчивое неверное соответствие. Перенесено из родительского #3839.
- **Списки офисов Greenhouse упорядочены детерминированно.** Greenhouse не обещает порядок `offices[]` между ответами, а набор, восстановленный из дерева `/offices`, идёт в порядке обхода. Оба попадают в поле `location` вакансии, поэтому перестановка офисов на стороне доски переписывала эту строку — и вакансия, в которой ничего не менялось, читалась как новая. Родитель исправил набор (#3839); **в web-ui обнаружился второй экземпляр того же дефекта, которого у родителя нет** — запасной вариант `offices[0]`, используемый когда у вакансии нет своей локации, — и отсортированы оба.

### Примечания
- **Исправление iCIMS (#3728) переносить не потребовалось.** Оно читает все записи `jobLocation` из JSON-LD страницы вакансии, потому что часть арендаторов ставит запись со сплошным `UNAVAILABLE` перед настоящим адресом. web-ui берёт локацию из HTML **страницы списка** и страницу вакансии не открывает вовсе — такого пути кода здесь нет.
- **Изменение `_http.mjs` переносить не потребовалось.** Оно выносит `isRefusedRedirectError`, чтобы `discover-ats.mjs` и слой повторов перестали расходиться в оценке одной и той же ошибки. Поведение `isRetryableError` не меняется, а второго потребителя в web-ui нет — выносить хелпер ради одного вызывающего было бы шумом.
- **Дедупликация по локации (#3751) не зеркалится.** Она живёт в конвейере `scan.mjs` CLI родителя; у маршрута сканирования web-ui своя.
- Реестр без изменений: **90 источников** (85 EN + 5 RU). Тесты: **2956 → 2962** (+6).

## [1.229.0] — 2026-09-03

**Добавлено — четыре источника сканирования из родительского career-ops v1.31.0: Built In, Feishu Jobs, Garena и MokaHR.**

### Добавлено
- **Четыре новых источника, 86 → 90** (85 EN + 5 RU), `ALL_ADAPTERS` **81 → 85**. Все четыре без токенов: ни ключа, ни входа.
  - **Built In** (`provider: builtin`) — американская техническая доска, по всем девяти её рынкам. Работодатели публикуются там напрямую, поэтому это агрегатор вроде RemoteOK, а не источник по одной компании. Дефолтного запроса у него **нет**: запись без `queries:` и `categories:` не сканирует ничего и прямо об этом сообщает — общий источник не должен нести чьи-то личные поисковые слова. Рынок вне списка разрешённых **отклоняется**, а не подменяется тихо национальной доской: опечатка не должна незаметно расширять скан.
  - **Feishu Jobs** (`provider: feishu-jobs`) — 飞书招聘, тот самый эндпоинт, который вызывает страница вакансий самого тенанта. Две формы хоста и никаких других: ровно `jobs.bytedance.com` либо тенант `*.jobs.feishu.cn`. Пути к странице вакансии у них **разные**, так что один шаблон URL дал бы половине битые ссылки.
  - **Garena** (`provider: garena`) — одна компания, один фиксированный хост. `office` формирует **ссылку** на вакансию, а не выдачу: апстрим проверил живьём, что любой код офиса, включая выдуманный, возвращает ту же доску.
  - **MokaHR** (`provider: mokahr`) — китайская HR SaaS. **Ответ зашифрован**: конверт AES-128-CBC, ключ к которому приезжает рядом с шифротекстом. Это обфускация, а не защита, но обычный разбор JSON не увидит ничего, поэтому расшифровка — часть чтения доски как таковой. Два тенанта отклоняются на этапе конфигурации: их robots.txt закрывает путь к вакансиям.

### Исправлено
- **`htmlToText` мог пропустить незакрытый открывающий тег в текст.** `safe &lt;img src=x onerror=1` выходил как `safe <img src=x onerror=1`: закрывающей скобки нет, снимать разметке нечего, и опенер доживал до вывода. Теперь за каждым декодированием следует снятие разметки, а у концевого опенера убирается угловая скобка — текст остаётся читаемым и становится безвредным. Портировано от родителя (#3491).

### Примечания
- **Из фикса Workday у родителя не портировано ничего.** Он спасает тенантов, у которых бэкенд CXS зажимает пагинацию на offset 2000, — а web-ui Workday вообще не пагинирует (один POST с `offset: 0`, 100 строк), так что зажимать нечего. Собственный потолок web-ui в 100 строк — отдельное, более грубое ограничение, и не тема этого релиза.
- **Фиксы `verify-cv-facts` и `merge-tracker` у родителя порта не потребовали**: web-ui запускает эти скрипты из чекаута родителя, а не зеркалит их логику, поэтому они приезжают вместе с ним.
- Зарплата из Built In приводится здесь к строке. Родительский `parseSalary` отдаёт `{ min, max }` для своих фильтров, а контракт web-ui хранит `salary` текстом — пропустив объект, мы получили бы `[object Object]` в трекере.
- Тесты: **2909 → 2956** (+47).


## [1.228.5] — 2026-08-30

**Исправлено — сообщаемая версия описывала файл на диске, а не исполняемый код.**

### Исправлено
- **`/api/health` перечитывал `package.json` на каждый запрос, поэтому устаревший процесс рапортовал версию, которую не исполнял.** QA нашёл локальный инстанс, отвечавший `version: 1.228.4` и при этом дававший 404 на `/api/ping` — маршрут, добавленный в 1.228.3: процесс был запущен до развёртывания, обслуживал старый код и сообщал версию нового файла. Именно так выглядит успешным развёртывание, которое скопировало файлы, но не перезапустило процесс: единственная строка, на которую смотрит оператор, — та самая, что не способна увидеть проблему. Это тот же класс, что симлинк из v1.228.1, где rsync обрывался посреди передачи и всё равно отчитывался об успехе.

  Версия теперь фиксируется при загрузке модуля и описывает работающий код. Устаревший процесс сообщит **старую** версию — правду, и сразу заметную. `parentVersion` остаётся почитанным на каждый запрос: он описывает родительский чекаут, который этот процесс не загружает и который законно меняется под работающим сервером.

## [1.228.4] — 2026-08-30

**Исправлено — проба живости без авторизации обращалась к файловой системе на каждый запрос.**

### Исправлено
- **`GET /api/ping` читал и разбирал `package.json` при каждом обращении.** Это единственная точка, доступная без учётных данных, поэтому чтение с диска и разбор JSON на каждый запрос — рычаг отказа в обслуживании, выданный кому угодно; CodeQL пометил это как отсутствие ограничения частоты. Версия теперь читается один раз при регистрации маршрута — измениться без перезапуска она всё равно не может, — и обработчик не делает ввода-вывода вовсе. Ограничение частоты урезало бы ущерб; удаление работы убирает сам рычаг.
- **Маскирование имени владельца профиля вне loopback теперь закреплено тестом.** `/api/health` и раньше заменял его на `hidden` при не-loopback привязке — но лишь потому, что эта строка делит охранник `hidden ?? value` с корнем проекта. Тест этого не проверял, и правка, давшая строке собственное значение, утекла бы настоящим именем в локальную сеть незамеченной. Замечание из ревью v1.228.3.

## [1.228.3] — 2026-08-30

**Исправлено — ассистент по документации не отвечал по-английски, названия компаний из Telegram приходили обрезанными, а мониторинг не мог увидеть, что сервис жив.**

### Исправлено
- **Ассистент отвечал «подходящих разделов справки не найдено» по-английски, хотя тот же вопрос по-русски работал.** Поиск был исправен — виноват сборщик контекста. В `buildAskPrompt` стояло `if (ctx.length + sec.body.length > MAX_CONTEXT) break` — именно `break`, поэтому один переросший раздел обрывал цикл и оставлял контекст **пустым**, а ассистент честно сообщал, что у него ничего нет. Английский §5 вырос до **16 081 байта при бюджете 14 336** (в том числе потому, что v1.228.0 добавила в него `telegram_channels`), русский весил 13 009 и влезал. Ранжирование в обоих языках одинаковое — §5 первый, — различался только размер.

  Две правки: раздел свыше 6 КБ теперь делится по границам `###`, поэтому `telegram_channels` стал отдельным чанком на 1 340 байт и занимает **первое место** в обоих языках; а переросший раздел пропускается или усекается, но больше не обрывает цикл. Руководство разбилось на 75 чанков вместо 32.
- **Метка компании в Telegram давала предложение вместо названия.** `Компания: FinCore Technology (продуктовая разработка, высоконагруженные системы)` превращалось в `FinCore Technology (продуктовая разработка, высоконагруженны` — обрезано на 60 символах, посреди слова. Три строки из 141. Теперь имя заканчивается на первом разделителе, вводящем описание: тире **в пробелах** (поэтому `Coca-Cola` уцелеет), запятая, точка с запятой, вертикальная черта или открывающая скобка; остаточное ограничение падает на границу слова. Этот источник приписывает компанию как факт, и обрезанное предложение неверно ровно так же, как выдуманное имя, — только менее заметно.

### Добавлено
- **`GET /api/ping` — проба живости, не раскрывающая ничего.** Только `{ ok, version }`. `/api/health` сообщает абсолютные пути, настоящее имя владельца профиля и то, какие API-ключи заданы: это законно для аутентифицированного оператора и недопустимо для открытого интернета. У мониторинга за Basic-аутентификацией до сих пор вообще не было способа увидеть, что сервис жив.

## [1.228.2] — 2026-08-30

**Исправлено — поиск из двух слов в `/api/scan-results` искал фразу, а не слова, и сильно недобирал.**

### Исправлено
- **`?q=` теперь сопоставляет каждое слово по отдельности, а не фразу целиком.** Запрос был одним `includes(q)` по заголовку, компании и локации, поэтому `продакт менеджер` находил только строки, где эти два слова стоят именно рядом, — **32 из 162**, которые содержат оба. «Менеджер продукта» и «Менеджер по развитию продукта» были для него невидимы: те же слова, но в другом порядке или с наполнителем между ними.

  Так узкое совпадение превращается в **неверный ответ**. Telegram-ассистент на просьбу найти всех продуктовых менеджеров сообщил 9 — честно, потому что 9 ему и ответил эндпоинт. Его число было правдивым, а вот число эндпоинта — нет. Порядок слов и то, что стоит между ними, человек, набирающий два слова, закреплять не собирался.

  Сопоставление идёт по И, так что расширение не стоит точности: строка обязана содержать каждое слово. `менеджер разработчик` вернёт ноль, а не объединение.

## [1.228.1] — 2026-08-29

**Исправлено — висячий симлинк в репозитории, молча обрезавший деплой.**

### Исправлено
- **`.claude/skills/refero-design` был отслеживаемым симлинком на путь, которого здесь никогда не было.** Он указывал на `../../.agents/skills/refero-design`, то есть на `.agents/` в корне репозитория; такой директории нет, а сама ссылка попала сюда в пакете паритета v1.118.0 с машины, где вложенность папок была другой. С тех пор любой, кто клонировал репозиторий, получал битую ссылку.

  И она не была безобидной. **rsync прерывается на ней посреди передачи** — сегодняшний деплой отчитался об успехе, пока сервер продолжал работать на прежней версии, и расхождение всплыло лишь потому, что версию проверили после. Родительский `node test-all.mjs` тоже падает, если чекаут web-ui лежит внутри его дерева: копия фикстур обходит рабочую директорию и не может сделать stat по ссылке. Ссылку никто не использовал: единственные совпадения `refero` в репозитории — польское слово *preferowany*.

## [1.228.0] — 2026-08-29

**Добавлено — Telegram-каналы как источник сканирования и постраничный режим запроса у `/api/scan-results`.**

### Добавлено
- **Telegram-каналы стали сканируемым источником.** Отдельный блок `telegram_channels:` в `portals.yml` перечисляет каналы; каждый читается через публичный веб-превью `https://t.me/s/<канал>`. Реестр: **85 → 86 источников** (81 EN + 5 RU), `ALL_ADAPTERS` **80 → 81**.

  У Telegram нет своего RSS, а Bot API не позволяет читать канал, которым бот неадминистрирует. Оба очевидных пути закрыты. Превью `/s/` — обычный HTML с сервера: без входа, без JS, без куки. Проверено на 16 каналах: 15 отдали по 20 постов на голый GET.

  `channel:` принимает любую запись — `rabotaphp`, `@rabotaphp`, `https://t.me/rabotaphp`, даже ссылку на конкретный пост. Парсер опирается на три вещи, которые редизайн Telegram сдвинет в последнюю очередь: `data-post="<канал>/<id>"` (он же разбивает страницу на посты и несёт единственный стабильный URL поста), контейнер текста сообщения и ISO-дату `<time datetime>`.
- **У `GET /api/scan-results` появился постраничный режим запроса.** `?q=<текст>&limit=50[&region=ru|en][&set=filtered|fresh][&offset=N]` возвращает `{ total, returned, offset, limit, set, rows }`, где **`total` — количество ДО пагинации**. Без параметров роут по-прежнему отдаёт снимок целиком: от этой формы зависит таблица `#/scan`.

### Исправлено
- **Telegram-ассистент отвечал по доле данных.** Его навык уже указывал на `/api/scan-results`, но роут умел отдавать только весь снимок — около 2 МБ на реальной машине, что не влезает в контекст агента. На «найди все Продуктовый менеджер» он сообщил **9** совпадений там, где в снимке их были сотни, и честно сказал, что «работает с ограниченным локальным индексом». Это было правдой: эндпоинт он видел, а съесть не мог. С постраничным режимом тот же вопрос стоит 2 КБ и возвращает честный `total`. Навык тоже переписан: он предлагал сводить *новые* посты, то есть указывал на `fresh` (только последний прогон) вместо `filtered` (всё оставленное).

### Примечания
- **Пост канала — это проза, а не запись о вакансии.** В отличие от всех прочих источников здесь нет структурированных полей: источник берёт заголовок из первой содержательной строки, компанию/город/вилку — по явным меткам («Компания:», «Локация:», сумма с валютой), а полный текст кладёт в описание. **Отделять настоящие вакансии от рекламы и дайджестов оставлено существующему `title_filter`** — тому же, что уже настроен для других источников. Это лиды, а не готовый список.
- **Компания никогда не угадывается.** Без метки «Компания:» запись атрибутируется каналу (`@rabotaphp`), потому что неверный работодатель попадает в трекер как факт.
- **Пустой разбор — это ошибка, а не «вакансий нет».** На закрытый или несуществующий канал t.me отвечает редиректом; вернуть ноль строк значило бы показать спокойный день и навсегда спрятать опечатку в конфиге. `jobGeeks` из исходного списка ведёт себя именно так (HTTP 302) и намеренно **не** добавлен.
- Определение удалёнки использует Unicode-границу слова вместо `\b`: `\b` работает только с ASCII, поэтому пробел перед кириллическим словом границей не является и `\bудал` никогда не совпадал с «удалёнка» — та же ловушка, что и в фильтре заголовков в v1.227.3, здесь пойманная тестом до релиза.
- Настроено и проверено живьём 15 каналов: **299 постов, 15/15 каналов**, `salary_pm` дал 13 вилок из 20. 30 новых тестов. Тесты: **2865 → 2895**.
- **Счётчики тестов в README подскочили на сотни — и нет, этот релиз их не добавил.** В локализованных README лежали замороженные значения: 1856 / 1945 / 1955 / 549 / 419 / 284 / 2527 — каждое устарело со своего релиза, и некоторые противоречили бейджу на той же странице. Теперь везде **2895**, и это проверено прогоном набора, а не переписыванием бейджа. Так же устарели Playwright (70 / 90 / 12 → **101**), smoke e2e (20 → **21**), модули маршрутов (12 / 32 → **37**), потоки smoke (12 → **22**) и число файлов тестов (218 → **329**). Строка `Итого` дублировала счётчик юнитов и потому не сходилась с тремя наборами ниже — теперь там настоящая сумма (**3040**).

## [1.227.5] — 2026-08-29

**Исправлено — региональный скан выедал память сервера, и один Playwright-сьют флейкал на собственном teardown.**

### Исправлено
- **Региональный скан ронял сервер по OOM.** `ru-scanner` накапливал **все сырые хиты по всем запросам** и дедуплицировал с фильтрацией только в самом конце. Список запросов намеренно состоит из близких синонимов (`Golang`, `Go разработчик`, `Golang разработчик`, `Senior Go`…), поэтому одна и та же вакансия возвращается по разу на запрос, и массив держал кратно больше объектов, чем уникальных — вместе с описаниями, которые составляют основную массу объекта и почти все всё равно отсеиваются фильтрами. Замерено: **742 МБ** кучи на реальном прогоне из 21 запроса. Сервер ограничивает кучу Node **490 МБ** (V8 считает лимит от 956 МБ RAM машины), поэтому скан валил сервис с `FATAL ERROR: Reached heap limit` — **четыре раза за сутки**. Теперь дедуп и фильтрация идут по каждому запросу, и тот же прогон уложился в **177 МБ**, минус 76%. Отчётные числа не изменились: уникальный итог несёт `Set` из URL-строк, так что «Total found» значит ровно то же, что и раньше.
- **`net::ERR_SOCKET_NOT_CONNECTED` роняла CI случайным образом.** Три Playwright-сьюта вызывают `window.stop()`, чтобы оборвать незавершённые запросы перед проверкой «консольных ошибок нет». Chromium сообщает об одном и том же намеренном действии **тремя** именами в зависимости от состояния сокета: `ERR_ABORTED` при отмене запроса, `ERR_EMPTY_RESPONSE` в полёте и `ERR_SOCKET_NOT_CONNECTED`, когда сокет уже разорван. Общий фильтр знал только первые два, поэтому один и тот же teardown ронял прогон наугад. Всплыло на прогоне по локалям в `tr` — сьюте с наибольшим числом навигаций, — тогда как `playwright-forms` и `playwright-smoke`, которые тоже вызывают `window.stop()`, были в одном неудачном прогоне от того же.

### Примечания
- **Почему скан начал падать только сейчас.** Список запросов вырос с 14 до 21, когда добавили поиски продакт-менеджера. Память старой реализации росла с общим числом хитов, поэтому +50% запросов пробили потолок машины. С новыми запросами всё в порядке — они лишь вскрыли схему выделения памяти, которая давно была в одном изменении конфига от отказа.
- **Правка дедупа сохраняет поведение, и это проверено, а не заявлено.** `tests/ru-scanner-dedup-order.test.mjs` фиксирует неудобное направление — более поздний дубликат, НЕ проходящий фильтры, обязан вытеснить более ранний прошедший, потому что старый проход в конце оставлял только последнюю копию и затем её отфильтровывал — и прогоняет 200 рандомизированных сверок против прежней реализации «накопить, потом отфильтровать».
- **Фильтр консоли остаётся узким.** Безобидно только семейство отменённых запросов. Отказ в соединении, сбой DNS, сброс, любой 5xx и любое необработанное JS-исключение по-прежнему валят проверку — это закреплено новыми тестами.
- Две гипотезы были проверены и отброшены, а не отправлены в релиз: таймаут Caddy (конфиг чистый, `flush_interval -1` для SSE верен) и дефолтный 5-минутный `requestTimeout` Node (уменьшенная репродукция показала, что он длинный стриминговый ответ не рвёт). Настоящей уликой стал `Reached heap limit` в журнале. Тесты: **2860 → 2865**.

## [1.227.4] — 2026-08-29

**Исправлено — `word:` / `stem:` всё ещё игнорировались в `content_filter`; v1.227.3 починил только фильтр заголовков.**

### Исправлено
- **`content_filter` не учитывал префиксы.** v1.227.3 научил `title_filter` родительским префиксам `word:` / `stem:`, но `content_filter` остался на собственном сыром `lower.includes(k)`, так что запись с префиксом там по-прежнему сопоставлялась как буквальный текст `"word:java"` — та же молчаливая пустышка, на один фильтр дальше. `content_filter` читает ОПИСАНИЕ вакансии, и его умолчание всегда было простой подстрокой — именно поэтому голый негатив `java` отбрасывает всё, где просто упомянут «JavaScript»; префикс и есть способ отказаться от этого для одной записи. Портирован родительский `compileContentKeyword`: он использует ту же машинерию префиксов, но намеренно **не** привязывает короткие ключи к границам слова — фильтр заголовков привязывает аббревиатуры из 2–3 букв, потому что «COO» внутри «Coordinator» всегда ошибка, тогда как короткая последовательность внутри абзаца прозы обычно осмысленна (`aws`, `gcp`, `sql`, `go`).

### Примечания
- **Найдено в AI-ревью PR v1.227.3**, которое заметило, что код не покрывает то, что заявляет его же комментарий — там было «`title_filter` / `content_filter`», хотя подключён был только путь заголовков. Теперь комментарий явно разделяет: префиксы работают в обоих фильтрах, автопривязка аббревиатур — только в заголовках.
- **Тоже проверено по тому ревью и не изменилось:** перевод правила для аббревиатур 2–3 букв с `\b` на Unicode-границу поведенчески идентичен для ASCII и строже только там, где нужно — `COO_lead` и `coo1` по-прежнему сохраняются (подчёркивание и цифра — символы слова в обоих вариантах), `Coordinator` сохраняется, «Директор COO» и «VP «Продукт»» корректно отбрасываются. Проверено 14 краевых случаев.
- Записи `content_filter` без префикса сохраняют прежнее сопоставление байт в байт, пустое/отсутствующее описание по-прежнему проходит, а голый `word:` / `stem:` не матчит ничего вместо всего. 6 новых кейсов в `tests/content-filter.test.mjs`. Тесты: **2852 → 2858**.

## [1.227.3] — 2026-08-29

**Исправлено — префиксы `word:` / `stem:` в фильтре заголовков молча игнорировались, превращая эти строки фильтра в пустышку.**

### Исправлено
- **web-ui не реализовывал родительские префиксы `word:` / `stem:`.** `title_filter` (и `content_filter`) по умолчанию сопоставляют **подстроку** без учёта регистра — именно поэтому голый негатив `intern` отбрасывает и «International Product Manager», и «Internal Tools Engineer». Вместо смены умолчания (ломающей все настроенные установки) родитель сделал точность опциональной для каждой записи: `word:intern` матчит только целое слово, `stem:agent` должен начинать слово и может продолжаться (отделяя «Agentforce» от «Reagents»). В web-ui не было ни того, ни другого, поэтому запись с префиксом сопоставлялась как **буквальный текст** `"word:intern"`, которого нет ни в одном заголовке вакансии. Строка становилась молчаливой пустышкой: тот же `portals.yml` корректно фильтровал через CLI и совсем не фильтровал здесь, так что все стажёрские вакансии, которые пользователь исключил, всё равно попадали в таблицу `#/scan`. Портировано из родительского `title-keywords.mjs`, включая границу слова с учётом Unicode (`\p{L}\p{M}\p{N}_`) вместо `\b`, который работает только с ASCII и матчил внутри слова в заголовках с диакритикой и кириллицей.

### Примечания
- **Найдено при проверке живого `portals.yml`**, куда только что добавили `word:intern` **именно** чтобы голый `intern` перестал выбрасывать «International Product Manager». В CLI префикс работал; web-ui тихо оставлял все стажёрские вакансии. Теперь поведение идентично.
- **Умолчания не изменились.** Запись без префикса сохраняет прежнее сопоставление: подстрока для фраз и всего, что содержит не-буквы (`.NET`, `SAP `, `L&D`), и привязка к границам слова для аббревиатур из 2–3 букв. Голый `word:` / `stem:` без продолжения считается опечаткой и не матчит **ничего** — как негатив пустой шаблон совпал бы с любым заголовком и зарубил бы весь скан из-за одного случайного двоеточия.
- Правило для аббревиатур 2–3 букв переведено на ту же Unicode-границу: раньше оно использовало `\b`, из-за чего `coo` и `word:coo` расходились на не-ASCII заголовках — два написания одного правила.
- 6 новых кейсов в `tests/title-filter.test.mjs`. Тесты: **2846 → 2852**.

## [1.227.2] — 2026-08-29

**Исправлено — `#/config` уезжал вбок на узких телефонах.**

### Исправлено
- **MOBILE-1: `#/config` выходил за экран по горизонтали на вьюпортах ≤ 352 px.** Каждое поле-селект несло **инлайновый** `min-width: 300px`. Инлайновый стиль перебивает таблицу стилей, поэтому контрол не мог ужаться вместе с контейнером: на вьюпорте 320 px правый край селекта оказывался на 353 px, страница получала горизонтальную прокрутку, а контролы уезжали за правый край. Затронуты все 18 селектов провайдеров/моделей (`cfg-llm-provider` и все `cfg-*-model`). Теперь `min-width: min(300px, 100%)` + `max-width: 100%`: комфортные 300 px применяются только когда есть место. Устройства на пороге и ниже: iPhone SE 1-го поколения (320 px), старые Android, внешний экран Galaxy Fold.

### Примечания
- **Почему все прошлые мобильные прогоны это пропускали.** На 360 и 375 px запаса ровно хватает, чтобы overflow не проявился, а Playwright-наборы работают в окне 1280 px. Ни один тест в CI никогда не мерил вьюпорт уже 375 px. Новый `tests/playwright-narrow-viewport.mjs` проходит маршруты с формами на **320 px** и падает на любом горизонтальном overflow, называя в сообщении самый широкий элемент-нарушитель; второй кейс фиксирует, что ни один селект `#/config` не выходит за вьюпорт. Сначала было проверено, что тест воспроизводит баг (`scrollWidth 353 > clientWidth 320`, 18 селектов с правым краем 353). Playwright: **99 → 101**.
- **Известно и намеренно пока не под гейтом: 280 px** (внешний экран Galaxy Fold) всё ещё уезжает примерно на 5 px, но по другой причине — длинный контент внутри блоков `<details>` карточки конфига и сайдбар 256 px против вьюпорта 280 px, а не инлайновый `min-width`, который чинит этот релиз. Переверстка под ширину, которой не заявляет ни один массовый телефон, — отдельная работа; список `WIDTHS` в сьюте к ней готов.
- Также исправлен QA-промт v1.227.1, где маршрут фоллоу-апа был назван `#/modes/followup`. Mode-страницы регистрируются «голым» слагом (`Router.register(cfg.slug)`), поэтому маршрут — `#/followup`, а `#/modes/followup` корректно отдаёт 404. Только документация.
- Юнит-сьют без изменений — **2846** (новое покрытие браузерного уровня).

## [1.227.1] — 2026-08-29

**Исправлено — разбивка источников в справке не сходилась сама с собой, а GitHub-баннер печатался в приложении сырой разметкой.**

### Исправлено
- **§17 давал итог и разбивку, которые не складывались.** Во всех бандлах справки было «registry `server/lib/sources/` поставляется с **85** адаптерами — **78 английских + 5 русских**». 78 + 5 = 83, а не 85; в ja/ko дрейф зашёл дальше — 77 (= 82). Каждый новый источник двигал итог и не трогал слагаемое, и числа расходились уже несколько релизов. Теперь разбивка — **80 EN + 5 RU** во всех 17 локалях. Это важнее косметики: `docs/help/<lang>.md` — единственный корпус для «Ask the docs», поэтому на вопрос «сколько источников поддерживается» пользователь получал два противоречащих числа из одного предложения.
- **Там же был устаревший якорь версии.** «По состоянию на **v1.213.0** registry …» дожил в 16 локалях, пока счётчик двигали дважды (83 в v1.219.0, 85 в v1.226.0). Якорь уже чинили вручную в v1.210.1 — и он вернулся. Теперь он убран в пользу формулировки без версии: сам счётчик проверяется против живого реестра, который устареть не может.
- **Баннер провайдеров печатался экранированной разметкой вверху `#/help`.** В v1.225.0/v1.225.1 записано, что его «вырезает `UI.md()` (нет поддержки картинок)». Это смешивает две разные вещи: `UI.md()` экранирует-первым — он экранирует каждый байт источника до любых markdown-преобразований, и это правильная XSS-граница, но противоположность вырезанию. Баннер никто не удалял, потому что этого никто и не пытался, — и все 17 локалей открывали `#/help` 268 символами литерального `<p align="center"><img src="https://…`. Новый общий вход (`server/lib/help-markdown.mjs::stripGithubOnlyBlocks`) убирает отдельно стоящие HTML-блоки с картинкой при чтении — и для `GET /api/help/:lang`, и для корпуса docs-assistant. В файлах баннер остаётся, поэтому на GitHub витрина по-прежнему рендерится.
- **Колонка «#» на доске каденции была пустой с v1.117.0.** Она читала `entry.appNum`, тогда как `followup-cadence.mjs` отдаёт `num` — поля `appNum` в payload не было никогда. Теперь читаются оба. Строка «Next up» из v1.227.0 унаследовала ту же ошибку и починена вместе с ней.

### Примечания
- **Все четыре прожили незамеченными, потому что их ничто не проверяло.** Гейты справки (`canonical-docs-coverage`, `help-ui`, `help-ru-config-section`) считают ЗАГОЛОВКИ — 32 H2 / 121 H3, и ни один тест никогда не читал число внутри абзаца и не смотрел, что реально отдаёт вход справки. Это закрывают два новых сьюта: `tests/help-source-counts.test.mjs` (заявленный итог равен `SOURCES.length`; слагаемые равны живому EN/RU-разбиению и дают в сумме итог; в абзаце нет версии) и `tests/help-banner-strip.test.mjs` (баннер остаётся на диске для GitHub, не переживает strip, первая отрисованная строка каждого бандла — его заголовок, strip остаётся узким, и оба потребителя идут через него). Оба проверены на падение против ровно этих багов до коммита. Тесты: **2837 → 2846**.
- Реестр, сканер и контракты роутов не менялись; источников по-прежнему **85** (80 EN + 5 RU), `ALL_ADAPTERS` **80**.

## [1.227.0] — 2026-08-28

**Добавлено — SEEK Гонконг (JobsDB), фильтр «только срочные» для фоллоу-апов и нераскрытые работодатели теперь показывают посредника.**

### Добавлено
- **JobsDB Гонконг через существующий источник Jobstreet/SEEK.** Гонконг работает под брендом JobsDB, но стоит на той же платформе SEEK за тем же эндпоинтом v5, поэтому отдельный источник не нужен — достаточно `hk.jobsdb.com` в списке разрешённых хостов и `siteKey: HK-Main`. Реестр не меняется: **85 источников** (80 EN + 5 RU), `ALL_ADAPTERS` **80** — это новый РЫНОК на существующем адаптере, а не новый адаптер.
- **Фильтр «только срочные» на доске каденции** (`#/modes/followup`). Новый чекбокс сужает доску до того, что требует действия *прямо сейчас*. Срочность читается из `urgency`, которую уже вычислил родительский `followup-cadence.mjs` (`urgent` / `overdue`), и никогда из `status` трекера — там `applied`/`responded`/`interview`, что не совпало бы ни с чем. Когда срочного нет, доска называет ближайший запланированный фоллоу-ап вместо пустого экрана. Логика вынесена в чистый модуль `public/js/lib/followup-view.js` — зеркало родительского `followup-view.mjs`.
- **Нераскрытые работодатели атрибутируются агентству-посреднику.** Вакансия со скрытым работодателем записывается в трекер как `?`, а рекрутёр попадает в колонку `Via`. `#/tracker` показывал голый `?`, поэтому несколько нераскрытых строк были неразличимы, а резолверу логотипов не за что было зацепиться. Теперь такая строка читается как **«Не раскрыт · через \<агентство\>»**, логотип берётся по имени агентства, и строка находится по этому имени в поиске. Это только отображение — каноническое поле `company` никогда не перезаписывается рекрутёром. Новый чистый модуль `public/js/lib/company-presentation.js` — зеркало родительского `company-presentation.mjs`.

### Исправлено
- **Jobstreet/SEEK обращался к устаревшему эндпоинту.** Источник по-прежнему указывал на chalice-search v4 (`/api/chalice-search/v4/search`), который SEEK отключил; родитель перешёл на `/api/jobsearch/v5/search`, а web-ui за ним не последовал. Теперь любая запись Jobstreet/SEEK запрашивает v5. Форма элемента v5 читается нативно (URL строится из `id`, `locations[0].label`, `advertiser.description`, `salaryLabel`), проекционный параметр `solrFields` из v4 убран, а запись, у которой в `portals.yml` всё ещё зафиксирован мёртвый путь v4, сохраняет хост своего рынка, а путь пересобирается на v5 — правки от пользователя не нужны.

### Примечания
- Паритет с career-ops 1.30.0 (HEAD родителя `8d64f65` → `6cc46a4`; его `VERSION` всё ещё 1.30.0 — release-please отстаёт от `main`). Не портировано, с причинами: **заполнение локации из JSON-LD у iCIMS** — у источника iCIMS в web-ui вообще нет хука `enrichDate()` (только страницы списка), поэтому чинить нечего; **нефатальный stderr CLI (#1974)** — раннер web-ui определяет сбой только по коду выхода и никогда не имел этой эвристики; **трассировка путей Turbopack** — в web-ui нет сборщика; **фиксы `cv-sync-check`** — ретранслируются только на чтение и fail-soft; **последовательный Playwright в `scan.mjs`, ответы Block H, guard-ы updater/doctor/`update-system`, рекурсивная проверка синтаксиса, дашборд на Go и добавленные самотесты `jd-skill-gap`/`verify-cv-facts`** — только-CLI поверхности, в которые web-ui не заходит, либо самотесты без изменения поведения. Тесты: **2818 → 2837**.

## [1.226.0] — 2026-08-27

**Добавлено — два вьетнамских источника вакансий: ITviec и CareerViet.**

### Добавлено
- **ITviec + CareerViet (Вьетнам).** Две вьетнамские площадки с привязкой хоста подключаются к сканеру записями `provider: itviec` / `provider: careerviet`: **ITviec** (itviec.com, техно) и **CareerViet** (careerviet.vn, широкий охват). Оба — HTML-скраперы без токенов с SSRF-привязкой хоста, только-HTTPS, fail-soft по компаниям и лимитами страниц. Реестр теперь несёт **85 источников** (80 EN + 5 RU), `ALL_ADAPTERS` **80** — оба видны в фильтре «Источник» на `#/scan`.

### Примечания
- Паритет с career-ops 1.30.0 (parentVersion 1.29.0 → 1.30.0). Не портировано (только CLI/сообщество или уже есть): Hired Wall, `/calibrate`, clean-markers, паки шаблонов, doctor, pdf/LaTeX и CLI eval/tracker/scan, в которые web-ui не заходит; фикс `_html-to-text` и `roleFuzzyMatch` уже были; префикс `title-filter stem:` отложен. Тесты: **2818**.

## [1.225.1] — 2026-08-27

**Исправлено — баннер логотипов провайдеров теперь отображается в README и руководстве.**

### Исправлено
- Баннер v1.225.0 не показывался на GitHub: у отдельного `images/providers.svg` был некорректный `font-family` (вложенные двойные кавычки), который GitHub отклоняет; `<img>` использовал относительный `src` (raw HTML `<img>` не разрешает относительные пути на GitHub); а у блока `<p>` не было завершающей пустой строки, из-за чего он сливался со следующей строкой. Теперь он указывает на отрендеренный `images/providers.png` по абсолютному raw-URL, с разделением пустыми строками; SVG также исправлен до валидного XML.

### Примечания
- Только доки; без изменений кода и тестов. Витрина cvstart.org и плитки в приложении уже были корректны (встроенный SVG, не этот файл).  **2784**

## [1.225.0] — 2026-08-27

**Добавлено — баннер логотипов провайдеров в README и руководстве.**

### Добавлено
- **Баннер логотипов провайдеров.** Единый самодостаточный `images/providers.svg` — **18 LLM-провайдеров**: **реальный логотип бренда** ([simple-icons](https://simpleicons.org), CC0) для 11, публикующих иконку, и монограмма в цвете бренда для 7 остальных — теперь возглавляет README (×17) и встроенное руководство. (Само приложение рисует плитки с реальными логотипами; баннер отображается на GitHub, а XSS-безопасный рендерер приложения его убирает.)

### Примечания
- Только доки — без изменений кода и тестов; SVG встроенный (без удалённых ресурсов). Источники сканирования без изменений: **83**. Тесты: **2784**.

## [1.224.0] — 2026-08-26

**Изменено — плитки LLM-провайдеров теперь показывают реальные логотипы брендов.**

### Изменено
- **Реальные логотипы провайдеров.** Плитки провайдеров — поля API-ключей в Настройках, строки Использования, чип дашборда, сводка «Активен» в Настройках, результат ⚡-оценки и приветственный баннер — теперь отображают **реальный логотип бренда** для 11 провайдеров, публикующих open-source-иконку (Anthropic · Gemini · OpenAI · Qwen · OpenRouter · GitHub · DeepSeek · Kimi · MiniMax · Mistral · Ollama; одно-путевые SVG из [simple-icons](https://simpleicons.org), CC0). Семь без опубликованной иконки (Hermes · GLM/Z.ai · Grok · Together · Fireworks · BytePlus Ark · Volcengine Ark) сохраняют монограмму в цвете бренда. По-прежнему **CSP-безопасно по построению**: путь логотипа — встроенная статическая константа, без удалённых ресурсов и без `innerHTML`.

### Примечания
- Без изменений маршрутов и поведения; тот же API `ProviderLogo.el(slug)`. `provider-logo.js` вырос (встроенные пути логотипов). Источники сканирования без изменений: **83**. Тесты: **2784**.

## [1.223.0] — 2026-08-26

**Исправлено — успешная ⚡ live-оценка красилась как ошибка; плюс установка coworker OpenWorker прямо из его GitHub-URL или .zip.**

### Исправлено
- **Тон бейджа результата ⚡-оценки.** Успешная live-оценка больше не красится как провал. In-process провайдеры (OpenRouter, DeepSeek, …) не возвращают код выхода подпроцесса, поэтому строгое `code === 0` было ложным и любой успех выходил **красным** (при этом показывая «exit 0»). Тон теперь следует за результатом — красным красит только подпроцесс с ненулевым кодом; live-результат без кода — `badge-ok`, а суффикс «· exit N» показывается только при реальном коде выхода.

### Изменено
- **Установка [coworker OpenWorker](https://github.com/Fighter90/career-ops-coworker) из его GitHub-URL / .zip.** В документации coworker теперь описаны установщик одной командой (`curl … | bash`, идемпотентный — переиспользует существующие career-ops / web-ui) и три пути установки в OpenWorker (GitHub URL · `.zip` · импорт одного файла) в `docs/integrations/openworker.md`, встроенной справке §32 (×17) и README.

### Примечания
- Закрыта давняя дыра в покрытии: у источника Recruitee появился сквозной тест на фикс v1.214.2 с кавычками вокруг угловой скобки (`>` внутри атрибута тега не должен утекать в описание). Источники сканирования без изменений: **83**. Тесты: **2783**.

## [1.222.0] — 2026-08-26

**Изменено — подсказки полей расширенных провайдеров (DeepSeek … Volcengine Ark) теперь локализованы на все 17 языков.**

### Изменено
- **Локализация подсказок полей провайдеров.** Каждый `config.<slug>Hint` для 11 OpenAI-совместимых провайдеров, добавленных в v1.216.0–v1.217.0 (DeepSeek · GLM/Z.ai · Kimi · MiniMax · Mistral · Grok · Together · Fireworks · Ollama · BytePlus Ark · Volcengine Ark) — подсказки API-ключа, модели и базового URL в разделе **Настройки → API-ключи** — теперь переведён на все 17 локалей вместо отката к английскому. URL регистрации, id моделей и маркер ⚡ live-eval сохранены дословно.

### Примечания
- Поведение не меняется — английский `hintFallback` в `field-specs.js` по-прежнему страхует отсутствующий ключ, а новый контрактный тест проверяет, что каждый `hintKey` дескрипторов полей резолвится во всех 17 локалях. Источники сканирования без изменений: **83**. Тесты: **2779**.

## [1.221.0] — 2026-08-26

**Безопасность — эшелонированная защита от DNS-rebinding на пути fetch сканера.**

### Безопасность
- HTTP-ядро сканера (`fetchJson` / `fetchText` в `server/lib/http-json.mjs`) теперь резолвит хост каждого источника на реальном сетевом пути и **отказывается подключаться, если он резолвится в приватный, loopback, link-local, CGNAT или адрес облачных метаданных** — закрывая вектор DNS-rebinding. Хосты сканера уже привязаны к публичным доменам, так что это эшелонированная защита; путь пользовательских URL уже имел более сильную привязку соединения (`safe-fetch.mjs`). Guard работает **только** на реальном транспорте `fetch` — внедрённый в тестах `fetchImpl` никогда не резолвится.

### Примечания
- Для нормального сканирования поведение не меняется. Источники сканирования без изменений: **83**. Тесты: **2775**.

## [1.220.0] — 2026-08-26

**Добавлено — сканируйте несколько категорий Get on Board из одной записи.**

### Добавлено
- Запись `getonbrd` теперь может задать `categories: [programming, operations-management, machine-learning-ai]` (или один `category:`) вместо только фида `programming`; вакансии дедуплицируются по URL между категориями, лимит — 12. Существующие записи не меняются (по умолчанию по-прежнему `programming`).

### Примечания
- Только сервер; SSRF-привязка хоста (www.getonbrd.com) и `redirect:'error'` неизменны, а неверный слаг категории отклоняется до запроса. Источники сканирования без изменений: **83**. Тесты: **2771**.

## [1.219.0] — 2026-08-26

**Добавлено — Torre в сканере, и теперь каждая метка провайдера корректна.**

### Добавлено
- **Torre** (`provider: torre`) — пан-латиноамериканский маркетплейс талантов torre.ai, публичный поиск без авторизации с удалёнными вакансиями, которых нет в Greenhouse/Lever/Ashby; один ограниченный запрос на запись. Реестр теперь содержит **83** источника (78 английских + 5 русских). Плюс монограмма активного провайдера в приветственном баннере.

### Исправлено
- Сканирование a16z Speedrun завершалось раньше времени на доске с ротацией (короткая страница в середине обхода принималась за последнюю, обрезая до 149 из ~300); теперь доверяет `total_pages` фида. А приветственный баннер определял провайдера по карте из 4 записей (показывая сырой слаг для остальных 14) — теперь использует метку+монограмму из 18; заметка о провайдерах в настройках теперь 18 вместо 7.

### Примечания
- Опция нескольких категорий на запись у Get on Board ещё не подключена (нужен рефакторинг на уровне источника). Тесты: **2768**.

## [1.218.0] — 2026-08-26

**Добавлено — значки провайдеров (и правильные имена) везде, где показан провайдер.**

### Добавлено
- Монограммы провайдеров на чипе дашборда, в сводке «Активен» настроек и в заголовке результата ⚡ оценки.

### Исправлено
- Устаревшие метки: три экрана определяли имя из карты на 5 записей (результат оценки помечал любого не-Anthropic провайдера как «Gemini»); теперь используют общий `ProviderStatus.label` (18). Счётчик ключей показывал «/ 7» → 18. В список слагов подсказки `LLM_PROVIDER` добавлены `ark` / `arkcn` во всех 17 языках.

### Примечания
- Только клиент, безопасно для CSP. Без изменений сервера. Источники сканирования: **82**. Тесты: **2758**.

## [1.217.1] — 2026-08-26

**Усиление тестов — покрытие эндпоинтов для всего списка провайдеров.**

### Добавлено
- Параметризованный тест `run<Provider>`, проверяющий эндпоинт, модель по умолчанию и Bearer-аутентификацию для **Kimi, MiniMax, Mistral и Fireworks**.

## [1.217.0] — 2026-08-26

**Добавлено — Ark присоединяется к списку LLM (18 провайдеров).** BytePlus Ark и Volcengine Ark — модели Doubao от ByteDance — теперь провайдеры ⚡ живой оценки через тот же OpenAI-совместимый код и базовый URL по региону.

### Добавлено
- **BytePlus Ark** (`ARK_API_KEY`, международный) и **Volcengine Ark** (`ARK_CN_API_KEY`, Китай) — OpenAI-совместимый Chat Completions через `runOpenAICompatible()`. Задайте `ARK_MODEL` / `ARK_CN_MODEL` (имя модели Doubao или ваш id эндпоинта `ep-…`); `ARK_BASE_URL` / `ARK_CN_BASE_URL` переключают регион. Оба встают в порядок `auto` (последними), с монограммой в настройках и «Использовании». Веб-интерфейс охватывает теперь **18 провайдеров**.

### Примечания
- Тот же путь доверенной конфигурации + проверки схемы http(s), что и у остальных (никогда не SSRF-валидатор URL вакансий). Источники сканирования без изменений: **82**. Тесты: **2755**.

## [1.216.0] — 2026-08-26

**Добавлено — ещё девять LLM-провайдеров, у каждого своя фирменная плитка.** ⚡ Живую оценку теперь можно запускать через DeepSeek, GLM (Z.ai), Kimi (Moonshot), MiniMax, Mistral, Grok (xAI), Together, Fireworks или полностью локальный Ollama — достаточно одного ключа, рядом с полем каждого в настройках стоит монограмма.

### Добавлено
- **Девять OpenAI-совместимых провайдеров.** DeepSeek, GLM (Z.ai), Kimi (Moonshot), MiniMax, Mistral, Grok (xAI), Together AI, Fireworks AI и **Ollama** (полностью локально, без ключа). Задайте ключ — или `OLLAMA_BASE_URL` для Ollama — в **Настройках**, и провайдер встанет в порядок `auto` после Hermes; закрепите любой через `LLM_PROVIDER`. Together также хостит **Inkling** от Thinking Machines (`thinkingmachines/Inkling`). Веб-интерфейс охватывает теперь **16 провайдеров** живой оценки.
- **Монограммы провайдеров.** CSP-безопасная плитка с инициалом в фирменном цвете отмечает каждого провайдера рядом с полем ключа в настройках и в строке страницы «Использование» — встроенный SVG, без внешних логотипов, ничего не покидает вашу машину.

### Примечания
- Базовые URL провайдеров — доверенная конфигурация, проходящая через `runOpenAICompatible()` с проверкой схемы http(s) (петля Ollama разрешена), а не через SSRF-валидатор URL вакансий. Источники сканирования без изменений: **82**. Тесты: **2752**.

## [1.215.0] — 2026-08-26

**Добавлено — запускайте весь поиск работы из OpenWorker.** Новый coworker управляет этим пайплайном из open-source-приложения AI-coworker от Andrew Ng, и встроенная справка теперь его покрывает, так что ассистент «Спросить у документации» может провести вас по шагам.

### Добавлено
- **Coworker для OpenWorker.** Coworker без кода для [OpenWorker](https://github.com/andrewyng/openworker) — [Fighter90/career-ops-coworker](https://github.com/Fighter90/career-ops-coworker) — выполняет весь поиск работы (скан, оценка, подгонка, трекинг, follow-up) и может **запустить этот дашборд**, всё на вашем рабочем столе. Это персона из одного файла на 17 языках, установка которой проверена собственным загрузчиком OpenWorker.
- **Справка §32 «Run it from OpenWorker»** (все 17 локалей) документирует это, чтобы встроенный ассистент **«Спросить у документации»** и плавающая кнопка справки могли отвечать на вопросы о нём. Также добавлены ссылки в футере лендинга, README, вики и `docs/integrations/openworker.md`.

### Примечания
- Только документация + сайт — без изменений кода приложения. Источники сканирования без изменений — **82**. Набор тестов: **2742**.



## [1.214.2] — 2026-08-25

**Исправлено — общий конвейер HTML→текст (описания Greenhouse / Recruitee) больше не протаскивает атрибуты тега в описание, когда значение атрибута содержит символ `>`.**

### Исправлено
- **HTML-описания с `>` внутри атрибута очищаются корректно.** Средство удаления тегов за описаниями Greenhouse и Recruitee использовало наивное совпадение, которое останавливалось на первом `>`, поэтому тег вроде `<a title="salary > 100k">` оставлял хвост атрибута (`100k">`) в описании. Теперь оно уважает значения атрибутов в кавычках (одинарных или двойных), удаляет разметку *до* декодирования сущностей (чтобы закодированная кавычка внутри атрибута не стала ложным разделителем) и по-прежнему сохраняет литеральный `<>` в простом тексте.

### Примечания
- Набор тестов: **2742**. Источники сканирования без изменений — **82**. Ничего нового от родителя в этот раз: остальные изменения ретранслируются только на чтение (правки follow-up / weekly-digest / rejection-latency / salary-gap не требуют изменений в web-ui) или не относятся к web-ui (внешний X/Xquik BYO-key плагин, правки CLI/verify). Опциональные пункты — детальные описания SmartRecruiters, пропуск агрегаторов в detect-reposts, префиксы заголовков `stem:` / `word:`, защита от DNS-rebinding — остаются в очереди.



## [1.214.1] — 2026-08-25

**Исправлено — описания вакансий Ashby теперь обрезаются до той же длины, что у Greenhouse и Recruitee, так что контент-фильтр ведёт себя одинаково на всех трёх досках и одна доска не может раздуть кэш сканирования.**

### Исправлено
- **Описания Ashby теперь соблюдают тот же предел длины, что и другие доски.** v1.214.0 подавала тело вакансии Ashby в контент-фильтр, но — в отличие от Greenhouse и Recruitee, которые режут до 4000 символов — у Ashby оно мапилось напрямую без предела (иногда в 4 раза длиннее). Из-за этого одно и то же слово `content_filter` могло совпасть на Ashby, но не на Greenhouse — только в зависимости от позиции в тексте, а одна доска могла добавить сотни КБ в кэш сканирования, который грузит `#/scan`. Теперь Ashby режется до тех же 4000 символов (его текст уже простой, поэтому он усекается, а не очищается от тегов).

### Примечания
- Набор тестов: **2738**. Источники сканирования без изменений — **82**.



## [1.214.0] — 2026-08-25

**Исправлено — описания вакансий Greenhouse, Ashby и Recruitee теперь попадают в контент-фильтр, так что фильтры по словам / стране / визе реально совпадают с этими досками; а "Associate <старший титул>" (включая академические звания) больше не относят к entry-level.**

### Исправлено
- **Контент-фильтры теперь работают на Greenhouse, Ashby и Recruitee.** Эти доски теперь несут полный текст каждой вакансии как её описание, поэтому `content_filter` (или фильтр по словам страны / визы) совпадает с телом вакансии, а не пропускает доску вслепую. Ashby и Recruitee отдают тело в списочном payload бесплатно; Greenhouse уже это делал. Все три очищаются одним общим конвейером HTML→текст.
- **"Associate Director" / "Associate Professor" — это senior, а не entry-level.** Классификатор уровня (используется `skip_tiers:`) считал слово "associate" меткой entry-level, даже когда оно предваряет старший титул. Теперь он читает "Associate <старшее существительное>" — включая академические звания (Associate Professor, Associate Dean) — как senior, сохраняя настоящие junior-варианты (Associate Attorney, Associate Editor) как entry-level, и по-прежнему относит ведущий "Intern, Associate Dean" к стажировке.

### Примечания
- Набор тестов: **2736**. Источники сканирования без изменений — **82**. В очереди на потом: опциональное обогащение описаний SmartRecruiters (запрос детали по каждой вакансии), пропуск досок `aggregator: true` в detect-reposts и префиксы заголовков с якорным словом `stem:` / `word:` — каждое опциональная настройка, а не изменение поведения по умолчанию. Защита от DNS-rebinding по-прежнему в очереди на отдельный релиз безопасности.
...
**fix(test): v1.59.11 — e2e-comprehensive 12 падений в CI разобраны и закрыты.** Двенадцать кейсов (Pipeline · Activity · Health · 7 mode-страниц · 404 · Profile) падали в каждом CI run начиная с v1.58.x, потому что `page.goto(baseUrl + '/#/X')` в Playwright — no-op для изменения только хэша. После того как CV-step ставил хэш `#/cv`, все следующие `goto` молча оставляли страницу на `#/cv`. Фикс: новый helper `goRoute(hash)` в [tests/e2e-comprehensive.mjs](https://github.com/Fighter90/career-ops-ui/blob/main/tests/e2e-comprehensive.mjs) делает bounce через `about:blank` перед каждым `goto`, форсируя реальную навигацию и ребутстрап SPA. Все 17 вызовов `page.goto(`${baseUrl}/#/...`)` заменены. Результат: **23 / 23 кейса зелёные** · 988 / 988 unit · 20 / 20 smoke e2e. (e2e-harness-r1)
...
**fix(ux): UX-A5-r4 (v1.59.9) — debug-маркер `data-toc-spy="active"` + поведенческий lock-test scroll-spy на Help TOC.** Шестой цикл закрытия: предыдущие 5 закрытий проходили статические тесты, но баг возвращался. v1.59.9 добавляет маркер (любой тестер проверит `document.body.dataset.tocSpy === 'active'`), синхронный initial paint, double-rAF re-compute, resize listener и полную очистку на hashchange. Новый [tests/help-toc-spy-behavior.test.mjs](https://github.com/Fighter90/career-ops-ui/blob/main/tests/help-toc-spy-behavior.test.mjs) гоняет алгоритм против 6 сценариев синтетической геометрии + 1 algorithm-parity check против help.js. 973 → **982** юнит-тестов. (UX-A5-r4)
...
**fix(ux): UX-A6 (NEW-M4-r1) — каждая saved-research карточка проходит через единый helper `renderSavedCard()`.** Верификационный регресс v1.58.51 показал: одна карточка рендерилась без структурных детей (title+date как один text node — `software-engineer-generalyesterday`), а другая с правильной `<span>` + `<time>` структурой. Правка в [public/js/views/deep.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/deep.js#L26-L75): извлечён `renderSavedCard(f)`, всегда выдающий `.saved-card__title` + `.saved-card__date datetime=…`. Любой render-путь (page-load `renderArchive`, post-`Run live`, будущие точки) проходит через единый helper — flex-gap layout M-4 v1.58.11 работает только с правильными структурными детьми. 948 → **949** модульных. (UX-A6)

---

## [1.58.52] — 2026-05-20

**fix(ux): UX-A5 (NEW-K1) — scroll-spy для TOC на `#/help` снова работает (регресс из v1.58.45).** Верификационный регресс v1.58.51 показал: 18 H2-элементов с id, 18 TOC `<a>` ссылок, но **0** ссылок получают `.toc-current` при скролле. Причина: в v1.58.45 observer был обернут в `setTimeout(0)`, который срабатывал ДО того, как роутер монтировал view в `#content`; `document.querySelectorAll(".help-article h2[id]")` не находил ничего. Правка в [public/js/views/help.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/help.js#L155-L200): новый `mountTocSpy()` наблюдает за **синхронно построенными refs `headings`** (без re-query документа); deferred через **двойной `requestAnimationFrame`** — наблюдатель привязывается после первого paint, который включает смонтированный view. 947 → **948** модульных. (UX-A5)
...
**chore(tooling): TOOL-1 — `make clean-test-fixtures` + `scripts/clean-test-fixtures.mjs`.** v1.58.36 аудит: `${CAREER_OPS_ROOT}/data/pipeline.md` накопил 1252+ строки с `example.com/job/<n>` от регрессионных прогонов. Чистка вручную утомительна. Новый [scripts/clean-test-fixtures.mjs](https://github.com/Fighter90/career-ops-ui/blob/main/scripts/clean-test-fixtures.mjs) читает `${CAREER_OPS_ROOT or ..}/data/pipeline.md`, удаляет каждую строку с `example.com` (case-insensitive), сохраняет реальные ATS URL без изменений, печатает счёт и завершается с кодом 0. Флаг `--dry-run` печатает результат в stdout, не трогая файл. Новый [Makefile](https://github.com/Fighter90/career-ops-ui/blob/main/Makefile) с целями `make clean-test-fixtures` и `make clean-test-fixtures-dry-run`. 4 новых CI-isolated теста в [tests/clean-test-fixtures.test.mjs](https://github.com/Fighter90/career-ops-ui/blob/main/tests/clean-test-fixtures.test.mjs) (через `mkdtempSync` — никаких реальных file-writes). 942 → **946** модульных (+4 новых TOOL-1 кейса). (TOOL-1)

---

## [1.58.48] — 2026-05-20

**fix(ux/onboarding): UX-D-B — на `#/dashboard` появляется глобальный предупреждающий баннер, если пользователь всё ещё на дефолтном fixture-профиле.** v1.58.36 аудит: `/api/health` уже возвращает `{ name: "Profile customized", ok: false }` (`server/lib/store.mjs` проверяет `Acceptance Test` / `Jane Smith` / прочие шаблонные имена), но пользователь видел это только зайдя на `#/health`. При этом каждое advisor-output (Apply / Followup / Contacto / Deep) обращалось к fixture-имени — испорченное первое впечатление. Новый `profileFixtureBanner()` в [public/js/views/dashboard.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/dashboard.js) рендерит `.hero-banner.hero-banner--warning` в начале маршрута, когда чек fail, с локализованным сообщением (`onboarding.fixtureWarning` × 8) + CTA-кнопкой на `#/config` (`onboarding.fixProfile` × 8). Когда пользователь меняет профиль, /api/health возвращает `ok: true` и следующий mount dashboard молча убирает баннер. Новые CSS правила `.hero-banner` + `.hero-banner--warning`. 941 → **942** модульных. (UX-D-B)

---

## [1.58.47] — 2026-05-20

**fix(ux/naming): UX-D-C — кнопка `Quick scan` в шапке переименована в `Открыть Scan`, чтобы метка соответствовала поведению.** v1.58.36 аудит: «Quick scan» намекало на мгновенный скан с дефолтными настройками, но обработчик на [public/js/app.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/app.js#L140) только навигирует на `#/scan` — сам скан не запускается. Переименовано через ключ `top.quickscan` во всех 8 локалях (en `Open Scan` / es `Abrir Scan` / pt-BR `Abrir Scan` / ko `Scan 열기` / ja `Scan を開く` / ru `Открыть Scan` / zh-CN `打开 Scan` / zh-TW `開啟 Scan`). Дефолтный текст в HTML также обновлён. Поведение без изменений; только честность метки. 940 → **941** модульных. (UX-D-C)

---

## [1.58.46] — 2026-05-20

**fix(ux): UX-D-D — checklist на `#/apply` подставляет `{company}-{role}` слагами, вычисленными из URL/JD.** v1.58.36 аудит: item 5 чеклиста читался как `Save filled answers to interview-prep/{company}-{role}.md before submitting.` — литералы `{company}-{role}` отрисовывались дословно, пользователь должен был мысленно подставлять значения (или, хуже, оставлял их как есть). Новые `extractSlugs(url, jd)` + `substitutePlaceholders(text, url, jd)` в [public/js/views/apply.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/apply.js#L36-L93): white-list хостов (`greenhouse / lever / ashby / workable / smartrecruiters / workday`) выделяет `company` из path или поддомена, затем `role` из последнего сегмента пути (с обрезкой trailing numeric id) — или, как fallback, из первой строки JD. Если извлечение не удалось (unknown host / нет JD), плейсхолдеры становятся `[company]` / `[role]` (square-bracket convention для «заполни сам»). Substitution выполняется один раз перед `parseChecklist`, поэтому live checklist и Copy-unchecked output остаются согласованными. 939 → **940** модульных. (UX-D-D)

---

## [1.58.45] — 2026-05-20

**fix(ux): UX-D-K — scroll-spy в TOC `#/help` подсвечивает текущую секцию.** v1.58.36 аудит: при прокрутке тела help страница TOC (sticky, ~92 H2-секций) не показывал, какую секцию читает пользователь. Новый `IntersectionObserver` в [public/js/views/help.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/help.js#L155-L185) наблюдает за каждым `.help-article h2[id]` и применяет `.toc-current` к соответствующей `<a>` ссылке TOC, когда H2 попадает в верхнюю треть viewport (`rootMargin: "-30% 0% -60% 0%"`). CSS [public/css/app.css](https://github.com/Fighter90/career-ops-ui/blob/main/public/css/app.css) задаёт `.toc-current` левый бренд-бордер + цвет `var(--rausch)` + `font-weight: 600`. При hashchange observer отключается рядом с существующим scroll-listener — никаких утечек. 938 → **939** модульных. (UX-D-K)

---

## [1.58.44] — 2026-05-20

**fix(ux): UX-D-L — на `#/deep` у открытого Saved-research brief появилась inline-кнопка × для закрытия.** v1.58.36 аудит: клик по карточке Saved-research отрисовывал тело brief'а внутри страницы (Copy / Download / Open in tab / Generate PDF в шапке), но **закрыть** brief можно было только прокруткой или навигацией. Новая кнопка × в [public/js/views/deep.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/deep.js) в шапке `showResult()` — очищает `out.innerHTML`, повторяет паттерн закрытия модалки (api.js UI.modal × button), доступна с клавиатуры, `aria-label` + `title` из нового ключа `deep.closeBrief` × 8 локалей. 937 → **938** модульных. (UX-D-L)

---

## [1.58.43] — 2026-05-20

**fix(ux): UX-D-F — пустой submit на `#/evaluate` теперь показывает отдельный локализованный toast (раньше показывал «too short»).** До правки клик Evaluate при пустой textarea проваливался в существующую проверку `<50 chars` и выдавал `JD too short (min 50 chars)` — точно, но бесполезно: реальная проблема — «вы ничего не ввели», а не «вы ввели мало». В [public/js/views/evaluate.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/evaluate.js#L28-L45) теперь первый чек `!jd` показывает новый `eval.emptyJd` («JD обязателен — вставьте полное описание вакансии», локализовано × 8) и фокусирует textarea, чтобы пользователь сразу начал печатать. Существующий `eval.shortJd` всё ещё срабатывает для 1–49 символов. 936 → **937** модульных. (UX-D-F)

---

## [1.58.42] — 2026-05-20

**fix(ux): UX-D-J — паритет ETA-чипа на всех advisor-страницах с `#/auto` (UX-6 v1.55.4).** v1.58.36 аудит: только `#/auto` показывал честный «⏱ ~1–2 min» рядом с Run; остальные 7 LLM-страниц (`#/evaluate`, `#/deep`, 5 mode-страниц — project/training/followup/contacto/interview-prep/patterns) совершали похожий 10-60 с вызов и не давали подсказку про время. Добавлен `<span class="advisor-eta">⏱ ~30s</span>` (локализация через новый ключ `advisor.eta` × 8) рядом с `UI.providerCostHint(t)` в [evaluate.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/evaluate.js), [deep.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/deep.js), [mode-page.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/mode-page.js). CSS правило `.auto-eta` расширено на `.advisor-eta` с тем же стилем. Оригинальный `auto.eta` остаётся «~1–2 min» — это единственный multi-step SSE-пайплайн. 935 → **936** модульных. (UX-D-J)

---

## [1.58.41] — 2026-05-20

**fix(ux/truthfulness): UX-D-I — cost-hint теперь перечитывает `/api/status/providers` при возврате фокуса на вкладку и по событию `providers-changed` (последствие M-7 v1.58.12).** v1.58.12 связал `UI.providerCostHint(t)` с `/api/status/providers`, но запрос делался ОДИН раз при создании ноды. Если пользователь открывал `#/config` в другой вкладке, менял провайдера и возвращался — cost-line молча отображал старое значение, пока не происходил переход с маршрута и обратно. Правка в [public/js/api.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/api.js#L676-L740): извлечена именованная функция `refreshCostLine()` и привязана к `document.visibilitychange` (фокус вернулся на вкладку) + новому `CustomEvent("providers-changed")`. Обработчик Save в [public/js/views/config.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/config.js) диспатчит этот event после успешного POST, и cost-line на `#/auto`, `#/deep`, `#/evaluate`, mode-страницах обновляется **без** перезагрузки страницы или re-mount маршрута. 934 → **935** модульных. (UX-D-I)

---

## [1.58.40] — 2026-05-20

**fix(ux/docs): UX-D-H — regression-lock: каждая видимая ссылка `career-ops.org/docs/<path>` обязана быть кликабельной.** v1.58.36 аудит подтвердил живьём: каждая такая ссылка в `public/js/views/*.js` уже находится внутри `c("a", { href, target: "_blank", rel: "noopener noreferrer" }, …)` (apply.js / batch.js / config.js / reports.js), и каждая ссылка в `docs/help/*.md` использует markdown `[text](https://github.com/Fighter90/career-ops-ui/blob/main/url)`. Поэтому в этом релизе только **regression lock**: новый [tests/external-doc-links.test.mjs](https://github.com/Fighter90/career-ops-ui/blob/main/tests/external-doc-links.test.mjs) парсит каждый `views/*.js` и `docs/help/*.md` и валит сборку, если `career-ops.org/docs/<path>` URL рендерится как plain child text. Бренд-упоминания `career-ops.org` без `/docs/` пути допускаются (например, «career-ops.org schema» в прозе). 932 → **934** модульных. (UX-D-H)

---

## [1.58.39] — 2026-05-20

**fix(ux): NEW-D2 — на `#/dashboard` появилась кнопка `↻ Refresh` в шапке с явным toast-фидбэком (отличная от Refresh из connection-banner — M-9 v1.58.14).** До правки на `#/dashboard` не было in-place обновления — пользователь использовал banner Refresh, который делает полный `location.reload()` и теряет позицию скролла. Новая кнопка в [public/js/views/dashboard.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/dashboard.js) перечитывает `/api/dashboard` и перерисовывает view через `Router.go("/dashboard")`. Toast-пайплайн: «Обновление…» → «Панель обновлена» (success) / локализованная ошибка. Два новых i18n-ключа × 8 локалей: `dash.refreshAria`, `dash.refreshed`. 931 → **932** модульных. (NEW-D2)
...
**fix(ux): U-8 — блок сгенерированного prompt по умолчанию свёрнут на 7 mode-страницах.** Раньше после клика «Generate prompt» inline `<pre>` уходил на 1200+ px и ронял Copy + Run-live ниже сгиба. В [public/js/views/mode-page.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/mode-page.js#L272-L300) `showPrompt()` теперь оборачивает `<pre>` в `<details class="prompt-block">` (по умолчанию закрыт); summary показывает «Показать prompt (N строк)» через новые ключи `prompt.show` / `prompt.lines` × 8 локалей. Copy + Run-live сразу видны. Параллельно исправлена регрессия теста BUG-007/008: после U-7 (v1.58.27) между `UI.dismissToast()` и `UI.modal(t(…))` появился промежуточный `const stripped = …`; жёсткое требование «соседних строк» ослаблено до «dismissToast вызывается перед modal в окне ≤ 1200 символов». 920 → **921** модульных. (U-8)

---

## [1.58.27] — 2026-05-20

**fix(ux): U-7 — ASCII-разделители `===` от `verify-pipeline.mjs` убраны из модалки результата.** Скрипт печатает `=========` (≥ 50 символов) между секциями; в 14 px моноширинной модалке такая полоса визуально ломала ширину. Теперь в [public/js/views/health.js](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/health.js#L23-L38) перед рендером применяется `.replace(/^={10,}$/gm, "")` — секции по-прежнему разделяются пустыми строками. 919 → **920** модульных. (U-7)
...
**WS6.4 — редакторы массивов Profile + аудит WS6.2 API-keys.** `PUT /api/profile` принимает `{ arrays }` (комбинируется с `{ fields }`): Target roles/Superpowers (списки), Archetypes (name/level/fit), Proof points (name/url/hero-metric). Та же гарантия merge-not-replace; пустые строки отброшены; пустой список удаляет ключ. 4 add/remove-редактора в #/config. i18n 6 ключей ×8. Аудит: KNOWN_KEYS ≡ FIELDS, гэпа нет. +7 тестов. 597 → 604. Подробно — [`CHANGELOG.md`](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.md).

---

## [1.34.0] — 2026-05-17

**WS5 — экран Auto-pipeline в один клик (`#/auto`).** Модалка повышена до отдельной линкуемой страницы. Один клик: валидация→загрузка→оценка→сохранение отчёта→трекер (SSE). Доступный stepper (`aria-current`, live-region), deep-link на отчёт/трекер, ручной режим без ключа, линкуется `#/auto?url=…&go=1`. Пункт в сайдбаре; кнопка ✨ дашборда ведёт сюда. i18n 14 ключей ×8. help §1 ×8 + README ×8. +8 тестов. 589 → 597. Подробно — [`CHANGELOG.md`](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.md).

---

## [1.33.0] — 2026-05-17

**WS4 — аудит паритета с career-ops 1.8.0 + `location_filter`.** В родительском `scan.mjs` появился `location_filter` (#570); in-process сканеры web-ui не делегируют ему, поэтому не флоу. Новый `server/lib/location-filter.mjs` повторяет семантику дословно; подключён в оба сканера. help §5 ×8. +8 тестов. 581 → 589. Остальной дельта классифицирован в PARENT-PARITY.md. Подробно — [`CHANGELOG.md`](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.md).

---

## [1.32.0] — 2026-05-17

**Вкладка Profile в `#/config` — сырой YAML-блоб → форма по полям (WS1).** 3 сворачиваемые секции (Кандидат / Нарратив / Компенсация), 14 скалярных путей. Сохранение по полям **мёржит** в `config/profile.yml`: архетипы, proof points и кастомные ключи сохраняются нетронутыми. Raw-YAML escape-hatch сохранён в *Advanced* (хранит комментарии). 23 i18n-ключа ×8. +7 тестов (вкл. инвариант выживания неизвестных ключей). 574 → 581. Подробно — [`CHANGELOG.md`](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.md).

---

## [1.31.0] — 2026-05-17

**Синхронизация с career-ops 1.8.0 — `#/batch` отдаёт `--model` + `--start-from`.** Родительский проект 1.7.1 → 1.8.0; в `batch-runner.sh` добавлены `--model NAME` (#504) и `--start-from N`. web-ui выводит их в `#/batch` (поля **Модель** и **Старт с #**) с серверной defense-in-depth валидацией (charset для model, целое 1..100000 для start-from). i18n ×8. +7 тестов. 567 → 574. Полностью — в [`CHANGELOG.md`](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.md).
...
- Тесты: **520 → 540** (+20). Полные подробности в [`CHANGELOG.md`](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.md).
...
До v1.28.1 `Router.go('/evaluate?url=…')` производил hash, чей первый сегмент по `split('/')` был литералом `"evaluate?url=…"`, который никогда не совпадал с зарегистрированной ветвью → `__not_found__` (404). Однострочный фикс: `hash.split('?')[0]` перед разбиением имени. Закрывает оба известных клика: `#/pipeline → ▶` и «App settings → Modes». Опциональная строка `HH_USER_AGENT` убрана из `/api/health` (подсказка про 403-вне-РФ остаётся в help-bundle §16 и в stderr самого ru-scanner). **515 → 520** unit + acceptance (+5 новых). Полные подробности в [`CHANGELOG.md`](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.md).
...
- **506 → 515** unit + acceptance (+ 9 новых). Playwright 32/32 без изменений. Полные подробности в [`CHANGELOG.md`](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.md).
...
В v1.26.0 декларация `min-height: 44px` в `.btn` отсутствовала, header-кнопки рендерились на 39-41 px (нарушение WCAG 2.5.5). v1.26.1 восстанавливает 44-пиксельный пол + `flex-shrink: 0` + `line-height: 1.2`. **502 → 506** unit, Playwright 32/32 без изменений. Детали в [`CHANGELOG.md`](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.md).
...
- **`fix(auto-pipeline): G-014 — honour mode:'manual' short-circuit`** ([`server/lib/routes/auto-pipeline.mjs:158-195`](https://github.com/Fighter90/career-ops-ui/blob/main/server/lib/routes/auto-pipeline.mjs#L158-L195)) — до v1.25 маршрут всегда обращался к LLM. Передача `mode: 'manual'` (по аналогии с `/api/evaluate` начиная с v1.10.2) тихо игнорировалась, запрос висел 1–3 минуты на Anthropic. Теперь обработчик:
...
- **Тесты** — [`tests/auto-pipeline-manual-mode.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/auto-pipeline-manual-mode.test.mjs): 3 теста подтверждают (1) `mode: 'manual'` возвращает ответ менее чем за 2 с со всеми 5 ключами этапов, (2) даже при заданном `ANTHROPIC_API_KEY` короткое замыкание всё равно срабатывает (исходный симптом), (3) старые вызовы с `evalMode: 'manual'` продолжают работать.
...
- **`fix(dashboard): dedup ✨ glyph in auto-pipeline button label`** ([`public/js/lib/i18n-dict.js:219`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/lib/i18n-dict.js#L219)) — `dash.autoPipeline` нёс ведущий `✨` в строке каждой локали, И `public/js/views/dashboard.js:58` добавлял ещё один `✨` во вьюхе. Результат: кнопка рендерилась как `✨ ✨ Auto-pipeline …`. v1.25 убирает ведущий глиф из значения DICT каждой локали; префикс вьюхи остаётся единственным источником. Тот же проход аудита прошёлся по остальному i18n-бандлу — других случаев двойного глифа не обнаружено.
...
- **`fix(config): G-015 — replace removed Element.prototype.also call in config.js`** ([`public/js/views/config.js:371`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/config.js#L371)) — v1.22.0 N-2 удалил глобальный monkey-patch `Element.prototype.also` и перевёл `cv.js` на шаблон со свободными выражениями, но **пропустил `config.js`**. В итоге `#/config` падал при первом обращении в каждой локали с ошибкой `c(...).also is not a function`. v1.24.1 применяет тот же шаблон миграции, что и в `cv.js:188-201` — вынести дерево в `const root = c(...)`, выполнить блок активации отдельно и затем `return root;`.
...
- **`docs(help): refresh en.md from 5 canonical career-ops.org/docs URLs`** ([`docs/help/en.md`](https://github.com/Fighter90/career-ops-ui/blob/main/docs/help/en.md)) — до v1.24 EN-бандл содержал 1113 строк и перечислял 5 канонических URL во front-matter, но не раскрывал их в теле. v1.24 загружает все 5 URL через WebFetch и углубляет соответствующие H2-разделы:
...
- **`docs(translate): CHANGELOG.ru.md retry agent — full body translation`** ([`CHANGELOG.ru.md`](https://github.com/Fighter90/career-ops-ui/blob/main/CHANGELOG.ru.md)) — релиз v1.23.0 вышел с переводческим агентом для RU CHANGELOG ещё в работе (он один раз упал с ошибкой сокета и был перезапущен). v1.24 подхватывает 1542-строчный полный перевод агента: каждая запись от v1.23.0 до v1.6.0 получает русское тело публикационного уровня, больше никаких EN-телесных заглушек. Стилевая дисциплина соответствует обновлению качества README в v1.22.0: «функциональность» / «возможности» / «поведение» вместо неуклюжего «функционал»; «через» / «с помощью» вместо «при помощи»; активный залог над пассивным; «эндпоинт», «лимит запросов», «состояние гонки», «санитайзинг» как канонические термины; английские аббревиатуры (TOCTOU, CSP, SSRF, WCAG, ATS, JD, SSE, REST, API) сохранены.
...
- **`refactor(client): split DICT into i18n-dict.js (data) + i18n.js (logic)`** — до v1.23 файл `public/js/lib/i18n.js` содержал 639 строк. Основная часть (строки 23-586) — таблица переводов `DICT`, чистые структурированные данные. v1.23.0 выносит её в [`public/js/lib/i18n-dict.js`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/lib/i18n-dict.js) (578 LOC, освобождён от лимита LOC по правилу CLAUDE.md «Exempt from these limits: generated files, migrations, test fixtures, lock files, vendored code» — таблица переводов квалифицируется как фикстура), оставляя в [`public/js/lib/i18n.js`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/lib/i18n.js) 86 LOC чистой логики модуля (значительно ниже целевого лимита в 400 LOC).
...
- **Обновление тестовой инфраструктуры:** [`tests/i18n-coverage.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/i18n-coverage.test.mjs), [`tests/help-ui.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/help-ui.test.mjs), [`tests/canonical-docs-coverage.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/canonical-docs-coverage.test.mjs) теперь прогоняют оба файла через тестовый VM-контекст (или конкатенируют их исходники для regex-обхода), сохраняя все существующие проверки.
...
- **`docs(readme): wire each locale README at its locale-specific PNG`** — до v1.23 только `README.pt-BR.md` ссылался на `dashboard-pt-BR.png`; остальные 6 не-EN README продолжали указывать на `dashboard-en.png`. Скриншоты (уже снятые в цикле v1.22.0 скриптом [`scripts/capture-dashboard-screenshots.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/scripts/capture-dashboard-screenshots.mjs)) лежали в `images/`, но не использовались. v1.23.0 обновляет 14-ю строку каждого `README.{es,ja,ko-KR,ru,zh-CN,zh-TW}.md` на собственный `dashboard-<locale>.png`.
...
- **`fix(security): M-4 — entity-aware stripDangerousMarkdown`** ([`server/lib/security.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/server/lib/security.mjs)) — regex до v1.22 сопоставлял `<script>`, `javascript:`, `on*=` как литеральные подстроки. `&lt;script&gt;`, `java&#115;cript:` и `<img src="data:image/svg+xml,<svg onload=…>">` проходили насквозь. Очистка теперь декодирует `&lt;`, `&gt;`, `&amp;`, `&quot;`, числовые (`&#NN;`) и шестнадцатеричные (`&#xHH;`) сущности **до** запуска regex-очистки. Подтверждено 11 тестами в [`tests/cv-xss-bypasses.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/cv-xss-bypasses.test.mjs). Основная защита по-прежнему лежит на клиентском конвейере `UI.md` с приоритетом escape; этот патч закрывает файл на диске.

- **`fix(security): L-2 — bash --noprofile --norc on the batch runner`** ([`server/lib/routes/batch.mjs:108`](https://github.com/Fighter90/career-ops-ui/blob/main/server/lib/routes/batch.mjs#L108)) — `spawn('bash', [PATHS.batchRunner, ...])` ранее подхватывал пользовательский `~/.bashrc`. Враждебный rc-файл мог влиять на выполнение. Теперь `spawn('bash', ['--noprofile', '--norc', PATHS.batchRunner, ...])`.

### 🔒 Устойчивость

- **`fix(client): M-6 — exponential backoff on health ping`** ([`public/js/api.js:22-48`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/api.js#L22-L48)) — поллер состояния «нет соединения» совершал 28 800 запросов к мёртвому серверу за ночь. Теперь 3 с → 6 с → 12 с → 24 с → 60 с, сброс к 3 с при первом успешном восстановлении (2xx). Реализация на цепочке `setTimeout` (не `setInterval`), поэтому каждый шаг подхватывает новую задержку.

- **`fix(client): M-5 — Safari private-mode localStorage guard`** ([`public/js/lib/i18n.js:572-583`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/lib/i18n.js#L572-L583)) — Safari в приватном режиме выбрасывает `SecurityError` на каждый вызов `localStorage.getItem/setItem`. IIFE во время загрузки роняла весь модуль i18n, и SPA рендерил сырые ключи. Оба вызова обёрнуты в try/catch, в качестве запасного варианта используется `detect()` по языку браузера.

- **`fix(server): M-2 — body-size cap on outbound preview fetches (test + verify)`** — функция `safeGet` из v1.21.0 уже стримила чанки и ограничивала размер `opts.maxBytes`. v1.22 добавляет явный регрессионный тест в [`tests/ssrf-redirect-rebind.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/ssrf-redirect-rebind.test.mjs), фиксирующий контракт: 100 КБ вверх по потоку + лимит 4 КБ → ответ ≤ 4 КБ.

- **`fix(client): L-5 — clear setTimeout on hashchange in scan.js`** ([`public/js/views/scan.js:6-22, :113-120`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/scan.js#L6-L22)) — таймер `refreshResults()` на 300 мс после завершения утекал, если пользователь уходил с `#/scan` в это окно. Дескриптор теперь захватывается и очищается в `__cancelActiveScanPoll`.

- **`fix(client): L-4 — multi-line SSE data: joiner`** ([`public/js/lib/auto-pipeline.js:158-176`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/lib/auto-pipeline.js#L158-L176)) — парсер SSE использовал `match()` (одна строка). По спецификации событие может нести несколько строк `data:`, которые потребитель должен соединить через `\n`. Сейчас сервер шлёт однострочный JSON, и старый код работал — но был хрупким к любой будущей многострочной полезной нагрузке.

### ♿ Доступность

- **`feat(a11y): M-3 — WCAG 1.4.1 redundant cues on score pills + connection banner`** ([`public/css/app.css:602-625, :812-822`](https://github.com/Fighter90/career-ops-ui/blob/main/public/css/app.css#L602-L625)) — `score-high` / `score-mid` / `score-low` ранее передавали состояние только оттенком (красный/жёлтый/зелёный). Пользователи без цветовосприятия не имели альтернативы. Каждый уровень теперь получает дополнительный глиф через `::before` (✓ / ◐ / ○). Баннер соединения получает префиксный глиф `⚠` в офлайн-состоянии. Места рендеринга не тронуты — чисто CSS-усиление.
...
- **`fix(a11y): M-7 — null-guard on UI.el() htmlFor alias`** ([`public/js/api.js:194-198`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/api.js#L194-L198)) — `htmlFor: null` ранее рендерил литеральный `for="null"`. Однострочное зеркало защиты `v != null && v !== false` из соседней ветки.
...
- **`fix(server): L-3 — Windows-safe entrypoint check`** ([`server/index.mjs:159-163`](https://github.com/Fighter90/career-ops-ui/blob/main/server/index.mjs#L159-L163)) — `import.meta.url === \`file://${process.argv[1]}\`` неправильно обрабатывает буквы дисков и обратные слэши в Windows. Заменено на `fileURLToPath(import.meta.url) === path.resolve(process.argv[1])`.

- **`refactor(client): N-2 — drop Element.prototype.also monkey-patch`** ([`public/js/views/cv.js:188-201`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/cv.js#L188-L201)) — глобальное загрязнение прототипа DOM. Заменено локальной переменной для корня дерева.

- **`test(canary): M-8 — 404 regression test for retired /api/scan-ru/config`** ([`tests/scan-consolidated.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/scan-consolidated.test.mjs)) — v1.20.0 убрал alias, но не добавил канарейку. Трёхстрочное добавление по образцу тестов вывода из эксплуатации в v1.18.
...
**Безопасность + параллелизм + полировка a11y по результатам двух независимых код-ревью.** Семь находок из [`docs/specs/V1.20.1-BACKLOG.md`](https://github.com/Fighter90/career-ops-ui/blob/main/docs/specs/V1.20.1-BACKLOG.md) отгружены одним релизом: один блокер (TOCTOU при DNS-rebind), шесть багов высокой степени серьёзности (распыление санитайзинга при path-traversal, отсутствие лимита запросов на LAN-деплое, состояние гонки при конкурентной записи, провал в покрытии i18n, висячий aria-describedby, отсутствие связей с метками). 34 новых теста; baseline вырос с 427 → 461 unit + 32/32 Playwright. Каждый патч закрыт именованным регрессионным тестом.

### 🛡️ Безопасность

- **`fix(security): B-1 — close DNS-rebind TOCTOU via safe-fetch.mjs`** ([`server/lib/safe-fetch.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/server/lib/safe-fetch.mjs)) — прежний паттерн делал один явный `dnsLookup` для валидации, а затем `fetch()` выполнял свой собственный независимый lookup. Атакующий через DNS-rebind с TTL=0 мог вернуть публичный IP на первом lookup и `127.0.0.1` / `169.254.169.254` / адрес LAN на втором, обходя `isPrivateOrLoopbackHost`. Новая функция `safeGet` резолвит ОДИН раз, привязывает TCP-соединение к конкретному IP через node:http(s) и выставляет SNI/Host так, чтобы проверка сертификата по-прежнему шла против исходного hostname. Используется `/api/pipeline/preview` и `/api/auto-pipeline`. Fail-CLOSED при ошибке lookup (обратно к прежнему `try { … } catch { /* fall through */ }`). Подтверждено 8 новыми тестами в [`tests/ssrf-redirect-rebind.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/ssrf-redirect-rebind.test.mjs).

- **`fix(security): H-4 — consolidate sanitizePathName across 10 routes`** ([`server/lib/security.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/server/lib/security.mjs)) — голый regex `replace(/[^\w\-.]/g, '')` дублировался в `jds.mjs`, `content.mjs`, `reports.mjs`, `llm.mjs`, `runners.mjs` и сохранял символы `.`, из-за чего `..pdf`, `....md`, имена с ведущей точкой выживали. Только `reports.mjs::sanitizeSlug` делал это правильно. v1.21.0 поднимает корректную версию (`sanitizePathName`) в `security.mjs`, удаляет 10 сломанных копий и отвергает пустые результаты с кодом 400. Подтверждено 12 тестами в [`tests/path-traversal.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/path-traversal.test.mjs).

- **`fix(security): H-5 — rate-limit LLM endpoints on public bind`** ([`server/lib/rate-limit.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/server/lib/rate-limit.mjs)) — у `/api/evaluate`, `/api/deep`, `/api/mode/:slug`, `/api/auto-pipeline` ранее не было троттлинга по IP. Loopback-пользователей это не затрагивает; LAN-деплои (`HOST=0.0.0.0`) получают 10 запросов/мин/IP с заголовками `Retry-After` и `X-RateLimit-*` при превышении. Настраивается через `LLM_RATE_LIMIT="N/Ws"`. Дешёвая промежуточная защита до auth-шлюза P-12 в v2.0. Подтверждено 6 тестами в [`tests/rate-limit.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/rate-limit.test.mjs).
...
- **`fix(a11y): H-1 — id="batch-tsv-hint" on the batch.js hint paragraph`** ([`public/js/views/batch.js`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/batch.js)) — в v1.20.0 был добавлен `aria-describedby="batch-tsv-hint"` на textarea TSV, но соответствующий `id` у блока `<p>` не появился. Скринридерам нечего было озвучивать. Исправлено.

- **`fix(a11y): H-2 — htmlFor on batch-parallel / batch-min-score labels`** ([`public/js/views/batch.js`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/views/batch.js)) — четыре инпута из v1.20.0 получили новые id, но их метки не были программно связаны. WCAG 3.3.2 теперь удовлетворён.

- Новая канарейка статического анализа в [`tests/a11y-form-wires.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/a11y-form-wires.test.mjs) — обходит каждый view-файл и утверждает, что каждая IDREF в `aria-describedby` / `htmlFor` указывает на соседнее объявление `id:`. Ловит регрессии класса опечаток в CI.

### 🌐 i18n

- **`fix(i18n): H-3 — 13 keys from v1.20.0 silently fell through to EN for 7 locales`** ([`public/js/lib/i18n.js`](https://github.com/Fighter90/career-ops-ui/blob/main/public/js/lib/i18n.js)) — `pipe.filter`, `pipe.count`, `pipe.preview*`, `pipe.openTab`, `pipe.evaluateAll*`, `eval.jdHint`, `batch.parallelAria`, `batch.minScoreAria`, плюс `common.delete`, `config.group{Core,Runtime,Regional}`, `config.profileEmpty`, `config.viewProfile`, `scan.atsBadge`, `scan.regionalBadge` упоминались как `t('key', 'EN fallback')`, но никогда не добавлялись в DICT. Пользователи скринридеров на русском, японском и китайском слышали английские `aria-label`, что прямо обнуляло выигрыш по WCAG 3.3.2, заявленный в v1.20.0. v1.21.0 добавляет все 19 ключей × 8 локалей (≈ 150 новых переводов) и расширяет [`tests/i18n-coverage.test.mjs`](https://github.com/Fighter90/career-ops-ui/blob/main/tests/i18n-coverage.test.mjs) проходом статического анализа, который сканирует каждый вызов `t('key', …)` в `public/js/**/*.js` и проверяет наличие ключа в DICT. Будущий дрейф ловится на этапе CI.
