...
**Изменено (LOW, сканер) — порт паритета Getro (родитель #2640): зарплата, все локации и определение удалёнки по `work_mode`.**

### Изменено
- Перенесена безопасная часть расширения родительского `providers/getro.mjs` (#2640) в `server/lib/sources/getro.mjs`. Вакансия Getro теперь несёт строку **зарплаты** (из `compensation_amount_min/max_cents` + валюта, только годовая — по строковому соглашению remotli/lever, которое клиентский фильтр зарплаты разбирает заново), её **локация** объединяет **все** `locations` (раньше только первую), а `work_mode: 'remote'` учитывается как сигнал удалёнки наряду с существующим флагом `remote` / текстом локации. Чистое отображение данных из ответа API, который web-ui и так запрашивает — без нового хоста, без изменения SSRF-поверхности.

### Примечания
- Намеренно НЕ перенесено: **авторазрешение** `collection_id` родителя (оно запрашивает собственный `careers_url` доски — произвольный хост — поэтому это отдельное изменение через SSRF-границу `safeGet` web-ui) и поднятый лимит в 1500 страниц (web-ui сохраняет лимит 200 страниц / 4000 вакансий для сканов из браузера). Остальные 11 родительских коммитов этой синхронизации не имеют зеркала в web-ui. Тесты: `tests/sources-getro.test.mjs` (+5). Набор: **2470** (+5).
