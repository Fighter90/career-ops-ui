...
> **🆕 最新版本 — v1.86.0**
>
> **按目标职位统计**（`#/stats`）：全新的 Analytics 页面读取你**个人资料中的目标职位**（并非硬编码）以及最近一次扫描,以图表展示**按国家/地区的职位空缺数与薪资中位数**,提供职位/国家筛选以及**已保存快照的趋势** —— 从扫描器已收集的稀疏数据中在客户端聚合(不编造数字),支持全部 16 种语言。在 v1.85.0（德语/意大利语/土耳其语语言包）、v1.84.0（重复投递冷却期）以及 v1.83.0（重复发布检测器）的基础上构建。
>
> _16 locales · 6 个 LLM 提供方 · 41 个扫描器适配器 · 按目标职位的市场统计 · 重复发布检测器 · 与父级 career-ops v1.16.0 对齐。_
