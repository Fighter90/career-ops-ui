...
> **🆕 最新版本 — v1.93.0**
>
> **记忆层（Epic 24）：** 全新的 **`#/memory`** 页面保存一条简短、可编辑的「关于我请记住这些」的备注。由于它被内联进共享的项目上下文,它会引导 **every** AI 请求 —— 评估、mock interview、人脉拓展、CV Studio —— 并横跨 **all** 提供方。它捕捉你的偏好和工作风格(绝非关于你的新事实),而一个 **从你的数据中提议** 的助手会从你的追踪器中草拟行为条目供你审阅。在 v1.92.0（CV Studio）、v1.91.0（人脉拓展规划器）以及 v1.90.0（mock interview）的基础上构建。
>
> _16 locales · 6 个 LLM 提供方 · 45 个扫描器适配器 · 记忆层 · CV Studio · 人脉拓展规划器 · mock interview · two-pager 市场契合度 · 按目标职位的市场统计 · 重复发布检测器 · 与父级 career-ops v1.16.0 对齐。_
