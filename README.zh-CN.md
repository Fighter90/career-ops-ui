...
> **🆕 最新版本 — v1.90.0**
>
> **Mock Interview 2.0（Epic 15）：** 全新的 **`#/mock-interview`** 页面会针对一份真实职位进行逐轮面试演练。每个回答都会获得 **STAR+R 反馈**、一个**评分**以及一个**追问** —— 均以你的简历、two-pager 和故事库为依据。可用任意 LLM 密钥实时运行,或返回一段可复制粘贴的提示词,并且你可以按公司/职位**保存对话记录**。在 v1.89.0（two-pager 候选人与市场契合度）、v1.88.0（Scan i18n + API 卫生）以及 v1.87.0（4 个新扫描源）的基础上构建。
>
> _16 locales · 6 个 LLM 提供方 · 45 个扫描器适配器 · mock interview · two-pager 市场契合度 · 按目标职位的市场统计 · 重复发布检测器 · 与父级 career-ops v1.16.0 对齐。_
