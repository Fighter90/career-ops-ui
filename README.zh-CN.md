...
> **🆕 最新版本 — v1.96.0**
>
> **职业方向（Epic 27）:** 全新的 **`#/orientation`** 页面回答“哪些方向真正适合我？”——就像职业测评能给你的那种解读,但不是靠问卷,而是从你自己的 CV 和档案中推断。点击 **生成画像**,模型会返回你的 **最契合的职业向量**（八种原型中哪一种契合,并附上证据）、一种职业类型倾向、推荐职位、与 CV 相关联的职业优势、工作风格倾向,以及发展建议。这是 **对你的 CV 读起来如何的一种 AI 反思——而非心理测验**:它不会虚构成就,也绝不会把数值分数当作实测结果来报告。可导出为 **Markdown 或 PDF**;不会向磁盘写入任何内容。在 v1.95.0（职业规划)和 v1.94.0（统计重构)的基础上构建。
>
> _16 locales · 6 个 LLM 提供方 · 45 个扫描器适配器 · 职业方向 · 职业规划 · 统计重构 · 记忆层 · CV Studio · 人脉拓展规划器 · mock interview · two-pager 市场契合度 · 与父级 career-ops v1.16.0 对齐。_
