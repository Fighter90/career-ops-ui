...
> **🆕 最新版本 — v1.189.0** — **`skip_tiers` 现在通晓各种语言** — 层级分类器现在能在任何文字系统中读取职位词后的罗马数字等级（I/II/III/IV/V）（"Инженер III"、"エンジニア I"、"Ingénieur IV"），不再仅限 ASCII 词；此前它们会落入 "mid" 并绕过 `skip_tiers`。 **2498 项测试。**
>
