...
> **🆕 最新版本 — v1.105.0**
>
> **AI 用量与成本页面。** 新的**AI 用量**页面按提供方显示你在实时 AI 生成上花费的代币,涵盖 24 小时 / 7 天 / 30 天 / 全部时间,并附预计 USD 成本。每次实时调用都会向 `data/llm-usage.jsonl` 追加一条 `{provider, in, out}` 记录(不离开你的机器);美元来自可编辑的各提供方价格表——代币数精确,USD 为估算,从不计费。
>
> _AI 用量与成本 · 公司徽标 · AI CLI 工具 · 向文档提问 · 简历定制 + 求职信 · 两页纸自动填充 · DOCX 导出 · 门户健康 · 应用内错误报告器 · 16 locales · 6 个 LLM 提供方 · 46 个扫描器适配器 · 职业方向 · 职业规划 · 统计重构 · 记忆层 · CV Studio · 人脉拓展规划器 · mock interview · two-pager 市场契合度 · 与父级 career-ops v1.16.0 对齐。_
