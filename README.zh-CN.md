...
> **🆕 最新版本 — v1.94.0**
>
> **统计功能,焕然一新（Epic 25）：** **`#/stats`** 页面现在是一个三标签页的 **统计** 版块,配有真实图表和多得多的数据。全新的 **市场报告** 标签页会为你的目标职位生成 AI 薪资与劳动力市场分析 —— 按职级并含 P10–P90 百分位、头部雇主、热门技能、福利、远程占比、趋势,以及谈判指导,在你所选的地区与 **货币** 下呈现(数字标注为模型的方向性估计,绝非抓取)。全新的 **我的管线** 标签页会绘制你自己的追踪器(评分、漏斗、头部公司/职位、时间线、转化率)。每一份报告都可导出为 **Markdown 或 PDF**。在 v1.93.0（记忆层)和 v1.92.0（CV Studio)的基础上构建。
>
> _统计重构 · 16 locales · 6 个 LLM 提供方 · 45 个扫描器适配器 · 记忆层 · CV Studio · 人脉拓展规划器 · mock interview · two-pager 市场契合度 · 重复发布检测器 · 与父级 career-ops v1.16.0 对齐。_
