...
> **🆕 最新版本 — v1.100.0**
>
> **两页纸 AI 自动填充 + 导出。** 两页纸(`#/two-pager`)现在会用你配置的提供方从简历 + 档案自动填充(绝不杜撰；无密钥时回退到复制提示词)，新增**预览并导出**视图，并获得 **PDF / DOCX / Markdown** 导出——DOCX 由一个新的零依赖 `.docx` 生成器完成，也接入了市场、职业规划与职业定位报告。
>
> _两页纸自动填充 · DOCX 导出 · 门户健康 · 应用内错误报告器 · 16 locales · 6 个 LLM 提供方 · 46 个扫描器适配器 · 职业方向 · 职业规划 · 统计重构 · 记忆层 · CV Studio · 人脉拓展规划器 · mock interview · two-pager 市场契合度 · 与父级 career-ops v1.16.0 对齐。_
