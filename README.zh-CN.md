...
> **🆕 最新版本 — v1.92.0**
>
> **CV Studio（Epic 21）：** 全新的 **`#/cv-studio`** 页面为你的简历提供三个诚实的工具 —— **简历诊断**（一个确定性的 0–100 分,并附带逐项检查的说明）、**隐私脱敏**（完全在浏览器中遮蔽邮箱/电话/链接/地址/姓名),以及 **make it human**（以 _你的_ 语气重写生硬的句子,以你的 voice-dna 和写作样本为依据,绝不编造事实）。在 v1.91.0（人脉拓展规划器）、v1.90.0（mock interview）以及 v1.89.0（two-pager 市场契合度）的基础上构建。
>
> _16 locales · 6 个 LLM 提供方 · 45 个扫描器适配器 · CV Studio · 人脉拓展规划器 · mock interview · two-pager 市场契合度 · 按目标职位的市场统计 · 重复发布检测器 · 与父级 career-ops v1.16.0 对齐。_
