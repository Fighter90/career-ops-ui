...
> **🆕 最新版本 — v1.89.0**
>
> **候选人与市场契合度 —— two-pager（Epic 14）：** 全新的 **`#/two-pager`** 页面以 “Mnookin two-pager” 格式记录_你_对下一份工作真正想要的东西（喜欢的、必备项、讨厌的、deal-breaker、不可谈判项）。**AI 填写助手**会根据你的简历 + 个人资料起草它,`#/scan` 上的每则职位都会获得一个**与你所想契合度 `◎` 评分**,并且 two-pager 会喂给每一次评估。在 v1.88.0（Scan i18n + API 卫生）、v1.87.0（4 个新扫描源）以及 v1.86.0（目标职位统计）的基础上构建。
>
> _16 locales · 6 个 LLM 提供方 · 45 个扫描器适配器 · two-pager 市场契合度 · 按目标职位的市场统计 · 重复发布检测器 · 与父级 career-ops v1.16.0 对齐。_
