...
> **🆕 最新版本 — v1.116.0**
>
> **用量计量重做 + 首个端到端组件测试。** AI 用量计量现固定在左侧边栏底部(不遮挡菜单——预留空间让导航自由滚动),实时刷新,并按窗口显示真实的令牌 · 预估费用。持久的 typeof 屏障关闭了反复出现的 CodeQL 误报,新的 Playwright 端到端测试在真实浏览器中驱动两个常驻组件。
>
> _用量计量重做 · 设计打磨 · 用量计量 · 浮动帮助助手 · 文档与 QA 整合 · 安全待办清理 · 文档与 QA ×16 · Scan 排除 · 流水线概览 · 安全加固 2 · 净化器加固 · 安全加固 · AI 用量与成本 · 公司徽标 · AI CLI 工具 · 向文档提问 · 简历定制 + 求职信 · 两页纸自动填充 · DOCX 导出 · 门户健康 · 应用内错误报告器 · 16 locales · 6 个 LLM 提供方 · 46 个扫描器适配器 · 职业方向 · 职业规划 · 统计重构 · 记忆层 · CV Studio · 人脉拓展规划器 · mock interview · two-pager 市场契合度 · 与父级 career-ops v1.16.0 对齐。_
