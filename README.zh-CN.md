...
> **🆕 最新版本 — v1.91.0**
>
> **人脉拓展与深度公司调研（Epic 16）：** 全新的 **`#/networking`** 页面会将一家公司转化为一份可执行计划 —— 一份**公司档案**、**该联系谁**（人物画像 + LinkedIn 搜索字符串）、**最温暖的引荐路径**,以及量身定制的**触达草稿** —— 全部以你的简历和 two-pager 为依据。可用任意 LLM 密钥实时运行,或返回一段可复制粘贴的提示词,并且你可以按公司**保存计划**。在 v1.90.0（mock interview）、v1.89.0（two-pager 候选人与市场契合度）以及 v1.88.0（Scan i18n + API 卫生）的基础上构建。
>
> _16 locales · 6 个 LLM 提供方 · 45 个扫描器适配器 · 人脉拓展规划器 · mock interview · two-pager 市场契合度 · 按目标职位的市场统计 · 重复发布检测器 · 与父级 career-ops v1.16.0 对齐。_
