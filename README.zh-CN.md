...
> **🆕 最新版本 — v1.111.0**
>
> **安全 — 清理 CodeQL 待办。** 三项纵深防御强化从根源关闭剩余的静态分析发现:CV markdown 净化器现在会转义任何被截断的危险标签开头,使其输出永不含存活的 `<script`/`<iframe`;CV 导入通过显式数值转换读取缓冲区大小;模式角色行改为用 `String.replace` 插值的模板字符串而非存储函数,移除了最后的动态派发调用。用户可见无变化。
>
> _安全待办清理 · 文档与 QA ×16 · Scan 排除 · 流水线概览 · 安全加固 2 · 净化器加固 · 安全加固 · AI 用量与成本 · 公司徽标 · AI CLI 工具 · 向文档提问 · 简历定制 + 求职信 · 两页纸自动填充 · DOCX 导出 · 门户健康 · 应用内错误报告器 · 16 locales · 6 个 LLM 提供方 · 46 个扫描器适配器 · 职业方向 · 职业规划 · 统计重构 · 记忆层 · CV Studio · 人脉拓展规划器 · mock interview · two-pager 市场契合度 · 与父级 career-ops v1.16.0 对齐。_
