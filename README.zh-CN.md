...
> **🆕 最新版本 — v1.95.0**
>
> **职业规划（Epic 26）：** 全新的 **`#/career-plan`** 页面会把你的 CV 与档案转化为一份具体、个性化的发展规划。选择一个 **周期**(6/12/24 个月)和一个可选的聚焦方向,模型便会写出起点快照、优势/成长 SWOT、以 **SMART / OKR / WOOP** 形式呈现的目标、备选发展轨迹、硬技能/软技能规划、一份 **逐月路线图**、进度追踪、常见陷阱与支持性举措 —— 立足于你自己的材料,绝不虚构你的过往经历。可就地编辑,**保存** 到用户层,并导出为 **Markdown 或 PDF**。在 v1.94.0（统计重构)和 v1.93.0（记忆层)的基础上构建。
>
> _16 locales · 6 个 LLM 提供方 · 45 个扫描器适配器 · 职业规划 · 统计重构 · 记忆层 · CV Studio · 人脉拓展规划器 · mock interview · two-pager 市场契合度 · 与父级 career-ops v1.16.0 对齐。_
