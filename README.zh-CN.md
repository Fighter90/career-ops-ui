...
> **🆕 最新版本 — v1.228.3** — **文档助手无法用英文作答。** 问题不在检索，而在上下文构建：遇到第一个放不下的章节就 `break`，把上下文留空。英文 §5 已涨到 16 081 字节，而预算是 14 336。超过 6 KB 的章节现在按 `###` 边界切分（32 → 75 块）。此外：Telegram 的公司标签给出的是句子而非名称，以及不泄露任何信息的存活探针 `GET /api/ping`。**2907 个测试。**
...
| 套件                       | 测试数 | 内容                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs`(unit + integration) | **2907** | 每个端点,临时端口,无外网。覆盖 parser、scanner(已 mock)、runner、anthropic、安全 header、XSS(含实体解码)、JD sanitize、URL 校验、SSRF 重定向 / rebind、并发互斥、路径遍历、速率限制、i18n 对等。 |
...
- 推送前先跑 `npm test` —— **2907 项全绿** 是底线(触碰 UI 时再加上 90 个 Playwright)。
