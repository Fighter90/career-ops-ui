...
> **🆕 最新版本 — v1.103.0**
>
> **设置：AI CLI 工具。** 应用设置中的新**AI CLI 工具**标签页显示 Claude Code、Codex、Gemini CLI、OpenCode、GitHub Copilot CLI、Qwen、Antigravity 中哪些安装在运行服务器的机器上及其路径。只读的 PATH 扫描：只检查每个二进制是否存在，**绝不运行它**(无 `--version`，不执行)，不写入任何内容，不读取用户数据。
>
> _AI CLI 工具 · 向文档提问 · 简历定制 + 求职信 · 两页纸自动填充 · DOCX 导出 · 门户健康 · 应用内错误报告器 · 16 locales · 6 个 LLM 提供方 · 46 个扫描器适配器 · 职业方向 · 职业规划 · 统计重构 · 记忆层 · CV Studio · 人脉拓展规划器 · mock interview · two-pager 市场契合度 · 与父级 career-ops v1.16.0 对齐。_
