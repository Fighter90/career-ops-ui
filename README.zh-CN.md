...
> **🆕 最新版本 — v1.93.0**
>
> **记忆层（Epic 24）：** 全新的 **`#/memory`** 页面保存一条简短、可编辑的「关于我请记住这些」的备注。由于它被内联进共享的项目上下文,它会引导 **每一项** AI 请求 —— 评估、mock interview、人脉拓展、CV Studio —— 并横跨 **所有** 提供方。它捕捉你的偏好和工作风格(绝非关于你的新事实),而一个 **从你的数据中提议** 的助手会从你的追踪器中草拟行为条目供你审阅。在 v1.92.0（CV Studio）、v1.91.0（人脉拓展规划器）以及 v1.90.0（mock interview）的基础上构建。
>
