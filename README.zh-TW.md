...
> **🆕 最新版本 — v1.131.2**
>
> **`app.css` 拆分(重構)。** v1.131.2 償還一項檔案大小規範債務:約 1990 行的 `app.css` 拆分為三個依序載入的樣式表 — `app.css`(基礎/外框/按鈕)、**`components.css`**(卡片、表格、表單、徽章、分頁器……)、**`overlays.css`**(提示訊息、彈窗、抽屜、`[dir="rtl"]`、docs-fab、usage-hud)— 每個檔案皆在 800 行的硬性目標之內。切分方式為連續且依原順序,因此串接結果**逐位元組完全相同**;`index.html` 依序以三個 `<link>` 載入全部三者。無行為/標記/i18n 變更;已於瀏覽器中驗證(三份樣式表皆能解析且生效,0 個主控台錯誤)。**2138 個測試**。 **配接器主機固定強化(修補版)。** v1.131.1 針對 v1.130.0 兩個來源的程式碼審查跟進項目進行處理(縱深防禦,對合法輸入無行為變化):**a16z Speedrun** 配接器現在會重新驗證端點覆寫值(HTTPS + 精確主機)並在失敗時降級回退至規範動態消息 — 與 **Cryptocurrency Jobs** 一致 — Cryptocurrency Jobs 解析器的 `cleanUrl` 現使用與其 SSRF 檢查相同的精確主機把關(原為 `endsWith`,會誤放行子網域),確保解析器絕不會比把關更寬鬆。**2135 個測試**。 **CRM 階段分頁看板。** v1.131.0 將父專案 web app 的 `/pipeline` 檢視移植到 `#/tracker`:漏斗晶片列 + 狀態下拉選單,變成一條**階段分頁列** — **All** 外加每個規範狀態各一個分頁(Evaluated · Applied · Responded · Interview · Offer · Rejected · Discarded · SKIP · Hired),每個分頁都附即時計數,**包含零計數的階段**,讓完整漏斗隨時可見。分頁來自新的唯讀 `GET /api/tracker/stages`(規範標籤 + 來自 `templates/states.yml` 的別名摺疊 — 客戶端永不硬編碼白名單),各列新增可選的品牌標誌,且無障礙名稱皆帶有計數。**2133 個測試**。 **父專案 v1.24.0 一致性。** v1.130.0 移植兩個新掃描來源 — **a16z Speedrun**(a16z Speedrun 人才網絡全站動態消息)與 **Cryptocurrency Jobs**(Web3 看板 `cryptocurrencyjobs.co`,透過其公開 RSS)—,將註冊表擴增至 **72 個來源(67 個英文 + 5 個俄文)**,並在 `#/scan` 的 Source 篩選器與 cvstart.org 落地頁中同步呈現。同時鏡像兩項父專案修復:**echojobs** 現在讓**混合制**(hybrid)職缺與遠端職缺保持可區分(`City · Hybrid`,不再併入 `Remote`),**radancy** 學會辨識舊版 TalentBrew 標記語言以及 JSON 結果片段傳輸。**2123 個測試**。 **AI 審查跟進(修補)。** v1.129.1 修復 v1.128/v1.129 web 移植的諮詢性問題:job-facets 級別優先序(`Senior Engineering Manager` → `senior`,而非 `lead`)、`states.mjs` 不再因啟動時父專案故障而固定回退(+損毀檔案警告)、評分色調將無評分列顯示為中性(非紅色)、`domainFromName` 跳過非 ASCII slug。 **掃描級別面向 + 時長欄。** v1.129.0 將 v1.128.0 的 `job-facets.js` 函式庫接入 `#/scan`:新增**級別**篩選(lead/staff/senior/mid/junior/intern,像國家面向一樣從結果自動填入)、級別徽章欄,以及零 token 的**時長**欄(`今天`/`N天`)。 **從父專案 Web 應用移植四項。** v1.128.0 以原生 JS 重新實作 career-ops Next.js 應用的四項解決方案:追蹤器即時從 `templates/states.yml` 讀取規範狀態(去除硬編碼白名單)、ATS 託管列透過名稱→網域對映顯示品牌標誌、更細的 4 級評分色調,以及零 token 的 job-facets 函式庫。**2066 個測試**。 **父專案 v1.23.0 一致性。** v1.127.0 移植三個新掃描來源 — **Flowxtra**、**VDAB**、**iCIMS**(註冊表現 **70 個配接器**,65 EN + 5 RU)—,鏡像 agenticjobs 的 HTML→REST 遷移、Greenhouse 城市還原與 role-matcher 修復,並將 **Cursor** 重新加入 CLI 名單(parent #2115 — `cli-detect` 回報 10 個工具)。 **CLI 名單漂移修補。** v1.126.1 修復了 v1.126.0 掃描遺漏的兩處:`#/config` API keys 分頁導語(i18n ×17)現列出 Antigravity + Grok Build,說明指南 ×17(及 CI 建置的網站 help)中過時的 `Cursor / Gemini CLI` 列現為完整的 8 個 CLI 名單。 **文件與 CLI 名單重新同步。** v1.126.0 將每個文件面(help ×17、README ×17、wiki)與即時 career-ops.org/docs(全部 31 頁已讀)核對,並讓 `#/config` 的 AI CLI 工具掃描偵測全部 8 個一級 CLI — **Grok Build CLI** 與 **Kimi CLI** 加入 Claude Code、Cursor、Codex、OpenCode、Antigravity、Qwen 和 Copilot(現回報 **9 個工具**,仍從不執行)。 **維護同步。** v1.125.4 彙總了 dependabot 的 site 相依升級(`sharp` 0.35.3、`svgo` 4.0.2、`fast-uri` 3.1.4),並記錄父專案一致性巡檢(v1.22.0 之後,#2108–#2168): set-status 錯列防護、在地化模式 Risk Summary 與更新器資訊清單驗證均在 CLI 側 — 無需移植(**1969 個測試** 不變)。 **提示詞語言修復。** v1.125.3 讓丹麥語與印地語在所有 AI 提示詞中成為一等公民 — 深度研究(即時與手動)、模式執行、評估、面試、人脈與 CV Studio 現在在這兩個語言下都會輸出 `# Output language` 指令(先前回答為英文),語言閘門現在遍歷全部 17 個語言(**1969 個測試**)。 **外部貢獻者包。** v1.125.2 合併了 [@Alien10140](https://github.com/Alien10140) 的首批社群 PR — 即時深度研究 HTTP 502 修復(面向 API 供應方的無工具 headless 提示詞),以及把 Gemini 預設模型從已棄用的 `gemini-2.0-flash` 升至 `gemini-3.6-flash`,由新的 5 測試防漂移閘門固定(**1957 個測試**)。v1.125.1 讓多品牌 SuccessFactors RMK 租戶保留品牌路徑(父專案 #2099)。**登陸頁新增求職來源區塊。** v1.125.0 在 cvstart.org 新增一個區塊,將全部 **70 個掃描來源** 以可點擊標籤列出,並於建置時與登錄檔同步、由漂移關卡把關。在此之前,**父專案 v1.22.0 對齊。** v1.124.0 移植了 **五個掃描來源** — Welcome to the Jungle、Agentic Engineering Jobs、Jobvite、Gem 與 Alibaba Group(現有 **70 個轉接器**) — 以及 Arbeitsagentur 完全遠端驗證(`homeofficetyp=VOLLSTAENDIG`)與 SmartRecruiters 公開網址修正。
>
> _app.css 拆分(三個樣式表) · 檔案大小規範債務清償 · 位元組級相同的串接結果 · 配接器主機固定強化 · a16z 覆寫值重新驗證 · cryptocurrencyjobs cleanUrl 精確主機 · CRM 階段分頁看板 · 追蹤器 /pipeline 移植 · 階段分頁 + 計數 · `GET /api/tracker/stages` · 追蹤器品牌標誌 · 父專案 v1.24 一致性 · +2 個來源(a16z Speedrun/Cryptocurrency Jobs) · 72 個配接器 · echojobs 混合制 · radancy TalentBrew · AI 審查修補 · 級別優先序 · states 回退 · 中性色調 · 掃描級別面向 · 時長欄 · job-facets 接入 · web/ 四移植 · states.yml 即時 · 名稱→網域標誌 · 評分色調 · 面向 · 父專案 v1.23 一致性 · +3 來源 · 70 配接器 · Cursor · CLI 名單漂移修補 · 文件與 CLI 名單重新同步 · 維護同步 · da/hi 提示詞語言修復 · 外部貢獻者包 · 深度研究 502 修復 · Gemini 3.6 預設 · SF 品牌路徑修復 · 登陸頁來源區塊 · 5 個來源 · 67 個配接器 · 父專案 v1.22 對齊 · Oracle Cloud · 62 個配接器 · 父專案 v1.21 對齊 · 印地語 · 17 種語言 · 宣言 · cvstart.org 方法論／授權／更新日誌頁面 · 父專案 v1.19 對齊 · 61 個轉接器 · 落地頁即時星標 + 貢獻者 · 父專案 v1.18 對齊 · 54 個轉接器 · Hired 狀態 · 累計統計 · 對齊包 · 50 轉接器 · 用量計量重做 · 設計打磨 · 用量計量 · 浮動說明助手 · 文件與 QA 整合 · 安全待辦清理 · 文件與 QA ×16 · Scan 排除 · 流水線概覽 · 安全強化 2 · 淨化器強化 · 安全強化 · AI 用量與成本 · 公司標誌 · AI CLI 工具 · 向文件提問 · 履歷客製 + 求職信 · 兩頁紙自動填入 · DOCX 匯出 · 入口網站健康 · 應用程式內錯誤回報器 · 16 個 locales · 6 個 LLM 提供者 · 46 個掃描器轉接器 · 職業方向 · 職涯規劃 · 統計重構 · 記憶層 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 與父層 career-ops v1.16.0 對齊。_
