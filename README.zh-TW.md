...
> **🆕 最新版本 — v1.91.0**
>
> **人脈拓展與深度公司研究（Epic 16）：** 全新的 **`#/networking`** 頁面會將一家公司轉化為一份可執行計畫 —— 一份**公司檔案**、**該聯繫誰**（人物誌 + LinkedIn 搜尋字串）、**最溫暖的引薦路徑**，以及量身打造的**觸達草稿** —— 皆以你的履歷和 two-pager 為依據。可用任意 LLM 金鑰即時執行，或回傳一段可複製貼上的提示詞，而且你可以依公司**儲存計畫**。在 v1.90.0（mock interview）、v1.89.0（two-pager 候選人與市場契合度）和 v1.88.0（Scan i18n + API 衛生）的基礎上構建。
>
> _16 locales · 6 個 LLM 提供者 · 45 個掃描器轉接器 · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 依目標職務的市場統計 · 重複刊登偵測器 · 與父層 career-ops v1.16.0 對齊。_
