...
> **🆕 最新版本 — v1.114.0**
>
> **側邊欄 AI 用量與費用計量。** 每個頁面側邊欄底部的精簡用量區塊(左下;RTL 右下)以綠色進度條和 24 小時預估費用展示 24h / 7d / 30d 的 LLM 權杖用量——來自唯讀本地用量日誌,與用量頁面同源。可折疊;費用為估算,手動模式執行免費。
>
> _用量計量 · 浮動說明助手 · 文件與 QA 整合 · 安全待辦清理 · 文件與 QA ×16 · Scan 排除 · 流水線概覽 · 安全強化 2 · 淨化器強化 · 安全強化 · AI 用量與成本 · 公司標誌 · AI CLI 工具 · 向文件提問 · 履歷客製 + 求職信 · 兩頁紙自動填入 · DOCX 匯出 · 入口網站健康 · 應用程式內錯誤回報器 · 16 個 locales · 6 個 LLM 提供者 · 46 個掃描器轉接器 · 職業方向 · 職涯規劃 · 統計重構 · 記憶層 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 與父層 career-ops v1.16.0 對齊。_
