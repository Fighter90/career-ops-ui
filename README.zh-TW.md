...
> **🆕 最新版本 — v1.100.0**
>
> **兩頁紙 AI 自動填入 + 匯出。** 兩頁紙(`#/two-pager`)現在會用你設定的供應商從履歷 + 檔案自動填入(絕不杜撰；無金鑰時回退到複製提示詞)，新增**預覽並匯出**檢視，並獲得 **PDF / DOCX / Markdown** 匯出——DOCX 由一個新的零相依 `.docx` 產生器完成，也接入了市場、職涯規劃與職涯定位報告。
>
> _兩頁紙自動填入 · DOCX 匯出 · 入口網站健康 · 應用程式內錯誤回報器 · 16 個 locales · 6 個 LLM 提供者 · 46 個掃描器轉接器 · 職業方向 · 職涯規劃 · 統計重構 · 記憶層 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 與父層 career-ops v1.16.0 對齊。_
