...
>
> _父專案 v1.18 對齊 · 54 個轉接器 · Hired 狀態 · 累計統計 · 對齊包 · 50 轉接器 · 用量計量重做 · 設計打磨 · 用量計量 · 浮動說明助手 · 文件與 QA 整合 · 安全待辦清理 · 文件與 QA ×16 · Scan 排除 · 流水線概覽 · 安全強化 2 · 淨化器強化 · 安全強化 · AI 用量與成本 · 公司標誌 · AI CLI 工具 · 向文件提問 · 履歷客製 + 求職信 · 兩頁紙自動填入 · DOCX 匯出 · 入口網站健康 · 應用程式內錯誤回報器 · 16 個 locales · 6 個 LLM 提供者 · 46 個掃描器轉接器 · 職業方向 · 職涯規劃 · 統計重構 · 記憶層 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 與父層 career-ops v1.16.0 對齊。_
...
```bash
npm test                       # 1824 個 unit/integration 測試
...
| Suite                       | 測試數 | 內容                                                                                                         |
| --------------------------- | ----- | ------------------------------------------------------------------------------------------------------------ |
| `node --test tests/*.test.mjs`(unit + integration) | **1824** | 每個端點、暫時性伺服器、無網路。涵蓋解析器、scanner(已 mock)、runner、anthropic、CSP / 安全 header、實體感知 XSS、JD sanitize、URL 驗證、SSRF redirect rebind、檔案互斥下的並行 tracker 寫入、`llmRateLimit`、路徑統一 sanitization、i18n parity。 |
...
- 推送前執行 `npm test` — **1824 項檢查綠燈**為門檻(若觸碰 UI 則加上 90 個 Playwright)。
