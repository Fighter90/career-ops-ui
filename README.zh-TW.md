...
> **🆕 最新版本 — v1.233.2** — **兩條高危 CodeQL `js/remote-property-injection` 告警**，通過把既有不變式由「斷言」改為「結構」而消除。把儲存值寫入 `process.env` 的迴圈原本遍歷由請求體建立的物件；現在直接遍歷常量陣列。**行為完全相同。** **3022 項 · 瀏覽器 116 項。**
