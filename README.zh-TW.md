...
> **🆕 最新版本 — v1.122.0**
>
> **印地語加入 — 17 種語言。** v1.122.0 為應用程式全面新增 हिन्दी(印地語)作為第 17 種介面語言：應用程式 UI、應用程式內說明指南、README、更新日誌，以及 cvstart.org 的每個頁面(落地頁 + 方法論／授權／更新日誌／說明)。
>
> _印地語 · 17 種語言 · 宣言 · cvstart.org 方法論／授權／更新日誌頁面 · 父專案 v1.19 對齊 · 61 個轉接器 · 落地頁即時星標 + 貢獻者 · 父專案 v1.18 對齊 · 54 個轉接器 · Hired 狀態 · 累計統計 · 對齊包 · 50 轉接器 · 用量計量重做 · 設計打磨 · 用量計量 · 浮動說明助手 · 文件與 QA 整合 · 安全待辦清理 · 文件與 QA ×16 · Scan 排除 · 流水線概覽 · 安全強化 2 · 淨化器強化 · 安全強化 · AI 用量與成本 · 公司標誌 · AI CLI 工具 · 向文件提問 · 履歷客製 + 求職信 · 兩頁紙自動填入 · DOCX 匯出 · 入口網站健康 · 應用程式內錯誤回報器 · 16 個 locales · 6 個 LLM 提供者 · 46 個掃描器轉接器 · 職業方向 · 職涯規劃 · 統計重構 · 記憶層 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 與父層 career-ops v1.16.0 對齊。_
