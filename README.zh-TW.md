...
> **🆕 最新版本 — v1.86.0**
>
> **依目標職務統計**（`#/stats`）：全新的 Analytics 頁面讀取你**個人檔案中的目標職務**（並非硬編碼）以及最近一次掃描,以圖表呈現**依國家/地區的職缺數與薪資中位數**,提供職務/國家篩選以及**已儲存快照的趨勢** —— 從掃描器已收集的稀疏資料中在用戶端彙整(不捏造數字),支援全部 16 種語言。在 v1.85.0（德語/義大利語/土耳其語語系）、v1.84.0（重新申請冷卻）和 v1.83.0（重複刊登偵測器）的基礎上構建。
>
> _16 locales · 6 個 LLM 提供者 · 41 個掃描器轉接器 · 依目標職務的市場統計 · 重複刊登偵測器 · 與父層 career-ops v1.16.0 對齊。_
