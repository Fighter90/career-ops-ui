...
> **🆕 最新版本 — v1.109.0**
>
> **Scan 排除篩選 + 流水線概覽。** 掃描搜尋框現在把逗號視為或("要找的職位"),新增排除欄位隱藏符合任一逗號分隔詞的列;兩者都保存在已儲存搜尋中。流水線頁面新增精簡概覽列——收件匣 N、已追蹤 N,以及來自追蹤器的 Applied / Responded / Interview / Offer 計數,各自連結到 `#/tracker`。
>
> _Scan 排除 · 流水線概覽 · 安全強化 2 · 淨化器強化 · 安全強化 · AI 用量與成本 · 公司標誌 · AI CLI 工具 · 向文件提問 · 履歷客製 + 求職信 · 兩頁紙自動填入 · DOCX 匯出 · 入口網站健康 · 應用程式內錯誤回報器 · 16 個 locales · 6 個 LLM 提供者 · 46 個掃描器轉接器 · 職業方向 · 職涯規劃 · 統計重構 · 記憶層 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 與父層 career-ops v1.16.0 對齊。_
