...
> **🆕 最新版本 — v1.126.0**
>
> **文件與 CLI 名單重新同步。** v1.126.0 將每個文件面(help ×17、README ×17、wiki)與即時 career-ops.org/docs(全部 31 頁已讀)核對,並讓 `#/config` 的 AI CLI 工具掃描偵測全部 8 個一級 CLI — **Grok Build CLI** 與 **Kimi CLI** 加入 Claude Code、Codex、OpenCode、Antigravity、Qwen 和 Copilot(現回報 **9 個工具**,仍從不執行)。 **維護同步。** v1.125.4 彙總了 dependabot 的 site 相依升級(`sharp` 0.35.3、`svgo` 4.0.2、`fast-uri` 3.1.4),並記錄父專案一致性巡檢(v1.22.0 之後,#2108–#2168): set-status 錯列防護、在地化模式 Risk Summary 與更新器資訊清單驗證均在 CLI 側 — 無需移植(**1969 個測試** 不變)。 **提示詞語言修復。** v1.125.3 讓丹麥語與印地語在所有 AI 提示詞中成為一等公民 — 深度研究(即時與手動)、模式執行、評估、面試、人脈與 CV Studio 現在在這兩個語言下都會輸出 `# Output language` 指令(先前回答為英文),語言閘門現在遍歷全部 17 個語言(**1969 個測試**)。 **外部貢獻者包。** v1.125.2 合併了 [@Alien10140](https://github.com/Alien10140) 的首批社群 PR — 即時深度研究 HTTP 502 修復(面向 API 供應方的無工具 headless 提示詞),以及把 Gemini 預設模型從已棄用的 `gemini-2.0-flash` 升至 `gemini-3.6-flash`,由新的 5 測試防漂移閘門固定(**1957 個測試**)。v1.125.1 讓多品牌 SuccessFactors RMK 租戶保留品牌路徑(父專案 #2099)。**登陸頁新增求職來源區塊。** v1.125.0 在 cvstart.org 新增一個區塊,將全部 **67 個掃描來源** 以可點擊標籤列出,並於建置時與登錄檔同步、由漂移關卡把關。在此之前,**父專案 v1.22.0 對齊。** v1.124.0 移植了 **五個掃描來源** — Welcome to the Jungle、Agentic Engineering Jobs、Jobvite、Gem 與 Alibaba Group(現有 **67 個轉接器**) — 以及 Arbeitsagentur 完全遠端驗證(`homeofficetyp=VOLLSTAENDIG`)與 SmartRecruiters 公開網址修正。
>
> _文件與 CLI 名單重新同步 · 維護同步 · da/hi 提示詞語言修復 · 外部貢獻者包 · 深度研究 502 修復 · Gemini 3.6 預設 · SF 品牌路徑修復 · 登陸頁來源區塊 · 5 個來源 · 67 個配接器 · 父專案 v1.22 對齊 · Oracle Cloud · 62 個配接器 · 父專案 v1.21 對齊 · 印地語 · 17 種語言 · 宣言 · cvstart.org 方法論／授權／更新日誌頁面 · 父專案 v1.19 對齊 · 61 個轉接器 · 落地頁即時星標 + 貢獻者 · 父專案 v1.18 對齊 · 54 個轉接器 · Hired 狀態 · 累計統計 · 對齊包 · 50 轉接器 · 用量計量重做 · 設計打磨 · 用量計量 · 浮動說明助手 · 文件與 QA 整合 · 安全待辦清理 · 文件與 QA ×16 · Scan 排除 · 流水線概覽 · 安全強化 2 · 淨化器強化 · 安全強化 · AI 用量與成本 · 公司標誌 · AI CLI 工具 · 向文件提問 · 履歷客製 + 求職信 · 兩頁紙自動填入 · DOCX 匯出 · 入口網站健康 · 應用程式內錯誤回報器 · 16 個 locales · 6 個 LLM 提供者 · 46 個掃描器轉接器 · 職業方向 · 職涯規劃 · 統計重構 · 記憶層 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 與父層 career-ops v1.16.0 對齊。_
