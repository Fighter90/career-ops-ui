...
> **🆕 最新版本 — v1.102.0**
>
> **向文件提問——有據可依的說明對話。** 新增**向文件提問 💬**頁面，**僅**使用你所用語言的應用說明指南回答使用問題(顯示所用章節，絕不讀取你的履歷或求職資訊)。對指南 `##` 章節進行零相依關鍵字檢索；模型據此作答或承認指南未涵蓋。有 LLM 金鑰時即時，無則給出可複製提示詞。
>
> _向文件提問 · 履歷客製 + 求職信 · 兩頁紙自動填入 · DOCX 匯出 · 入口網站健康 · 應用程式內錯誤回報器 · 16 個 locales · 6 個 LLM 提供者 · 46 個掃描器轉接器 · 職業方向 · 職涯規劃 · 統計重構 · 記憶層 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 與父層 career-ops v1.16.0 對齊。_
