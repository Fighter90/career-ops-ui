...
> **🆕 最新版本 — v1.94.0**
>
> **統計功能,全面翻新（Epic 25）：** **`#/stats`** 頁面現在是一個三頁籤的**統計**版塊,配有真實圖表與多得多的資料。全新的**市場報告**頁籤會為你的目標職務產生 AI 薪資與勞動力市場分析 —— 依職級並含 P10–P90 百分位、頂尖雇主、熱門技能、福利、遠端占比、趨勢,以及談判指引,在你所選的地區與**貨幣**下呈現(數字標註為模型的方向性估計,絕非抓取)。全新的**我的管線**頁籤會繪製你自己的追蹤器(評分、漏斗、頂尖公司/職務、時間線、轉換率)。每一份報告都可匯出為 **Markdown 或 PDF**。在 v1.93.0（記憶層)與 v1.92.0（CV Studio)的基礎上構建。
>
> _統計重構 · 16 個 locales · 6 個 LLM 提供者 · 45 個掃描器轉接器 · 記憶層 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 重複刊登偵測器 · 與父層 career-ops v1.16.0 對齊。_
