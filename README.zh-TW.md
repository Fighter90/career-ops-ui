...
> **🆕 最新版本 — v1.106.0**
>
> **安全強化(CodeQL 分診)。** 路由渲染的錯誤路徑現在會在錯誤訊息到達 DOM 之前進行跳脫(伺服器錯誤可能回顯使用者輸入——XSS 邊界),檔案/設定的屬性寫入會拒絕 `__proto__` / `constructor` / `prototype` 鍵(原型汙染防護)。其餘靜態分析警示多為對掃描器合法 `data/*` 讀寫的誤報,已附理由駁回。
>
> _安全強化 · AI 用量與成本 · 公司標誌 · AI CLI 工具 · 向文件提問 · 履歷客製 + 求職信 · 兩頁紙自動填入 · DOCX 匯出 · 入口網站健康 · 應用程式內錯誤回報器 · 16 個 locales · 6 個 LLM 提供者 · 46 個掃描器轉接器 · 職業方向 · 職涯規劃 · 統計重構 · 記憶層 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 與父層 career-ops v1.16.0 對齊。_
