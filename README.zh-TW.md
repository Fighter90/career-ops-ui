...
> **🆕 最新版本 — v1.189.0** — **`skip_tiers` 現在通曉各種語言** — 層級分類器現在能在任何文字系統中讀取職位詞後的羅馬數字等級（I/II/III/IV/V）（「Инженер III」、「エンジニア I」、「Ingénieur IV」），不再僅限 ASCII 詞；此前它們會落入 "mid" 並繞過 `skip_tiers`。 **2498 項測試。**
>
