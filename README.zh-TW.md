...
> **🆕 最新版本 — v1.111.0**
>
> **安全 — 清理 CodeQL 待辦。** 三項縱深防禦強化從根源關閉剩餘的靜態分析發現:CV markdown 淨化器現在會逸出任何被截斷的危險標籤開頭,使其輸出永不含存活的 `<script`/`<iframe`;CV 匯入透過明確數值轉換讀取緩衝區大小;模式角色行改為用 `String.replace` 插值的範本字串而非儲存函式,移除了最後的動態派發呼叫。使用者可見無變化。
>
> _安全待辦清理 · 文件與 QA ×16 · Scan 排除 · 流水線概覽 · 安全強化 2 · 淨化器強化 · 安全強化 · AI 用量與成本 · 公司標誌 · AI CLI 工具 · 向文件提問 · 履歷客製 + 求職信 · 兩頁紙自動填入 · DOCX 匯出 · 入口網站健康 · 應用程式內錯誤回報器 · 16 個 locales · 6 個 LLM 提供者 · 46 個掃描器轉接器 · 職業方向 · 職涯規劃 · 統計重構 · 記憶層 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 與父層 career-ops v1.16.0 對齊。_
