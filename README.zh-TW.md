...
> **🆕 最新版本 — v1.108.0**
>
> **安全強化(CodeQL 分診第 2 輪)。** 模式提示建構器按自有鍵 + `typeof === function` 解析角色行,PDF 檔名 slug 在其正規式之前限制長度(多項式 ReDoS),文件匯入將陣列 `filename` 強制轉換為字串(type-confusion)。v1.106–v1.108 期間靜態分析積壓從 167 降至約 14,所有真實發現均已修復,其餘已附理由駁回。
>
> _安全強化 2 · 淨化器強化 · 安全強化 · AI 用量與成本 · 公司標誌 · AI CLI 工具 · 向文件提問 · 履歷客製 + 求職信 · 兩頁紙自動填入 · DOCX 匯出 · 入口網站健康 · 應用程式內錯誤回報器 · 16 個 locales · 6 個 LLM 提供者 · 46 個掃描器轉接器 · 職業方向 · 職涯規劃 · 統計重構 · 記憶層 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 與父層 career-ops v1.16.0 對齊。_
