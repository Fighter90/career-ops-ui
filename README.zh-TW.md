...
> **🆕 最新版本 — v1.93.0**
>
> **記憶層（Epic 24）：** 全新的 **`#/memory`** 頁面存放一則簡短、可編輯的「關於我要記住的事」筆記。由於它會內嵌進共享的專案情境,它會引導**每一次** AI 請求 —— 評估、mock interview、人脈拓展、CV Studio —— 橫跨**所有**供應商。它捕捉的是你的偏好與工作風格（絕不包含關於你的新事實）,而一個**從你的資料提出建議**的小工具會從你的追蹤器草擬行為條目供你審閱。在 v1.92.0（CV Studio）、v1.91.0（人脈拓展規劃器）和 v1.90.0（mock interview）的基礎上構建。
>
> _16 locales · 6 個 LLM 提供者 · 45 個掃描器轉接器 · 記憶層 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 依目標職務的市場統計 · 重複刊登偵測器 · 與父層 career-ops v1.16.0 對齊。_
