...
> **🆕 最新版本 — v1.89.0**
>
> **候選人與市場契合度 —— two-pager（Epic 14）：** 全新的 **`#/two-pager`** 頁面以「Mnookin two-pager」格式記錄_你_對下一份工作真正想要的東西（喜歡的、必備項、討厭的、deal-breaker、不可談判項）。**AI 填寫助手**會根據你的履歷 + 個人檔案起草它，`#/scan` 上的每則職缺都會取得一個**與你所想契合度 `◎` 評分**，而 two-pager 會餵給每一次評估。在 v1.88.0（Scan i18n + API 衛生）、v1.87.0（4 個新掃描來源）和 v1.86.0（目標職務統計）的基礎上構建。
>
> _16 locales · 6 個 LLM 提供者 · 45 個掃描器轉接器 · two-pager 市場契合度 · 依目標職務的市場統計 · 重複刊登偵測器 · 與父層 career-ops v1.16.0 對齊。_
