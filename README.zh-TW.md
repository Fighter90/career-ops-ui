...
> **🆕 最新版本 — v1.95.0**
>
> **職涯規劃（Epic 26）：** 全新的 **`#/career-plan`** 頁面會把你的 CV 與檔案轉化為一份具體、個人化的發展規劃。選擇一個 **週期**(6/12/24 個月)與一個可選的聚焦方向,模型便會寫出起點快照、優勢/成長 SWOT、以 **SMART / OKR / WOOP** 形式呈現的目標、替代發展軌跡、硬技能/軟技能規劃、一份 **逐月路線圖**、進度追蹤、常見陷阱與支援性舉措 —— 立足於你自己的素材,絕不虛構你的過往經歷。可就地編輯,**儲存**到使用者層,並匯出為 **Markdown 或 PDF**。在 v1.94.0（統計重構)與 v1.93.0（記憶層)的基礎上構建。
>
> _16 個 locales · 6 個 LLM 提供者 · 45 個掃描器轉接器 · 職涯規劃 · 統計重構 · 記憶層 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 與父層 career-ops v1.16.0 對齊。_
