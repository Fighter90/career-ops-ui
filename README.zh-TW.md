...
> **🆕 最新版本 — v1.115.0**
>
> **設計打磨(保守,保留珊瑚色品牌)。** 對共享設計系統的一次輕量精修——不重構、不改調色盤:指標卡片懸停時帶珊瑚色邊框抬起,按鈕獲得靜止陰影與柔和懸停抬升,大數字用 tabular-nums 對齊,互動控件獲得柔和的珊瑚色聚焦光暈。動效尊重 prefers-reduced-motion。
>
> _設計打磨 · 用量計量 · 浮動說明助手 · 文件與 QA 整合 · 安全待辦清理 · 文件與 QA ×16 · Scan 排除 · 流水線概覽 · 安全強化 2 · 淨化器強化 · 安全強化 · AI 用量與成本 · 公司標誌 · AI CLI 工具 · 向文件提問 · 履歷客製 + 求職信 · 兩頁紙自動填入 · DOCX 匯出 · 入口網站健康 · 應用程式內錯誤回報器 · 16 個 locales · 6 個 LLM 提供者 · 46 個掃描器轉接器 · 職業方向 · 職涯規劃 · 統計重構 · 記憶層 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 與父層 career-ops v1.16.0 對齊。_
