...
> **🆕 最新版本 — v1.121.0**
>
> **CareerOps 宣言 + cvstart.org 專區。** v1.120.0(對齊父專案 v1.20.0)已在應用程式中呈現 [CareerOps 宣言](https://career-ops.org/manifesto) — 側邊欄頁尾連結、16 種語言皆有的說明指南第 29 節、README 專節。v1.121.0 為 [cvstart.org](https://cvstart.org) 新增四個專區:**方法論**(六維度評分準則,取自 [career-ops.org/methodology](https://career-ops.org/methodology))、**比較**、**授權**與**更新日誌**(全部 16 種語言),並在文件與說明指南中補上方法論連結。
>
> _宣言 · cvstart.org 方法論／授權／更新日誌頁面 · 父專案 v1.19 對齊 · 61 個轉接器 · 落地頁即時星標 + 貢獻者 · 父專案 v1.18 對齊 · 54 個轉接器 · Hired 狀態 · 累計統計 · 對齊包 · 50 轉接器 · 用量計量重做 · 設計打磨 · 用量計量 · 浮動說明助手 · 文件與 QA 整合 · 安全待辦清理 · 文件與 QA ×16 · Scan 排除 · 流水線概覽 · 安全強化 2 · 淨化器強化 · 安全強化 · AI 用量與成本 · 公司標誌 · AI CLI 工具 · 向文件提問 · 履歷客製 + 求職信 · 兩頁紙自動填入 · DOCX 匯出 · 入口網站健康 · 應用程式內錯誤回報器 · 16 個 locales · 6 個 LLM 提供者 · 46 個掃描器轉接器 · 職業方向 · 職涯規劃 · 統計重構 · 記憶層 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 與父層 career-ops v1.16.0 對齊。_
