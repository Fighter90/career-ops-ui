...
> **🆕 最新版本 — v1.107.0**
>
> **淨化器強化(靜態 XSS)。** 履歷/職缺 markdown 淨化器現在會將危險 HTML 清除**至不動點**(以捕捉會重構酬載的清除),匹配末尾帶雜物的 script/style 結束標籤(`</script foo>`),並清除未閉合的可執行開標籤。位於渲染時跳脫用戶端之後的縱深防禦;關閉相應的 CodeQL 發現。
>
> _淨化器強化 · 安全強化 · AI 用量與成本 · 公司標誌 · AI CLI 工具 · 向文件提問 · 履歷客製 + 求職信 · 兩頁紙自動填入 · DOCX 匯出 · 入口網站健康 · 應用程式內錯誤回報器 · 16 個 locales · 6 個 LLM 提供者 · 46 個掃描器轉接器 · 職業方向 · 職涯規劃 · 統計重構 · 記憶層 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 與父層 career-ops v1.16.0 對齊。_
