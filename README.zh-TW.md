...
> **🆕 最新版本 — v1.92.0**
>
> **CV Studio（Epic 21）：** 全新的 **`#/cv-studio`** 頁面為你的履歷提供三項誠實的工具 —— **履歷診斷**（一個確定性的 0–100 分數,並附各項檢查的說明）、**隱私遮罩**（完全在瀏覽器中遮蔽電子郵件/電話/連結/地址/姓名），以及**讓文字更像人味**（以 _你_ 的語調重寫生硬的句子,依據你的 voice-dna 與寫作範本,絕不捏造事實）。在 v1.91.0（人脈拓展規劃器）、v1.90.0（mock interview）和 v1.89.0（two-pager 市場契合度）的基礎上構建。
>
> _16 locales · 6 個 LLM 提供者 · 45 個掃描器轉接器 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 依目標職務的市場統計 · 重複刊登偵測器 · 與父層 career-ops v1.16.0 對齊。_
