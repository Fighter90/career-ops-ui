...
> **🆕 最新版本 — v1.104.0**
>
> **掃描表中的公司標誌(保護隱私)。** 新的外觀開關會在 `#/scan` 的公司名稱旁繪製其標誌。標誌是**公司自己網域的 favicon**,透過 SSRF 安全抓取器在伺服器端代理(`GET /api/logo`),因此沒有第三方標誌服務能得知你查看哪些雇主。共享徵才入口主機改用字母徽章;預設關閉。
>
> _公司標誌 · AI CLI 工具 · 向文件提問 · 履歷客製 + 求職信 · 兩頁紙自動填入 · DOCX 匯出 · 入口網站健康 · 應用程式內錯誤回報器 · 16 個 locales · 6 個 LLM 提供者 · 46 個掃描器轉接器 · 職業方向 · 職涯規劃 · 統計重構 · 記憶層 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 與父層 career-ops v1.16.0 對齊。_
