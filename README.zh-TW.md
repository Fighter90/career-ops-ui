...
> **🆕 最新版本 — v1.116.0**
>
> **用量計量重做 + 首個端到端元件測試。** AI 用量計量現固定在左側邊欄底部(不遮擋選單——預留空間讓導覽自由捲動),即時刷新,並按視窗顯示真實的權杖 · 預估費用。持久的 typeof 屏障關閉了反覆出現的 CodeQL 誤報,新的 Playwright 端到端測試在真實瀏覽器中驅動兩個常駐元件。
>
> _用量計量重做 · 設計打磨 · 用量計量 · 浮動說明助手 · 文件與 QA 整合 · 安全待辦清理 · 文件與 QA ×16 · Scan 排除 · 流水線概覽 · 安全強化 2 · 淨化器強化 · 安全強化 · AI 用量與成本 · 公司標誌 · AI CLI 工具 · 向文件提問 · 履歷客製 + 求職信 · 兩頁紙自動填入 · DOCX 匯出 · 入口網站健康 · 應用程式內錯誤回報器 · 16 個 locales · 6 個 LLM 提供者 · 46 個掃描器轉接器 · 職業方向 · 職涯規劃 · 統計重構 · 記憶層 · CV Studio · 人脈拓展規劃器 · mock interview · two-pager 市場契合度 · 與父層 career-ops v1.16.0 對齊。_
