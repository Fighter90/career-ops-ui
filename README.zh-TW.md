...
> **🆕 最新版本 — v1.90.0**
>
> **Mock Interview 2.0（Epic 15）：** 全新的 **`#/mock-interview`** 頁面會針對一份真實職缺進行逐輪面試演練。每個回答都會取得 **STAR+R 回饋**、一個**評分**以及一個**追問** —— 皆以你的履歷、two-pager 和故事庫為依據。可用任意 LLM 金鑰即時執行，或回傳一段可複製貼上的提示詞，而且你可以依公司/職務**儲存對話記錄**。在 v1.89.0（two-pager 候選人與市場契合度）、v1.88.0（Scan i18n + API 衛生）和 v1.87.0（4 個新掃描來源）的基礎上構建。
>
> _16 locales · 6 個 LLM 提供者 · 45 個掃描器轉接器 · mock interview · two-pager 市場契合度 · 依目標職務的市場統計 · 重複刊登偵測器 · 與父層 career-ops v1.16.0 對齊。_
