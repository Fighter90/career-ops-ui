...
> **🆕 最新版本 — v1.228.5** — **上報的版本描述的是磁碟上的檔案，而不是正在執行的程式碼。** `/api/health` 在每個請求上重新讀取 `package.json`：部署前啟動的行程一邊提供舊程式碼，一邊上報新檔案的版本 —— QA 發現一個實例回答 `1.228.4`，卻對 1.228.3 新增的路由回傳 404。只複製檔案、不重啟的部署正是這樣看起來像成功的。版本現在在模組載入時擷取：陳舊的行程會上報**舊**版本，也就是真相。**2909 個測試。**
...
```bash
npm test                       # 2909 個 unit/integration 測試
...
| Suite                       | 測試數 | 內容                                                                                                         |
| --------------------------- | ----- | ------------------------------------------------------------------------------------------------------------ |
| `node --test tests/*.test.mjs`(unit + integration) | **2909** | 每個端點、暫時性伺服器、無網路。涵蓋解析器、scanner(已 mock)、runner、anthropic、CSP / 安全 header、實體感知 XSS、JD sanitize、URL 驗證、SSRF redirect rebind、檔案互斥下的並行 tracker 寫入、`llmRateLimit`、路徑統一 sanitization、i18n parity。 |
...
| **總計** (四個套件合計)                   | **3040** | **0 fails、0 flakes**                                                                                       |
...
- 推送前執行 `npm test` — **2909 項檢查綠燈**為門檻(若觸碰 UI 則加上 90 個 Playwright)。
