...
import { BROWSER_LIKE_USER_AGENT, fetchText } from '../http-json.mjs';
 ...
 * tracker and the results table.
 * @param {{min?: number, max?: number}|undefined} band
 */
function formatSalary(band) {
  if (!band || typeof band !== 'object') return '';
  const k = (n) => `$${Math.round(n / 1000)}K`;
  const { min, max } = band;
  if (min && max && max !== min) return `${k(min)}–${k(max)}`;
  const one = min || max;
  return one ? k(one) : '';
}

/**
 * Fetch + normalize a Built In market.
 *
 ...
 */
export async function fetchBuiltin(_url, opts = {}) {
  const { fetchImpl = fetch, signal, company = {} } = opts;
  const { queries, categories, maxPages, host, scope } = readConfig(company);
  const label = company?.name ?? 'entry';

  if (host === null) {
    const raw = company?.builtin?.host ?? company?.host;
  ...
  if (bases.length === 0) {
    // Neutralized default (see readConfig): a shared source must never ship one
    // user's search terms, so an entry with neither key has nothing to scan.
    console.error(`⚠️  builtin: ${label} has no queries: or categories: — nothing to scan`);
    return [];
  }

  const pages = Math.min(HARD_MAX_PAGES, Math.max(1, Number(maxPages) || DEFAULT_MAX_PAGES));
  const seen = new Set();
  const out = [];
  // Drift-guard counters, aggregated across every page so one odd page cannot
  ...
  for (const base of bases) {
    ...
    for (let page = 1; page <= pages; page++) {
      ...
      for (const j of jobs) {
        ...
        added++;
        out.push({
          id: `builtin-${jobIdFromUrl(j.url) || j.url}`,
          title: j.title,
          company: j.company || '',
          url: j.url,
          ...
          workplaceType: '',
          relocates: false,
          date: j.postedAt ? new Date(j.postedAt).toISOString() : '',
  ...
  // The guard catches a LAYOUT CHANGE (near-total loss), not a page whose jobs
  // genuinely lack a field — hence the deliberately loose thresholds.
  if (guardRows >= GUARD_MIN_ROWS) {
    if (guardCards === 0) {
      console.error(`⚠️  builtin: ${label} parsed ${guardRows} rows but ZERO job cards — card markup changed. location_filter and posting-age are INERT for this entry until the parser is fixed.`);
    } else if (guardCards >= GUARD_MIN_ROWS && guardLocated / guardCards < GUARD_MIN_LOCATION) {
      console.error(`⚠️  builtin: ${label} resolved a location for only ${guardLocated}/${guardCards} cards — location markup may have changed.`);
    }
  }
