...
**Шар пам'яті (Epic 24).** Нова сторінка `#/memory` містить коротку редаговану нотатку «запам'ятай це про мене», яку асистент тримає в голові на **every** завданні:

- **Одна нотатка, всюди** — оскільки вона вбудована в `bundleProjectContext`, нотатка автоматично доходить до кожного AI-запиту (оцінка, пробна співбесіда, нетворкінг, CV Studio) в усіх **all** провайдерах. Напишіть її раз — вона керує всім.
