...
Ключи, которые нужны чаще всего. Это выборка, а не реестр: полный список всех распознаваемых ключей, сгруппированный и с подсказкой к каждому, показывает `#/config`.
...
- **Сохранение** (`POST /api/config`) валидирует каждое значение,
  ...
  Перезапуск не нужен.
- **Пустое значение удаляет** ключ — настройка возвращается в состояние «не задана», и снова действует умолчание проекта.
- **Выпадающие списки начинаются с «Использовать умолчание (…)».** Выбор этого пункта означает, что ключа в `.env` **нет**: приложение берёт умолчание проекта, и вы будете получать новое каждый раз, когда проект его поменяет. Выбор того же значения из списка ниже **закрепляет** его — оно останется даже после смены умолчания.
- **Поле, которого вы не касались, не записывается.** Незаданное поле показывает умолчание, чтобы вы видели, что использует сервер; это показанное значение не сохраняется, пока вы действительно не отредактируете поле.
- **Сохранение сообщает, что изменилось.** Тост считает и записанные ключи, и удалённые, поэтому очистка поля показывает «· 1», а не «· 0».
...
**Закрепление тенанта Personio (v1.237.0).** Многие компании встраивают свою
доску Personio как **iframe** на брендированную страницу карьеры, поэтому
`careers_url` указывает на собственный домен компании, а фид на самом деле
находится по адресу `<slug>.jobs.personio.de`. Без закрепления такие доски не
резолвятся ни во что. Укажите слаг тенанта явно — он побеждает `careers_url`:
...
Слаг ограничен по набору символов (буквы, цифры и дефисы, не может начинаться
с дефиса, не более 63 символов), а итоговый URL всё равно проходит ту же
проверку allowlist хостов и HTTPS — слаг, содержащий точку, слэш, `@` или `:`,
отклоняется, и запись возвращается к `careers_url`.

**Отображение типа занятости в заголовке (Jobstreet / SEEK, v1.237.0).**
Включите `appendWorkType`, и типы занятости вакансии добавляются к заголовку,
чтобы фильтры по заголовку и триаж видели их уже на этапе скана. По умолчанию
выключено:
...
`siteKey` выбирает рынок: `ID-Main` (id.jobstreet.com), `SG-Main`, `MY-Main`,
`HK-Main` (hk.jobsdb.com), `AU-Main` (au.seek.com, www.seek.com.au) и `NZ-Main`
(nz.seek.com, www.seek.co.nz). Ссылка на вакансию строится по-своему для каждого рынка —
только индонезийские хосты используют локальный префикс `/id/`, поэтому
ссылка, построенная с ним для любого другого рынка, возвращает 404
(исправлено в v1.237.0).
...
| `hh.ru: HTTP 451` или `403` в логе скана | Исходящий IP вне России либо hh.ru счёл его VPN/датацентром | Запускайте с российского **домашнего** IP. Ключа или настройки User-Agent не существует — сканер сам отправляет браузерный UA. |
...
career-ops-ui рассматривает каждый job-сайт как **adapter** — единый файл в [`server/lib/sources/<slug>.mjs`](../../server/lib/sources/), который умеет fetch'ить и нормализовать результаты одного сайта. На текущий момент registry `server/lib/sources/` поставляется с **94** адаптерами — **89 английских + 5 русских** досок. Английский набор охватывает основные ATSes (Greenhouse / Ashby / Lever / Workable / SmartRecruiters / Workday), агрегаторы уровня всей доски, выбираемые через явный `provider:` (RemoteOK, Remotive, We Work Remotely, NoDesk, Get on Board, Amazon, …), и ATSes на тенант, автоматически определяемые по хосту `careers_url` или явному `api:` URL (BambooHR, Personio, Recruitee, Teamtailor, Avature, SAP SuccessFactors, …). **Полный список никогда не нужно пересчитывать здесь вручную — он автоматически обнаруживается из `server/lib/sources/` и показывается вживую в выпадающем списке Source на `#/scan`.** См. §5 для YAML и `docs/portals-examples.md` для готовых copy-paste-записей.
...
**Как построено само это приложение.** Манифест — о том, как вести поиск работы; а само приложение, которое его реализует, построено с помощью агентов кодирования, и у этой практики есть собственный каталог: [Паттерны агентного кодинга](https://mokevnin.github.io/agentic-coding-design-patterns/ru/) (*Agentic Coding Design Patterns*) Кирилла Мокевнина
([английское издание](https://mokevnin.github.io/agentic-coding-design-patterns/en/)). career-ops-ui сознательно следует ей — исполняемые ограждения в закоммиченном pre-commit-хуке, упакованные в виде навыков рабочие процессы, изолированные параллельные ответвления, словарь предметной области, ADR, журнал прогресса и оценки рабочих процессов. Что из этого действительно реализовано, а что нет, фиксируется на странице вики проекта *Agentic Practices*.
