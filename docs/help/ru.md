...
**Hermes от Nous Research** — это открытый автономный агент: вызов инструментов, навыки и более 20 каналов обмена сообщениями, включая Telegram. Этот раздел объясняет, как можно запустить career-ops-ui на облачном сервере и связать его события с Telegram через агента Hermes. **Это план, а не готовая функция:** Hermes как LLM-провайдер пока не подключён (заблокирован до подтверждения контракта API у Nous Portal), поэтому сегодня никакой код приложения не вызывает Hermes. Полное руководство по дизайну и развёртыванию — в `docs/integrations/HERMES.md`, а скилл `hermes-bridge` проводит по шагам.

### Что такое Hermes

Hermes — это агентный runtime, а не, насколько мы можем судить, обычный размещённый API chat-completions. Это определяет интеграцию двумя способами. **Форма A:** если эндпоинт Nous Portal окажется OpenAI-совместимым, Hermes добавляется как ещё один LLM-провайдер в каскад приложения. **Форма B:** если он доступен только как агентный runtime, приложение общается с ним через выделенный relay-маршрут или локально запущенного агента. Что именно применимо — подтверждается из Nous Portal и репозитория `NousResearch/hermes-agent` *до* написания кода; именно поэтому провайдер намеренно ещё не реализован.
