...
>
> **Sağlayıcılar (v1.57.0).** Headless canlı değerlendirme artık **Anthropic → Gemini → OpenAI → Qwen → OpenRouter → GitHub Models → Hermes → DeepSeek → GLM (Z.ai) → Kimi (Moonshot) → MiniMax → Mistral → Grok (xAI) → Together → Fireworks → Ollama** kapsar (`auto` sırası; `LLM_PROVIDER` birini sabitler). **OpenRouter** — tek bir `OPENROUTER_API_KEY` 300+ modelin önüne geçer; `OPENROUTER_MODEL` açılır menüsü OpenRouter'ın canlı kataloğunu yükler (sunucu tarafı proxy, seçilmiş çevrimdışı yedek). Ayrıca düzeltildi: sonda satır sonu / çevresinde boşluk ile yapıştırılan anahtarlar artık doğrulamadan önce kırpılır, böylece `/#/config` hiçbir sağlayıcı için artık "validation failed" göstermez.
