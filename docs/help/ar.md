...
أكثر المفاتيح لزومًا. وهذه انتقاءٌ لا سجلّ: فـ`#/config` يعرض **كلّ** المفاتيح المعروفة مُجمَّعةً ومع تلميحٍ لكلٍّ منها.
...
- **القيمة الفارغة تحذف** المفتاح — فيعود الإعداد إلى حال *غير مضبوط*، ويسري افتراضيّ المشروع من جديد.
- **تبدأ القوائم المنسدلة بـ«استخدام القيمة الافتراضية (…)».** واختيارها يعني أنّ المفتاح **ليس** في `.env`: فيستعمل التطبيق افتراضيّ المشروع، وتظلّ تتلقّى الجديد كلّما غيّره المشروع. أمّا اختيار القيمة نفسها من القائمة أدناه فيُثبّتها، فتبقى ولو تحرّك الافتراضيّ.
- **والحقل الذي لا تمسّه لا يُكتب أبدًا.** فالحقل غير المضبوط يعرض الافتراضيّ لتَرى ما سيستعمله الخادم؛ ولا تُحفظ تلك القيمة المعروضة ما لم تُحرّر الحقل فعلًا.
- **ويُبلّغ الحفظُ بما تغيّر.** إذ يعدّ التنبيه المفاتيحَ المكتوبة *والمحذوفة* معًا، فإفراغُ حقلٍ يُظهر «· 1» لا «· 0».
...
| `hh.ru: HTTP 451` أو `403` في سجلّ المسح | عنوان الخروج خارج روسيا، أو عدّه hh.ru عنوان VPN/مركز بيانات | شغّله من عنوان **منزليّ** روسيّ. ولا وجود لمفتاحٍ ولا لإعداد User-Agent — فالماسح يرسل UA متصفّحٍ من تلقاء نفسه. |
