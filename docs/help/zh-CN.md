...
**AI CLI 工具。** **AI CLI 工具**标签页显示服务器上安装了哪些智能体 CLI(Claude Code、Codex、Gemini、OpenCode、Copilot、Qwen、Antigravity)——只读 PATH 扫描,不运行。**外观 → 显示公司徽标**(默认关闭)从各公司自己域名获取 favicon 显示在扫描表中(非第三方服务)。
...
career-ops-ui 将每个招聘站点视为一个 **adapter** — [`server/lib/sources/<slug>.mjs`](../../server/lib/sources/) 下的单一文件,知道如何获取并规范化某个站点的结果。截至 v1.119.0,`server/lib/sources/` 注册表自带 **61** 个 adapter —— **56 个英文 + 5 个俄文**板块。英文集合涵盖主流 ATS(Greenhouse / Ashby / Lever / Workable / SmartRecruiters / Workday)、由显式 `provider:` 选择的全板块聚合器(RemoteOK、Remotive、We Work Remotely、NoDesk、Get on Board、Amazon、…),以及从 `careers_url` 主机或显式 `api:` URL 自动检测的按租户 ATS(BambooHR、Personio、Recruitee、Teamtailor、Avature、SAP SuccessFactors、…)。**完整列表永远无需在此手工统计 —— 它会从 `server/lib/sources/` 自动发现,并在 `#/scan` 的 Source 下拉框中实时显示。** YAML 见 §5,可复制粘贴的条目见 `docs/portals-examples.md`。
...
career-ops——这款应用所依托的父项目——是 [CareerOps 宣言](https://career-ops.org/manifesto?utm_source=career-ops-ui)(父项目 v1.20.0)的首个参考实现。这份宣言为这整套工具链的存在理由命名:像工程师运营生产系统那样运营求职——讲证据、讲纪律,并把工具放在候选人这一边。
