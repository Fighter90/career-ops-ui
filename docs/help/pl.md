...
Klucze potrzebne najczęściej. To wybór, a nie rejestr: `#/config` wymienia **wszystkie** rozpoznawane klucze, pogrupowane i z podpowiedzią przy każdym.
...
- **Pusta wartość usuwa** klucz — ustawienie wraca do stanu *nieustawione* i znów obowiązuje wartość domyślna projektu.
- **Listy rozwijane zaczynają się od „Użyj wartości domyślnej (…)”.** Wybór tej pozycji oznacza, że klucza **nie ma** w `.env`: aplikacja korzysta z domyślnej wartości projektu i będziesz dostawać nową za każdym razem, gdy projekt ją zmieni. Wybranie tej samej wartości z listy poniżej **przypina** ją — zostanie nawet po zmianie domyślnej.
- **Pole, którego nie dotkniesz, nigdy nie jest zapisywane.** Nieustawione pole pokazuje wartość domyślną, żebyś widział, czego użyje serwer; ta pokazana wartość nie zostanie zapisana, dopóki naprawdę nie edytujesz pola.
- **Zapis informuje, co się zmieniło.** Powiadomienie liczy klucze zapisane *i* usunięte, więc wyczyszczenie pola pokazuje „· 1”, a nie „· 0”.
...
**Przypinanie tenanta Personio (v1.237.0).** Wiele firm osadza swoją tablicę Personio jako
**iframe** na własnej stronie kariery, więc `careers_url` wskazuje domenę firmy,
podczas gdy kanał faktycznie znajduje się pod adresem `<slug>.jobs.personio.de`. Bez
przypięcia takie tablice nie rozwiązują się do niczego. Podaj slug tenanta jawnie, a
wygrywa on z `careers_url`:
...
Slug ma ograniczony zestaw znaków (litery, cyfry i myślniki, bez myślnika na początku,
maksymalnie 63 znaki), a wynikowy URL nadal przechodzi przez tę samą listę dozwolonych
hostów i sprawdzenie HTTPS — slug zawierający kropkę, ukośnik, `@` lub `:` jest
odrzucany, a wpis wraca do `careers_url`.

**Pokazywanie typu zatrudnienia w tytule (Jobstreet / SEEK, v1.237.0).** Włącz to przez
`appendWorkType`, a typy zatrudnienia z ogłoszenia zostaną dopisane do tytułu, dzięki
czemu filtry tytułu i sortowanie widzą je już w momencie skanowania. Domyślnie wyłączone:
...
`siteKey` wybiera rynek: `ID-Main` (id.jobstreet.com), `SG-Main`, `MY-Main`,
`HK-Main` (hk.jobsdb.com), `AU-Main` (au.seek.com, www.seek.com.au) i `NZ-Main` (nz.seek.com, www.seek.co.nz).
Link do oferty budowany jest osobno dla każdego rynku — tylko hosty indonezyjskie
używają prefiksu lokalizacji `/id/`, więc link zbudowany dla innego rynku z tym
prefiksem zwraca 404 (naprawione w v1.237.0).
...
| `hh.ru: HTTP 451` lub `403` w logu skanowania | Wyjściowy IP jest poza Rosją albo hh.ru uznał go za VPN/centrum danych | Uruchamiaj z rosyjskiego IP **domowego**. Nie istnieje żaden klucz ani ustawienie User-Agent — skaner sam wysyła przeglądarkowe UA. |
...
career-ops-ui traktuje każdy portal pracy jako **adapter** — pojedynczy plik w [`server/lib/sources/<slug>.mjs`](../../server/lib/sources/), który wie jak pobrać i znormalizować wyniki jednego portalu. Obecnie rejestr `server/lib/sources/` dostarcza **94** adapterów — **89 angielskich + 5 rosyjskich** portali. Zestaw angielski obejmuje główne ATS-y (Greenhouse / Ashby / Lever / Workable / SmartRecruiters / Workday), agregatory ogólnoportalowe wybierane przez jawny `provider:` (RemoteOK, Remotive, We Work Remotely, NoDesk, Get on Board, Amazon, …) oraz ATS-y na tenant automatycznie wykrywane z hosta `careers_url` lub jawnego adresu `api:` (BambooHR, Personio, Recruitee, Teamtailor, Avature, SAP SuccessFactors, …). **Pełnej listy nigdy nie trzeba liczyć tu ręcznie — jest automatycznie wykrywana z `server/lib/sources/` i pokazywana na żywo w rozwijanym menu Source na `#/scan`.** Patrz §5 po YAML oraz `docs/portals-examples.md` po gotowe wpisy do skopiowania.
...
Karta **Raport rynkowy** prosi model o analizę wynagrodzeń i rynku pracy dla *twoich* docelowych ról — czyta twoje CV i profil, aby wiedzieć, które role i poziom seniority uwzględnić. Wpisz **Region / rynek** (na przykład `Russia`, `EU-remote`, `US` lub `Germany`), wybierz **Walutę** i kliknij **Wygeneruj raport rynkowy**. Otrzymasz uporządkowany raport z podsumowaniem wykonawczym, wynagrodzeniami według stopnia (mediana plus P10/P25/P75/P92), czołowymi pracodawcami, tabelą poszukiwanych umiejętności, częstotliwością benefitów, podziałem na biuro/hybryda/zdalnie, trendami na 12–24 miesiące łącznie z wpływem AI oraz wskazówkami negocjacyjnymi. Każda liczba to **orientacyjne oszacowanie z wiedzy treningowej modelu** — nie dane pobierane ani na żywo — i raport o tym informuje; traktuj liczby jako zakresy, nie jako oferty. Bez ustawionego klucza API otrzymasz zamiast tego prompt do skopiowania i wklejenia, a nie zmyślony raport. Użyj **Pobierz .md**, **Zapisz jako PDF** lub **Kopiuj**, aby wyciągnąć raport z aplikacji.
...
**Jak zbudowana jest sama aplikacja.** Manifest mówi o tym, jak prowadzić poszukiwania pracy; oprogramowanie, które je obsługuje, jest budowane przez agentów kodujących, a ta praktyka ma własny katalog: [Agentic Coding Design Patterns](https://mokevnin.github.io/agentic-coding-design-patterns/en/) autorstwa Kirilla Mokevnina ([rosyjskie wydanie](https://mokevnin.github.io/agentic-coding-design-patterns/ru/)). career-ops-ui stosuje go świadomie — egzekwowalne zabezpieczenia w zatwierdzonym hooku pre-commit, zapakowane przepływy pracy jako skille, izolowane równoległe rozgałęzienia, słownik domenowy, ADR-y, dziennik postępu i ewaluacje przepływów pracy. To, co faktycznie zostało wdrożone, a co nie, jest audytowane na stronie *Agentic Practices* wiki projektu.
