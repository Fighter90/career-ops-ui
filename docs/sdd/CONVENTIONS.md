...
- `server/index.mjs` (~174 LOC after **P-2 phase 2** in v1.9.0). Pure orchestrator — middleware + `register*Routes(app)` calls + SPA catch-all. New routes go into `server/lib/routes/<topic>.mjs` exporting `register<Topic>Routes(app)`. Currently 17 route modules: `activity`, `auto-pipeline`, `batch`, `config`, `content`, `health`, `help`, `jds`, `llm`, `openrouter`, `pipeline`, `reports`, `runners`, `scan`, `stats` (v1.86.0 — target-role stats snapshot store), `tracker`, `two-pager` (v1.89.0 — candidate two-pager GET/PUT/draft; writes `config/two-pager.yml`).
...
- Help-bundle markdown lives in `docs/help/<locale>.md` (**16 files**: `en, es, pt-BR, ko-KR, ja, ru, zh-CN, zh-TW, fr, pl, uk, da, ar, de, it, tr` — pl/uk/ar added in v1.71.1; da in v1.77.0; de/it/tr in v1.85.0). CI invariant: **21 H2 sections** (`tests/canonical-docs-coverage.test.mjs` + `tests/help-ui.test.mjs` `SECTION_COUNT`) and **81 H3** (`tests/help-ru-config-section.test.mjs`). §19 "Localizing the app" added in v1.60.0; §20 "Statistics by target roles" (+3 H3) added in v1.86.0; §21 "Your two-pager" (+3 H3) added in v1.89.0.
