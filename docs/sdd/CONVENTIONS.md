...
- `server/index.mjs` (~174 LOC after **P-2 phase 2** in v1.9.0). Pure orchestrator — middleware + `register*Routes(app)` calls + SPA catch-all. New routes go into `server/lib/routes/<topic>.mjs` exporting `register<Topic>Routes(app)`. Currently 24 route modules: `activity`, `auto-pipeline`, `batch`, `career-plan` (v1.95.0 — `GET`/`PUT /api/career-plan` writes `config/career-plan.md`; `POST /api/career-plan/generate` AI development plan), `config`, `content`, `cv-studio` (v1.92.0 — CV humanize/voice-match), `health`, `help`, `interview` (v1.90.0 — mock interview turn/save/sessions), `jds`, `llm`, `market` (v1.94.0 — `POST /api/stats/market` AI salary/market report from CV+profile target roles + region + currency; no writes), `memory` (v1.93.0 — about-me note GET/PUT/suggest; writes `config/memory.md`), `networking` (v1.91.0 — networking plan/save/plans; writes `networking/`), `openrouter`, `orientation` (v1.96.0 — `POST /api/orientation/generate` AI career-orientation profile from CV+profile+two-pager+memory; reflection-not-test framing, no writes), `pipeline`, `reports`, `runners`, `scan`, `stats` (v1.86.0 — target-role stats snapshot store), `tracker`, `two-pager` (v1.89.0 — candidate two-pager GET/PUT/draft; writes `config/two-pager.yml`). Shared live-LLM cascade lives in `server/lib/llm-dispatch.mjs` (`runActiveProvider` / `providerAvailable`), used by new live-LLM routes.
...
- Help-bundle markdown lives in `docs/help/<locale>.md` (**16 files**: `en, es, pt-BR, ko-KR, ja, ru, zh-CN, zh-TW, fr, pl, uk, da, ar, de, it, tr` — pl/uk/ar added in v1.71.1; da in v1.77.0; de/it/tr in v1.85.0). CI invariant: **28 H2 sections** (`tests/canonical-docs-coverage.test.mjs` + `tests/help-ui.test.mjs` `SECTION_COUNT`) and **102 H3** (`tests/help-ru-config-section.test.mjs`). §19 "Localizing the app" added in v1.60.0; §20 "Statistics by target roles" (+3 H3) added in v1.86.0; §21 "Your two-pager" (+3 H3) added in v1.89.0; §22 "Mock interview" (+3 H3) added in v1.90.0; §23 "Networking & deep company research" (+3 H3) added in v1.91.0; §24 "CV Studio" (+3 H3) added in v1.92.0; §25 "Memory" (+3 H3) added in v1.93.0; §26 "Statistics" (rework — market report / pipeline / trend, +3 H3) added in v1.94.0; §27 "Career plan" (+3 H3) added in v1.95.0; §28 "Career orientation" (+3 H3) added in v1.96.0.
- `tests/i18n-coverage.test.mjs` enforces locale parity. If you add a key and forget a locale, that test fails.
- CHANGELOG parity: all 16 `CHANGELOG*.md` files (EN + 15 locales) must reference the same `## [vX.Y.Z]` top — `scripts/check-changelog-parity.mjs` is the gate (part of `npm run test:ci`).
...
- Every user-facing string is i18n-keyed across all 16 locales (the
