...
[![tests](https://img.shields.io/badge/tests-2138%20passed-brightgreen)](#الاختبارات)
...
> **🆕 أحدث إصدار — v1.131.2**
>
> **تقسيم `app.css` (إعادة هيكلة).** يسدّد v1.131.2 ديْن عقد حجم الملف: يُقسَّم `app.css` البالغ نحو 1990 سطرًا إلى ثلاث صفحات أنماط مرتّبة — `app.css` (الأساس/الإطار/الأزرار)، و**`components.css`** (البطاقات، الجداول، النماذج، الشارات، مقسِّم الصفحات، ...)، و**`overlays.css`** (التنبيه المنبثق، النافذة المنبثقة، الدرج، `[dir="rtl"]`، docs-fab، usage-hud) — كل منها ضمن الحد الصارم البالغ 800 سطر. القطع متواصل ومرتّب، لذا يكون التتالي **مطابقًا حرفيًا بايتًا ببايت**؛ يحمّل `index.html` الملفات الثلاثة كعناصر `<link>` مرتّبة. لا تغيير في السلوك/الترميز/الترجمة؛ جرى التحقق داخل المتصفح (تُحلَّل الصفحات الثلاث جميعها وتُطبَّق، صفر أخطاء في وحدة التحكم). **2138 اختبارًا**. **تعزيز تثبيت مضيف المحوّلات (تصحيح).** يعالج v1.131.1 متابعات مراجعة الذكاء الاصطناعي على مصدرَي v1.130.0 (دفاع في العمق، بلا تغيير في السلوك للمدخلات الصالحة): يعيد محوّل **a16z Speedrun** الآن التحقّق من تجاوز نقطة النهاية (HTTPS + مضيف مطابق تمامًا) ويعود إلى التغذية القياسية عند الفشل — تكافؤًا مع **Cryptocurrency Jobs** — ويستخدم `cleanUrl` الخاص بمُحلِّل Cryptocurrency Jobs حارس المضيف المطابق تمامًا نفسه المستخدَم في فحص SSRF (كان يستخدم `endsWith`، الذي يقبل النطاقات الفرعية)، بحيث لا يكون المُحلِّل أبدًا أكثر تساهلًا من الحارس. **2135 اختبارًا**. **لوحة تبويبات المراحل بأسلوب CRM.** ينقل v1.131.0 عرض `/pipeline` من تطبيق الويب الخاص بالمشروع الأصل إلى `#/tracker`: يتحوّل شريط رقائق القمع + القائمة المنسدلة للحالة إلى **شريط تبويبات مراحل** — تبويب **الكل** بالإضافة إلى تبويب لكل حالة قياسية (Evaluated · Applied · Responded · Interview · Offer · Rejected · Discarded · SKIP · Hired)، يعرض كل منها عداداً حياً، **مع تضمين المراحل ذات العدّ الصفري** بحيث يظل القمع بالكامل مرئياً دائماً. تأتي التبويبات من مسار جديد للقراءة فقط `GET /api/tracker/stages` (تسميات قياسية + طيّ الأسماء البديلة من `templates/states.yml` — لا تُبرمج القائمة البيضاء أبداً من جهة العميل)، وتكتسب الصفوف شعارات علامات تجارية اختيارية، وتحمل الأسماء المتاحة للقراءة الآلية العدادات. **2133 اختباراً**. **تكافؤ الأب v1.24.0.** ينقل v1.130.0 مصدرَي مسح جديدين — **a16z Speedrun** (تغذية a16z Speedrun لشبكة المواهب على مستوى اللوحة بأكملها) و**Cryptocurrency Jobs** (لوحة الويب3 `cryptocurrencyjobs.co`، عبر تغذية RSS العامة الخاصة بها) — ليصل السجل إلى **72 مصدرًا (67 إنجليزية + 5 روسية)** ويظهر كلاهما في مرشّح Source بـ`#/scan` وعلى الصفحة الرئيسية لـcvstart.org. كما يعكس إصلاحين من المشروع الأصل: تبقي **echojobs** الآن الوظائف **الهجينة** قابلة للتمييز عن العمل عن بُعد (`City · Hybrid`، لا تنطوي أبداً ضمن `Remote`)، وتتعلّم **radancy** ترميز TalentBrew القديم إضافةً إلى نقل جزء نتائج JSON. **2123 اختبارًا**. **متابعات مراجعة الذكاء الاصطناعي (تصحيح).** يُصلح v1.129.1 ملاحظات استشارية من منافذ web في v1.128/v1.129: أسبقية المستوى في job-facets (`Senior Engineering Manager` → `senior`، لا `lead`)، ولم يعد `states.mjs` يثبّت fallback عند تعثّر الأب أثناء الإقلاع (+تحذير عند ملف تالف)، ويُظهر التدرّج الصفَّ بلا درجة محايدًا (لا أحمر)، ويتخطّى `domainFromName` الـslugs غير ASCII. **وجه المستوى + عمود المدة في المسح.** يربط v1.129.0 مكتبة `job-facets.js` من v1.128.0 بـ `#/scan`: مرشّح **المستوى** جديد (lead/staff/senior/متوسط/junior/متدرب، يُملأ من النتائج كوجه الدولة)، وعمود شارة للمستوى، وعمود **المدة** بلا رموز (`اليوم` / `Nي`). **أربعة نقلات من تطبيق ويب المشروع الأب.** يعيد v1.128.0 تنفيذ أربعة حلول من تطبيق Next.js الخاص بـ career-ops بجافاسكريبت خام: يقرأ المتتبِّع الحالات القياسية مباشرةً من `templates/states.yml` (بلا قائمة بيضاء مبرمَجة)، وشعارات العلامات الآن في الصفوف المستضافة على ATS عبر خريطة اسم→نطاق، ودرجة لون رباعية أدق، ومكتبة أوجه بلا رموز. **2066 اختبارًا**. **تكافؤ الأب v1.23.0.** ينقل v1.127.0 ثلاثة مصادر مسح جديدة — **Flowxtra** و**VDAB** و**iCIMS** (السجل الآن **70 محوّلاً**، 65 EN + 5 RU) —، ويعكس انتقال agenticjobs إلى REST واستعادة المدينة في Greenhouse وإصلاحات role-matcher، ويعيد **Cursor** إلى قائمة الـCLI (parent #2115 — يُبلّغ `cli-detect` عن 10 أدوات). **ترقيع انحراف قائمة CLI.** يُصلح v1.126.1 نقطتين فوّتهما مسح v1.126.0: مقدّمة تبويب API keys في `#/config` (i18n ×17) تسرد الآن Antigravity + Grok Build، وصفّ قديم `Cursor / Gemini CLI` في دليل المساعدة ×17 (وhelp الموقع) صار يحمل قائمة الـ8 واجهات كاملةً. **إعادة مزامنة التوثيق وقائمة CLI.** يُطابق v1.126.0 كل واجهة توثيق (help ×17، README ×17، wiki) مع career-ops.org/docs الحيّة (قُرئت الـ31 صفحة) ويُعلّم مسح أدوات AI CLI في `#/config` اكتشاف كل الواجهات الثماني من الدرجة الأولى — تنضمّ **Grok Build CLI** و**Kimi CLI** إلى Claude Code وCursor وCodex وOpenCode وAntigravity وQwen وCopilot (الآن **9 أدوات**، وما زالت لا تُشغَّل أبداً). **مزامنة صيانة.** يجمع v1.125.4 تحديثات dependabot في site (`sharp` 0.35.3 و`svgo` 4.0.2 و`fast-uri` 3.1.4) ويسجّل مسح تكافؤ مع المشروع الأب (بعد v1.22.0، #2108–#2168): حارس الصف الخاطئ في set-status وRisk Summary للأوضاع المترجمة والتحقق من manifest المحدِّث كلها في جانب CLI — لا شيء يُنقل (**1969 اختبارًا** دون تغيير). **إصلاح لغة المطالبات.** يجعل v1.125.3 الدنماركية والهندية مدعومتين بالكامل في كل مطالبة ذكاء اصطناعي — البحث المعمّق (المباشر واليدوي)، الأوضاع، التقييم، المقابلة، التواصل وCV Studio تُصدر الآن توجيه `# Output language` في هاتين اللغتين (كانت الإجابات تأتي بالإنجليزية)، وبوابة اللغات تفحص الآن كل اللغات الـ17 (**1969 اختبارًا**). **حزمة المساهمين الخارجيين.** يدمج v1.125.2 أولى مساهمات المجتمع من [@Alien10140](https://github.com/Alien10140) — إصلاح HTTP 502 في البحث المعمّق المباشر (موجّهات بلا أدوات لمزوّدي API) ورفع نماذج Gemini الافتراضية من `gemini-2.0-flash` الموقوف إلى `gemini-3.6-flash`، مثبّتة ببوابة انحراف جديدة من 5 اختبارات (**1957 اختبارًا**). ويحافظ v1.125.1 على مسار العلامة لمستأجري SuccessFactors RMK متعددي العلامات (الأصل ‎#2099). **مصادر الوظائف على الصفحة الرئيسية.** يضيف الإصدار v1.125.0 قسمًا على cvstart.org يسرد **جميع مصادر المسح الـ67** كشرائح قابلة للنقر، تُزامَن مع السجل عند البناء وتُحمى من الانحراف. وقبل ذلك، **تكافؤ مع الأصل v1.22.0.** يضيف الإصدار v1.124.0 **خمسة مصادر مسح** — Welcome to the Jungle وAgentic Engineering Jobs وJobvite وGem وAlibaba Group (**70 محوّلاً** الآن) — إضافةً إلى التحقّق من العمل عن بُعد بالكامل في Arbeitsagentur (`homeofficetyp=VOLLSTAENDIG`) وإصلاح رابط الوظيفة العام في SmartRecruiters.
>
> _تقسيم `app.css` (3 صفحات أنماط) · سداد ديْن عقد حجم الملف · تتالٍ مطابق بايتًا ببايت · تعزيز تثبيت مضيف المحوّلات · إعادة تحقّق تجاوز a16z Speedrun · cleanUrl مضيف مطابق تمامًا في cryptocurrencyjobs · لوحة تبويبات المراحل بأسلوب CRM · نقل عرض /pipeline إلى المتتبّع · تبويبات المراحل + العدادات · GET /api/tracker/stages · شعارات العلامات في المتتبّع · تكافؤ الأب v1.24 · +2 مصادر (a16z Speedrun/Cryptocurrency Jobs) · 72 محوّلاً · echojobs هجين · radancy TalentBrew · تصحيح مراجعة الذكاء · أسبقية المستوى · fallback states · تدرّج محايد · وجه المستوى · عمود المدة · ربط job-facets · 4 نقلات web/ · states.yml مباشرة · شعارات اسم→نطاق · درجة اللون · أوجه · تكافؤ الأب v1.23 · +3 مصادر · 70 محوّلاً · Cursor · ترقيع انحراف قائمة CLI · إعادة مزامنة التوثيق وقائمة CLI · مزامنة صيانة · إصلاح لغة المطالبات da/hi · حزمة المساهمين الخارجيين · إصلاح 502 للبحث المعمّق · افتراضيات Gemini 3.6 · إصلاح مسار علامة SF · قسم مصادر الوظائف على الصفحة · 5 مصادر · 67 محوّلاً · تكافؤ مع الأب v1.22 · Oracle Cloud · 62 محوّلاً · تكافؤ مع الأب v1.21 · الهندية · 17 لغة · بيان CareerOps · صفحات منهجية/رخصة/سجل تغييرات cvstart.org · تكافؤ مع الأب v1.19 · 61 محوّلاً · نجوم مباشرة + مساهمون على الصفحة · تكافؤ الأب v1.18 · 54 محوّلًا · حالة Hired · إحصاءات إجمالية · حزمة التكافؤ · 50 محوّلاً · إعادة تصميم مقياس الاستخدام · صقل التصميم · مقياس الاستخدام · مساعد مساعدة عائم · دمج التوثيق وضمان الجودة · إغلاق متأخّرات الأمان · الوثائق وضمان الجودة ×16 · استبعاد Scan · نظرة عامة على المسار · تعزيز الأمان 2 · تعزيز أداة التعقيم · تعزيز الأمان · استخدام الذكاء الاصطناعي وتكلفته · شعارات الشركات · أدوات CLI للذكاء الاصطناعي · اسأل الدليل · تكييف السيرة + خطاب · تعبئة تلقائية للوثيقة · تصدير DOCX · صحة البوابات · مُبلّغ الأخطاء المدمج · الإحصاءات، مُعاد تصميمها · 16 locales · 6 مزوّدات LLM · 46 محوّل ماسح · التوجيه المهني · خطة المسار المهني · طبقة الذاكرة · CV Studio · مخطّط تواصل مهني · مقابلة تجريبية · ملاءمة السوق عبر two-pager · تكافؤ مع المشروع الأصلي career-ops v1.16.0._
