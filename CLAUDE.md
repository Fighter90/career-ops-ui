...
| Layer | Tech | Files |
|---|---|---|
| Server | Node ≥18, Express 4, js-yaml | `server/index.mjs` (~130 lines, orchestrator only), `server/lib/*.mjs`, `server/lib/routes/*.mjs` (20 modules) |
...
- **Versioning:** `package.json` is the source of truth (currently 1.92.0). The footer reads it via `/api/health`. The parent's `VERSION` file is reported separately as `parentVersion` — they drift independently.
