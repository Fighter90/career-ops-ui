...
### 新增
- **`#/scan` 級別面向 + 時長欄** — v1.128.0 的 `job-facets.js` 函式庫現已接入掃描 UI(先前僅邏輯)。新的**級別**下拉將每筆職缺標題歸入 lead/staff/senior/mid/junior/intern(`JobFacets.seniorityFromTitle`),並依結果中實際存在的項自動填入(與國家面向一致);無級別詞的標題一律通過。可在已儲存搜尋、重設與套用中保持。結果表新增**級別**徽章欄與零 token 的**時長**欄(`今天`/`N天`,來自 `JobFacets.daysSince`)。12 個 i18n 鍵 ×17,+3 個測試 → **2073**。
