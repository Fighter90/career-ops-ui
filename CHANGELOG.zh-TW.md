...
### 說明
- 新增 `server/lib/discover-ats.mjs`（固定主機、字元集驗證的 slug 經 DNS 固定的 `safeGet` 探測，每次請求 ≤12 次）+ `POST /api/portals/discover`（唯讀）與 `POST /api/portals/track`（顯式寫入：`withFileLock` + 文字拼接 + 重新解析保護 + 原子重新命名；僅已知 ATS 主機、冪等）。複用掃描器的配接器註冊表確認招聘板並統計職缺。i18n ×17。`tests/discover-ats-resolver.test.mjs` + `tests/discover-ats-route.test.mjs`。測試套件：**2586**。
