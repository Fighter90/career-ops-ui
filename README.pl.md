...
> **🆕 Najnowsze wydanie — v1.233.0** — **Trzy ustalenia w ścieżce zapisu `#/config`; CONFIG-3 wywołała sama poprawka z v1.232.1.** Świadomy wybór równy zasianej wartości był odrzucany jako „nietknięty"; teraz zapisujemy **`seededFromFile`**. Usunięcie klucza pokazywało „· 0" — serwer zwraca też `removed`. Listy rozwijanej nie dało się wyczyścić: każda zaczyna się teraz od **„Użyj wartości domyślnej (…)"**. **3018 testów · 116 przeglądarkowych.**
