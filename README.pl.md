...
[![tests](https://img.shields.io/badge/tests-3164%20passed-brightgreen)](#testy)
...
> **🆕 Najnowsze wydanie — v1.236.0** — **Parytet z rodzicem career-ops `main` @ `6a9c84c`: dwa nowe źródła i dwie poprawki, które po cichu gubiły oferty.** Stanowisko **Workday** otwarte w kilku miastach zgłasza *liczbę* — `"53 Locations"` — tam, gdzie każda inna oferta niesie miejsce, więc nie pasowało do żadnego wpisu `allow: [austin]` i było odrzucane (53 z 291 ofert u źródła). Jego prawdziwe miejsca pochodzą teraz z dokumentu szczegółów CXS, z limitem 200 zapytań na wpis. **`role-matcher`** w ogóle nie miał obsługi poziomów, więc `"Insurance Specialist"` i `"Insurance Specialist II"` zwijały się w jedną powtórkę — cały mechanizm rodzica jest przeniesiony, z cyframi rzymskimi i arabskimi zwiniętymi do jednej liczby. Tablica **Avature** przypięta do filtra przechodziła całą globalną tablicę, bo ciąg zapytania był odrzucany przy odbudowie URL wyszukiwania. Nowe źródła: **Python.org Jobs** i **Generalist World** — **92 → 94** (89 EN + 5 RU). Dodatkowo `devalue` 5.8.1 → 5.9.2, zamykając otwarty alert Dependabot. **3164 testów · 116 przeglądarkowych.**
