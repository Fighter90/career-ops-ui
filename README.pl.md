...
[![tests](https://img.shields.io/badge/tests-2909%20passed-brightgreen)](#testy)
...
> **🆕 Najnowsze wydanie — v1.228.5** — **Zgłaszana wersja opisywała plik na dysku, a nie uruchomiony kod.** `/api/health` odczytywał `package.json` przy każdym żądaniu: proces uruchomiony przed wdrożeniem zgłaszał wersję nowego pliku, serwując stary kod — QA znalazł instancję odpowiadającą `1.228.4` i zwracającą 404 na trasie dodanej w 1.228.3. Tak właśnie wygląda wdrożenie, które kopiuje pliki bez restartu. Wersja jest teraz przechwytywana przy ładowaniu modułu: przestarzały proces zgłasza **starą** wersję, czyli prawdę. **2909 testów.**
...
└─ web-ui/          ← to repozytorium
   ...
   └─ tests/        # 2909 testów jednostkowych + 101 Playwright + 44 e2e
```
...
```bash
npm test                    # 2909 testów jednostkowych/integracyjnych
