...
[![tests](https://img.shields.io/badge/tests-3018%20passed-brightgreen)](#testy)
...
> **🆕 Najnowsze wydanie — v1.232.0** — **Dwie usterki z przeglądarkowego QA; żadna z v1.231.5.** **`#/profile` przewijał całą stronę w bok**: wartość była anonimowym `<div>` bez reguły łamania, a URL LinkedIn nie daje CSS miejsc podziału — **min-content równał się całemu ciągowi**, a `min-width:auto` nie pozwalał zejść niżej, rozsadzając kartę, wiersz i dokument. Naprawione klasą `.card-value` z **`overflow-wrap: anywhere`**. **`POST /api/config` przyjmował dowolną wartość pola select** i potem zawodził *po cichu*. Walidowany jest tylko dostawca; wartość już zapisana jest tolerowana. **3018 testów.**
